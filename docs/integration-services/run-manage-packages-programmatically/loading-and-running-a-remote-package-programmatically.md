---
title: "Le chargement et l’exécution d’un Package distant par programme | Documents Microsoft"
ms.custom: 
ms.date: 03/17/2017
...
> [!NOTE]  
>  Vous ne pouvez pas exécuter les packages en dehors de [!INCLUDE[ssBIDevStudio](../../includes/ssbidevstudio-md.md)] sur un ordinateur client qui n’a pas [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] installé et les termes de votre [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] licences ne peut-être pas vous permettent d’installer [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] sur des ordinateurs supplémentaires. ([!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] est un composant serveur et n'est pas redistribuable aux ordinateurs clients.)  
  
 Alternativement, vous pouvez exécuter un package distant à partir d'un ordinateur local sur lequel [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] est installé. Pour plus d’informations, consultez [chargement et exécution d’un Package Local Programmatically](../../integration-services/run-manage-packages-programmatically/loading-and-running-a-local-package-programmatically.md).  
  
##  <a name="top"></a>Exécution d’un Package à distance sur l’ordinateur distant  
 Comme mentionné précédemment, il existe plusieurs manières d'exécuter un package distant sur un serveur distant :  
  
-   [Utiliser SQL Server Agent pour exécuter le package distant par programme](#agent)  
  
-   [Utiliser un service Web ou un composant distant pour exécuter le package distant par programme](#service)  
  
 Presque toutes les méthodes qui sont utilisés dans cette rubrique pour charger et enregistrer des packages requièrent une référence à la **Microsoft.SqlServer.ManagedDTS** assembly. L’exception est l’approche ADO.NET présenté dans cette rubrique pour exécuter le **sp_start_job** procédure stockée, ce qui ne nécessite qu’une référence à **System.Data**. Après avoir ajouté la référence à la **Microsoft.SqlServer.ManagedDTS** assembly dans un nouveau projet, importez le <xref:Microsoft.SqlServer.Dts.Runtime> espace de noms avec un **à l’aide de** ou **importations** instruction.  
  
###  <a name="agent"></a>À l’aide de l’Agent SQL Server pour exécuter un Package distant par programme sur le serveur  
 L'exemple de code suivant montre comment utiliser par programme l'Agent [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] pour exécuter un package distant sur le serveur. L’exemple de code appelle la procédure stockée système, **sp_start_job**, qui lance un [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] travail de l’Agent. Le travail que la procédure lance est nommé `RunSSISPackage`, et ce travail se trouve sur l'ordinateur distant. Le travail `RunSSISPackage` exécute alors le package sur l'ordinateur distant.  
  
> [!NOTE]  
>  La valeur de retour de la **sp_start_job** procédure stockée indique si la procédure stockée a été en mesure de démarrer le [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent correctement le travail. La valeur de retour n'indique pas si le package a réussi ou a échoué.  
  
 Pour plus d’informations sur la résolution des packages qui sont exécutés à partir de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] des travaux de l’Agent, consultez l’article Microsoft [un package SSIS ne s’exécute pas lorsque vous appelez le package SSIS à partir d’une étape de travail de l’Agent SQL Server](http://support.microsoft.com/kb/918760).  
...
###  <a name="service"></a>À l’aide d’un Service Web ou un composant distant pour exécuter un Package distant par programme  
...
> [!IMPORTANT]  
>  Avec ses paramètres par défaut pour l'authentification et l'autorisation, un service Web ne dispose généralement pas des autorisations suffisantes pour accéder à SQL Server ou au système de fichiers afin de charger et d'exécuter des packages. Vous devrez peut-être affecter des autorisations appropriées au service Web en configurant ses paramètres d’authentification et d’autorisation dans le **web.config** fichier et l’assignation d’autorisations de système de base de données et le fichier comme il convient. La description complète des autorisations Web, de base de données et de système de fichiers sort de la portée de cette rubrique.  
...
#### <a name="creating-the-web-service"></a>Création du service Web  
 Un package [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] peut être chargé directement à partir d'un fichier, directement à partir de SQL Server ou à partir du magasin de packages SSIS, lequel gère le stockage des packages dans SQL Server et des dossiers spéciaux du système de fichiers. Cet exemple prend en charge toutes les options disponibles en utilisant un **Select Case** ou **commutateur** construction pour sélectionner la syntaxe appropriée au démarrage du package et pour concaténer les arguments d’entrée convenablement. La méthode de service Web LaunchPackage renvoie le résultat de l'exécution du package sous la forme d'un entier plutôt que d'une valeur <xref:Microsoft.SqlServer.Dts.Runtime.DTSExecResult> afin que les ordinateurs clients ne requièrent pas de référence aux assemblys [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)].  
...
2.  Ajoutez une référence à **Microsoft.SqlServer.ManagedDTS** et ajoutez un **importations** ou **à l’aide de** instruction dans le fichier de code pour le **Microsoft.SqlServer.Dts.Runtime** espace de noms.  
...
-   Vidéo, [Comment : automatiser l’exécution du Package SSIS à l’aide de l’Agent SQL Server (vidéo liée à SQL Server)](http://technet.microsoft.com/sqlserver/ff686764.aspx), sur technet.microsoft.com  
