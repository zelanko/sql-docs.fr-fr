...
---
# S&#233;lectionner les tables et les vues sources (Assistant Importation et Exportation SQL Server)
...
## <a name="screen-shot---what-you-see-if-youre-going-to-copy-tables"></a>Capture d’écran : ce que vous voyez si vous allez copier des tables  
 La capture d’écran suivante montre la page **Sélectionner les tables et les vues sources** de l’Assistant quand vous avez auparavant sélectionné l’option **Copier les données à partir d’une ou plusieurs tables ou vues** dans la page **Spécifier la copie ou l’interrogation de table**. La liste indique toutes les tables et vues disponibles à partir de la source de données.
 
Dans cet exemple, l’utilisateur souhaite copier la table **Sales.Customer** à partir de la source vers la nouvelle table **Sales.CustomerNew** dans la destination. 
...
## <a name="screen-shot---what-you-see-if-you-provided-a-query"></a>Capture d’écran : ce que vous voyez si vous avez fourni une requête  
 La capture d’écran suivante montre la page **Sélectionner les tables et les vues sources** de l’Assistant quand vous avez auparavant sélectionné l’option **Écrire une requête pour spécifier les données à transférer** dans la page **Spécifier la copie ou l’interrogation de table**, et que vous avez ensuite fourni la requête dans la page **Fournir une requête source**. La liste ne comprend qu’une seule ligne, dans laquelle l’élément de la colonne Source représente la requête que vous avez fournie.
...
**Source**  
Activez les cases à cocher pour sélectionner dans la liste les tables et les vues à copier. Par défaut, les données de la source de données sont copiées sans modification. Si vous créez une table de destination, le schéma est également copié sans modification à partir de la source de données.

Si vous fournissez une requête, la liste ne contient qu’un élément portant le nom \[Requête\]. 
...
**Valeurs Null ou manquantes dans les données importées**. Quand une colonne Excel semble contenir différents types de données (par exemple, des valeurs numériques et des valeurs de texte), dans les huit premières lignes échantillonnées par l’Assistant, ce dernier choisit le type de données majoritaire comme type de données de la colonne et retourne des valeurs Null pour les cellules qui contiennent des données d’autres types. Il n’existe aucun moyen de modifier ce comportement de l’Assistant.
...
**Spécification d’une plage de données existante**. Quand vous spécifiez une plage existante comme destination, vous obtenez une erreur si la plage n’est pas assez large. Autrement dit, s’il y a moins de colonnes que dans les données sources. Toutefois, si la plage que vous spécifiez n’est pas assez longue (autrement dit, s’il y a moins de lignes que dans les données sources), l’Assistant continue d’écrire des lignes et étend la définition de la plage pour qu’elle corresponde au nouveau nombre de lignes.
...
-   Si la table de destination est créée par l’Assistant, l’instruction **CREATE TABLE** doit utiliser **LONGTEXT** (ou un de ses synonymes) comme type de données de la colonne mémo. Vérifiez l’instruction **CREATE TABLE** en cliquant sur **Modifier SQL** à côté de l’option **Créer une table de destination** dans la page **Mappages de colonnes**.
