---
title: "Passez en revue le Type de données de mappage (SQL Server Assistant Importation et exportation) | Documents Microsoft"
ms.custom: 
ms.date: 03/16/2017
...
## <a name="screen-shot-of-the-review-data-type-mapping-page"></a>Capture d’écran de la page Vérifier le mappage de type de données
 La capture d’écran suivante montre un exemple de la **révision Data Type Mapping** page de l’Assistant.
 
 Dans cet exemple :
 -   L’utilisateur a spécifié un mappage dans le **mappages de colonnes** boîte de dialogue qui ne peut pas réussir.
 ...
 -   L’icône d’avertissement sur la première ligne dans le **mappage de type de données** liste indique que le mappage à partir de la **int** type de données de la colonne source à la **smalldatetime** type de données de la colonne de destination peut provoquer une perte de données.
 
 ![Page vérifier la correspondance de Type de données de l’Assistant Importation et exportation](../../integration-services/import-export-data/media/review-mapping.png "le mappage de Type de données de vérification de page de l’Assistant Importation et exportation") 
 
## <a name="review-the-source-and-destination-tables"></a>Vérifier les tables source et de destination  
 La section supérieure de la page **Vérifier le mappage de type de données** est une liste **Table** qui répertorie les tables à copier de la source vers la destination. Pour consulter les informations de conversion d’une table spécifique, sélectionnez une table dans la liste **Table** . Les informations de conversion pour les colonnes de la table sélectionnée s’affiche dans la partie inférieure de la page dans le **mappage de type de données** grille.

Dans cet exemple, les résultats de la requête fournies par l’utilisateur seront être copiées dans la table de Sales.CustomerNew2 à la destination. L’icône d’avertissement indique qu’il existe un problème de conversion d’au moins une colonne de données à partir des résultats de la requête à un type de données compatibles dans la table de destination.
...
## <a name="review-the-data-type-mappings"></a>Passez en revue les mappages de types de données  
 La section centrale de la page **Vérifier le mappage de type de données** contient la liste **Mappage de type de données** . Cette grille inclut des informations de conversion détaillées concernant les colonnes de la table source qui est sélectionné dans le **Table** liste dans la partie supérieure de la page.

Dans cet exemple, chaque colonne dans la source de doivent être copiée dans une colonne avec le même nom et le type de données à la destination. L’icône d’avertissement sur la première ligne dans le **mappage de type de données** liste indique que le mappage à partir de la **int** type de données de la colonne source à la **smalldatetime** type de données de la colonne de destination peut provoquer une perte de données.
...
Cet exemple montre les valeurs par défaut pour les deux options de gestion des erreurs global.
