---
title: "Composant de flux de données, les méthodes de conception | Documents Microsoft"
ms.custom: 
ms.date: 03/14/2017
...
> [!NOTE]  
>  Le <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSCustomProperty100> prise en charge pour les valeurs de propriété de type est limitée à interface **objet**. Le seul objet que vous pouvez stocker en tant que valeur d'une propriété personnalisée est un tableau de types simples tels que des chaînes ou des entiers.  
  
 Vous pouvez indiquer que votre propriété personnalisée prend en charge les expressions de propriété en définissant la valeur de sa <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSCustomProperty100.ExpressionType%2A> propriété **CPET_NOTIFY** à partir de la <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.DTSCustomPropertyExpressionType> énumération, comme illustré dans l’exemple suivant. Vous ne devez pas ajouter de code pour gérer ou valider l'expression de propriété entrée par l'utilisateur. Vous pouvez définir une valeur par défaut pour la propriété, validez sa valeur, ainsi que lire et utiliser normalement sa valeur.  
...
```  
 ...
 Vous pouvez spécifier une boîte de dialogue d'éditeur personnalisée pour la valeur de votre propriété personnalisée en utilisant la propriété <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSCustomProperty100.UITypeEditor%2A>, comme indiqué dans l'exemple suivant. Tout d’abord, vous devez créer un type personnalisé qui hérite de l’éditeur **System.Drawing.Design.UITypeEditor**, si vous ne trouvez pas une classe de l’éditeur de type interface utilisateur existante qui correspond à vos besoins.  
...
```  
 ...
 Pour plus d’informations, consultez « Implémentation d’un éditeur de Type l’interface utilisateur » dans le [MSDN Library](http://go.microsoft.com/fwlink/?LinkId=7022).  
