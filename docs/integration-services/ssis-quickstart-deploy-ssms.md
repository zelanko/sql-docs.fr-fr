...
3.  Cliquez avec le bouton droit sur le nœud **Projets** et sélectionnez **Déployer le projet**. L’Assistant Déploiement d’Integration Services s’ouvre. Vous pouvez déployer un projet à partir du catalogue actif ou du système de fichiers.
...
3.  Dans la page **Sélectionner la destination**, sélectionnez la destination du projet.
    -   Entrez le nom complet du serveur. Si le serveur cible est un serveur Azure SQL Database server, le nom est au format suivant : `<server_name>.database.windows.net`.
