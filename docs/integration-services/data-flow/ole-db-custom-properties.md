...
|OpenRowset|Chaîne|Quand AccessMode est défini sur **OpenRowset**, nom de la table ou de la vue à laquelle la destination OLE DB a accès.|  
|OpenRowsetVariable|Chaîne|Quand AccessMode est défini sur **OpenRowset à partir de Variable**, nom de la variable contenant le nom de la table ou de la vue à laquelle la destination OLE DB a accès.|  
|SqlCommand|Chaîne|Quand AccessMode est défini sur **Commande SQL**, instruction Transact-SQL que la destination OLE DB utilise pour spécifier les colonnes de destination pour les données.|  
