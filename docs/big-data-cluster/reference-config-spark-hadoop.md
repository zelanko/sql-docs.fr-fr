---
title: Propriétés de configuration Apache Spark et Apache Hadoop
titleSuffix: SQL Server big data clusters
description: Article de référence sur les propriétés de configuration d’Apache Spark et Apache Hadoop (HDFS).
author: MikeRayMSFT
ms.author: mikeray
...
ms.locfileid: "88645750"
---
# <a name="apache-spark--apache-hadoop-hdfs-configuration-properties"></a>Propriétés de configuration d’Apache Spark et Apache Hadoop (HDFS)

[!INCLUDE[SQL Server 2019](../includes/applies-to-version/sqlserver2019.md)]

Pour configurer Apache Spark et Apache Hadoop dans des clusters Big Data, vous devez modifier le profil des clusters (bdc.json) au moment du déploiement.

## <a name="supported-configurations"></a>Configurations prises en charge
Les clusters Big Data utilisent les mêmes valeurs de configuration par défaut que le projet open source respectif.
...
> [!NOTE]
> Il existe certains paramètres de configuration pour lesquels nous modifions la valeur par défaut. Le code JSON ci-dessous répertorie ces paramètres et leurs valeurs par défaut. Ces paramètres peuvent toujours être modifiés. Pour les paramètres non répertoriés dans le JSON, reportez-vous aux liens ci-dessus.

> [!NOTE]
> Pour inclure Spark dans le pool de stockage, définissez la valeur booléenne `includeSpark` dans le fichier de configuration `bdc.json` à `spec.resources.storage-0.spec.settings.spark`. Consultez [Configurer Apache Spark et Apache Hadoop dans les clusters Big Data](configure-spark-hdfs.md) pour plus d’instructions.


###  <a name="big-data-clusters-specific-default-configurations"></a>Configurations par défaut spécifiques des clusters Big Data
Les paramètres répertoriés dans le JSON sont les valeurs par défaut spécifiques au cluster Big Data. Ils peuvent être modifiés au moment du déploiement, si nécessaire. Ils sont répertoriés ici, car ils diffèrent de leurs configurations de projet par défaut respectives.
