...
Pour utiliser le stockage persistant pendant le déploiement, définissez les valeurs de **usePersistentVolume** clé *true* et **className** clés pour le nom de la classe de stockage que vous souhaitez utiliser pour le pool respectif. Vous pouvez également personnaliser la taille des revendications de volume persistant créé dans le cadre du déploiement. En tant que meilleure pratique, nous vous recommandons d’utiliser les classes de stockage avec un *conserver* [récupérer de la stratégie](https://kubernetes.io/docs/concepts/storage/storage-classes/#reclaim-policy).

> [!NOTE]
> Dans CTP 2.5, vous ne pouvez pas modifier post-déploiement de paramètre de configuration stockage. En outre, seuls `ReadWriteOnce` mode d’accès pour l’ensemble du cluster est prise en charge.
...
> [!NOTE]
> Dans le texte généré dans le fichier de configuration de déploiement pour kubeadm *kubeadm-dev-test.json*, la valeur par défaut **usePersistentVolume** clé est *true*, vous devez donc définir la valeur pour **className** sinon les validations avant le déploiement échouera. Déploiement a également une étape de validation qui vérifie l’existence de la classe de stockage, mais pas pour les volumes persistants nécessaires. Vous devez vous assurer de que vous créez suffisamment volumes en fonction de l’échelle de votre cluster. Dans CTP2.5, pour la taille de cluster par défaut, vous devez créer au moins 23 volumes. [Ici](https://github.com/Microsoft/sql-server-samples/tree/master/samples/features/sql-big-data-cluster/deployment/kubeadm/ubuntu) est un exemple sur la façon de créer des volumes persistants à l’aide du fournisseur local.
...
L’exemple suivant met à jour uniquement la taille de volume persistant des revendications dans le pool de stockage à Gi 32 :
