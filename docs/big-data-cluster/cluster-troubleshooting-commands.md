...
> [!TIP]
> Il s’agit d’une façon d’afficher les services avec **kubectl**, mais il est également possible d’utiliser `mssqlctl cluster endpoints list` commande pour afficher ces points de terminaison. Pour plus d’informations, consultez [obtenir les points de terminaison de cluster big data](deployment-guidance.md#endpoints).
