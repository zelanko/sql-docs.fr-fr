---
title: Applies SQL Server | Microsoft Docs
ms.custom: ''
ms.date: 07/27/2017
ms.prod: sql
...
ms.locfileid: "39228652"
---
# <a name="sql-applies-to-and-includes"></a>« Applies to » et « Includes » dans SQL
Les références dans la documentation peuvent être facilement modifiées sans changer le texte d’articles individuels à l’aide d’« Includes » dans Markdown. Il existe trois types d’« includes » dans le monde du contenu SQL : Versions de SQL, Applies-to et texte de référence. Les versions SQL sont utilisées pour indiquer la version de SQL dont il est question (par exemple, SQL 2016 vs 2017), et applies-to indique à quelle version de SQL Server le document s’applique (SQL Server sur Linux vs Azure SQL Database). Le texte référentiel contient des includes qui ne font pas partie des deux autres catégories, par exemple l’include Get Help, une liste de liens que les clients peuvent utiliser afin d’obtenir une aide pour SQL. 

Cet article est destiné à être utilisé comme point de référence pour les deux premiers types d’includes. 

## <a name="sql-server-version-includes"></a>Includes de la versions de SQL Server

Les rédacteurs de contenu SQL ont souvent besoin d’inclure le nom du produit et de la version de SQL Server. De cette façon, si quelque chose change dans le nom, l’include est mis à jour au lieu de mettre à jour manuellement de la valeur dans chacun des articles. Ces fichiers Include sont utilisés comme espaces réservés pour les noms de produits, mais n’ont pas été utilisées de manière cohérente dans toute la documentation SQL. SQL Server vNext fait référence à une version future de SQL qui n’a pas encore d’un numéro de version et fait exception à cela.  
...
## <a name="sql-server-non-version-specific-applies-to"></a>applies-to non spécifique à une version de SQL Server
...
## <a name="sql-server-version-specific-applies-to"></a>applies-to spécifique à une version de SQL Server
Applies-to spécifie ici les versions de SQL auxquelles s’applique la documentation. 
