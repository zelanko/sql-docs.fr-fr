...
-   Nous recommandons d’exécuter [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] sur des ordinateurs dotés du format de fichier NTFS ou ReFS. L’installation de [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] sur un système de fichiers FAT32 est prise en charge mais n’est pas recommandée car elle est moins sécurisée que le système de fichiers NTFS ou ReFS.  
...
-   [Configuration matérielle et logicielle requise](../../sql-server/install/hardware-and-software-requirements-for-installing-sql-server.md#hwswr)  
  
-   [Processeur, mémoire et système d’exploitation requis](../../sql-server/install/hardware-and-software-requirements-for-installing-sql-server.md#pmosr)  
  
-   [Prise en charge de langues différentes](../../sql-server/install/hardware-and-software-requirements-for-installing-sql-server.md#CrossLanguageSupport)  
  
-   [Espace disque nécessaire](../../sql-server/install/hardware-and-software-requirements-for-installing-sql-server.md#HardDiskSpace)  
  
-   [Types de stockage pour les fichiers de données](../../sql-server/install/hardware-and-software-requirements-for-installing-sql-server.md#StorageTypes)  
  
-   [Installation de SQL Server sur un contrôleur de domaine](../../sql-server/install/hardware-and-software-requirements-for-installing-sql-server.md#DC_support)  
...
|Internet|Les fonctionnalités Internet requièrent un accès à Internet (des frais peuvent s'appliquer).|  
 ...
 Remarque : l’exécution de [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] sur une machine virtuelle est plus lente qu’un fonctionnement en mode natif en raison de la charge de la virtualisation.  
...
> [!NOTE]  
>  L’installation de [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] est prise en charge uniquement sur les processeurs x64. Elle n’est plus prise en charge sur les processeurs x86.  
  
 *La mémoire minimale requise pour installer le composant [!INCLUDE[ssDQSServer](../../includes/ssdqsserver-md.md)] dans [!INCLUDE[ssDQSnoversion](../../includes/ssdqsnoversion-md.md)] DQS est de 2 Go de RAM, qui est différente de la mémoire minimale nécessaire pour la condition de mémoire minimum de [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] . Pour obtenir des informations sur l'installation de DQS, consultez [Install Data Quality Services](../../data-quality-services/install-windows/install-data-quality-services.md).  
  
 **Prise en charge de la configuration WOW64 :**  
  
 WOW64 (Windows 32 bits sur Windows 64 bits) est une fonctionnalité des éditions 64 bits de Windows qui permet aux applications 32 bits de s'exécuter de façon native en mode 32 bits. Les applications fonctionnent en mode 32 bits, même si le système d'exploitation sous-jacent est un système 64 bits. WOW64 n’est pas pris en charge pour les installations [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] . Les outils de gestion sont cependant pris en charge dans WOW64.  
  
 **Prise en charge du système d’exploitation :**  
  
 Les éditions de [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] sont classées comme suit :  
  
-   [Principales éditions de SQL Server 2016](../../sql-server/install/hardware-and-software-requirements-for-installing-sql-server.md#TOP_Principal)  
  
-   [Éditions transversales de SQL Server 2016](../../sql-server/install/hardware-and-software-requirements-for-installing-sql-server.md#TOP_Breadth)  
  
> [!NOTE]  
>  Les fonctionnalités de Business Intelligence suivantes ne sont pas prises en charge par le système d’exploitation. Ces fonctionnalités peuvent être installées sur Windows Server 2008 R2 SP1 ou version ultérieure.  
...
##  <a name="HardDiskSpace"></a> Espace disque nécessaire  
 Pendant l'installation de [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)], Windows Installer crée des fichiers temporaires sur le lecteur système. Avant d'exécuter le programme d'installation ou de mise à niveau de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], vérifiez que vous disposez d'au moins 6,0 Go d'espace disponible sur le lecteur système pour ces fichiers. Cette configuration requise s'applique même lorsque vous installez tous les composants [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sur un lecteur différent du lecteur par défaut.  
  
 L'espace disque nécessaire peut varier selon la configuration du système et les fonctionnalités installées. Le tableau suivant présente l'espace disque nécessaire pour les composants [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] .  
