---
title: Référence du langage Analysis Services | Microsoft Docs
ms.date: 06/04/2018
ms.prod: sql
ms.technology: analysis-services
ms.custom: mdx
ms.topic: reference
ms.author: owend
ms.reviewer: owend
author: minewiskan
manager: kfile
ms.openlocfilehash: 3c51a9b1364131630daddabec316c01992ffedfd
ms.sourcegitcommit: f7fced330b64d6616aeb8766747295807c92dd41
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 04/23/2019
ms.locfileid: "62521386"
---
# <a name="analysis-services-language-reference"></a>Référence du langage Analysis Services


  [Référence DAX (Data Analysis Expressions)](https://msdn.microsoft.com/70a82136-0926-4a91-bcb3-e18e82593b0d)  
  
 [Référence MDX &#40;Multidimensional Expressions&#41;](../mdx/multidimensional-expressions-mdx-reference.md)  
  
 [Informations de référence sur le langage DMX &#40;Data Mining Extensions&#41;](../dmx/data-mining-extensions-dmx-reference.md)  
  
 [Fonctions VBA dans MDX et DAX](../mdx/vba-functions-in-mdx-and-dax.md)  
  
## <a name="see-also"></a>Voir aussi  
 [Référence PowerShell pour PowerPivot pour SharePoint](../analysis-services/powershell/powershell-reference-for-power-pivot-for-sharepoint.md)   
 [Référence PowerShell Analysis Services](../analysis-services/powershell/analysis-services-powershell-reference.md)   
 [Procédures stockées d’exploration de données &#40;Analysis Services - Exploration de données&#41;](../analysis-services/data-mining/data-mining-stored-procedures-analysis-services-data-mining.md)  
  
  







