...
##  <a name="Top"></a> Dans ce Guide  
 [Principes de base de transaction](#Basics)  
  
 [Verrouillage et concepts de base de contrôle de version de ligne](#Lock_Basics)  
...
##  <a name="Basics"></a> Principes de base sur les transactions  
 ...
 Durabilité  
 Lorsqu'une transaction durable est terminée, ses effets sur le système sont permanents. Les modifications sont conservées même en cas de défaillance du système. [!INCLUDE[ssSQL14](../includes/sssql14-md.md)]et versions ultérieures permettent les transactions à durabilité retardée. Les transactions durables retardées sont validées avant de consigner l'enregistrement du journal des transactions sur le disque. Pour plus d’informations sur la durabilité des transactions retardées, consultez la rubrique [Durabilité des transactions](../relational-databases/logs/control-transaction-durability.md).  
...
-   Lectures manquantes et en double provoquées par des mises à jour de ligne  
    ...
    -   Manquer une ou plusieurs lignes qui n'étaient pas la cible de la mise à jour  
         ...
         Lorsque vous utilisez READ UNCOMMITTED, si votre requête lit des lignes à l’aide d’une analyse d’ordre d’allocation (à l’aide des pages IAM), vous risquez de manquer des lignes si une autre transaction provoque un fractionnement de page. Cela ne peut pas se produire lorsque vous utilisez la lecture validée car un verrou de table est maintenu pendant un fractionnement de page et ne se produit pas si la table n'a pas d'index cluster, car les mises à jour ne provoquent pas de fractionnements de page.  
...
|Mise à jour en bloc (BU)|Utilisé lorsque en bloc copie des données dans une table et la **TABLOCK** indicateur est spécifié.|  
...
>  Un verrou intent exclusif (IX) est compatible avec un mode de verrouillage IX car IX signifie l'intention de mettre à jour uniquement certaines lignes et non pas toutes les lignes. Les autres transactions qui essaient de lire ou de mettre à jour certaines lignes sont aussi autorisées si elles ne mettent pas à jour les lignes en cours de mise à jour par les autres transactions. De plus, si deux transactions essaient de mettre à jour la même ligne, un verrou IX sera accordé aux deux transactions au niveau de la table et de la page. Toutefois, un verrou X sera accordé à une transaction au niveau de la ligne. L'autre transaction doit attendre jusqu'à ce que le verrouillage au niveau de la ligne soit supprimé.  
 ...
 ![Matrice de compatibilité de verrouillage diagramme montrant](media/lockconflicttable.gif "matrice de compatibilité de verrouillage affichage Diagramme")  
...
#### Exemples  
 ...
 ![Table de base de données avec illustration de b-tree d’index](media/btree4.gif "table de base de données avec illustration de b-tree d’index")  
...
### Verrouillage dynamique  
 ...
 ![Diagramme affichant le coût par rapport à la granularité](media/lockcht.gif "diagramme montrant les coûts par rapport à la granularité")  
...
-   La transaction B demande un verrou exclusif sur la ligne 1, mais elle est bloquée jusqu'à la fin de la transaction A qui libérera le verrou partagé sur la ligne 1.  
 ...
 ![Diagramme montrant le blocage de transaction](media/dedlck1.gif "diagramme montrant le blocage de transaction")  
...
-   **Ressources MARS (Multiple Active Result Sets)**. Ces ressources servent à contrôler l'entrelacement de plusieurs demandes actives sous MARS. Pour plus d’informations, consultez [Multiple Active Result Sets (MARS) dans SQL Server](http://msdn.microsoft.com/library/ms345109(v=SQL.90).aspx).  
 ...
 ![Blocage de processus d’utilisateur d’affichage Diagramme logique. ] (media/udb9-logicflowexamplec.gif "Blocage de processus logique utilisateur de l’affichage Diagramme.")  
...
###### Evénement Deadlock Graph de SQL Profiler  
 ...
 ![Blocage de processus logique diagramme montrant utilisateur. ] (media/udb9-profilerdeadlockgraphc.gif "Blocage de processus logique utilisateur de l’affichage Diagramme de flux.")  
  
 Pour plus d’informations sur l’exécution du [!INCLUDE[ssSqlProfiler](../includes/sssqlprofiler-md.md)] deadlock graph, consultez [enregistrer les événements Deadlock Graph &#40;SQL Server Profiler&#41;](../relational-databases/performance/save-deadlock-graphs-sql-server-profiler.md).  
...
##### Accès aux objets dans le même ordre  
 ...
 ![Diagramme montrant la prévention de blocage](media/dedlck2.gif "prévention de blocage d’affichage des diagrammes")  
...
#### Présentation du partitionnement de verrous  
...
#### Mise en œuvre et surveillance du partitionnement de verrous  
...
##  <a name="Row_versioning"></a> Niveaux d’isolement basé sur le contrôle de version de ligne dans le moteur de base de données  
...
#### A. Utilisation du niveau d'isolement d'instantané  
...
#### B. Utilisation d'une transaction validée en lecture à l'aide du contrôle de version de ligne  
...
### Activation des niveaux d'isolement selon le contrôle de version de ligne  
...
### Utilisation de niveaux d'isolement basés sur le contrôle de version de ligne  
...
>  Dans [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], utilisez la vue de gestion dynamique **sys.dm_os_waiting_tasks** pour déterminer si un processus est bloqué et identifier l’auteur du blocage. Dans les versions antérieures de [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], utilisez la procédure stockée système **sp_who**.  
 ...
 Pour déterminer le paramètre LOCK_TIMEOUT actuel, exécutez le @@LOCK_TIMEOUT fonction :  
...
```  
 ...
 Chaque appel à COMMIT TRANSACTION ou COMMIT WORK s'applique à la dernière instruction BEGIN TRANSACTION exécutée. Si les instructions BEGIN TRANSACTION sont imbriquées, une instruction COMMIT s'applique uniquement à la dernière transaction imbriquée, qui est la transaction la plus interne. Même si une TRANSACTION de validation *argument* instruction dans une transaction imbriquée fait référence à la transaction de la transaction externe, la validation s’applique uniquement à la transaction la plus interne.  
  
 Il n’est pas autorisée pour le *argument* paramètre d’une instruction ROLLBACK TRANSACTION pour faire référence aux transactions internes d’un ensemble de transactions nommées imbriquées. *transaction_name* ne peut faire référence qu’au nom de la transaction la plus extérieure. Si une instruction ROLLBACK TRANSACTION *transaction_name* utilisant le nom de la transaction externe est exécutée à n’importe quel niveau d’un ensemble de transactions imbriquées, toutes les transactions imbriquées sont restaurées. Si une instruction ROLLBACK WORK ou ROLLBACK TRANSACTION sans un *argument* paramètre est exécuté sur n’importe quel niveau d’un ensemble de transactions imbriquées, il restaure toutes les transactions imbriquées, y compris la transaction la plus extérieure.  
  
 Le @@TRANCOUNT fonction enregistre le niveau d’imbrication actuel de la transaction. Chaque instruction BEGIN TRANSACTION incrémente@TRANCOUNT d’une unité. Chaque décrémente d’instruction COMMIT TRANSACTION ou COMMIT WORK @@TRANCOUNT d’une unité. Un ROLLBACK WORK ou une instruction ROLLBACK TRANSACTION qui n’a pas un nom de transaction restaure les transactions imbriquées et décrémente @@TRANCOUNT à 0. Une instruction ROLLBACK TRANSACTION qui utilise le nom de la transaction de la transaction la plus externe d’un ensemble de transactions imbriquées restaure toutes les transactions imbriquées et décrémente @@TRANCOUNT à 0. Lorsque vous savez pas si vous êtes déjà dans une transaction, sélectionnez@TRANCOUNT pour déterminer si elle est de 1 ou plus. If @@TRANCOUNT est 0, vous n’êtes pas dans une transaction.  
...
### Utilisation de sessions associées  
 ...
 Pour participer à une session liée, une session appelle **sp_getbindtoken** ou **srv_getbindtoken** (via Open Data Services) pour obtenir le jeton une liaison. Un jeton d'association est une chaîne de caractères qui identifie de manière unique chaque transaction associée. Le jeton d'association est ensuite transmis aux autres sessions à associer à la session en cours. Les autres sessions se lient à la transaction en appelant **sp_bindsession** avec le jeton de liaison reçu de la première session.  
  
> [!NOTE]  
>  Une session doit posséder une transaction utilisateur active pour **sp_getbindtoken** ou **srv_getbindtoken** réussisse.  
...
-   Si une instance de serveur est arrêtée après une transaction active a effectué de nombreuses modifications non validées, la phase de récupération du redémarrage suivant peut être beaucoup plus longue que la durée spécifiée par la **l’intervalle de récupération** server option de configuration ou par l’instruction ALTER DATABASE... SET TARGET_RECOVERY_TIME. Ces options contrôlent la fréquence des points de contrôle actifs et indirects, respectivement. Pour plus d’informations sur les types de points de contrôle, consultez [Points de contrôle de base de données &#40;SQL Server&#41;](../relational-databases/logs/database-checkpoints-sql-server.md).  
  
-   Plus important, bien qu'une transaction en attente génère très peu d'entrées de journal, elle empêche la troncation du journal et entraîne ainsi sa croissance et son remplissage. Si le journal des transactions est rempli, la base de données ne peut plus effectuer de mises à jour. Pour plus d’informations, consultez [dépanner un journal des transactions saturé &#40;erreur SQL Server 9002&#41;](../relational-databases/logs/troubleshoot-a-full-transaction-log-sql-server-error-9002.md), et [du journal des transactions &#40;SQL Server&#41;](../relational-databases/logs/the-transaction-log-sql-server.md).  
...
## Voir aussi  
 [SQL Server 2005 Row Versioning des transactions Isolation](http://msdn.microsoft.com/library/ms345124(v=sql.90).aspx)   
 ...
 [Comment créer une transaction autonome dans SQL Server 2008](http://blogs.msdn.com/b/sqlprogrammability/archive/2008/08/22/how-to-create-an-autonomous-transaction-in-sql-server-2008.aspx)  
