...
> [!NOTE]  
>  A [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] connexion ne peut pas accéder à une base de données dans laquelle il ne dispose pas d’un utilisateur de base de données correspondante, sauf si **invité** est activée dans cette base de données. Pour plus d’informations sur la création d’un compte d’utilisateur de base de données, consultez [CREATE USER &#40;Transact-SQL&#41;](/sql/t-sql/statements/create-user-transact-sql).  
