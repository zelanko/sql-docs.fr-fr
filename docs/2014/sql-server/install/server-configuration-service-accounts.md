---
title: Configuration du serveur - comptes de Service | Documents Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
## <a name="options"></a>Options  
 ...
 Sur Windows 7 et [!INCLUDE[nextref_longhorn](../../includes/nextref-longhorn-md.md)] R2, la plupart des comptes par défaut à un compte virtuel.  
  
 Si vous configurez des services pour utiliser des comptes de domaine, [!INCLUDE[msCoName](../../includes/msconame-md.md)] recommande de configurer les comptes de service individuellement afin de fournir des privilèges moindres pour chaque service, où [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] services sont accordés au minimum les autorisations nécessaires à l’exécution leurs tâches. Pour plus d’informations et les descriptions des types de comptes, consultez [configurer les comptes de Service Windows et les autorisations](../../database-engine/configure-windows/configure-windows-service-accounts-and-permissions.md).  
  
 **Configurer [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] comptes de service individuellement (recommandé)**  
 Utilisez la grille pour configurer chaque [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] service avec un nom d’utilisateur d’ouverture de session et le mot de passe et pour définir le type de démarrage pour le service. Vous pouvez utiliser des comptes système intégrés, un compte local, groupe local, groupe de domaine ou comptes d’utilisateur de domaine pour [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] services.  
...
|-------------------------|----------------------------------------------|  
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent|Le service qui exécute les travaux, surveille, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]et autorise l’automatisation des tâches d’administration.<br /><br /> Il n'y a aucun compte d'ouverture de session par défaut pour ce service.<br /><br /> Le type de démarrage par défaut est Manuel.|  
