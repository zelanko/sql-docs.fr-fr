---
title: Exemple Script Reporting Services rs.exe pour migrer le contenu entre les serveurs de rapports | Documents Microsoft
ms.custom: ''
ms.date: 07/27/2015
...
# <a name="sample-reporting-services-rsexe-script-to-migrate-content-between-report-servers"></a>Exemple de script Reporting Services rs.exe pour migrer le contenu entre des serveurs de rapports
  Cette rubrique présente et décrit un exemple [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] script RSS qui copie les éléments de contenu et des paramètres à partir d’un [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] serveur de rapports sur un autre serveur de rapports à l’aide de la **RS.exe** utilitaire. RS.exe est installé avec [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)], en mode natif et SharePoint. Le script copie les éléments de [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)], par exemple les rapports et les abonnements, de serveur à serveur. Il prend en charge le mode SharePoint et les serveurs de rapports en mode natif.  
...
-   [Plus d'exemples](#bkmk_more_examples)  
    ...
    -   [Mode natif vers mode natif – Machine virtuelle Windows Azure](#bkmk_native_to_native_Azure_vm)  
  
    -   [Mode SharePoint – Collection de sites « bi » vers un serveur en mode natif sur une machine virtuelle Windows Azure](#bkmk_sharepoint_site_to_native_Azure_vm)  
...
-   [!INCLUDE[ssKilimanjaro](../../../includes/sskilimanjaro-md.md)]  
 ...
 Il peut être utilisé pour copier le contenu entre des serveurs de rapports ayant le même mode ou des modes différents. Par exemple, vous pouvez exécuter le script pour copier le contenu à partir d’un [!INCLUDE[ssKilimanjaro](../../../includes/sskilimanjaro-md.md)] le serveur de rapports en mode natif à un [!INCLUDE[ssSQL11SP1](../../includes/sssql11sp1-md.md)] serveur de rapports en mode SharePoint. Exécutez le script à partir de n'importe quel serveur sur lequel RS.exe est installé. Par exemple, dans le déploiement suivant, vous pouvez :  
...
|Mes rapports|**Non**|**Non**|La fonctionnalité « Mes rapports » du mode natif utilise les connexions utilisateur individuelles, par conséquent, le service de script n’a pas accès au contenu des dossiers « Mes rapports » pour les utilisateurs, sauf pour le paramètre **–u** utilisé pour exécuter le script rss. En outre, « Mes rapports » n'est pas une fonctionnalité du mode SharePoint de [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] et les éléments dans les dossiers ne peuvent pas être copiés dans un environnement SharePoint. Par conséquent, le script ne copie pas les éléments de rapport qui se trouvent dans les dossiers « Mes rapports » sur un serveur de rapports en mode natif source. Pour migrer le contenu des dossiers « Mes rapports » avec ce script, procédez comme suit :<br /><br /> (1) créez de nouveaux dossiers dans le Gestionnaire de rapports. Éventuellement, créez des dossiers ou des sous-dossier pour chaque utilisateur.<br /><br /> 2) connexion en tant qu’un des utilisateurs avec un contenu « Mes rapports ».<br /><br /> (3) dans le Gestionnaire de rapports, cliquez sur le **Mes rapports** dossier.<br /><br /> 4) cliquez sur le **détails** affichage pour le dossier.<br /><br /> 5), sélectionnez chaque rapport que vous voulez copier.<br /><br /> 6) cliquez sur **déplacer** dans la barre d’outils du Gestionnaire de rapports.<br /><br /> 7), sélectionnez le dossier de destination souhaité.<br /><br /> 8) Répétez les étapes 2 à 7 pour chaque utilisateur.<br /><br /> 9) exécutez le script.|  
|Historique|**Non**|**Non**||  
...
-   Les SOURCE_URL and TARGET_URL doivent être des URL de serveur de rapports valides qui pointent vers la source et cible [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] serveur de rapports. En mode natif, une URL de serveur de rapports est similaire à ce qui suit :  
