...
## <a name="enabling-and-disabling-report-builder"></a>Activation et désactivation du Générateur de rapports  
 Le Générateur de rapports est activé par défaut. Les administrateurs de serveur de rapports ont la possibilité de désactiver la fonctionnalité de générateur de rapports en définissant la propriété de système de serveur de rapports `EnableReportDesignClientDownload` à `false`. Les téléchargements du Générateur de rapports effectués pour ce serveur de rapports sont ainsi désactivés.  
