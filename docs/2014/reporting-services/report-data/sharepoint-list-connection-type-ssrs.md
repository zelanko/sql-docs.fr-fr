...
##  <a name="Credentials"></a>Informations d’identification  
 ...
 Le **tableau 1** concerne les rapports déployés sur un site Windows SharePoint hérité. Un site Windows hérité prend en charge uniquement l'authentification Kerberos, NTLM et à base de formulaires (FBA). Le **tableau 2** concerne les rapports déployés sur un site SharePoint basé sur les revendications.  
 ...
 **Les informations d’identification ne sont pas requises (ne pas utiliser les informations d’identification) :**  
...
-   **Masqué** Indique si le champ s’affiche dans l’affichage de liste SharePoint.  
...
-   **Filtres appliqués**  Limite les données retournées à partir de la liste SharePoint, avant que les données ne soient retournées au rapport. Sélectionnez le nom de champ, l'opérateur et la valeur à utiliser pour limiter les données récupérées dans la liste. Les opérateurs varient selon le type de données de la valeur que vous sélectionnez.  
...
-   **Résultats** de la requête  Affiche des exemples de lignes qui sont retournées lors de l’exécution de la requête. Si les valeurs de liste SharePoint changent fréquemment sur le site SharePoint, les valeurs que vous voyez dans le volet des résultats de la requête peuvent différer de celles que vous voyez dans le rapport.  
