...
6.  Ajoutez l’entrée suivante au fichier RSReportDesigner.config sous le `Designer` élément. Vous devez remplacer uniquement le `Name` attribut avec le nom que vous avez fourni dans les entrées précédentes.  
...
4.  Ajouter un groupe de codes pour le [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] assembly de fournisseur de données qui octroie `FullTrust` autorisation. Votre groupe de codes doit ressembler au code suivant :  
