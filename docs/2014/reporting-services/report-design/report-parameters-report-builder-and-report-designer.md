 ...
 **Faire varier la présentation du rapport**  
...
##  <a name="UserInterface"></a>Volet des paramètres  
 Lorsque vous visualisez un rapport, la barre d'outils de la visionneuse de rapports affiche chaque paramètre de sorte que l'utilisateur puisse spécifier des valeurs de manière interactive. L’illustration suivante montre la zone de paramètres d’un rapport qui a @StartDatedes @EndDateparamètres @Subcategory,, @ShowAllRowset.  
...
2.  **@StartDateparamètres @EndDate et** le paramètre @StartDate est le type `DateTime`de données. L'invite de la date de début s'affiche en regard de la zone de texte. Pour modifier la date, tapez une nouvelle date dans la zone de texte ou utilisez le contrôle calendrier.  
  
     Le paramètre @EndDate apparaît en regard @StartDatede.  
  
3.  ** @Subcategory paramètre** Le paramètre @Subcategory est le type `Text`de données. Étant @Subcategory donné que contient une liste de valeurs disponibles, les valeurs valides s’affichent dans une liste déroulante. Vous devez choisir des valeurs dans cette liste. Étant @Subcategory donné que est à valeurs multiples, une option **Sélectionner tout** s’affiche pour vous permettre d’effacer tout et de sélectionner toutes les valeurs de la liste.  
  
4.  ** @ShowAllRows paramètre** Le paramètre @ShowAllRows est le type `Boolean`de données. Utilisez les cases d'option pour spécifier `True` ou `False`.  
  
5.  **Handle d’affichage ou de masquage de la zone de paramètres** Dans la barre d’outils de la visionneuse de rapports, cliquez sur cette flèche pour afficher ou masquer le volet Paramètres.  
  
6.  **Bouton Paramètres** Dans Générateur de rapports aperçu, sur le ruban, cliquez sur le bouton **paramètres** pour afficher ou masquer le volet Paramètres.  
  
7.  **Bouton afficher le rapport** Dans la barre d’outils de la visionneuse de rapports, cliquez sur **afficher le rapport** pour exécuter le rapport après avoir entré les valeurs des paramètres. Si tous les paramètres possèdent des valeurs par défaut, le rapport s'exécute automatiquement au premier affichage.  
...
|Type de données|Lorsque des valeurs disponibles sont définies pour un paramètre, l’utilisateur choisit des valeurs dans une liste déroulante, même si le type `DateTime`de données est. Un paramètre de rapport doit avoir l'un des types de données suivants :<br /><br /> `Boolean`. L'utilisateur sélectionne True ou False à l'aide d'une case d'option.<br /><br /> `DateTime`. L'utilisateur sélectionne une date à l'aide d'un contrôle calendrier.<br /><br /> **Entier**. L'utilisateur tape des valeurs dans une zone de texte.<br /><br /> Valeur **float**. L'utilisateur tape des valeurs dans une zone de texte.<br /><br /> `Text`. L'utilisateur tape des valeurs dans une zone de texte.<br /><br /> Pour plus d'informations sur les types de données de rapport, consultez [RDL Data Types](../reports/report-definition-language-ssrs.md#bkmk_RDL_Data_Types).|  
...
##  <a name="bkmk_Related_Topics"></a> Contenu associé  
 [Configuration des paramètres de rapport SSRS (Quiz)](https://www.trenovision.com/quiz/sql-server-reporting-services-ssrs-quiz/)  
