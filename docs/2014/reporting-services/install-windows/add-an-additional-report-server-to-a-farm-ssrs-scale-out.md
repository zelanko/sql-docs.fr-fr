...
> [!IMPORTANT]  
>  Le déploiement avec montée en puissance parallèle de [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] n'est pas pris en charge par toutes les éditions de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Pour plus d’informations, consultez la [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] section de [fonctionnalités prises en charge par les éditions de SQL Server 2014](../../../2014/getting-started/features-supported-by-the-editions-of-sql-server-2014.md).  
...
-   [L’équilibrage de charge](#bkmk_loadbalancing)  
...
|Installez un produit SharePoint 2010.|(1) sélectionnez le **batterie de serveurs** le type d’installation.<br /><br /> (2) sélectionnez **Complete** pour le type de serveur.<br /><br /> 3) Lorsque l’installation est terminée, n’exécutez pas l’Assistant Configuration des produits SharePoint si SharePoint 2010 SP1 est installé sur votre batterie de serveurs SharePoint existante. Vous devez installer SharePoint SP1 avant d'exécuter l'Assistant Configuration des produits SharePoint.|  
|Installer SharePoint Server 2010 SP1|Si votre batterie de serveurs SharePoint existante a installé SharePoint 2010 SP1 Téléchargez et installez SharePoint 2010 SP1 à partir de :[http://support.microsoft.com/kb/2460045](http://go.microsoft.com/fwlink/p/?linkID=219697).<br /><br /> Pour plus d'informations sur SharePoint 2010 SP1, consultez [Problèmes connus lors de l'installation d'Office 2010 SP1 et de SharePoint 2010 SP1](http://support.microsoft.com/kb/2532126):|  
|Exécutez l'Assistant Configuration des produits SharePoint pour ajouter le serveur à la batterie.|(1) dans le **produits Microsoft SharePoint 2010** groupe de programmes, cliquez sur **Assistant Configuration des produits Microsoft SharePoint 2010**.<br /><br /> 2) dans le **se connecter à une batterie de serveurs** page Sélectionnez **se connecter à une batterie de serveurs existante** et cliquez sur **suivant**.<br /><br /> 3) dans le **spécifier les paramètres de Configuration de base de données** , tapez le nom du serveur de base de données utilisé pour la batterie de serveurs existant et le nom de la base de données de configuration. Cliquez sur **Suivant**.<br />**\*\* Important \* \***  si vous voyez un message d’erreur semblable au suivant et que vous avez vérifié que vous disposez des autorisations, puis vérifiez les protocoles activés pour la Configuration du réseau SQL Server dans **Sql Server Gestionnaire de configuration**: « Impossible de se connecter au serveur de base de données. Vérifiez que la base de données existe, est un serveur Sql Server, et que vous disposez des autorisations appropriées pour accéder au serveur. »<br />**\*\* Important \* \***  si vous voyez la page **des produits et état des correctifs**, vous devez examiner les informations sur la page et mettre à jour le serveur avec les fichiers nécessaires avant de pouvoir continuer à joindre le serveur à la batterie de serveurs.<br /><br /> 4) dans le **spécifier les paramètres de sécurité batterie** page tapez la phrase secrète de batterie de serveurs, puis cliquez sur **suivant**. Cliquez sur **Suivant** dans la page de confirmation pour exécuter l'Assistant.<br /><br /> 5) cliquez sur **suivant** pour exécuter le **Assistant Configuration de batterie de serveurs**.|  
|Vérifiez que le serveur a été ajouté à la batterie de serveurs SharePoint.|1) Dans l’Administration centrale de SharePoint, cliquez sur **Gérer les serveurs de cette batterie** dans le groupe **Paramètres système** .<br /><br /> 2) Vérifiez que le nouveau serveur est ajouté et que l'état est correct.<br /><br /> 3) Notez que vous ne voyez pas le service **Service SQL Server Reporting Services** en cours d’exécution. Le service sera installé à l'étape suivante.<br /><br /> (4) pour supprimer ce serveur du rôle de serveur Web frontal, cliquez sur **gérer les services sur le serveur** et arrêter le service **Application Web de Microsoft SharePoint Foundation**.|  
|Installez et configurez le mode SharePoint de Reporting Services.|Exécutez l'installation de [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] . Pour plus d’informations sur l’installation de [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] mode SharePoint, consultez [Install Reporting Services SharePoint Mode for SharePoint 2010](../../../2014/sql-server/install/install-reporting-services-sharepoint-mode-for-sharepoint-2010.md) si le serveur doit uniquement être utilisé car un serveur d’applications et le serveur ne seront pas utilisés en tant que un serveur Web frontal, vous n’avez pas besoin de sélectionner **complément Reporting Services pour les produits SharePoint** sur :<br /><br /> le **rôle d’installation** page, sélectionnez **Installation de fonctionnalités SQL Server**<br /><br /> le **sélection des fonctionnalités** page, sélectionnez **Reporting Services - SharePoint**<br /><br /> -ou-<br /><br /> le **Configuration de Reporting Services** page vérifier la **installer uniquement** option est sélectionnée pour **Reporting Services SharePoint Mode**.|  
...
##  <a name="bkmk_additional"></a> Configuration supplémentaire  
 ...
 Le service Web Reporting Services n'est pas activé. Configurer au moins une instance de Reporting Services SharePoint Service avoir \<IsWebServiceEnable > défini à true. Pour plus d’informations, consultez [Modifier un fichier de configuration &#40;RSreportserver.config&#41;](../report-server/modify-a-reporting-services-configuration-file-rsreportserver-config.md).  
  
## <a name="see-also"></a>Voir aussi  
 [Ajouter des serveurs web ou d’applications aux batteries de serveurs dans SharePoint 2013](http://technet.microsoft.com/library/cc261752.aspx)   
 [Configurer les services (SharePoint Server 2010)](http://technet.microsoft.com/library/ee794878.aspx)  
