...
1.  **Sources de données dans le volet données du rapport** Une source de données apparaît dans le volet des données de rapport une fois que vous avez créé une source de données incorporée ou ajouté une source de données partagée.  
  
2.  **Boîte de dialogue Connexion** Utilisez la boîte de dialogue Connexion pour créer une chaîne de connexion ou pour coller une chaîne de connexion.  
  
3.  **Informations de connexion de données** La chaîne de connexion est passée à l’extension de données.  
  
4.  **Informations d’identification** Les informations d’identification sont gérées séparément de la chaîne de connexion.  
...
6.  **Sources de données externes** Récupérez des données de bases de données relationnelles, de bases de données multidimensionnelles, de listes SharePoint, de services Web ou de modèles de rapport.  
...
|[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]base de données d’instance|`Data Source=localhost\MSSQL12.InstanceName; Initial Catalog= AdventureWorks2012`|Définissez `SQL Server` comme type de source de données.|  
...
|Source de données XML, document XML incorporé|*Vidé*|Définissez `XML` comme type de source de données. Les données XML sont incorporées dans la définition de rapport.|  
