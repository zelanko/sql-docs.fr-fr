...
### <a name="querying-using-datetime-wrapper-functions"></a>Recherche à l'aide de fonctions Wrapper Datetime  
 ...
 Les fonctions wrapper générées prennent en charge la convention suivante pour parcourir systématiquement la chronologie de capture de données modifiées : Il est probable que le @end_time utiliser le paramètre de l’intervalle précédent en tant que le @start_time paramètre de l’intervalle suivant. La fonction wrapper se charge de mapper les valeurs datetime aux valeurs LSN et de garantir qu'aucune donnée n'est perdue ou répétée si cette convention est suivie.  
