...
> [!NOTE]  
>  L'option d'installation avancée `-k`[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] permet à un administrateur de base de données de limiter le comportement d'E/S de point de contrôle en fonction du débit du sous-système d'E/S pour certains types de points de contrôle. Le `-k` option d’installation s’applique aux points de contrôle automatique non sinon les points de contrôle manuels et internes.  
