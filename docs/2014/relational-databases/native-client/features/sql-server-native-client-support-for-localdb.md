...
-   Utiliser le mot clé de chaîne de connexion `AttachDBFilename` pour spécifier le fichier de base de données auquel le serveur doit se rattacher. Lorsque vous utilisez `AttachDBFilename`, si vous ne spécifiez pas le nom de la base de données avec le **base de données** mot-clé de chaîne de connexion, la base de données sera retiré l’instance de LocalDB lorsque l’application se ferme.  
