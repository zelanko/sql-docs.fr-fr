...
|*Domaine MachineName*$@*Domain*|Spécification directe d'un compte d'ordinateur.<br /><br /> (Si le serveur à lequel vous vous connectez est en cours d’exécution sous les comptes LOCAL SYSTEM ou NETWORK SERVICE, pour obtenir l’authentification `ServerSPN` Kerberos, peut être dans le format*Domaine* *MachineName.)*$@|  
|*KDCKey*/*MachineName (en)*|Nom principal de service spécifié par l'utilisateur.<br /><br /> *KDCKey* est une chaîne alphanumérique conforme aux règles d'une clé du centre de distribution de clés.|  
