 ...
 *szTable*  
 ...
 Si *eDirection* est DB_OUT et qu’une instruction SELECT est spécifiée à l’aide de [bcp_control](bcp-control.md) avant [bcp_exec](bcp-exec.md) est appelée, **bcp_init *** szTable*doit avoir la valeur NULL.  
  
 *szDataFile*  
 Nom du fichier utilisateur depuis ou vers lequel s'effectue la copie. Si données sont copiées directement à partir de variables à l’aide de [bcp_sendrow](bcp-sendrow.md), définissez *szDataFile* avec la valeur NULL.  
