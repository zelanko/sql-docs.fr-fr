...
ms.locfileid: "62664265"
---
# <a name="blocked-process-report-event-class"></a>Blocked Process Report (classe d'événements)
  La classe d’événements **blocked process Report** indique qu’une tâche a été bloquée pendant plus d’un laps de temps spécifié. Elle ne s'applique pas aux tâches système ou aux tâches en attente de ressources à blocage non détectable.  
...
|**IndexID contient**|**int**|ID de l'index de l'objet affecté par l'événement. Pour déterminer l’ID d’index d’un objet, utilisez la colonne **indid** de la table système **sysindexes** .|24|Oui|  
...
|**Arguments**|**int**|ID affecté par le système à l'objet sur lequel a été acquis le verrou (si disponible et le cas échéant).|22|Oui|  
|**Nom du serveur**|**nvarchar**|Nom de l'instance [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] tracée.|26||  
