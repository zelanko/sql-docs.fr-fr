...
# <a name="audit-add-role-event-class"></a>Audit Add Role (classe d'événements)
  La classe **d’événements audit Add Role** se produit lorsqu’un rôle de base de données est ajouté ou supprimé dans une base de données. Cette classe d’événements est utilisée par les procédures stockées **sp_addrole** et **sp_droprole** .  
  
 Cette classe d’événements peut être supprimée dans une version [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]future de. Nous vous recommandons d’utiliser la casse d’événements **Audit Database Principal Management** à la place.  
