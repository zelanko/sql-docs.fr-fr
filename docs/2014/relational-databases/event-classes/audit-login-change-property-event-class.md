...
# <a name="audit-login-change-property-event-class"></a>Audit Login Change Property event class (classe d'événements)
  La classe **d’événements Audit login change Property** se produit lorsque vous utilisez la procédure stockée **sp_defaultdb** , la procédure stockée **SP_DEFAULTLANGUAGE** ou l’instruction ALTER login pour modifier une propriété d’une connexion.  
