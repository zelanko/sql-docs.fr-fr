...
3.  **Assurez-vous SQL Server Agent Service est démarré et en cours d’exécution :**  Démarrez SQL Server Agent s’il n’est pas en cours d’exécution.  
  [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)] nécessite l'exécution de SQL Server Agent sur l'instance pour effectuer les opérations de sauvegarde.  Vous pouvez configurer l'exécution automatique de SQL Server Agent, pour vous assurer que les opérations de sauvegarde se déroulent régulièrement.  
  
4.  **Déterminez la période de rétention :** Déterminez la période de rétention pour les fichiers de sauvegarde. La période de rétention est spécifiée en jours, sur une plage de 1 à 30.  
...
7.  **Activer et configurer la notification pour l’état d’intégrité :** [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)] contient une procédure stockée qui crée un travail de l’agent pour envoyer des notifications par courrier électronique d’erreurs ou d’avertissements susceptibles de nécessiter votre attention. Les étapes suivantes décrivent la procédure d'activation et de configuration des notifications par courrier électronique :  
    ...
    3.  **Activer les notifications par courrier électronique pour recevoir des erreurs et des avertissements de sauvegarde :** À partir de la fenêtre de requête, exécutez les instructions Transact-SQL suivantes :  
...
9. **Affichez les fichiers de sauvegarde dans le compte stockage Microsoft Azure :** Connectez-vous au compte de stockage à partir de SQL Server Management Studio ou du Portail de gestion Azure. Vous verrez un conteneur pour l'instance de SQL Server qui héberge la base de données que vous avez configurée pour utiliser la [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]. Vous pourrez voir aussi une base de données et une sauvegarde de journal 15 minutes après la création de la base de données.  
  
10. **Surveiller l’état d’intégrité :**  Vous pouvez surveiller les notifications par courrier électronique que vous avez configurées précédemment ou surveiller activement les événements consignés. Voici quelques exemples d'instructions Transact SQL utilisées pour afficher les événements :  
