...
```  
 ...
 les programmes 64 bits lors de la lecture du `Installed Versions` clé s’affiche les valeurs qui pointent vers les versions 64 bits de la **SQLUserInstance** DLL, tandis que les programmes 32 bits (en cours d’exécution sur Windows 64 bits en mode WOW64) seront automatiquement redirigés vers un `Installed Versions` clé se trouve sous le `Wow6432Node` hive. Cette clé contient des valeurs qui pointent vers les versions 32 bits de le **SQLUserInstance** DLL.  
