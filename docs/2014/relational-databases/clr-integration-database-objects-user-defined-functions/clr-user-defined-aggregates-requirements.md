...
|`Terminate`|public return_type Terminate ();|Cette méthode termine le calcul d'agrégation et retourne le résultat de l'agrégation. Le *return_type* doit être un type [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] de données managé qui est l’équivalent managé de *return_sqltype* spécifié dans `CREATE AGGREGATE` l’instruction. Le *return_type* peut également être un type défini par l’utilisateur.|  
