...
4.  Effectuez l'une des actions décrites dans le tableau suivant :  
    ...
    |Définir l'option clustered|Dans la grille sous **Concepteur de tables**, sélectionnez **créer comme Clustered** , puis dans la liste déroulante, choisissez Oui pour créer un index cluster et non pour créer un index non-cluster. Il ne peut exister qu'un seul index cluster par table. Si un index cluster existe dans la table, vous devez effacer ce paramètre sur l'index d'origine.|  
