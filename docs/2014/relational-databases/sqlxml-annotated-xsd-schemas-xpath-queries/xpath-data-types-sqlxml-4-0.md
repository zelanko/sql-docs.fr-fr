...
|time<br /><br /> time.tz|string|SUBSTRING(CONVERT(nvarchar(4000), EmployeeID, 126), 1 + CHARINDEX(N'T', CONVERT(nvarchar(4000), EmployeeID, 126)), 24)|  
 ...
 Les conversions de date et d’heure sont conçues pour fonctionner, que la valeur soit stockée dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] `datetime` la base de données `string`à l’aide du type de données ou d’un. Notez que le [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] `datetime` type de données n’utilise `timezone` pas et a une précision inférieure à celle `time` du type de données XML. Pour inclure le type de données `timezone` ou apporter une précision supplémentaire, stockez les données dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] avec un type `string`.  
...
```  
 ...
 Si @m est du type `fixed14.4` de données XDR, la conversion du type de données XDR en type de données XPath s’effectue à l’aide de :  
...
### <a name="a-convert-a-data-type-in-an-xpath-query"></a>R. Convertir un type de données dans une requête XPath  
 Dans la requête XPath suivante spécifiée par rapport à un schéma XSD annoté, la requête sélectionne tous les nœuds **Employee** avec la valeur d’attribut **EmployeeID** de E-1, où « e- » est `sql:id-prefix` le préfixe spécifié à l’aide de l’annotation.  
 ...
 **EmployeeID** étant l’une des valeurs `id` de`idref`type `idrefs`de `nmtoken`données `nmtokens`(,,,, etc.) dans le schéma XSD, **EmployeeID** est converti en type `string` de données XPath à l’aide des règles de conversion décrites précédemment.  
...
### <a name="b-perform-several-data-type-conversions-in-an-xpath-query"></a>B. Effectuer plusieurs conversions de types de données dans une requête XPath.  
 ...
 Cette requête XPath retourne tous les ** \<éléments OrderDetail>** qui satisfont le `@UnitPrice * @OrderQty > 98`prédicat. Si **UnitPrice** est annoté avec un `fixed14.4` type de données dans le schéma annoté, ce prédicat équivaut à l’expression SQL :  
...
```  
 ...
 En supposant que l’attribut **OrderQty** n’a pas de type de données XSD, `number` **OrderQty** est converti en type de données XPath en une seule conversion :  
