...
## <a name="j-bulk-loading-in-xml-data-type-columns"></a>J. Chargement en masse dans les colonnes de type de données xml  
 Si le schéma de mappage spécifie une [type de données xml](/sql/t-sql/xml/xml-transact-sql) colonne à l’aide de la `sql:datatype="xml"` annotation, le chargement en masse XML peut copier des éléments enfants XML pour le champ mappé à partir du document source vers cette colonne.  
