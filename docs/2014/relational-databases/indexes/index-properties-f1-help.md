...
### <a name="bounding-box"></a>Cadre englobant  
 ...
 Le panneau affiche la **(*`X-min`*,*`Y-min`*)** et **(*`X-max`*,*`Y-max`*)** coordonnées du rectangle englobant. Il n'y a pas de valeurs de coordonnées par défaut. Par conséquent, lorsque vous créez un index spatial sur une colonne de type `geometry`, vous devez spécifier les valeurs des coordonnées.  
