...
ms.locfileid: "36052650"
---
# <a name="register-a-database-as-a-dac"></a>Inscrire une base de données en tant que DAC
  Utilisez le **inscrire l’Assistant Création d’applications de couche données** ou un script Windows PowerShell pour créer une définition de couche données (DAC) d’application qui décrit les objets d’une base de données existante et enregistrer la définition de la DAC dans le `msdb` base de données système (**master** dans [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]).  
