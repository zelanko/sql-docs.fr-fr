 ...
 **Version (utiliser x.x.x.x, où x est un nombre)** - Valeur numérique qui identifie la version de la DAC. La version de la DAC est utilisée dans Visual Studio pour identifier la version de la DAC sur laquelle les développeurs travaillent. Lorsque vous déployez une DAC, la version est stockée dans le `msdb` de base de données et peut être affichée ultérieurement sous le **Applications de couche données** nœud [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)].  
  
 **Description** : Facultatif. Décrit la DAC. Lorsque vous déployez une DAC, la description est stockée dans le `msdb` de base de données et peut être affichée ultérieurement sous le **Applications de couche données** nœud [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)].  
...
###  <a name="BuildPackage"></a> Page générer le Package  
