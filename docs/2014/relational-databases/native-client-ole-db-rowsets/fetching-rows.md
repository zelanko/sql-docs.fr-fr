...
ms.locfileid: "37415058"
---
# <a name="fetching-rows"></a>Extraction de lignes
  Le **IRowset** interface est l’interface de l’ensemble de lignes de base. Le **IRowset** interface fournit des méthodes pour extraire les lignes séquentiellement, obtenir les données à partir de ces lignes et la gestion des lignes. Les consommateurs utilisent les méthodes de **IRowset** pour toutes les opérations de l’ensemble de lignes de base. Ces opérations incluent l'extraction et la libération des lignes, ainsi que l'obtention des valeurs des colonnes.  
  
 Quand un consommateur Obtient un pointeur d’interface sur un ensemble de lignes, la première étape consiste généralement à déterminer les capacités de l’ensemble de lignes à l’aide de la **IRowsetInfo::GetProperties** (méthode). Cette opération retourne les informations sur les interfaces exposées par l'ensemble de lignes, ainsi que les fonctions de l'ensemble de lignes n'apparaissant pas comme interfaces distinctes, telles que le nombre maximal de lignes actives et le nombre de lignes qui peuvent avoir simultanément des mises à jour en attente.  
  
 L'étape suivante pour les consommateurs consiste à déterminer les caractéristiques, ou métadonnées, des colonnes de l'ensemble de lignes. Pour cela, ils utilisent le **IColumnsInfo** méthode pour les informations de colonne simple ou le **IColumnsRowset** méthode pour les informations de colonnes étendues. Le **GetColumnInfo** méthode retourne les informations suivantes :  
...
-   Pointeur vers un stockage de toutes les valeurs de chaîne au sein d'un bloc d'allocation unique.  
 ...
 Le **IColumnsInfo** et **IColumnsRowset** interfaces sont utilisées pour extraire des informations sur les colonnes dans l’ensemble de lignes. Le **IColumnsInfo** interface retourne un ensemble limité d’informations, alors que **IColumnsRowset** fournit toutes les métadonnées.  
  
> [!NOTE]  
>  Dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] version 7.0 et versions antérieure, la colonne de métadonnées facultative DBCOLUMN_COMPUTEMODE retournée par **IColumnsInfo::GetColumnsInfo** retourne DBSTATUS_S_ISNULL (au lieu des valeurs décrivant si la colonne est calculé), car il est impossible de déterminer si la colonne sous-jacente est calculée.  
