...
|BCP_OPTION_DELAYREADFMT|Une valeur booléenne définie sur true entraîne la lecture d’[IBCPSession::BCPReadFmt](ibcpsession-bcpreadfmt-ole-db.md) au moment de l’exécution. Si la valeur est false (valeur par défaut), IBCPSession :: BCPReadFmt lit immédiatement le fichier de format. Une erreur de séquence se produit `BCP_OPTION_DELAYREADFMT` si a la valeur true et que vous appelez IBCPSession :: BCPColumns ou IBCPSession :: BCPColFmt.<br /><br /> Une erreur de séquence se produira également si `IBCPSession::BCPControl(BCPDELAYREADFMT, (void *)FALSE))` vous appelez `IBCPSession::BCPControl(BCPDELAYREADFMT, (void *)TRUE)` après l’appel de et IBCPSession :: BCPWriteFmt.<br /><br /> Pour plus d’informations, consultez [Découverte des métadonnées](../native-client/features/metadata-discovery.md).|  
