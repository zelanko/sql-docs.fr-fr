...
## <a name="sqlsetdescfield-support-for-enhanced-date-and-time-features"></a>Prise en charge par SQLSetDescField des fonctionnalités de date et heure améliorées  
 Les fonctionnalités de date/heure ont été améliorées dans ODBC. Pour plus d’informations sur le champ de descripteur fourni pour les types date/heure de nouveau, consultez [paramètre et les métadonnées de résultat](../native-client-odbc-date-time/metadata-parameter-and-result.md).  
