...
# <a name="sqlbindcol"></a>SQLBindCol
 ...
 Pour les types de données de valeur élevée, si la mémoire tampon fournie par l’utilisateur n’est pas assez grand pour contenir la valeur entière de la colonne, `SQL_SUCCESS_WITH_INFO` est retourné et les « données de chaîne ; avertissement « troncation à droite. L'argument `StrLen_or_IndPtr` contient le nombre de chars/octets stockés dans la mémoire tampon.  
