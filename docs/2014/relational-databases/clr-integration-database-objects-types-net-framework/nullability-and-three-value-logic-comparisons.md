...
## <a name="operations-functions-and-null-values"></a>Opérations, fonctions et valeurs Null  
 Tous les opérateurs arithmétiques (+, -, \*, /, %), opérateurs de bits (~, &, et |), et la plupart des fonctions retournent NULL si un des opérandes ou arguments de `SqlTypes` ont la valeur NULL. La propriété `IsNull` retourne toujours une valeur true ou false.  
