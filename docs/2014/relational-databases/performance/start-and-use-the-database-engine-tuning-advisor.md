...
> [!NOTE]  
>  Assistant Paramétrage du moteur de base de données ne met pas à jour automatiquement les informations de lignes pour les tables affichées sous l’onglet **général** . Au lieu de cela, il s’appuie sur les métadonnées de la base de données. Si vous pensez que les informations sur les lignes ne sont plus à jour, exécutez la commande DBCC UPDATEUSAGE pour les objets concernés.  
