...
```  
 ...
 Si vous voulez ne consulter qu’un seul schéma de la collection de schémas XML, vous pouvez spécifier la requête XQuery portant sur la `xml` type de résultat qui est retourné par `xml_schema_namespace`.  
