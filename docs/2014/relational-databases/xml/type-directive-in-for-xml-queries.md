...
```  
 ...
 Interne `SELECT … FOR XML` requête retourne un `xml` résultat auquel de type externe `SELECT` s’applique le `query()` méthode à la `xml` type. Notez la directive `TYPE` spécifiée.  
