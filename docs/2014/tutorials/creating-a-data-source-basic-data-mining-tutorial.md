---
title: Création d’une Source de données (didacticiel d’exploration de données de base de données) | Documents Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
> [!IMPORTANT]  
>  Le nom de la base de données est [!INCLUDE[ssSampleDBDWobject](../includes/sssampledbdwobject-md.md)]. Si vous n’avez pas déjà installé cette base de données, consultez la [Microsoft SQL Sample Databases](http://go.microsoft.com/fwlink/?LinkId=88417) page.  
  
### <a name="to-create-a-data-source"></a>Pour créer une source de données  
  
1.  Dans **l’Explorateur de solutions**, avec le bouton droit le **des Sources de données** et sélectionnez **nouvelle Source de données**.  
  
2.  Sur le **Bienvenue dans l’Assistant Source de données** , cliquez sur **suivant**.  
  
3.  Sur le **sélectionner la méthode de définition de la connexion** , cliquez sur **nouveau** pour ajouter une connexion à la [!INCLUDE[ssSampleDBDWobject](../includes/sssampledbdwobject-md.md)] base de données.  
  
4.  Dans le **fournisseur** liste **Gestionnaire de connexions**, sélectionnez **natif OLE DB\SQL Server Native Client 11.0**.  
