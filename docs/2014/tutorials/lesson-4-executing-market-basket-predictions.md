---
title: 'Leçon 4 : Exécution de prédictions Market Basket | Documents Microsoft'
ms.custom: ''
ms.date: 03/06/2017
...
## <a name="singleton-prediction-join-statement"></a>Instruction singleton PREDICTION JOIN  
 La première étape consiste à créer une requête singleton, à l’aide de la **SELECT FROM \<modèle > PREDICTION JOIN** syntaxe et en fournissant un ensemble unique de valeurs comme entrée. L'exemple générique suivant utilise l'instruction singleton :  
...
-   Créer une requête qui prédit les autres éléments, un client est susceptible d’acheter, en fonction des éléments existant déjà dans leur panier d’achat. Vous allez créer cette requête à l’aide du modèle d’exploration de données avec la valeur par défaut *MINIMUM_PROBABILITY*.  
  
-   créer une requête pour prédire quels autres articles un client est susceptible d'acheter, en vous basant sur des articles déjà présents dans son panier d'achat. Cette requête est basée sur un modèle différent, dans lequel *MINIMUM_PROBABILITY* a été défini sur 0.01. Étant donné que la valeur par défaut *MINIMUM_PROBABILITY* dans les modèles d’association étant égale à 0.3, la requête sur ce modèle doit retourner davantage d’éléments possibles que la requête sur le modèle par défaut.  
...
1.  Dans **l’Explorateur d’objets**, cliquez sur l’instance de [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)], pointez sur **nouvelle requête**, puis cliquez sur **DMX** pour ouvrir l’éditeur de requête.  
...
8.  Dans la barre d’outils, cliquez sur le **Execute** bouton.  
 ...
 Pour savoir comment utiliser DMX dans un autre scénario prédictif, consultez [didacticiel DMX Bike Buyer](../../2014/tutorials/bike-buyer-dmx-tutorial.md).  
  
## <a name="see-also"></a>Voir aussi  
 [Exemples de requêtes de modèle association](../../2014/analysis-services/data-mining/association-model-query-examples.md)   
 [Interfaces de requête d’exploration de données](../../2014/analysis-services/data-mining/data-mining-query-tools.md)  
