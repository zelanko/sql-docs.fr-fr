---
title: Explorer les données (SQL Server Data Mining Add-ins) | Documents Microsoft
ms.custom: ''
ms.date: 12/29/2017
...
# <a name="explore-data-sql-server-data-mining-add-ins"></a>Explorer des données (Compléments d'exploration de données SQL Server)
  ![L’Assistant exploration de données](media/dmc-explore.gif "Assistant Exploration des données")  
...
-   **Graphique à barres.** Ce graphique regroupe les valeurs selon le nombre de cas pour chaque valeur.  
 ...
 Si vous souhaitez regrouper vos données dans d'autres plages, vous devez le faire dans Excel avant d'analyser les données. Ou bien, vous pouvez également réétiqueter les données à l’aide de la [réétiquetage](relabel-sql-server-data-mining-add-ins.md) Assistant.  
  
## <a name="using-the-explore-data-wizard"></a>Utilisation de l'Assistant Explorer les données  
  
1.  Dans le **d’exploration de données** du ruban, cliquez sur **Explorer les données**.  
...
3.  Dans le **sélectionner la colonne** boîte de dialogue, choisissez la colonne à analyser, à partir d’exemples de données affichés dans le volet.  
  
4.  Dans le **Explorer les données** boîte de dialogue, choisissez les types de graphiques pour l’affichage de la distribution des données.  
