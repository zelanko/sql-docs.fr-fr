...
##  <a name="AllocationScheme"></a> Équilibrage de charge  
 L'une des fonctions qu'assure le service PowerPivot consiste à déterminer où les données Analysis Services seront chargées parmi les instances du service PowerPivot disponibles. Le `AllocationMethod` paramètre spécifie les critères selon lesquels une instance de service est sélectionnée.  
