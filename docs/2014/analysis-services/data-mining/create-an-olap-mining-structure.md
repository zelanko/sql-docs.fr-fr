...
##  <a name="bkmk_Reqs"></a>Configuration requise pour la structure et les modèles d’exploration de données OLAP  
...
1.  **Sélectionnez la méthode de définition**: vous sélectionnez ici un type de source de données, puis choisissez **à partir d’un cube existant**.  
...
4.  **Sélectionnez la clé de cas**: dans la dimension que vous venez de sélectionner, choisissez un attribut (colonne) qui servira d’identificateur unique pour vos données de cas.  
  
     En général, une colonne sera présélectionnée pour vous, mais vous pouvez en changer si en fait il existe plusieurs clés.  
  
5.  **Sélection des colonnes de niveau de cas**: vous choisissez ici les attributs de la dimension sélectionnée, ainsi que les mesures associées, qui sont pertinentes pour votre analyse. Cette étape est équivalente à la sélection de colonnes d'une table.  
...
6.  **Spécifier l’utilisation des colonnes du modèle d’exploration de données**: pour chaque attribut ou mesure que vous avez ajouté à la structure, vous devez spécifier si l’attribut doit être utilisé pour la prédiction ou utilisé comme entrée. Si vous ne sélectionnez aucune de ces options, les données sont traitées mais ne sont pas utilisées pour l'analyse ; toutefois, elles sont disponibles comme données d'arrière-plan au cas où vous activeriez ultérieurement l'extraction.  
...
8.  **Spécifier le type de contenu et de données des colonnes**: à ce stade, vous avez ajouté toutes les données qui seront utilisées pour l’analyse, et vous devez spécifier le *type de données* et le type de *contenu* pour chaque attribut.  
...
9. **Découpage du cube source**: vous pouvez ici définir des filtres dans un cube pour sélectionner uniquement un sous-ensemble de données et former des modèles qui sont plus ciblés.  
...
|----------|---------------------|---------------|  
|Regrouper des membres en clusters|Segmentez une dimension de clients en fonction des propriétés de membre des clients, des produits achetés par les clients et du montant dépensé par les clients.|
  [!INCLUDE[msCoName](../../includes/msconame-md.md)] Clustering)|  
|Rechercher des membres intéressants ou anormaux|Identifiez des magasins intéressants ou anormaux dans une dimension de magasins en fonction des ventes, des bénéfices, de la situation géographique et de la taille des magasins.|
  [!INCLUDE[msCoName](../../includes/msconame-md.md)] Decision Trees)|  
|Rechercher des cellules intéressantes ou anormales|Identifiez les ventes des magasins qui ne suivent pas les tendances générales dans le temps.|
  [!INCLUDE[msCoName](../../includes/msconame-md.md)] Time Series)|  
|Rechercher des corrélations|Identifiez les facteurs qui sont liés au temps mort de serveur, notamment la zone, le type de l'ordinateur, le système d'exploitation ou la date d'achat.|
