...
##  <a name="bkmk_Syntax"></a> Filter Syntax  
 ...
 Dans les tables imbriquées, vous pouvez également utiliser le `EXISTS` et `NOT EXISTS` opérateurs. Une condition `EXISTS` est évaluée à `true` si la sous-requête retourne au moins une ligne. Ceci est utile dans les cas où vous souhaitez restreindre le modèle aux cas qui contiennent une valeur particulière dans la table imbriquée : par exemple, les clients qui ont acheté un élément au moins une fois.  
...
> [!NOTE]  
>  Quel que soit le type de données, ces opérateurs ne peut pas être appliqués à une colonne qui a le type `Discrete`, `Discretized`, ou `Key`.  
