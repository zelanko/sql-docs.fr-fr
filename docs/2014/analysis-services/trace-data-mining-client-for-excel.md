---
title: Trace (Client d’exploration de données pour Excel) | Documents Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
# <a name="trace-data-mining-client-for-excel"></a>Trace (Client d'exploration de données pour Excel)
 ...
 Le **suivi** boîte de dialogue vous permet de surveiller les instructions sont envoyées à l’instance de [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] que vous utilisez pour l’exploration de données. Après avoir créé une connexion à une instance de [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)], toutes les interactions entre le client et le serveur sont enregistrées le **suivi** volet, y compris des instructions qui créent des structures, ajouter les modèles d’exploration de données, puis apportez prédictions, ainsi que certains messages retournés à partir du serveur.  
 ...
 **Trace** analyse uniquement la connexion actuelle et le contenu de la **suivi** boîte de dialogue ne sont pas stockées.  
...
## <a name="tracing-temporary-structures-and-models"></a>Traçage des structures et modèles temporaires  
 ...
 Si vous souhaitez conserver le travail lorsque l’un des outils d’analyse de Table, vous pouvez désélectionner l’option, **utiliser des modèles de session**, pour être enregistrées sur le serveur de manière permanente de vos modèles. Vous pouvez également copier les instructions figurant dans le **suivi** volet dans un fichier afin que vous pouvez recréer votre travail plus tard.  
