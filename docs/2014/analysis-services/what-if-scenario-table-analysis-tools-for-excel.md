---
title: Scénario (outils d’analyse de Table pour Excel) | Documents Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "36043451"
---
# <a name="what-if-scenario-table-analysis-tools-for-excel"></a>Scénario (Outils d'analyse de table pour Excel)
  ![Bouton scénarios des outils d’analyse de Table de](media/tat-whatif.gif "bouton scénario quoi dans Outils d’analyse de Table")  
  
 Le **simulation** outil scénario analyse les séquences dans les données existantes, puis vous permet d’évaluer les effets de modifications dans une colonne aurait sur la valeur d’une autre colonne.  
...
2.  Cliquez sur **scénarios**, puis sélectionnez **simulation**.  
  
3.  Dans le **scénario** , sélectionnez la colonne qui contient la valeur à modifier et spécifiez la modification comme une valeur spécifique ou comme un pourcentage de modification (soit augmenté ou diminué) sur les valeurs actuelles.  
...
6.  Dans le **spécifier la ligne ou table** , choisissez si vous souhaitez que l’impact soit évalué pour la ligne actuellement sélectionnée uniquement ou pour l’ensemble complet des données de la table.  
...
8.  Si vous sélectionnez **toute table**, l’outil affiche un message d’état dans la boîte de dialogue et ajoute deux nouvelles colonnes à la table de données d’origine. Cliquez sur **fermer** pour afficher les résultats complets dans la feuille de calcul.  
...
-   Si vous utilisez la **valeur** option, vous devez taper une valeur dans la zone ou sélectionnez une valeur dans la liste.  
  
-   Si vous utilisez la **pourcentage** option, définissez la modification comme un pourcentage d’augmentation ou de diminution. La valeur par défaut est de 120 %, ce qui correspond à une augmentation de 20 % par rapport à la valeur actuelle.  
...
## <a name="understanding-the-results-of-what-if-analysis"></a>Explication des résultats de l'analyse de scénarios  
 Lorsque vous créez un **simulation** scénario, l’outil effectue trois opérations :  
...
-   il crée une requête de prédiction pour chacune des valeurs que vous spécifiez.  
 ...
 Vous pouvez produire toutes les prédictions en même temps en spécifiant **toute table**. L'outil crée ensuite deux nouvelles colonnes dans la table de données d'origine.  
 ...
 Par exemple, si vous essayez de déterminer l’effet d’augmenter la durée de vie du client sur la disposition du client à acheter et l’âge du client à 25 ans, le **simulation** outil interroge le modèle d’exploration de données et retourne le résultat suivant :  
...
## <a name="related-tools"></a>Outils connexes  
 ...
 Pour plus d’informations sur l’algorithme utilisé par le **simulation** scénario outil, consultez la rubrique « Microsoft algorithme Logistic Regression » dans la documentation en ligne de SQL Server.  
