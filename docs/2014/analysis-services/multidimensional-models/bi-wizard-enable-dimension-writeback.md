...
## <a name="setting-dimension-writeback-capability"></a>Définition de la fonctionnalité d'écriture en différé de la dimension  
 Dans la deuxième page **Activer l’écriture différée de la dimension** de l’Assistant, vous paramétrez réellement l’option **Activer l’écriture différée de la dimension** . Cette option automatiquement définit le `WriteEnabled` propriété de la dimension à `True`. Désactivation automatique de cette option affecte à la propriété `False`.  
