...
|`AttributeHierarchyOrdered`|Détermine si la hiérarchie d'attribut associée est ordonnée. La valeur par défaut est `True`. Toutefois, si une hiérarchie d’attribut ne sera pas utilisée pour des requêtes, vous pouvez enregistrer le temps de traitement en modifiant la valeur de cette propriété sur `False`.|  
|`AttributeHierarchyVisible`|Détermine si la hiérarchie d'attribut est visible par des applications clientes. La valeur par défaut est `True`. Toutefois, si une hiérarchie d’attribut ne sera pas utilisée pour des requêtes, vous pouvez enregistrer le temps de traitement en modifiant la valeur de cette propriété sur `False`.|  
