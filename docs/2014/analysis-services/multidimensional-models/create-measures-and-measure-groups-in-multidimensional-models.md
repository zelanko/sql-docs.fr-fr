...
|**agrégation**|Par défaut, les mesures sont additionnées avec chaque dimension. Toutefois, la propriété `AggregateFunction` vous permet de modifier ce comportement. Pour obtenir la liste des fonctions d'agrégation, consultez [Use Aggregate Functions](use-aggregate-functions.md) .|  
