---
title: Configuration d’erreur pour le Cube, Partition et le traitement des dimensions (SSAS - multidimensionnel) | Documents Microsoft
ms.custom: ''
ms.date: 06/13/2017
...
-   La conversion en membre inconnu se produit par défaut, en raison du `ConvertToUnknown` définition pour `KeyErrorAction`. Les enregistrements alloués à un membre inconnu sont mis en quarantaine dans la base de données en tant que preuve d'un problème que vous pouvez examiner une fois le traitement terminé.  
...
-   Est ignoré lorsque vous définissez manuellement la `KeyErrorAction` propriété `DiscardRecord`.  
 ...
 Les erreurs ayant un impact élevé, notamment une clé manquante ou une valeur NULL dans un champ clé, doivent être traitées rapidement. Par défaut, ces erreurs adhèrent aux `ReportAndContinue` des comportements de serveur, où le serveur intercepte l’erreur, l’ajoute au nombre d’erreurs, puis continue le traitement (sauf le nombre maximal d’erreurs est zéro, et le traitement s’arrête immédiatement).  
...
|`NullKeyNotAllowed`<br /><br /> Se produit lorsque `NullProcessing`  =  `Error` est défini sur un attribut de dimension ou lorsqu’il existe des valeurs null dans une colonne clé d’attribut utilisée pour identifier de façon unique un membre.|`ReportAndContinue` consigne et comptabilise l’erreur.|`ReportAndStop` signale l’erreur et arrête le traitement immédiatement, quel que soit le nombre maximal d’erreurs.<br /><br /> `IgnoreError` journaux ni comptabilise l’erreur ; le traitement continue tant que le nombre d’erreurs est sous la limite maximale. Les enregistrements qui déclenchent cette erreur sont convertis en membre inconnu par défaut, mais vous pouvez définir le `KeyErrorAction` propriété à les ignorer.|  
|`NullKeyConvertedToUnknown`<br /><br /> Se produit lorsque des valeurs NULL sont, par la suite, converties en membre inconnu. Paramètre `NullProcessing`  =  `ConvertToUnknown` sur une dimension attribut déclenche cette erreur.|`IgnoreError` journaux ni comptabilise l’erreur ; le traitement continue tant que le nombre d’erreurs est sous la limite maximale.|Pour afficher cette erreur à titre d'information, conservez la valeur par défaut. Sinon, vous choisissez `ReportAndContinue` de façon à signaler l'erreur dans la fenêtre de traitement et la comptabiliser dans le nombre maximal d'erreurs.<br /><br /> `ReportAndStop` signale l’erreur et arrête le traitement immédiatement, quel que soit le nombre maximal d’erreurs.|  
  
 **Propriétés générales**  
  
|**Propriété**|**Valeurs**|  
|------------------|----------------|  
|`KeyErrorAction`|L’action effectuée par le serveur quand un `KeyNotFound` erreur se produit. Les réponses valides à cette erreur sont `ConvertToUnknown` ou `DiscardRecord`.|  
...
2.  Vous pouvez également, pour une seule dimension, cliquez sur la dimension dans l’Explorateur de solutions, choisissez `Process`, puis choisissez **modifier les paramètres** dans la boîte de dialogue traiter la Dimension. Les options de configuration d'erreur s'affichent dans l'onglet Erreurs de clé de dimension.  
...
##  <a name="bkmk_missing"></a> Clés manquantes (KeyNotFound)  
 ...
 Le serveur génère le `KeyNotFound` erreur lors du traitement de partition, lorsqu’une table de faits enregistrement contient une valeur de clé étrangère, mais la clé étrangère n’a aucun enregistrement correspondant dans une table de dimension associées. Cette erreur se produit également lors du traitement des tables de dimension associées ou en flocons, où un enregistrement dans une dimension spécifie une clé étrangère qui n'existe pas dans la dimension associée.  
  
 Lorsqu'une erreur `KeyNotFound` se produit, l'enregistrement incriminé est alloué au membre inconnu. Ce comportement est contrôlé via la **Action clé**, affectez la valeur `ConvertToUnknown`, de sorte que vous pouvez afficher les enregistrements alloués pour un examen approfondi.  
...
4.  Remplacez la valeur par `Error` pour exclure tous les enregistrements ayant une valeur null, ce qui évite la conversion null numérique (zéro). Cette modification vous permet d’éviter les erreurs de clé dupliquée reliés à plusieurs enregistrements contenant un zéro dans la colonne de clé et également d’éviter `KeyNotFound` erreurs lorsqu’une clé étrangère de valeur zéro ne possède aucune clé primaire équivalente dans une table de dimension associées.  
  
##  <a name="bkmk_nulldim"></a> Clés NULL dans une dimension  
 Pour continuer le traitement lorsque les valeurs null sont détectées dans les clés étrangères dans une dimension en flocons, traiter tout d’abord les valeurs null en définissant `NullProcessing` sur la `KeyColumn` de l’attribut de dimension. Cela permet d'ignorer ou de convertir l'enregistrement, avant que l'erreur `KeyNotFound` ne se produise.  
...
3.  Dans Propriétés, développez **KeyColumns** pour afficher `NullProcessing` propriété. Elle a la valeur **Automatic** par défaut, qui convertit les valeurs NULL en zéros pour les champs contenant des données numériques. Remplacez la valeur par `Error` ou `UnknownMember`.  
  
     Cette modification supprime les conditions sous-jacentes qui déclenchent `KeyNotFound` en ignorant ou en convertissant l’enregistrement avant son archivage des erreurs.  
  
     Selon la configuration d'erreur, ces actions entraînent une erreur qui est signalée et comptabilisée. Vous devrez peut-être ajuster les propriétés supplémentaires, telles que la définition `KeyNotFound` à `ReportAndContinue` ou `KeyErrorLimit` à une valeur différente de zéro, pour permettre le traitement continue lorsque ces erreurs sont signalées et comptabilisées.  
...
##  <a name="bkmk_dupe"></a> Clés dupliquées résultant de relations incohérentes (KeyDuplicate)  
 ...
 Pour modifier ce comportement, affectez `KeyDuplicate` à `ReportAndContinue` ou `ReportAndStop` pour signaler l’erreur. Vous pourrez ensuite examiner l'erreur pour déterminer les failles potentielles dans la conception de la dimension.  
...
##  <a name="bkmk_limit"></a> Modifier le nombre maximal d'erreurs ou l'action lorsque le nombre maximal d'erreurs est atteint  
 ...
 Une fois le nombre maximal d'erreurs atteint, spécifiez que le traitement s'arrête ou que l'enregistrement dans le journal s'arrête. Par exemple, supposons que vous définissez l’action `StopLogging` sur un nombre maximal d’erreurs de 100. À la 101è erreur, le traitement continue, mais les erreurs ne sont plus consignées ou comptabilisées. Les actions d’erreur limite sont spécifiées en tant que `KeyErrorLimitAction` dans `ErrorConfiguration` propriétés dans [!INCLUDE[ssBIDevStudio](../../includes/ssbidevstudio-md.md)], ou en tant que **sur l’action d’erreur** dans l’onglet Configuration d’erreur des propriétés des dimensions, cubes ou groupes de mesures dans [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] .  
