...
##  <a name="bkmk_how_imper"></a>Avantageuse  
 L' *emprunt d’identité* est la capacité d’une application serveur, telle que Analysis Services, à assumer l’identité d’une application cliente. Analysis Services s'exécute en utilisant un compte de service ; toutefois, lorsque le serveur établit une connexion à une source de données, il utilise l'emprunt d'identité afin d'effectuer les vérifications d'accès pour l'importation et le traitement des données.  
 ...
 **Description des informations d’identification côté serveur**  
 ...
 **Description des informations d’identification côté client**  
