 ...
 **Connexions HTTP (s) pour les classeurs PowerPivot (fichiers .xlsx, .xlsb ou .xlsm)**  
  
 `Data Source=<URL>`, où l'URL est le chemin d'accès SharePoint à un classeur PowerPivot publié dans une bibliothèque SharePoint. Par exemple, « Source de données =http://localhost/Shared Sales.xlsx ».  
  
 **Connexions HTTP(s) aux fichiers de connexion de modèle sémantique BI**  
  
 `Data Source=<URL>` où l'URL est le chemin d'accès SharePoint au fichier .bism. Par exemple, « Source de données =http://localhost/Shared Sales.bism ».  
