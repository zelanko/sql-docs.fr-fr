...
|`SEARCHABLE`|`DBTYPE_UI4`||Entier qui indique comment le type de données peut être utilisé dans les recherches si le fournisseur prend en charge `ICommandText` ; sinon, `NULL`.<br /><br /> Les valeurs possibles pour cette colonne sont les suivantes :<br /><br /> -   `DB_UNSEARCHABLE` Indique que le type de données ne peut pas être utilisé dans un `WHERE` clause.<br />-   `DB_LIKE_ONLY` Indique que le type de données peut être utilisé dans un `WHERE` clause uniquement avec la `LIKE` prédicat.<br />-   `DB_ALL_EXCEPT_LIKE` Indique que le type de données peut être utilisé dans un `WHERE` clause avec tous les opérateurs de comparaison sauf `LIKE`.<br />-   `DB_SEARCHABLE` Indique que le type de données peut être utilisé dans un `WHERE` clause avec n’importe quel opérateur de comparaison.|  
...
|`MINIMUM_SCALE`|`DBTYPE_I2`||Si l'indicateur de type est `DBTYPE_VARNUMERIC`, `DBTYPE_DECIMAL` ou `DBTYPE_NUMERIC`, nombre minimal de chiffres autorisés à droite de la virgule décimale. Dans le cas contraire, `NULL`.|  
...
|`GUID`|`DBTYPE_GUID`||(Destiné à une utilisation ultérieure) `GUID` du type, si le type est décrit dans une bibliothèque de types. Dans le cas contraire, `NULL`.|  
