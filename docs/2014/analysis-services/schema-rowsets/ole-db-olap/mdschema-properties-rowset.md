---
title: Ensemble de lignes MDSCHEMA_PROPERTIES | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
|`PROPERTY_ORIGIN`|`DBTYPE_UI2`||Identifie le type de hiérarchie auquel s'applique la propriété :<br /><br /> -   `MD_USER_DEFINED` (`1`) indique la propriété est sur une hiérarchie définie par l’utilisateur<br />-   `MD_SYSTEM_ENABLED` (`2`) indique la propriété est sur une hiérarchie d’attribut<br />-   `MD_SYSTEM_DISABLED` (`4`) indique la propriété est sur une hiérarchie d’attribut qui n’est pas activée.|  
