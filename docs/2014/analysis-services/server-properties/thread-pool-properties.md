...
-   `IOProcess` est utilisé pour les travaux d’e/s associés aux requêtes de moteur de stockage dans le moteur multidimensionnel. Le travail exécuté par ces threads n'a normalement pas de dépendances sur d'autres threads. Ces threads analysent généralement un seul segment d'une partition et filtrent et agrègent les données du segment. `IOProcess` les threads sont particulièrement sensibles aux configurations matérielles NUMA. Par conséquent, ce pool de threads possède le `PerNumaNode` propriété de configuration qui peut être utilisée pour ajuster les performances si nécessaire.  
  
-   `Process` est de plus longue durée stockage travaux du moteur, y compris les agrégations, l’indexation et des opérations de validation. Le mode de stockage ROLAP utilise également des threads du pool de threads Processing.  
...
##  <a name="bkmk_pernumanode"></a> Définir PerNumaNode pour configurer l'affinité des threads d'E/S aux processeurs dans un nœud NUMA  
 Pour les instances multidimensionnelles d’Analysis Services, vous pouvez définir `PerNumaNode` sur la `IOProcess` pool de threads pour optimiser davantage la planification des threads et l’exécution. Tandis que `GroupAffinity` identifie l’ensemble de processeurs logiques à utiliser pour un pool de threads spécifique, `PerNumaNode` va plus loin en spécifiant s’il faut créer plusieurs pools de threads, plus une affinité avec un sous-ensemble des processeurs logiques.  
...
###  <a name="bkmk_workdistrib"></a> Répartition du travail entre les threads IOProcess  
 Si vous envisagez de définir le `PerNumaNode` propriété, en sachant comment `IOProcess` threads sont utilisés peut vous aider à prendre une décision plus avisée.  
  
 N’oubliez pas que `IOProcess` est utilisée pour les travaux d’e/s associés aux requêtes de moteur de stockage dans le moteur multidimensionnel.  
...
-   Pour les groupes de mesures sous-partitionnés (par exemple, comportant une seule partition), augmentez le nombre de partitions. Si vous utilisez une seule partition, le moteur met systématiquement en file d'attente les tâches sur un seul pool de threads (le pool de threads 0). L'ajout de davantage de partitions permet au moteur d'utiliser des pools de threads supplémentaires.  
     ...
     Alternativement, si vous ne pouvez pas créer des partitions supplémentaires, essayez de définir `PerNumaNode`= 0 qui permet d’augmenter le nombre de threads disponibles pour le pool de threads 0.  
  
-   Pour les bases de données dans le segment qui analyses sont équitablement distribuées entre plusieurs partitions, en attribuant `PerNumaNode` sur 1 ou 2 peut améliorer les performances, car elle augmente le nombre total de `IOProcess` utilisés par le système de pools de threads.  
  
-   Pour les solutions qui ont plusieurs partitions, mais un seul est fortement analysée, essayez de définir `PerNumaNode`= 0 pour voir si cela améliore les performances.  
