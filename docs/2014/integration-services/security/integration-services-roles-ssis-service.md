...
# <a name="integration-services-roles-ssis-service"></a>Rôles Integration Services (Service SSIS)
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)][!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] comprend les trois rôles fixes au niveau de la `db_ssisadmin`base de données,, **db_ssisltduser**et **db_ssisoperator**, pour contrôler l’accès aux packages. Les rôles ne peuvent être implémentés que sur les packages enregistrés `msdb` dans la [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]base de données dans. Vous affectez des rôles à un package à l'aide de [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]. Les attributions de rôles sont enregistrées `msdb` dans la base de données.  
...
## <a name="access-to-packages"></a>Accès aux packages  
 ...
 Si vous souhaitez utiliser des rôles définis par l’utilisateur, vous devez les ajouter à `msdb` la base de données avant de pouvoir les affecter à des packages. Vous pouvez créer de nouveaux rôles de base de données dans [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)].  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)](le service MSSQLSERVER) doit être démarré pour que vous puissiez vous connecter au Moteur de base de données et accéder `msdb` à la base de données.  
