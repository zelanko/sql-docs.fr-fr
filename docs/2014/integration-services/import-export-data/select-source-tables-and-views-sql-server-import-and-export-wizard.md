...
> [!NOTE]  
>  Si vous effectuez une suspension à ce stade de l’Assistant pour créer une table de destination dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] à l’aide de [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] ou un autre outil, la nouvelle table n’est pas immédiatement visible dans la liste des tables de destination disponibles. Pour actualiser la liste des tables de destination, étape sauvegarder deux pages pour la **choisir une Destination** page, sélectionnez à nouveau la base de données de destination, puis avancez de nouveau jusqu'à la **sélectionner des Tables Source et vues**.  
 ...
 **Aperçu**  
 Afficher un aperçu des données sources dans le **aperçu des données** boîte de dialogue pour les vérifier avant de procéder de l’importation ou d’exportation. Le **aperçu des données** boîte de dialogue affiche jusqu'à 200 lignes de données.  
