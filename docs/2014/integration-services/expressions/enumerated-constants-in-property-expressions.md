...
|Remplacer| 1|  
 ...
 `BackupAction` propriété : définie à l’aide de valeurs à partir de la `BackupTaskType` énumération. Cette propriété fonctionne avec la `BackupIsIncremental` propriété pour définir le type de sauvegarde qui exécute la tâche.  
