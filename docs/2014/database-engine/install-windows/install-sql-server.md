...
|Rubrique|Description|  
|-----------|-----------------|  
|[Configurer le pare-feu Windows pour autoriser l’accès SQL Server](../../sql-server/install/configure-the-windows-firewall-to-allow-sql-server-access.md)|Cette rubrique fournit une vue d'ensemble de la configuration du pare-feu et décrit comment configurer le Pare-feu Windows.|  
