---
title: Changements essentiels pour la recherche en texte intégral | Documents Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
|CONTAINSTABLE et **ItemCount**|Suppression de lignes de table de base avant de pousser les modifications vers MSSearch.|CONTAINSTABLE retourne un enregistrement fantôme. **ItemCount** n’est pas modifiée.|CONTAINSTABLE ne retourne pas d'enregistrements fantômes.|  
|**ItemCount**|La table contient des documents ou des colonnes de type Null.|En plus des documents indexés, les documents qui sont null ou qui ont des types null sont comptés dans la **ItemCount** valeur.|Seuls les documents indexés sont comptés la **ItemCount** valeur.|  
|Catalogue **ItemCount**|Colonne blob avec une extension NULL.|Cette valeur est exprimée en **ItemCount** du catalogue|Il n’est pas compté **ItemCount** du catalogue.|  
|**UniqueKeyCount**|Interrogation d'un nombre de clés uniques d'un catalogue, par exemple deux tables (table1 et table2) chacune avec trois mots : word1, word2 et word3.|**UniqueKeyCount** = 9. Le tableau suivant résume comment cette valeur est atteinte :<br /><br /> table1 = 3<br /><br /> EOF pour l'index de recherche en texte intégral de table1 = 1<br /><br /> table2 = 3<br /><br /> EOF pour l'index de recherche en texte intégral de table2 = 1<br /><br /> catalogue de texte intégral = 1|Pour chaque table, **UniqueKeyCount** est le nombre de mots clés distincts + 1 (0xFF).  Cela ne traite PAS les mêmes mots dans > 1 doc comme nouvelle clé unique.<br /><br /> Pour un catalogue, **UniqueKeyCount** est la somme des **UniqueKeyCount** de chacune des tables dans le catalogue. Les mots identiques de tables différentes sont traités comme des clés uniques. Dans ce cas, le nombre de clés uniques est 8.|  
|**précalculer le rang** option de niveau serveur|Optimisation des performances des requêtes FREETEXTTABLE.|Lorsque l’option est définie sur 1, les requêtes FREETEXTTABLE spécifié avec *top_n_by_rank* utilisent les données de classement précalculées stockées dans les catalogues de texte intégral.|Non prise en charge.|  
...
|Mots vides spécifiques aux paramètres régionaux|Interroge les variantes inlocale spécifiques d’un langage parent, tel que le Français de Belgique et le Français du Canada.|Variantes spécifiques inlocale de requêtes sont traitées par les composants (analyseurs lexicaux, générateurs de formes dérivées et mots vides) de leur langage parent. Par exemple, les composants Français (France) sont utilisés pour analyser le Français (Belgique).|Vous devez ajouter des mots vides de manière explicite pour chaque identificateur de paramètres régionaux (LCID). Par exemple, vous devriez spécifier un LCID pour la Belgique, le Canada et la France.|  
...
|[DATABASEPROPERTYEX](/sql/t-sql/functions/databasepropertyex-transact-sql)|À l’aide de la **IsFullTextEnabled** propriété.|Le **IsFullTextEnabled** paramètre indique si la recherche en texte intégral est activée dans une base de données.|La valeur de cette colonne est sans effet. Les bases de données utilisateur sont toujours activées pour la recherche en texte intégral.|  
