---
title: Dépannage de SQL Server Managed Backup to Windows Azure | Documents Microsoft
ms.custom: ''
ms.date: 03/08/2017
...
ms.locfileid: "36038392"
---
# <a name="troubleshooting-sql-server-managed--backup-to-windows-azure"></a>Dépannage de SQL Server Managed Backup to Windows Azure
...
2.  **Les modifications apportées aux valeurs de clé de l’accès de stockage :** si les valeurs de clé de stockage sont modifiées pour le compte Windows Azure, mais les informations d’identification SQL n’est pas mis à jour avec les nouvelles valeurs [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] échoue lors de l’authentification pour le stockage et n’effectue pas de sauvegarde bases de données configurées pour utiliser ce compte.  
  
3.  **Modifications apportées au compte de stockage Windows Azure :** suppression ou la modification du nom de compte de stockage sans entraînent des modifications correspondantes apportées à l’information d’identification SQL [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] échoue et aucune sauvegarde ne va être retirée. Si vous supprimez un compte de stockage, assurez-vous que les bases de données sont reconfigurées avec des informations de compte de stockage valides. Si un compte de stockage est renommé ou les valeurs de clé sont modifiées, vérifiez que ces changements sont répercutés dans les informations d'identification SQL utilisées par la [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)].  
  
4.  **Modifications apportées aux propriétés de la base de données :** les modifications apportées aux modèles de récupération ou la modification du nom peut provoquer des sauvegardes d’échouer.  
  
5.  **Modifications apportées au mode de récupération :** si le mode de récupération de la base de données est modifié de simple de complet ou journalisé en bloc, sauvegardes s’arrêtent et les bases de données seront ignorées par [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)]. Pour plus d’informations, consultez [SQL Server Managed Backup dans Windows Azure : interopérabilité et Coexistence](../../2014/database-engine/sql-server-managed-backup-to-windows-azure-interoperability-and-coexistence.md)  
...
3.  **Erreurs de connectivité :**  
    ...
    -   **Erreurs liées à la connectivité SQL :** ces erreurs se produisent lorsqu’il existe des problèmes de connexion à une instance de SQL Server. Les événements étendus exposent ces types d'erreurs via le canal d'administration. Voici deux événements étendus que vous pouvez voir pour les erreurs relatives à ce type de problème de connectivité :  
         ...
         *« Une erreur interne s’est produite lors de la configuration de sauvegarde managée SQL Server pour les paramètres par défaut de Windows Azure pour l’instance. Erreur peut être temporaire. »*  
  
         *« Rencontre probablement de problèmes de connectivité dans SQL Server. Elle est ignorée dans l’itération actuelle. »*  
  
         *« Interrogation des informations sur l’utilisation de journal a échoué. L'échec peut être temporaire. Elle est ignorée dans l’itération actuelle. »*  
         ...
         *« SSMBackup2WA a rencontré exception SQL while... "*  
...
-   **Sqlservr.exe ne répond plus ou s’arrête lorsque [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] est en cours d’exécution :** si SQL Server cesse de fonctionner, l’Agent SQL s’arrête, [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] également s’arrête et les événements sont consignés dans le fichier SQL Agent.out.  
     ...
     *trace de la pile, le message et code d’erreur seront affichera dans un événement étendu du canal admin, ainsi que des informations supplémentaires, telles que :*   
    *« Rencontre probablement de problèmes de connectivité dans SQL Server. Base de données est ignorée dans l’itération actuelle »*  
  
-   **L’Agent SQL ne répond plus ou s’arrête lorsque [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] est en cours d’exécution :**  
    ...
    *« Une mise à jour de progression n’a pas été reçu à partir de SQL Server dans plus de « + Constants.DBBackupInfoMsgMaxWaitTime + « heures pour la sauvegarde de base de données.   Sauvegarde de Cloud SSM doit continuer à attendre. »*  
