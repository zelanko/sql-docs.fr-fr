---
title: Définir les Options de contrôle de code Source | Documents Microsoft
ms.custom: ''
ms.date: 03/06/2017
...
3.  Définissez les informations d'identification de connexion pour chaque rôle de contrôle de code source que vous souhaitez utiliser. Cette page est disponible uniquement si un plug-in de contrôle de code source est installé.  
    ...
    |Role|Description|  
    |----------|-----------------|  
    |**Visual SourceSafe**|Spécifie que vous souhaitez utiliser les paramètres couramment utilisés par [!INCLUDE[msCoName](../includes/msconame-md.md)] les utilisateurs de Visual SourceSafe.|  
     ...
     **Effectuer des mises à jour en arrière-plan**  
     ...
     **Nom d'accès**  
     Spécifie le nom d'utilisateur à employer pour se connecter au fournisseur de contrôle de code source. Si la prise en charge par votre fournisseur de contrôle de code source, ce nom est renseigné automatiquement dans le **connexion** boîte de dialogue Atteindre votre serveur de contrôle de code source. Pour activer cette option, désactivez les connexions utilisateur automatiques à l'aide du programme d'administrateur de votre fournisseur de contrôle de code source, puis redémarrez [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)].  
...
5.  Dans le **paramètres d’environnement de contrôle de code Source** , sélectionnez le rôle sur lequel vous souhaitez définir les options de contrôle de code source.  
  
     [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] sélectionne automatiquement les options par défaut du contrôle de code source correspondant au rôle que vous avez choisi. Si vous désactivez les options par défaut, le **paramètres d’environnement de contrôle de code Source** zone affiche la **personnalisé** option pour indiquer que vous avez personnalisé le rôle sélectionné à l’origine.  
    ...
    |Action|Description|  
    |------------|-----------------|  
    |**Invite pour l’extraction**|Affiche la **Check Out** boîte de dialogue.|  
    |**Extraire automatiquement**|Extrait l’élément sans afficher le **Check Out** boîte de dialogue. Il s'agit de l'option par défaut.|  
    ...
    |------------|-----------------|  
    |**Invite pour l’extraction**|Affiche la **Check Out** boîte de dialogue.|  
    |**Demander des extractions exclusives**|Affiche la **Check Out** boîte de dialogue.|  
    |**Extraire automatiquement**|Extrait l’élément sans afficher le **Check Out** boîte de dialogue. Il s'agit de l'option par défaut.|  
    |**Ne rien faire**|N'extrait pas le fichier.|  
  
     **Autoriser les éléments archivés à modifier**  
     Spécifie que les éléments archivés peuvent être modifiés en mémoire. Si vous activez cette case à cocher, un **modifier** bouton s’affiche dans le **Check Out** boîte de dialogue lorsque vous essayez de modifier un élément archivé. Après avoir cliqué sur ce bouton, vous pouvez modifier l'élément. Si vous souhaitez enregistrer l'élément, vous devez l'extraire ou l'enregistrer dans un autre emplacement.  
