...
4.  Au sein du bloc `catch`, créez une instance de la classe <xref:Microsoft.SqlServer.MessageBox.ExceptionMessageBox>. Passer le <xref:System.Exception> objet géré par le `try` - `catch` bloc.  
