...
1. Créer un paramètre de clé de Registre pour configurer le répertoire cible d’audit local. 
    ...
    Pour le moteur de base de données et Integration Services, créez la clé dans *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL14.\<NOM_INSTANCE\>\\CPE*. 
    
    Pour Analysis Services, créez la clé dans *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSAS14.\<NOM_INSTANCE\>\\CPE*.
...
> Remarque : configurez le chemin d’accès du répertoire pour l’audit local à l’extérieur du chemin d’installation de SQL Server pour éviter que l’autorisation des fonctionnalités d’audit et de la correction entraîne des problèmes éventuels avec SQL Server.
  ...
  |------|-----------------|----------|  
  |![Case à cocher](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Case à cocher")|Disponibilité de l’espace |Pour une charge de travail modérée avec 10 bases de données environ, prévoyez environ 2 Mo d’espace disque par jour et par instance.|  
...
|![Case à cocher](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Case à cocher")|Dossiers séparés |Utilisez un dossier spécifique pour chaque service. Par exemple, pour un nom d’instance donné, vous devez avoir un dossier pour le moteur de base de données. Si une instance SSAS utilise le même nom d’instance, créez un dossier distinct pour SSAS. Lorsque le moteur de base de données et les instances Analysis Services sont configurés dans le même dossier, l’audit local écrit dans le même fichier journal pour les deux instances.| 
|![Case à cocher](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Case à cocher")|Accorder des autorisations pour le compte de connexion de service de télémétrie SQL Server CEIP|Activer l’option **Lister le contenu des dossiers**, et l’accès **En lecture** et **En écriture** au compte de connexion de service de télémétrie SQL Server CEIP|


### <a name="grant-permissions-to-the-sql-server-ciep-telemetry-service-logon-account"></a>Accorder des autorisations pour le compte de connexion de service de télémétrie SQL Server CEIP
...
1. Accédez au chemin d’accès CPE approprié. 
    ...
    Pour le moteur de base de données et Integration Services, utilisez *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL14.\<NOM_INSTANCE\>\\CPE*. 
    
    Pour Analysis Services, utilisez *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSAS14.\<NOM_INSTANCE\>\\CPE*.
