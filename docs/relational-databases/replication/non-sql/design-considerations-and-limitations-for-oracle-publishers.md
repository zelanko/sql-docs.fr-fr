...
ms.topic: "article"
helpviewer_keywords: 
  - "publication Oracle [réplication SQL Server], problèmes et limitations de conception"
...
# Probl&#232;mes et limitations de conception des serveurs de publication Oracle
  Publication à partir d’une base de données Oracle est conçue pour fonctionne de façon similaire à la publication à partir d’un [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] base de données. Toutefois, soyez conscient des problèmes et limitations suivants :  
...
## Prise en charge des objets de publication d'Oracle  
...
-   Synonymes  
 ...
 Pour plus d’informations sur les types de données pris en charge, consultez la page [mappage de Type de données pour les serveurs de publication Oracle](../../../relational-databases/replication/non-sql/data-type-mapping-for-oracle-publishers.md).  
  
## Différences entre Oracle et SQL Server  
  
-   Oracle présente des limites de taille maximale différentes pour certains objets. Tout objet créé dans la base de données de publication Oracle doit respecter les limites de taille maximale des objets correspondants dans [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]. Pour plus d’informations sur les limites de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)], consultez la page [des spécifications de capacité maximale de SQL Server](../../../sql-server/maximum-capacity-specifications-for-sql-server.md).  
...
-   Incluez une colonne non LOB dans la mise à jour de ligne en plus de la colonne LOB ou mettez à jour une colonne non LOB de la ligne au cours de la même transaction Oracle. Dans les deux cas, la mise à jour de la colonne non LOB garantit l'activation du déclencheur.  
 ...
 Pour plus d’informations sur les objets volumineux, consultez [mappage de Type de données pour les serveurs de publication Oracle](../../../relational-databases/replication/non-sql/data-type-mapping-for-oracle-publishers.md).  
...
-   Toutes les colonnes incluses dans des contraintes uniques doivent contenir des types de données pris en charge. Pour plus d’informations sur les types de données, consultez la page [mappage de Type de données pour les serveurs de publication Oracle](../../../relational-databases/replication/non-sql/data-type-mapping-for-oracle-publishers.md).  
...
-   [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] prend en charge deux types de validation : binaire et du nombre de lignes. Les serveurs de publication Oracle prennent uniquement en charge la validation du nombre de lignes. Pour plus d’informations, consultez [valider les données répliquées](../../../relational-databases/replication/validate-replicated-data.md).  
  
-   [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] offre deux formats d’instantané : bcp en mode natif et en mode caractère. Les serveurs de publication Oracle prennent en charge les instantanés en mode caractères.  
...
-   Le compte utilisé par l'Agent d'instantané et l'Agent de lecture du journal pour connecter le serveur de distribution au serveur de publication est spécifié à l'aide de l'une des méthodes suivantes :  
    ...
    -   Le **@security_mode** paramètre de [sp_adddistpublisher & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-adddistpublisher-transact-sql.md) (vous spécifiez également les valeurs de **@login** et **@password** Si l’authentification Oracle)  
  
    -   Dans le **se connecter au serveur** boîte de dialogue dans SQL Server Management Studio, vous utilisez lorsque vous configurez le serveur de publication Oracle sur le [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] serveur de distribution.  
  
     Dans la réplication transactionnelle standard, le compte est spécifié avec [sp_addpublication_snapshot & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-addpublication-snapshot-transact-sql.md) et [sp_addlogreader_agent & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-addlogreader-agent-transact-sql.md).  
  
-   Impossible de modifier le compte sous lequel l’Agent de capture instantanée et l’Agent de lecture du journal établissent des connexions avec [sp_changedistpublisher & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-changedistpublisher-transact-sql.md) ou via une propriété de la feuille, mais le mot de passe peut être modifié.  
  
-   Si vous spécifiez une valeur de 1 (authentification intégrée Windows) pour le **@security_mode** paramètre de [sp_adddistpublisher & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-adddistpublisher-transact-sql.md):  
  
    -   Le compte de processus et le mot de passe utilisé pour l’Agent de capture instantanée et l’Agent de lecture du journal (la **@job_login** et **@job_password** paramètres de [sp_addpublication_snapshot & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-addpublication-snapshot-transact-sql.md) et [sp_addlogreader_agent & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-addlogreader-agent-transact-sql.md)) doit être le même que le compte et le mot de passe utilisé pour se connecter au serveur de publication Oracle.  
  
    -   Vous ne pouvez pas modifier le **@job_login** paramètre via [sp_changepublication_snapshot & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-changepublication-snapshot-transact-sql.md) ou [sp_changelogreader_agent & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-changelogreader-agent-transact-sql.md), mais le mot de passe peut être modifié.  
  
 Pour plus d’informations sur la sécurité de la réplication, consultez [sécurité et Protection & #40 ; Réplication & #41 ;](../../../relational-databases/replication/security/security-and-protection-replication.md).  
