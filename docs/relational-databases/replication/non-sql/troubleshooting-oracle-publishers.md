...
ms.topic: "article"
helpviewer_keywords: 
  - "Publication Oracle [réplication SQL Server], dépannage"
  - "dépannage [réplication SQL Server], publication Oracle"
...
---
# D&#233;pannage des serveurs de publication Oracle
...
## Une erreur s'est produite avec le logiciel client et réseau d'Oracle  
 Le compte sous lequel [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] s’exécute sur le serveur de distribution doit bénéficier de lecture et les autorisations d’exécution pour le répertoire (et tous les sous-répertoires) dans lequel est installé le logiciel réseau du client Oracle. Si les autorisations ne sont pas accordées ou si les composants du client Oracle ne sont pas installés correctement, vous recevez le message d'erreur suivant :  
...
## Le serveur de distribution SQL Server ne peut pas se connecter à l'instance de base de données Oracle  
...
-   Le dossier de base et le chemin d'accès d'Oracle corrects sont utilisés. Même si vous n'avez qu'un ensemble de fichiers binaires Oracle installés sur le serveur de distribution [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] , vérifiez que les variables d'environnement relatives au dossier de base d'Oracle sont définies correctement. Si vous modifiez des valeurs pour les variables d'environnement, vous devez arrêter et redémarrer [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] pour que les modifications soient prises en compte.  
 ...
 Pour plus d’informations sur la configuration et test de la connectivité, consultez la rubrique « Installing and Configuring logiciel mise en réseau de Client Oracle sur le [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] distributeur » dans [configurer un serveur de publication Oracle](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
...
-   « Instance de serveur oracle ' \<*OraclePublisherName*>' a été précédemment configuré pour utiliser ' \<*SQLServerDistributorName*> » en tant que serveur de distribution. Pour commencer à utiliser ' \<*NewSQLServerDistributorName*> » en tant que serveur de distribution, vous devez supprimer la configuration actuelle de la réplication sur l’instance de serveur Oracle, ce qui supprimera toutes les publications sur cette instance de serveur. »  
  
-   « Serveur oracle ' \<*OracleServerName*>' est déjà défini comme serveur de publication ' \<*OraclePublisherName*>' sur le serveur de distribution ' \<*SQLServerDistributorName*>.*\< DistributionDatabaseName>*». Supprimez le serveur de publication ou le synonyme public '*\< SynonymName>*' recréer. »  
...
## Une erreur SQL Server 21663 est provoquée par l'absence d'une clé primaire  
 ...
 « Aucune clé primaire valide ne trouvé pour la table source [\<*TableOwner*>]. [ \<*TableName*>] »  
...
## Une erreur SQL Server 21642 est provoquée par une connexion à un serveur lié en double  
 ...
 Cette erreur peut se produire si vous tentez de créer directement un serveur lié ou si vous avez précédemment supprimé la relation entre le serveur de publication Oracle et le serveur de distribution [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] , et que vous tentez de le reconfigurer. Si vous recevez cette erreur en tentant de reconfigurer le serveur de publication, supprimez le serveur lié avec [sp_dropserver & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-dropserver-transact-sql.md).  
  
 Si vous avez besoin pour vous connecter au serveur de publication Oracle via une connexion de serveur lié, créez un autre nom du service TNS et ensuite utiliser ce nom lors de l’appel [sp_addlinkedserver & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md). Pour des informations sur la création de noms de service TNS, consultez la documentation d'Oracle.  
  
## Une erreur SQL Server 21617 est signalée  
 La publication Oracle utilise l'application Oracle SQL*PLUS pour télécharger le package de code de prise en charge du serveur de publication dans la base de données Oracle. Avant de tenter de configurer le serveur de publication Oracle, [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] vérifie que SQL\*PLUS est accessible via le chemin d’accès système sur le serveur de distribution. Si SQL\*et ne peut pas être chargé, le message d’erreur suivant apparaît :  
...
## Une erreur SQL Server 21620 est signalée  
...
## Une erreur SQL Server 21624 ou 21629 est signalée  
...
## Une erreur SQL Server 21626 ou 21627 est signalée  
...
## Une erreur SQL Server 21628 est signalée  
...
## Vous rencontrez des problèmes d'autorisations pour le schéma utilisateur de réplication  
 Le schéma utilisateur de réplication doit disposer des autorisations décrites dans « Création manuelle du schéma utilisateur » dans [configurer un serveur de publication Oracle](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md).  
...
## Erreur Oracle ORA-01000  
 ...
 Pour éviter ce problème, assurez-vous que le **max_open_cursors** dans les bases de données Oracle est défini sur un nombre suffisamment élevé (au moins 1 000). Pour plus d'informations sur ce paramètre, consultez la documentation Oracle.  
...
2.  Avec le bouton droit **réplication**, puis cliquez sur **des propriétés de serveur de distribution**.  
...
-   Exécutez **sp_dropdistpublisher**. Pour plus d’informations, consultez [sp_dropdistpublisher & #40 ; Transact-SQL & #41 ;](../../../relational-databases/system-stored-procedures/sp-dropdistpublisher-transact-sql.md).  
