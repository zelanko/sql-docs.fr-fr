...
helpviewer_keywords: 
  ...
  - "websync.log"
  - "synchronisation web, serveurs IIS"
...
-   [! INCLURE [ssManStudioFull] (.. / Token/ssManStudioFull_md.md)]: [How to: Configure a Subscription to Use Web Synchronization \(SQL Server Management Studio\)](http://msdn.microsoft.com/library/ms345214.aspx)  
  
-   Réplication [!INCLUDE[tsql](../../includes/tsql-md.md)] programming : [Comment : configurer un abonnement pour utiliser la synchronisation Web (programmation de réplication Transact-SQL)](http://msdn.microsoft.com/library/ms345206.aspx)  
  
-   RMO : [Comment : configurer un abonnement pour utiliser la synchronisation Web (programmation RMO)](http://msdn.microsoft.com/library/ms345207.aspx)  
...
#### Pour spécifier un certificat pour un déploiement  
  
1.  Connectez-vous à l'ordinateur exécutant IIS en tant qu'administrateur.  
  
2.  Démarrer **Internet Information Services (IIS) Manager**:  
...
3.  Exécutez l'Assistant Certificat d'IIS :  
    ...
    1.  Dans **Gestionnaire des Services Internet (IIS)**, développez la **ordinateur local** nœud, puis développez le **Sites Web** dossier.  
  
    2.  Cliquez sur **Site Web par défaut**, puis cliquez sur **propriétés**.  
...
#### Pour spécifier un certificat pour des tests dans IIS 6.0  
  
1.  Connectez-vous à l'ordinateur exécutant IIS en tant qu'administrateur.  
  
2.  Téléchargez et installez SelfSSL. Par défaut, l’application est installée dans \<*lecteur*> : Files\IIS Resources\SelfSSL. Raccourcis d’application et la documentation sont copiés vers \<*lecteur*> : \Documents and Settings\All Users\Menu démarrer\programmes\iis Resources\SelfSSL.  
...
## Installation des composants de connectivité et de SQL Server Management Studio  
  
#### Pour installer les composants de connectivité de SQL Server et SQL Server Management Studio  
  
1.  Connectez-vous en tant qu'administrateur à l'ordinateur exécutant IIS.  
  
2.  Démarrez l'Assistant Installation de [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] à partir du disque d'installation de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] . Pour plus d’informations sur l’utilisation de cet Assistant, consultez la page [installer SQL Server 2016 à partir de l’Assistant Installation de & #40 ; Le programme d’installation & #41 ;](../../database-engine/install-windows/install-sql-server-2016-from-the-installation-wizard-setup.md).  
  
3.  Dans la page **Sélection de composant** , sélectionnez **Connectivité des outils clients**.  
  
4.  Si vous envisagez d’utiliser l’Assistant Configurer la synchronisation Web, sélectionnez **Outils de gestion - de base**.  
...
## Configuration de l'ordinateur exécutant IIS à l'aide de l'Assistant Configuration de la synchronisation Web  
...
3.  Développez le **Publications locales** dossier, avec le bouton droit de la publication, puis cliquez sur **configurer la synchronisation Web**.  
...
8.  Sur la page **Accès à l'annuaire** :  
    ...
    1.  Cliquez sur **Ajouter**puis, dans la boîte de dialogue **Sélectionner des utilisateurs ou des groupes** , ajoutez les comptes sous lesquels les Abonnés établiront des connexions à IIS. Ce sont les comptes que vous spécifiez sur la **informations sur le serveur Web** page de l’Assistant Nouvel abonnement ou en tant que la valeur de la [sp_addmergepullsubscription_agent](../../relational-databases/system-stored-procedures/sp-addmergepullsubscription-agent-transact-sql.md)*@internet_login* paramètre.  
...
## Configuration manuelle de l'ordinateur exécutant IIS  
...
1.  Créez un répertoire de fichiers sur l'ordinateur exécutant IIS pour y placer le fichier replisapi.dll. Vous pouvez créer le répertoire où vous souhaitez, mais nous vous recommandons de créer le répertoire du \<*lecteur*> : \Inetpub active. Par exemple, créez le répertoire \<*lecteur*> : \Inetpub\SQLReplication\\.  
...
1.  Créez un compte local sur l'ordinateur exécutant IIS :  
    ...
    1.  Avec le bouton droit **Poste de travail**, puis cliquez sur **gérer**.  
...
2.  Ajoutez le compte au groupe IIS_WPG :  
    ...
    2.  Avec le bouton droit **IIS_WPG**, puis cliquez sur **Ajouter au groupe**.  
  
    3.  Dans la **Propriétés de IIS_WPG** boîte de dialogue, cliquez sur **Ajouter**.  
    ...
    6.  Dans la **Sélectionner des utilisateurs** boîte de dialogue et les **Propriétés de IIS_WPG** boîte de dialogue, cliquez sur **OK**.  
  
3.  Attribuez au compte les autorisations minimales sur le dossier qui contient replisapi.dll :  
  
    1.  Recherchez le dossier que vous avez créé pour replisapi.dll, cliquez avec le bouton droit, puis cliquez sur **partage et sécurité**.  
    ...
    5.  Assurez-vous que le compte est accordé uniquement **en lecture**, **Lecture & exécution**, et **répertorier le contenu du dossier** autorisations.  
  
    6.  Sélectionnez tous les utilisateurs ou groupes qui ne requièrent pas l'accès au répertoire, puis cliquez sur **Supprimer**.  
  
    7.  Cliquez sur **OK**.  
  
4.  Créer un pool d’applications dans **Gestionnaire des Services Internet (IIS)**:  
  
    1.  Cliquez sur **Démarrer**, puis sur **Exécuter**.  
  
    2.  Dans la zone **Ouvrir** , tapez **inetmgr**, puis cliquez sur **OK**.  
  
    3.  Dans **Gestionnaire des Services Internet (IIS)**, développez la **ordinateur local** nœud.  
  
    4.  Avec le bouton droit **Pools d’applications**, pointez sur **New** puis **Pool d’applications**.  
...
5.  Associez le compte au pool d'applications :  
    ...
    1.  Dans **Gestionnaire des Services Internet (IIS)**, développez la **ordinateur local** nœud, puis développez **Pools d’applications**.  
  
    2.  Cliquez sur le pool d’applications que vous avez créé, puis cliquez sur **propriétés**.  
  
    3.  Dans la **\< Nom_pool_applications> propriétés** boîte de dialogue du **identité** sous l’onglet **Configurable**.  
...
6.  Associez le pool d'applications au répertoire virtuel utilisé pour la synchronisation Web :  
    ...
    1.  Dans **Gestionnaire des Services Internet (IIS)**, développez la **ordinateur local** nœud, puis développez **Sites Web**.  
  
    2.  Développez le site Web que vous utilisez pour la synchronisation Web, cliquez sur le répertoire virtuel que vous avez créé pour la synchronisation Web, puis cliquez sur **propriétés**.  
  
    3.  Sur le **répertoire virtuel** onglet de la **\< NomRépertoireVirtuel> propriétés** boîte de dialogue, à partir de la **pool d’applications** liste déroulante, sélectionnez le pool d’applications qui a été créé à l’étape 5.  
