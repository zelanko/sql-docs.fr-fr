...
ms.topic: "article"
helpviewer_keywords: 
  - "Agent de fusion, exécutables"
  - "Agent de fusion, référence de paramètre"
  - "agents [réplication SQL Server], Agent de fusion"
 ...
 **-Abonné** *nom_serveur*[**\\***instance_name*]  
 Nom de l'Abonné. Spécifiez *nom_serveur* pour l’instance par défaut de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] sur ce serveur. Spécifiez *nom_serveur***\\***instance_name* pour une instance nommée de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] sur ce serveur.  
 ...
 **-DownloadGenerationsPerBatch** *téléchargement_de_générations_par_lot*  
 ...
 **-DownloadReadChangesPerBatch** *téléchargement_de_modifications_de_lecture_par_lot*  
 ...
 **-FastRowCount** [**0**|**1**]  
 Spécifie le type de méthode de calcul du nombre de lignes à utiliser pour la validation du nombre de lignes. Valeur **1** (par défaut) indique la méthode rapide. La valeur **0** indique la méthode complète de calcul du nombre de lignes.  
  
 **-FileTransferType** [**0**|**1**]  
 Spécifie le type de transfert de fichier. Une valeur de **0** UNC (convention de dénomination universelle), et la valeur de **1** indique FTP (protocole de transfert de fichiers).  
  
 **-ForceConvergenceLevel** [**0**|**1**|**2** ( **Publisher**| **abonné**| **les**)]  
...
|**3**|Identique à **- HistoryVerboseLevel** = **2**, sauf que plusieurs messages de progression de l’agent sont enregistrés.|  
 ...
 **-InternetProxyServer**  *internet_proxy_server*  
 Spécifie le serveur proxy pour accéder à la ressource HTTP spécifiée dans *internet_url*.  
  
 **-InternetSecurityMode** [**0**|**1**]  
 Spécifie le mode de sécurité IIS utilisé lors de la connexion au serveur Web pendant la synchronisation Web. Une valeur de **0** l’authentification de base, et la valeur de **1** indique l’authentification intégrée Windows (par défaut).  
 ...
 **-MakeGenerationInterval** *make_generation_interval_seconds*  
 ...
 Makegeneration est le processus qui prépare les modifications du serveur de publication à télécharger vers les abonnés et peut constituer un goulot d'étranglement lors des téléchargements. Si le processus makegeneration est déjà exécuté dans l’intervalle spécifié par **- MakeGenerationInterval**, le processus est ignoré pour la session de synchronisation en cours. Cela peut profiter à la concurrence de synchronisation et s'avère particulièrement utile si les abonnés ne pensent pas télécharger les modifications.  
 ...
 **-MetadataRetentionCleanup** [**0**|**1**]  
 Spécifie si les métadonnées sont supprimées de [MSmerge_genhistory](../../../relational-databases/system-tables/msmerge-genhistory-transact-sql.md), [MSmerge_contents](../../../relational-databases/system-tables/msmerge-contents-transact-sql.md), [MSmerge_tombstone](../../../relational-databases/system-tables/msmerge-tombstone-transact-sql.md), [MSmerge_past_partition_mappings](../../../relational-databases/system-tables/msmerge-past-partition-mappings-transact-sql.md), et [MSmerge_current_partition_mappings](../../../relational-databases/system-tables/msmerge-current-partition-mappings.md) en fonction de la période de rétention de publication. La valeur par défaut est **1**, indiquant que le nettoyage doit se produire. La valeur **0** indique que le nettoyage ne doit pas avoir lieu automatiquement.  
 ...
 **-PublisherLogin** *publisher_login*  
 Nom de connexion du serveur de publication. Si **PublisherSecurityMode** est **0** (pour [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] l’authentification), ce paramètre doit être spécifié.  
  
 **-PublisherPassword** *publisher_password*  
 Mot de passe du serveur de publication. Si **PublisherSecurityMode** est **0** (pour [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] l’authentification), ce paramètre doit être spécifié.  
 ...
 **-StartQueueTimeout** *start_queue_timeout_seconds*  
 Est le nombre maximal de secondes pendant lesquelles l’Agent de fusion attend lorsque le nombre de processus de fusion simultanés en cours d’exécution a atteint la limite définie par le **@max_concurrent_merge** propriété du **sp_addmergepublication**. Si le nombre maximal de secondes est atteint et que l'Agent de fusion attend toujours, celui-ci se ferme. La valeur 0 signifie que l'Agent attend indéfiniment, bien qu'il soit possible de l'annuler.  
  
 **-SubscriberDatabasePath** *subscriber_database_path*  
 Est le chemin d’accès à la base de données Jet (fichier .mdb) si **SubscriberType** est **2** (autorise une connexion à une base de données Jet sans un nom de Source de données (DSN) ODBC).  
...
> [!NOTE]  
>  Lorsque vous utilisez des valeurs **2** et **3**, le chemin d’accès de base de données de l’abonné doit être spécifié dans le **SubscriberDatabasePath** option.  
  
 **-SubscriberLogin** *subscriber_login*  
 Nom de connexion de l'Abonné. Si **SubscriberSecurityMode** est **0** (pour [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] l’authentification), ce paramètre doit être spécifié.  
  
 **-SubscriberPassword** *subscriber_password*  
 Mot de passe de l'Abonné. Si **SubscriberSecurityMode** est **0** (pour [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] l’authentification), ce paramètre doit être spécifié.  
  
 **-SubscriberSecurityMode** [ **0**| **1**]  
 Spécifie le mode de sécurité de l'Abonné. Une valeur de **0** indique [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Authentication (par défaut) et la valeur **1** indique le Mode d’authentification Windows.  
  
 **-SubscriberConflictClean** [ **0**| **1**]  
 Si les tables de conflits sont nettoyées sur l’abonné pendant le processus de synchronisation, où la valeur d’est **1** indique que les tables de conflits sur l’abonné sont nettoyées. Ce paramètre est utilisé uniquement pour les abonnements aux publications avec une journalisation des conflits décentralisée.  
 ...
 **-SubscriptionType**[ **0**| **1**| **2**]  
 Spécifie le type d'abonnement pour la distribution. Une valeur de **0** indique un abonnement envoyé (par défaut), une valeur de **1** indique un abonnement par extraction et la valeur **2** indique un abonnement anonyme.  
 ...
 **-UploadGenerationsPerBatch** *télédéchargement_de_générations_par_lot*  
 ...
 **-UploadReadChangesPerBatch** *télédéchargement_de_modifications_de_lecture_par_lot*  
