...
helpviewer_keywords: 
  - "synchronisation web, meilleures pratiques de sécurité"
  - "synchronisation web, configuration"
...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx_md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]
  ...
  L’option de synchronisation Web pour [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] la réplication de fusion permet la réplication des données à l’aide du protocole HTTPS sur Internet. Pour utiliser la synchronisation Web, vous devez d'abord effectuer les actions de configuration suivantes :  
  
1.  Créer des comptes de domaine et mapper [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] connexions.  
...
> [!NOTE]  
>  Si vous souhaitez répliquer d’importants volumes de données ou utiliser des types de données de grande taille tels que **varchar (max)**, lisez la section « Réplication d’importants Volumes de données » dans cette rubrique.  
...
## Création de nouveaux comptes et mappage des connexions SQL Server  
 ...
 Le compte utilisé pour le [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] écouteur de réplication doit disposer des autorisations comme décrit dans [sécurité de l’Agent de fusion](../../relational-databases/replication/merge-agent-security.md), sous la section « Se connecter à la publication ou de distribution ». En résumé, le compte doit :  
...
## Configuration de l'ordinateur qui exécute IIS  
 ...
 La synchronisation Web est pris en charge à partir de IIS version 5.0. L'Assistant Configuration de la synchronisation Web n'est pas pris en charge sur IIS version 7.0. À partir de SQL Server 2012, pour utiliser le composant de synchronisation web sur le serveur IIS, il est recommandé que les utilisateurs installent SQL Server avec la réplication. Cela peut être l’édition gratuite de SQL Server Express.  
...
1.  Dans **Gestionnaire des Services Internet (IIS)**, développez le nœud du serveur local, puis cliquez sur le **Pool d’applications** nœud.  
  
2.  Sélectionnez le pool d’applications associé au site de synchronisation Web, puis cliquez sur **Paramètres avancés** sur la **Actions** volet.  
  
3.  Dans la boîte de dialogue Paramètres avancés, sous la **modèle de processus** titre, cliquez sur la ligne libellée **nombre maximal de processus de travail**. Modifiez la valeur de propriété, puis cliquez sur **OK**.  
...
## Configuration de la publication  
 ...
 Une fois la publication créée, activez l’option qui autorise la synchronisation Web en utilisant l’une des méthodes suivantes : [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)], [!INCLUDE[tsql](../../includes/tsql-md.md)], ou Replication Management Objects (RMO). Pour activer la synchronisation Web, vous devez fournir l'adresse du serveur Web pour les connexions des abonnés.  
 ...
 **Gen** est un mot réservé dans websync fichiers xml. N’essayez pas de publier des tables contenant des colonnes nommées **gen**.  
...
## Mise à niveau à partir d'une version antérieure de SQL Server  
 Si vous disposez d’une topologie de synchronisation Web existante configurée et que vous mettez à niveau [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], vous devez vous assurer que la version la plus récente de Replisapi.dll est copiée vers le répertoire virtuel utilisé par la synchronisation Web. Par défaut, la version la plus récente de Replisapi.dll se trouve dans C:\Program Files\Microsoft SQL Server\\< nnn\>\COM.  
...
-   Vous ne pouvez pas répliquer une colonne qui générerait un volume de données et de métadonnées supérieur à 25 Mo. Cela peut poser un problème lorsque vous répliquez des lignes qui contiennent des types de données volumineux, tels que **varchar (max)**.  
  
-   Si vous répliquez d'importants volumes de données, vous pouvez être amené à ajuster la taille de lot de l'Agent de fusion.  
  
 Taille de lot pour la réplication de fusion est mesurée en *générations*, qui sont des ensembles de modifications par article. Le nombre de générations dans un lot est spécifié en utilisant le –**DownloadGenerationsPerBatch** et –**UploadGenerationsPerBatch** paramètres de l’Agent de fusion. Pour plus d’informations, consultez [Replication Merge Agent](../../relational-databases/replication/agents/replication-merge-agent.md).  
...
-   Le [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] distributeur et l’éditeur peuvent être sur le même ordinateur (une installation par défaut pour la réplication de fusion). IIS doit cependant être installé sur un ordinateur distinct.  
...
-   Si les serveurs de la topologie de réplication se trouvent derrière un pare-feu, vous devrez peut-être ouvrir les ports dans le pare-feu afin d'activer la synchronisation Web.  
    ...
    -   L'ordinateur de l'abonné se connecte à l'ordinateur qui exécute IIS sur HTTPS à l'aide du protocole SSL, qui est généralement configuré pour utiliser le port 443. [!INCLUDE[ssEW](../../includes/ssew-md.md)] Les abonnés peuvent également se connecter via HTTP, ce qui est généralement configuré pour utiliser le port 80.  
  
    -   L'ordinateur chargé d'exécuter IIS se connecte généralement au serveur de publication ou au serveur de distribution via le port 1433 (instance par défaut). Lorsque le serveur de publication ou de distribution est une instance nommée sur un serveur avec une autre instance par défaut, le port 1500 est généralement utilisé pour se connecter à l’instance nommée.  
  
    -   Si l’ordinateur exécutant IIS est séparé du serveur de distribution par un pare-feu et un partage FTP est utilisé pour la remise d’instantané, les ports utilisés pour FTP doivent être ouvert. Pour plus d’informations, consultez [instantanés via FTP Transfer](../../relational-databases/replication/transfer-snapshots-through-ftp.md).  
