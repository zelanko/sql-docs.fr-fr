...
---
# Configurer la topologie (r&#233;plication d&#39;&#233;gal &#224; &#233;gal)
  Utilisez le **configurer la topologie** page pour effectuer des tâches de configuration courantes, telles que l’ajout de nouveaux nœuds, la suppression de nœuds et l’ajout de nouvelles connexions entre des nœuds existants. Le nœud sélectionné dans le **Publication** page de cet Assistant s’affiche sur l’aire de conception. Pour spécifier des options de configuration, cliquez avec le bouton droit sur un nœud, une connexion ou l'aire de conception.  
...
## Options  
 Le **configurer la topologie** page contient les éléments d’interface et les options qui sont disponibles lorsque vous cliquez sur un élément. Le tableau suivant décrit chaque élément d'interface.  
...
|![Premier nœud d'une topologie](../../relational-databases/replication/media/p2pwizard-firstnode.png "Premier nœud d'une topologie")|Nœud d'origine dans la topologie. Les nouveaux nœuds sont initialisés grâce à une copie de la base de données de publication du nœud d'origine.|  
|![Nœud pour lequel nous avons des informations complètes](../../relational-databases/replication/media/p2pwizard-complete.png "Nœud pour lequel nous avons des informations complètes")|Un nœud qui exécute une instance de [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] ou une version ultérieure, pour lesquels la réplication possède des informations complètes. Pour spécifier des options de configuration, cliquez avec le bouton droit sur le nœud.|  
|![Nœud pour lequel nous avons des informations incomplètes](../../relational-databases/replication/media/p2pwizard-incomplete.png "Nœud pour lequel nous avons des informations incomplètes")|Nœud sur lequel la réplication possède des informations incomplètes. Pour spécifier des options de configuration, cliquez avec le bouton droit sur le nœud.<br /><br /> La réplication possède des informations incomplètes pour l'une des raisons suivantes :<br /><br /> -Le nœud exécute une instance de [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)], qui ne stocke pas toutes les métadonnées requises par l’Assistant.<br /><br /> -Le nœud exécute une version ultérieure de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], mais la réplication ne peut pas récupérer les informations d’abonnement à partir du nœud. Pour résoudre ce problème :<br /><br /> Assurez-vous que la base de données au niveau du nœud est en ligne et que vous pouvez vous connecter à celle-ci en utilisant les mêmes informations d'identification que les Agents de distribution qui se connectent au nœud.<br /><br /> Assurez-vous que l'Agent de lecture du journal et les Agents de distribution qui se connectent au nœud sont en cours d'exécution.<br /><br /> Assurez-vous que le délai d'actualisation est suffisant pour collecter toutes les informations sur la topologie. Pour définir le délai d’attente, cliquez sur l’aire de conception, puis cliquez sur **définir un délai d’actualisation**.|  
...
### Options relatives à l'aire de conception  
...
### Options pour les flèches de connexion  
