...
ms.topic: "article"
helpviewer_keywords: 
  - "réplication transactionnelle, méthodes de propagation"
...
-   **sp_Msdel_ \<** *tablename* **>**, qui gère les suppressions.  
 ...
 Le **\<***tablename***>** utilisé dans la procédure varie selon la façon dont il a été ajouté à la publication et si la base de données d’abonnement contient une table portant le même nom avec un propriétaire différent.  
...
### Considérations sur l'utilisation des procédures stockées personnalisées  
...
-   Si vous modifiez le schéma d'une table publiée, les procédures personnalisées doivent être régénérées. Pour plus d’informations, consultez [régénérer des procédures transactionnelles personnalisées pour refléter les modifications de schéma](../../../relational-databases/replication/transactional/regenerate-custom-transactional-procedures-to-reflect-schema-changes.md).  
  
-   Si vous utilisez une valeur supérieure à 1 pour **- SubscriptionStreams** paramètre de l’Agent de Distribution, vous devez vous assurer que les mises à jour des colonnes de clé primaire sont réussies. Exemple :  
...
#### Syntaxe SCALL  
 Procédures stockées UPDATE  
 Les procédures stockées gérant des instructions UPDATE recevront les valeurs mises à jour uniquement pour les colonnes qui ont changé par les valeurs d’origine pour les colonnes de clé primaire, suivi par un masque de bits (**Binary**) paramètre qui indique les colonnes modifiées. Dans l'exemple suivant, la colonne 2 (c2) n'a pas été modifiée :  
...
#### Syntaxe MCALL  
 Procédures stockées UPDATE  
 Les procédures stockées gérant des instructions UPDATE recevront les valeurs mises à jour pour toutes les colonnes définies dans l’article, suivie des valeurs d’origine pour les colonnes de clé primaire, suivies d’un masque de bits (**Binary**) paramètre qui indique les colonnes modifiées :  
...
> [!NOTE]  
>  Lorsque vous utilisez XCALL, l’avant les valeurs de l’image pour **texte** et **image** les colonnes sont censées être NULL.  
