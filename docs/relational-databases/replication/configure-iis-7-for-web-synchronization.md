...
ms.topic: "article"
helpviewer_keywords: 
  - "configuration du serveur IIS 7 [réplication SQL Server]"
  - "synchronisation web, serveurs IIS 7"
...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx_md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]
  ...
  Les procédures décrites dans cette rubrique va vous guider dans le processus de configuration manuelle [!INCLUDE[msCoName](../../includes/msconame-md.md)] Internet Information Services (IIS) version 7 et versions ultérieur pour utilisation avec la synchronisation Web pour la réplication de fusion. 
  
 Configuration d’IIS 7 ou version ultérieure est le premier des trois étapes nécessaires pour activer la synchronisation Web.  
  
 Pour une vue d’ensemble du processus de configuration complète, consultez [configurer la synchronisation Web](../../relational-databases/replication/configure-web-synchronization.md).  
...
La synchronisation Web est pris en charge sur IIS, depuis la version 5.0. La configuration Web synchronisation Assistant des services IIS version 5 et 6, n’est pas disponible avec IIS version 7.0 et versions ultérieures. **À partir de SQL Server 2012, pour utiliser le composant de synchronisation web sur le serveur IIS, vous devez installer SQL Server avec la réplication. Cela peut être l’édition gratuite de SQL Server Express.**
  
#### Pour installer et configurer l'écouteur de réplication SQL Server  
  
1.  Installation de la réplication de SQL Server sur l’ordinateur IIS.

2. Créez un nouveau répertoire de fichiers pour replisapi.dll sur l'ordinateur exécutant IIS. Vous pouvez créer le répertoire où vous souhaitez, mais nous vous recommandons de créer le répertoire du \<*lecteur*> : \Inetpub active. Par exemple, créez le répertoire \<*lecteur*> : \Inetpub\SQLReplication\\.  
  
3.  Copiez replisapi.dll à partir du répertoire [!INCLUDE[ssInstallPathVar](../../includes/ssinstallpathvar-md.md)]com\ vers le répertoire du fichier que vous avez créé à l’étape 1.  
...
6.  Créez un répertoire virtuel dans IIS. Le répertoire virtuel doit être créé sous le site Web créé à l'étape 4 et doit être mappé au répertoire créé à l'étape 1. Soyez aussi restrictif que possible lors de l'attribution des autorisations sur ce répertoire. Vous devez sélectionner au moins les autorisations **Lire** et **Exécuter** .  
    ...
    1.  Dans **Gestionnaire des Services Internet (IIS)**, dans le **connexions** volet, avec le bouton droit **Site Web par défaut**, puis sélectionnez **Ajouter un répertoire virtuel**.  
...
7.  Configurez IIS pour permettre à replisapi.dll de s'exécuter.  
    ...
    1.  Dans **Gestionnaire des Services Internet (IIS)**, cliquez sur **Site Web par défaut**.  
    ...
    5.  À partir de la **Module** la liste déroulante, sélectionnez **IsapiModule**.  
    ...
    10. Vérifiez que Replisapi.dll apparaît sous les mappages de gestionnaire **Activé** . S’il s’agit de la **désactivé** liste, cliquez sur l’entrée Replisapi, puis **Modifier les autorisations de fonction**. Activez la case à cocher **Exécuter** , puis cliquez sur **OK**.  
...
2.  Dans le volet du milieu, double-cliquez sur **authentification**.  
...
1.  Dans **Gestionnaire des Services Internet (IIS)**, développez le nœud du serveur local, puis cliquez sur le **Site Web par défaut** (ou votre site Web de synchronisation s’il est différent du site Web par défaut).  
  
2.  Dans le volet du milieu, double-cliquez sur **paramètres SSL**.  
...
1.  Dans **Internet Information Services (IIS) Manager**, et cliquez sur le nœud du serveur local, puis, dans le volet central, double-cliquez sur **certificats de serveur**.  
  
2.  Dans la **Actions** volet, cliquez sur **créer un certificat auto-signé**.  
  
3.  Dans le **créer un certificat auto-signé** boîte de dialogue, entrez un nom pour le certificat, puis cliquez sur **OK**.  
  
###### Pour lier un certificat à un site Web  
  
1.  Dans la **connexions** volet, cliquez sur le **Site Web par défaut** (ou votre site Web de synchronisation, s’il est différent du site Web par défaut).  
  
2.  Dans le volet **Actions** , cliquez sur **Liaisons**, puis sur **Ajouter**. La boîte de dialogue **Ajouter la liaison de site** s'affiche.  
  
3.  À partir de la **Type** la liste déroulante, sélectionnez **https**. Laissez les paramètres par défaut pour **Adresse IP** et **Port**.  
  
4.  À partir de la **certificat SSL** cliquez sur le menu déroulant, sélectionnez le certificat créé dans « Pour créer un certificat auto-signé pour le test, » **OK**, puis cliquez sur **Fermer**.  
  
###### Pour tester le certificat  
  
1.  Dans **Gestionnaire des Services Internet (IIS)**, cliquez sur **Site Web par défaut.**  
  
2.  À partir de la **Actions** volet, cliquez sur **Parcourir \*: 443(https)**.  
  
3.  Internet Explorer ouvrira et affichera un message indiquant : « le certificat de sécurité de ce site Web présente un problème ». Cet avertissement indique que le certificat associé n'a pas été émis par une autorité de certification reconnue et n'est peut-être pas digne de confiance. Il s’agit d’un avertissement attendu, cliquez sur **poursuivre sur ce site Web (non recommandé)**.  
...
## Définition des autorisations pour l'écouteur de réplication SQL Server  
 ...
 En plus de suivre les étapes ci-dessous, vérifiez que les noms de connexion requis se trouvent dans la liste d'accès à la publication (PAL, Publication Access List). Pour plus d’informations sur la publication, consultez [sécuriser le serveur de publication](../../relational-databases/replication/security/secure-the-publisher.md).  
...
-   Être un membre de la liste d’accès Publication (PAL).  
...
1.  Créez un compte local sur l'ordinateur exécutant IIS :  
    ...
    1.  Ouvrez **Gestionnaire de serveur**. Dans le menu Démarrer, cliquez sur **ordinateur**, puis cliquez sur **gérer**.  
  
    2.  Dans **Gestionnaire de serveur**, développez **Configuration**, puis développez **Utilisateurs et groupes locaux**.  
  
    3.  Avec le bouton droit **utilisateurs**, puis cliquez sur **nouvel utilisateur**.  
...
2.  Ajoutez le compte au groupe IIS_IUSRS :  
    ...
    2.  Avec le bouton droit **IIS_IUSRS**, puis cliquez sur **Ajouter au groupe**.  
  
    3.  Dans la **Propriétés de IIS_IUSRS** boîte de dialogue, cliquez sur **Ajouter**.  
  
    4.  Dans la boîte de dialogue **Choisir des utilisateurs, des ordinateurs ou des groupes** , ajoutez le compte créé à l'étape 1.  
  
    5.  Vérifiez que **à partir de cet emplacement** affiche le nom de l’ordinateur local (et non un domaine). Si ce champ n'indique pas le nom de l'ordinateur local, cliquez sur **Emplacements**. Dans la boîte de dialogue **Emplacements** , choisissez l'ordinateur local, puis cliquez sur **OK**.  
  
    6.  Dans la **Sélectionner des utilisateurs** boîte de dialogue et les **Propriétés de IIS_IUSRS** boîte de dialogue, cliquez sur**OK**.  
  
3.  Attribuez des autorisations de compte minimales sur le dossier qui contient replisapi.dll :  
  
    1.  Dans l’Explorateur Windows, cliquez sur le dossier que vous avez créé pour replisapi.dll, puis cliquez sur **propriétés**.  
  
    2.  Sur l'onglet **Sécurité** , cliquez sur **Modifier**.  
  
    3.  Dans la **autorisations pour \< NomDossier>** boîte de dialogue, cliquez sur **Add** pour ajouter le compte que vous avez créé à l’étape 1.  
  
    4.  Vérifiez que **à partir de cet emplacement** affiche le nom de l’ordinateur local (et non un domaine). Si ce champ n'indique pas le nom de l'ordinateur local, cliquez sur **Emplacements**. Dans la boîte de dialogue **Emplacements** , choisissez l'ordinateur local, puis cliquez sur **OK**.  
  
    5.  Vérifiez que le compte est accordé uniquement **en lecture**, **Lecture & exécution**, et **répertorier le contenu du dossier** autorisations.  
...
4.  Créer un pool d’applications dans **Internet Information Services (IIS) Manager**:  
  
    1.  Dans **Gestionnaire des Services Internet (IIS)**, dans le **connexions** volet, développez le nœud du serveur local.  
  
    2.  Avec le bouton droit **Pools d’applications**, puis cliquez sur **Ajouter un Pool d’applications**.  
...
5.  Associez le compte au pool d'applications :  
    ...
    1.  Dans **Gestionnaire des Services Internet (IIS)**, développez le nœud du serveur local, puis cliquez sur **Pools d’applications**.  
  
    2.  Cliquez sur le pool d’applications que vous avez créé, puis cliquez sur **définition par défaut de Pool d’applications**.  
...
6.  Associez le pool d'applications au site Web de réplication :  
    ...
    4.  À partir de la **pool d’applications** la liste déroulante, sélectionnez le pool d’applications que vous avez créé à l’étape 4, puis cliquez sur **OK**.  
  
    5.  Cliquez à nouveau sur **OK** pour fermer Paramètres avancés.  
  
## Test de la connexion à replisapi.dll  
 Exécutez la synchronisation Web en mode de diagnostic pour tester la connexion à l’ordinateur exécutant IIS et vous assurer que le certificat Secure Sockets Layer (SSL) est correctement installé. Pour exécuter la synchronisation Web en mode de diagnostic, vous devez avoir la qualité d'administrateur sur l'ordinateur qui exécute les services Internet (IIS).  
...
2.  Sur l'Abonné, dans Internet Explorer, connectez-vous au serveur en mode de diagnostic en ajoutant `?diag` à l'adresse pour le fichier replisapi.dll. Par exemple : **https://server.domain.com/directory/replisapi.dll?diag**.  
    ...
    > [!NOTE]  
    >  Dans l’exemple ci-dessus, **serveur.domaine.com** doit être remplacé par le texte exact **délivré à** nom sous la **des certificats de serveur** section dans le Gestionnaire des services Internet.  
...
4.  Dans le **vous connecter à \< nom_serveur>** boîte de dialogue, spécifiez la connexion et le mot de passe que l’Agent de fusion utilisera pour se connecter à IIS. Ces informations d'identification seront aussi spécifiées dans l'Assistant Nouvel abonnement.  
