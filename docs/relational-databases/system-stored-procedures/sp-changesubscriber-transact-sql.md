...
`[ @password = ] 'password'`Mot de passe d’authentification. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] *Password* est de **%** **type sysname**, avec la valeur par défaut. **%** indique qu’il n’y a aucune modification de la propriété de mot de passe.  
...
`[ @frequency_interval = ] frequency_interval`Intervalle de *type_fréquence*. *frequency_interval* est de **type int**, avec NULL comme valeur par défaut.  
...
`[ @frequency_recurrence_factor = ] frequency_recurrence_factor`Fréquence de répétition de la tâche de distribution pendant le *frequency_type*défini. *frequency_recurrence_factor* est de **type int**, avec NULL comme valeur par défaut.  
