...
`[ @table_name = ] 'table_name'` Est le nom de la table qui contient la colonne spécifiée. *table_name* est **sysname**, avec NULL comme valeur par défaut.  
...
`[ @column_name = ] 'column_name'` Est le nom de la colonne pour laquelle les informations de privilège. *column_name* est **sysname**, avec une valeur par défaut NULL (tous les courant).  
  
## <a name="result-sets"></a>Jeux de résultats  
 Le tableau suivant présente que le jeu de résultats colonnes. Les résultats obtenus sont triés par **TABLE_QUALIFIER**, **TABLE_OWNER**, **TABLE_NAME**, **COLUMN_NAME**, et  **PRIVILÈGE**.  
