...
Pour les [!INCLUDE[sssdw-md](../../includes/sssdw-md.md)] et [!INCLUDE[sspdw-md](../../includes/sspdw-md.md)], `sp_spaceused` doit spécifier des paramètres nommés (par exemple, `sp_spaceused (@objname= N'Table1');` au lieu de s’appuyer sur la position ordinale des paramètres. 
...
|LOCAL_ONLY|Retourne les statistiques de stockage de la partie locale de l’objet ou de la base de données. Si l’objet ou la base de données ne prend pas en charge Stretch, retourne les mêmes statistiques que lorsque @mode = ALL.|  
...
|Value|Description|  
|-----------|-----------------|  
|0|Lorsque *\@objname* a la valeur null ou qu’il n’est pas spécifié, deux jeux de résultats sont retournés. Deux jeux de résultats sont le comportement par défaut.|  
|1|Lorsque *\@objname* = null ou n’est pas spécifié, un seul jeu de résultats est retourné.|  
  
 *oneresultset* est de valeur de **bit**, avec **0**comme valeur par défaut.  

`[ @include_total_xtp_storage] 'include_total_xtp_storage'` @ no__t-1**s’applique à :** [!INCLUDE[sssql17-md](../../includes/sssql17-md.md)], [!INCLUDE[sssds-md](../../includes/sssds-md.md)].  
...
|**database_size**|**varchar(18)**|Taille de la base de données actuelle en mégaoctets. **database_size** comprend à la fois les fichiers de données et les fichiers journaux.|  
...
|Nom de la colonne|Type de données|Description|  
|-----------------|---------------|-----------------|  
|**name**|**nvarchar(128)**|Nom de l'objet pour lequel ont été demandées les informations relatives à l'utilisation de l'espace.<br /><br /> Le nom de schéma de l'objet n'est pas renvoyé. Si le nom du schéma est requis, utilisez les vues de gestion dynamique [sys. DM _db_partition_stats](../../relational-databases/system-dynamic-management-views/sys-dm-db-partition-stats-transact-sql.md) ou [sys. DM _ db_index_physical_stats](../../relational-databases/system-dynamic-management-views/sys-dm-db-index-physical-stats-transact-sql.md) pour obtenir des informations de taille équivalentes.|  
...
|**reserved**|**varchar(18)**|Quantité totale d’espace réservé pour *nomobj*.|  
...
|-----------------|---------------|-----------------|  
|**database_name**|**nvarchar(128)**|Nom de la base de données en cours.|  
|**database_size**|**varchar(18)**|Taille de la base de données actuelle en mégaoctets. **database_size** comprend à la fois les fichiers de données et les fichiers journaux. Si la base de données a un groupe de fichiers MEMORY_OPTIMIZED_DATA, cela inclut la taille totale sur disque de tous les fichiers de point de contrôle dans le groupe de fichiers.|  
...
Le jeu de résultats suivant est retourné **uniquement si** la base de données a un groupe de fichiers MEMORY_OPTIMIZED_DATA avec au moins un conteneur : 
...
### <a name="e-displaying-space-usage-information-for-a-database-with-at-least-one-memory_optimized-file-group-in-a-single-result-set"></a>E. Affichage des informations sur l’utilisation de l’espace pour une base de données avec au moins un groupe de fichiers MEMORY_OPTIMIZED dans un seul jeu de résultats 
 L’exemple suivant résume l’utilisation de l’espace pour la base de données actuelle avec au moins un groupe de fichiers MEMORY_OPTIMIZED dans un seul jeu de résultats.
