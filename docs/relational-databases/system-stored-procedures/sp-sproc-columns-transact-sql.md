...
## <a name="arguments"></a>Arguments  
 [  **@procedure_name =** ] **'***nom***'**  
 Nom de la procédure utilisée pour renvoyer des informations de catalogue. *nom* est **nvarchar (** 390 **)**, avec une valeur par défaut %, ce qui signifie que toutes les tables dans la base de données actuelle. La recherche de correspondance avec des caractères génériques est prise en charge.  
 ...
 [  **@ODBCVer =**] **'***ODBCVer***'**  
 Est la version d’ODBC utilisée. *ODBCVer* est **int**, avec une valeur par défaut 2, ce qui indique ODBC version 2.0. Pour plus d’informations sur la différence entre la version 2.0 et ODBC version 3.0, reportez-vous à ODBC **SQLProcedureColumns** spécification pour ODBC version 3.0  
