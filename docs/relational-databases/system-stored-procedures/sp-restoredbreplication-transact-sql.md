...
`[ @srv_orig = ] 'original_server_name'` Le nom du serveur où la sauvegarde a été créée. *original_server_name* est **sysname**, sans valeur par défaut.  
  
`[ @db_orig = ] 'original_database_name'` Le nom de la base de données qui ont été sauvegardée. *original_database_name* est **sysname**, sans valeur par défaut.  
