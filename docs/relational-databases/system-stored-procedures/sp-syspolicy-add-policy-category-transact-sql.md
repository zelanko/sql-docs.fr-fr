...
`[ @name = ] 'name'` Est le nom de la catégorie de stratégie. *nom* est **sysname**et est requis. *nom* ne peut pas être NULL ou une chaîne vide.  
  
`[ @mandate_database_subscriptions = ] mandate_database_subscriptions` Détermine si l’abonnement de base de données est autorisé pour la catégorie de stratégie. *mandate_database_subscriptions* est un **bits** valeur, par défaut est 1 (activé).  
  
`[ @policy_category_id = ] policy_category_id` Est l’identificateur de la catégorie de stratégie. *policy_category_id* est **int**et est retourné en tant que sortie.  
