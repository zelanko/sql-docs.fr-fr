...
`[ @table = ] 'table'` est le nom de la table. *table* est de **type sysname**, sans valeur par défaut.  
...
`[ @owner = ] 'owner'` est le nom du propriétaire de la table. *owner* est de **type sysname**, avec NULL comme valeur par défaut.  
