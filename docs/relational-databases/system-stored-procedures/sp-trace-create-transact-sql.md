...
-   [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] n’effectue pas automatiquement charge ou vous inviter à charger les fichiers de substitution (si une de ces options de substitution de fichier sont configurée).  
  
-   La fonction fn_trace_gettable ne charge pas les fichiers de substitution (selon le *number_files* argument) où le nom de fichier d’origine se termine par un trait de soulignement et une valeur numérique. Cela ne s'applique pas au trait de soulignement et au nombre qui sont automatiquement ajoutés lors du remplacement d'un fichier.  
  
> [!NOTE]  
>  Une autre solution consiste à renommer les fichiers de trace pour supprimer les traits de soulignement dans le fichier d'origine. Par exemple, si le fichier d’origine se nomme **my_trace.trc**, et le fichier de substitution est nommé **my_trace_1.trc**, vous pouvez renommer les fichiers à **mytrace.trc** et **mytrace_1.trc** avant d’ouvrir les fichiers dans [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)].  
 ...
 **sp_trace_create** est un [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] procédure stockée qui effectue la plupart des actions précédemment exécutées par **xp_trace_\***  disponible dans les versions antérieures de SQL Server de procédures stockée étendues. Utilisez **sp_trace_create** au lieu de :  
...
-   C'est une trace de substitution. La valeur par défaut *file_count* est 2 mais peut être remplacée par l’utilisateur à l’aide *filecount* option.  
