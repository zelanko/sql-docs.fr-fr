...
`[ @publication = ] 'publication'` Est le nom de la publication dans laquelle de nouvelles plages d’identité sont réaffectées. *publication* est **sysname**, avec NULL comme valeur par défaut.  
  
`[ @table_name = ] 'table_name'` Est le nom de la table dans laquelle de nouvelles plages d’identité sont réaffectées. *table_name* est **sysname**, avec NULL comme valeur par défaut.  
  
`[ @table_owner = ] 'table_owner'` Est le propriétaire de la table sur le serveur de publication. *TABLE_OWNER* est **sysname**, avec NULL comme valeur par défaut. Si *table_owner* n’est pas spécifié, le nom de l’utilisateur actuel est utilisé.  
