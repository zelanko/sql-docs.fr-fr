...
`[ @allow_pull = ] 'allow_pull'`Spécifie si des abonnements par extraction de données peuvent être créés pour la publication donnée. *allow_pull* est de type **nvarchar (5)**, avec true comme valeur par défaut, qui autorise les abonnements par extraction de la publication. Vous devez spécifier la valeur true [!INCLUDE[ssEW](../../includes/ssew-md.md)] pour prendre en charge les abonnés.  
  
`[ @allow_anonymous = ] 'allow_anonymous'`Spécifie si des abonnements anonymes peuvent être créés pour la publication donnée. *allow_anonymous* est de type **nvarchar (5)**, avec true comme valeur par défaut, ce qui autorise les abonnements anonymes à la publication. Pour prendre [!INCLUDE[ssEW](../../includes/ssew-md.md)] en charge les abonnés, vous devez spécifier **true**.  
 ...
 **sp_addmergepublication** est utilisé dans la réplication de fusion.  
 ...
 Pour répertorier les objets de publication sur le ** \@** Active Directory à l’aide [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] du paramètre add_to_active_directory, l’objet doit déjà être créé dans le Active Directory.  
