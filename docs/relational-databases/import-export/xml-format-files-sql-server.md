...
ms.topic: "article"
helpviewer_keywords: 
  - "fichiers de format (SQL Server), fichiers de format XML"
...
-   Le composant \<ROW> décrit comment créer des lignes de données à partir d’un fichier de données lorsque les données du fichier sont importées dans une table [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
     ...
     Chaque élément \<COLUMN> ne peut être mappé que sur un seul champ du fichier de données. L’ordre des éléments de \<COLUMN> dans l’élément \<ROW> définit l’ordre dans lequel ils sont retournés par l’opération en bloc. Le fichier de format XML assigne à chaque élément \<COLUMN> un nom local qui n’a aucune relation avec la colonne dans la table cible d’une opération d’importation en bloc.  
...
####  <a name="SchemaElements"></a> Éléments du schéma  
 ...
 \<COLUMN >  
 Définit une colonne en tant qu’élément (\<COLUMN>). Chaque élément \<COLUMN> correspond à un élément \<FIELD> (dont l’ID est spécifié dans l’attribut SOURCE de l’élément \<COLUMN>).  
...
|---------------------|-----------------|------------------------------|  
|ID **="***fieldID***"**|Spécifie le nom logique du champ dans le fichier de données. L'ID d'un champ est la clé utilisée pour y faire référence.<br /><br /> \<FIELD ID**="***fieldID***"**/> est mappé sur \<COLUMN SOURCE**="***fieldID***"**/>|Requis|  
...
|----------------------|-----------------|------------------------------|  
|SOURCE **="***fieldID***"**|Spécifie l'ID du champ mappé à la colonne.<br /><br /> \<COLUMN SOURCE**="***fieldID***"**/> est mappé sur \<FIELD ID**="***fieldID***"**/>|Requis|  
...
###  <a name="HowUsesColumn"></a> Comment l’importation en bloc utilise l’élément \<COLUMN>  
 Pour l’importation de données en bloc dans une table, les éléments \<COLUMN> dans un fichier de format mappent un champ de fichier de données sur des colonnes de table en spécifiant :  
...
-   Le type de colonne, qui est utilisé pour convertir le type de données du champ au type de données de la colonne souhaitée.  
 ...
 De même, pour l’exportation de données en bloc à partir d’une table, chaque élément \<COLUMN> du fichier de format mappe la colonne de ligne de la table d’entrée sur son champ correspondant du fichier de données de sortie.  
...
###  <a name="MapXSItype"></a> D. Mappage de \<FIELD> xsi:type sur \<COLUMN> xsi:type  
