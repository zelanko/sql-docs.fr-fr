...
helpviewer_keywords: 
  ...
  - "déployer une DAC"
  - "application de la couche Données [SQL Server], déployer"
  - "procédure [DAC], déployer"
  - "Assistant [DAC], déployer"
...
###  <a name="SQLUtility"></a> Utilitaire SQL Server  
 Si vous déployez une DAC dans une instance gérée du moteur de base de données, la DAC déployée est incorporée dans l'utilitaire SQL Server lorsque le jeu d'éléments de collecte de l'utilitaire est envoyé de l'instance au point de contrôle de l'utilitaire. La DAC est alors présente dans le nœud **Applications de la couche Données déployées** de l’[!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]**Explorateur de l’utilitaire** et signalée dans la page détaillée **Applications de la couche Données déployées**.  
...
##  <a name="UsingDeployDACWizard"></a> Utilisation de l'Assistant Déployer une application de la couche Données  
 ...
 **Annuler** : met fin à l’Assistant sans déployer de DAC.  
 ...
 [&#91;Assistant de déploiement&#93;](#UsingDeployDACWizard)  
