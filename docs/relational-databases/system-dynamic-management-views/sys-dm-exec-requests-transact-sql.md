...
|request_id|**tiers**|ID de la demande. Unique dans le contexte de la session. N'accepte pas la valeur NULL.|  
|start_time|**Date/heure**|Horodateur lors la réception de la demande. N'accepte pas la valeur NULL.|  
|status|**nvarchar(30**|Statut de la demande. Il peut s’agir de l’une des propriétés suivantes :<br /><br /> Arrière-plan<br />Running<br />Exécutable<br />En état de veille<br />Suspended<br /><br /> N'accepte pas la valeur NULL.|  
|command|**nvarchar (32)**|Identifie le type de commande en cours de traitement. Les types de commandes courants comprennent notamment :<br /><br /> SELECT<br />INSERT<br />UPDATE<br />SUPPRIMER<br />BACKUP LOG<br />BACKUP DATABASE<br />DBCC<br />FOR<br /><br /> Le texte de la demande peut être extrait à l'aide de sys.dm_exec_sql_text avec le paramètre sql_handle correspondant pour la demande. Des processus système internes définissent la commande en fonction du type de tâche à exécuter. Il peut s'agir des tâches suivantes :<br /><br /> LOCK MONITOR<br />CHECKPOINTLAZY<br />WRITER<br /><br /> N'accepte pas la valeur NULL.|  
...
|statement_start_offset|**tiers**|Nombre de caractères dans le traitement ou la procédure stockée en cours d'exécution où les instructions en cours d'exécution commencent. Cette valeur peut être utilisée avec les fonctions de gestion dynamique sql_handle, statement_end_offset et sys.dm_exec_sql_text pour extraire l'instruction en cours d'exécution de la demande. Autorise la valeur NULL.|  
|statement_end_offset|**tiers**|Nombre de caractères dans le traitement ou la procédure stockée en cours d'exécution où les instructions en cours d'exécution se terminent. Cette valeur peut être utilisée avec les fonctions de gestion dynamique sql_handle, statement_end_offset et sys.dm_exec_sql_text pour extraire l'instruction en cours d'exécution de la demande. Autorise la valeur NULL.|  
...
|user_id|**tiers**|ID de l'utilisateur qui a envoyé la demande. N'accepte pas la valeur NULL.|  
...
|wait_time|**tiers**|Si la demande est actuellement bloquée, cette colonne retourne la durée de l'attente, en millisecondes. N'accepte pas la valeur NULL.|  
...
|open_transaction_count|**tiers**|Nombre de transactions ouvertes pour cette demande. N'accepte pas la valeur NULL.|  
|open_resultset_count|**tiers**|Nombre de jeux de résultats ouverts pour cette demande. N'accepte pas la valeur NULL.|  
...
|percent_complete|**non**|Le pourcentage de travail terminé pour les commandes suivantes :<br /><br /> ALTER INDEX REORGANIZE<br />Option AUTO_SHRINK avec ALTER DATABASE<br />BACKUP DATABASE<br />DBCC CHECKDB<br />DBCC CHECKFILEGROUP<br />DBCC CHECKTABLE<br />DBCC INDEXDEFRAG<br />DBCC SHRINKDATABASE<br />DBCC SHRINKFILE<br />RECOVERY<br />RESTORE DATABASE<br />ROLLBACK<br />TDE ENCRYPTION<br /><br /> N'accepte pas la valeur NULL.|  
|estimated_completion_time|**bigint**|Interne uniquement. N'accepte pas la valeur NULL.|  
|cpu_time|**tiers**|Quantité de temps UC (en millisecondes) utilisée par la demande. N'accepte pas la valeur NULL.|  
|total_elapsed_time|**tiers**|Temps total écoulé en millisecondes depuis l'arrivée de la demande. N'accepte pas la valeur NULL.|  
|scheduler_id|**tiers**|ID du planificateur qui planifie cette demande. N'accepte pas la valeur NULL.|  
...
|text_size|**tiers**|Valeur du paramètre TEXTSIZE pour la demande. N'accepte pas la valeur NULL.|  
...
|quoted_identifier|**64bits**|1 = QUOTED_IDENTIFIER est activé (ON) pour la demande. Sinon, la valeur est 0.<br /><br /> N'accepte pas la valeur NULL.|  
|arithabort|**64bits**|1 = ARITHABORT est activé (ON) pour la demande. Sinon, la valeur est 0.<br /><br /> N'accepte pas la valeur NULL.|  
|ansi_null_dflt_on|**64bits**|1 = ANSI_NULL_DFLT_ON est activé (ON) pour la demande. Sinon, la valeur est 0.<br /><br /> N'accepte pas la valeur NULL.|  
|ansi_defaults|**64bits**|1 = ANSI_DEFAULTS est activé (ON) pour la demande. Sinon, la valeur est 0.<br /><br /> N'accepte pas la valeur NULL.|  
|ansi_warnings|**64bits**|1 = ANSI_WARNINGS est activé (ON) pour la demande. Sinon, la valeur est 0.<br /><br /> N'accepte pas la valeur NULL.|  
|ansi_padding|**64bits**|1 = ANSI_PADDING est activé (ON) pour la demande.<br /><br /> Sinon, la valeur est 0.<br /><br /> N'accepte pas la valeur NULL.|  
|ansi_nulls|**64bits**|1 = ANSI_NULLS est activé (ON) pour la demande. Sinon, la valeur est 0.<br /><br /> N'accepte pas la valeur NULL.|  
|concat_null_yields_null|**64bits**|1 = CONCAT_NULL_YIELDS_NULL est activé (ON) pour la demande. Sinon, la valeur est 0.<br /><br /> N'accepte pas la valeur NULL.|  
...
|lock_timeout|**tiers**|Délai d'attente de verrou externe pour la demande, en millisecondes. N'accepte pas la valeur NULL.|  
|deadlock_priority|**tiers**|Paramètre DEADLOCK_PRIORITY de la demande. N'accepte pas la valeur NULL.|  
|row_count|**bigint**|Nombre de lignes retournées au client par cette demande. N'accepte pas la valeur NULL.|  
|prev_error|**tiers**|Dernière erreur générée pendant l'exécution de la demande. N'accepte pas la valeur NULL.|  
|nest_level|**tiers**|Niveau d'imbrication actuel du code en cours d'exécution sur la demande. N'accepte pas la valeur NULL.|  
|granted_query_memory|**tiers**|Nombre de pages allouées à l'exécution d'une requête dans la demande. N'accepte pas la valeur NULL.|  
|executing_managed_code|**64bits**|Indique si une demande spécifique est en train d'exécuter des objets CLR (Common Language Runtime) tels que des routines, des types et des déclencheurs. Cette valeur est définie pour toute la durée pendant laquelle un objet CLR réside dans la pile, même lorsqu'elle exécute [!INCLUDE[tsql](../../includes/tsql-md.md)] à partir du CLR. N'accepte pas la valeur NULL.|  
|group_id|**tiers**|ID du groupe de charge de travail auquel cette requête appartient. N'accepte pas la valeur NULL.|  
...
|dop |**tiers** |**S’applique à**: [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] et versions ultérieures.<br /><br /> Degré de parallélisme de la requête. |  
|parallel_worker_count |**tiers** |**S’applique à**: [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] et versions ultérieures.<br /><br /> Nombre de threads de travail parallèles réservés s’il s’agit d’une requête parallèle.  |  
...
|is_resumable |**64bits** |**S’applique à**: [!INCLUDE[sssqlv14-md](../../includes/sssqlv14-md.md)] et versions ultérieures.<br /><br /> Indique si la demande est une opération d’index pouvant être reprise. |  
