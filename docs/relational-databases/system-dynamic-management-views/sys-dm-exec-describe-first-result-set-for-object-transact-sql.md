...
|**source_server**|**sysname**|Nom du serveur d'origine retourné par la colonne dans ce résultat (s'il provient d'un serveur distant). Le nom est donné tel qu’il apparaît dans sys. Servers.  Retourne la valeur NULL si la colonne provient du serveur local, ou s’il est impossible de déterminer le serveur sur lequel elle provient. Est renseigné uniquement si les informations de navigation sont demandées.|  
