...
ms.topic: "article"
helpviewer_keywords: 
  - "type de données geometry [SQL Server], présentation"
  - "type de données geography [SQL Server], données spatiales"
  - "données spatiales planaires [SQL Server], type de données geometry"
...
### Les anneaux internes et externes ne sont pas importants dans le type de données geography  
 La spécification OGC Simple Features for SQL traite des anneaux externes et internes, mais cette distinction n’a que peu de sens pour le type de données **geography** dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ; tout anneau d’un polygone peut être accepté comme étant l’anneau externe.  
...
- [Référence de méthodes de type de données geography](../Topic/geography%20Data%20Type%20Method%20Reference.md)   
