---
title: SQLGetInfo - France Microsoft Docs
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
 ...
 **SQLGetInfo** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] dans Native Client diffère de **SQLGetInfo** dans le [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] conducteur ODBC (SQLSRV32. DLL) lorsque **SQLGetInfo** est appelé avec SQL_KEYWORDS et une longueur tampon de 0.  Le pilote [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client retourne SQL_SUCCESS, mais le pilote ODBC [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] retourne SQL_SUCCESS_WITH_INFO.  Cependant, lorsqu’on l’appelle avec une longueur tampon non zéro qui est [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] inférieure à la chaîne de mots clés de sortie, **SQLGetInfo** dans Native Client retourne SQL_SUCCESS_WITH_INFO et un SQLState de 01004.  
  
|fInfoType|rgbInfoValue|  
|---------------|------------------|  
|SQL_ACCESSIBLE_PROCEDURES|"Y"|  
|SQL_ACCESSIBLE_TABLES|"Y"|  
|SQL_ACTIVE_CONNECTIONS|Le nombre de connexions est limité par [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Le conducteur retourne 0 pour cette demande **SQLGetInfo.**|  
|SQL_ACTIVE_ENVIRONMENTS|Le nombre d'environnements n'est pas limité par le pilote. Le conducteur retourne 0 pour cette demande **SQLGetInfo.**|  
|SQL_ACTIVE_STATEMENTS|Le conducteur retourne 1 pour cette demande **SQLGetInfo.** Le nombre de descripteurs d'instruction disponibles pour une application n'est pas limité par le pilote, mais l'exécution par défaut sur un descripteur d'instruction bloquera l'exécution sur tout autre descripteur.|  
...
|SQL_CONVERT_DOUBLE|Aucune prise en charge de la conversion du type de données ODBC SQL_DOUBLE. Le [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] conducteur de Native Client ODBC prend en charge le type de données SQL_DOUBLE de l’ODBC comme SQL_FLOAT. Voir SQL_CONVERT_FLOAT ci-dessous.|  
...
|SQL_DATA_SOURCE_NAME|Nom de la source de données actuelle. Définit la valeur indiquée par *StringLengthPtr* à 0 si la connexion ne spécifiait pas un nom de source de données.|  
...
|SQL_IDENTIFIER_CASE|SQL_IC_MIXED si connecté à un serveur exécutant un ordre de tri insensible aux cas.<br /><br /> SQL_IC_SENSITIVE si connecté à un serveur exécutant l’ordre de tri sensible aux cas.|  
...
|SQL_INFO_SS_NETLIB_NAME|Attribut spécifique au pilote ODBC [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client. Nom de la bibliothèque réseau en cours d'utilisation par la connexion.<br /><br /> Par défaut, DBNETLIB est retourné.  Dans cette situation, DBNETLIB se réfère à la bibliothèque du réseau et n’est pas liée à dbnetlib.dll.|  
|SQL_INTEGRITY|"Y"|  
...
## <a name="see-also"></a>Voir aussi  
 [Fonction SQLGetInfo](https://go.microsoft.com/fwlink/?LinkId=59354)   
