---
title: SQLGetStmtAttr - France Microsoft Docs
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
  ...
  Le [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] conducteur de Native Client ODBC étend SQLGetStmtAttr pour exposer les attributs de déclaration spécifiques au conducteur.  
...
|Valeur|Description|  
|-----------|-----------------|  
|SQL_NC_OFF|NOCOUNT a la valeur OFF. SQLRowCount renvoie le nombre de rangées touchées.|  
|SQL_NC_ON|NOCOUNT a la valeur ON. Le nombre de lignes touchées n’est pas retourné par SQLRowCount et la valeur retournée est de 0.|  
  
 Si SQLRowCount retourne 0, l’application doit tester SQL_SOPT_SS_NOCOUNT_STATUS. Si SQL_NC_ON est retournée, la valeur de 0 de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] SQLRowCount indique seulement que cela n’a pas retourné un nombre de lignes. Si SQL_NC_OFF est retourné, cela signifie que l'option NOCOUNT est désactivée et la valeur 0 de SQLRowCount indique que l'instruction n'a affecté aucune ligne.  
  
 Les applications ne doivent pas afficher la valeur de SQLRowCount quand SQL_SOPT_SS_NOCOUNT_STATUS a la valeur SQL_NC_OFF. Les lots ou procédures stockées de grande taille peuvent contenir plusieurs instructions SET NOCOUNT. Il n'est donc pas possible de supposer que SQL_SOPT_SS_NOCOUNT_STATUS reste constant. Cette option doit être testée chaque fois que SQLRowCount renvoie 0.  
...
## <a name="sqlgetstmtattr-and-table-valued-parameters"></a>SQLGetStmtAttr et paramètres table  
 SQLGetStmtAttr peut être appelé pour obtenir la valeur de SQL_SOPT_SS_PARAM_FOCUS dans le descripteur de paramètres d’application (APD) lorsqu’il travaille avec des paramètres de valeur de table. Pour plus d'informations sur SQL_SOPT_SS_PARAM_FOCUS, consultez [SQLSetStmtAttr](../../relational-databases/native-client-odbc-api/sqlsetstmtattr.md).  
...
## <a name="see-also"></a>Voir aussi  
 [Fonction SQLSetStmtAttr](https://go.microsoft.com/fwlink/?LinkId=59370)   
