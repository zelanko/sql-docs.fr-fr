...
Une fois les parties I à IV de la rubrique Étapes de la configuration de la gestion de clés extensible à l’aide d’Azure Key Vault terminées, utilisez la clé Azure Key Vault pour chiffrer la clé de chiffrement de base de données à l’aide du chiffrement TDE. Pour plus d’informations sur la rotation des clés à l’aide de PowerShell, consultez [Effectuer une rotation du protecteur TDE (Transparent Data Encryption) à l’aide de PowerShell](/azure/sql-database/transparent-data-encryption-byok-azure-sql-key-rotation).
