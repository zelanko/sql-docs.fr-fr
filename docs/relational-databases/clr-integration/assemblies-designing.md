---
title: Conception d’assemblées (fr) Microsoft Docs
description: Cet article décrit les facteurs à prendre en compte lorsque vous concevez un assemblage à héberger sur SQL Server, y compris l’emballage, la gestion et les restrictions sur les assemblages.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
-   Assemblages d’emballages  
  
-   Gestion de la sécurité de l’assemblage  
  
-   Restrictions sur les assemblées  
...
## <a name="managing-assembly-security"></a>Gestion de la sécurité des assemblys  
 Vous pouvez contrôler la quantité d’accès d’un assemblage aux ressources protégées par la sécurité d’accès au code .NET lorsqu’elle exécute le code géré. Pour ce faire, vous spécifiez l'un des trois jeux d'autorisations lorsque vous créez ou modifiez un assembly : SAFE, EXTERNAL_ACCESS ou UNSAFE.  
...
### <a name="disallowed-net-framework-apis"></a>API .NET Framework interdites  
 Tout [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] API annoté avec l’un des **hostProtectionattributes refusés** ne peut pas être appelé à partir de SAFE et EXTERNAL_ACCESS assemblées.  
