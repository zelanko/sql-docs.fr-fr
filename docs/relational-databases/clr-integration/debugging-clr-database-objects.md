---
title: Débugging CLR Database Objects (fr) Microsoft Docs
description: SQL Server fournit un support pour déboguer les objets Transact-SQL et CLR dans la base de données intégrant SQL Server debugger avec Microsoft Visual Studio debugger.
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]
 ...
 Pour plus d’informations sur le débogage du code géré à l’aide de Visual Studio, voir le sujet «[Debugging Managed Code](https://go.microsoft.com/fwlink/?LinkId=120377)» dans la documentation Visual Studio.  
  
## <a name="debugging-permissions-and-restrictions"></a>Autorisations et restrictions de débogage  
 Debugging est une opération très privilégiée, et donc seuls les membres du rôle [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]de serveur fixe **sysadmin** sont autorisés à le faire dans .  
...
## <a name="overview-of-debugging-managed-database-objects"></a>Vue d'ensemble du débogage des objets de base de données managés  
 ...
 Pour plus d’informations et de scénarios sur le débogage géré procédures stockées, fonctions, déclencheurs, types définis par l’utilisateur, et les agrégats, voir le "[SQL Server CLR Integration Database Debugging](https://go.microsoft.com/fwlink/?LinkId=120378)" sujet dans la documentation Visual Studio.  
...
2.  Créez un type. Dans **Solution Explorer**, cliquez à droite sur le projet, sélectionnez **Ajouter** et **Nouvel Article...** De la fenêtre **Add New Item,** sélectionnez **la procédure stockée**, fonction définie par **l’utilisateur**, **type défini par l’utilisateur**, **déclencheur,** **agrégat,** ou **classe**. Spécifier un nom pour le fichier source du nouveau type et cliquez sur **Ajouter**.  
...
4.  Ajoutez un script qui teste le type. Dans **Solution Explorer**, étendre le répertoire **TestScripts** double clic **Test.sql** pour ouvrir le fichier source de script de test par défaut. Ajoutez à l'éditeur de texte un script de test qui appelle le code à déboguer. Un exemple de script est proposé ci-dessous.  
  
5.  Placez un ou plusieurs points d'arrêt dans le code source. Cliquez à droite sur une ligne de code dans l’éditeur de texte, dans la fonction ou la routine que vous souhaitez déboiffer, et sélectionnez **Breakpoint** et **Insert Breakpoint**. Le point d'arrêt est ajouté et la ligne de code est affichée en rouge.  
  
6.  Dans le menu **Debug,** sélectionnez **Start Debugging** pour compiler, déployer et tester le projet. Le script de test dans Test.sql est exécuté et l'objet de base de données managé est appelé.  
  
7.  Lorsque la flèche jaune qui désigne le pointeur d'instruction apparaît au niveau du point d'arrêt, l'exécution du code est  suspendue et vous pouvez commencer à déboguer votre objet de base de données managé. Vous pouvez **passer** du menu **Debug** pour faire avancer le pointeur d’instructions à la ligne suivante du code. La fenêtre **local est** utilisée pour observer l’état des objets actuellement mis en évidence par le pointeur d’instruction. Des variables peuvent être ajoutées à la fenêtre **Watch.** L'état des variables espionnées peut être consulté tout au long de la session de débogage, et non uniquement lorsque la variable figure dans la ligne de code mise en surbrillance par le pointeur d'instruction. Sélectionnez Continuer dans le menu Déboguer pour faire avancer le pointeur d'instruction jusqu'au point d'arrêt suivant ou pour terminer l'exécution de la routine s'il n'y a plus de points d'arrêt.  
