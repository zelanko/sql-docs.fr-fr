---
title: Sécurité d’accès du Code CLR Integration | Microsoft Docs
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
ms.reviewer: ''
...
-   Stratégie d’utilisateur : Il s’agit de la stratégie en vigueur pour le code managé hébergé par un processus. Pour [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)], la stratégie de l'utilisateur est spécifique au compte Windows sur lequel le service [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] s'exécute.  
 ...
 **EXTERNAL_ACCESS** assemblys présentent également les autorisations et les valeurs suivantes :  
...
|**EnvironmentPermission**|**Unrestricted :** Un accès complet aux variables d’environnement système et utilisateur est autorisé.|  
|**Autorisation EventLogPermission**|**Administrer :** Les actions suivantes sont autorisées : création d’une source d’événement, lecture de journaux existants, supprimer les sources d’événements ou journaux, répond aux entrées, effacement d’un journal des événements, écouter des événements et l’accès à une collection de tous les journaux des événements.|  
|**FileIOPermission**|**Unrestricted :** Accès complet aux fichiers et dossiers est accordé.|  
|**KeyContainerPermission**|**Unrestricted :** Un accès complet aux conteneurs de clé est autorisé.|  
|**NetworkInformationPermission**|**Accès :** Une commande ping est autorisée.|  
|**RegistryPermission**|Permet de droits de lecture sur **HKEY_CLASSES_ROOT**, **HKEY_LOCAL_MACHINE**, **HKEY_CURRENT_USER**, **HKEY_CURRENT_CONFIG**et  **HKEY_USERS.**|  
|**SecurityPermission**|**Assertion :** Possibilité de déclarer que tous les appelants de ce code ont l’autorisation requise pour l’opération.<br /><br /> **ControlPrincipal :** Possibilité de manipuler l’objet principal.<br /><br /> **Exécution :** Autorisation d’exécuter le code managé.<br /><br /> **SerializationFormatter :** Capacité à fournir des services de sérialisation.|  
|**SmtpPermission**|**Accès :** Connexions sortantes vers le port 25 hôte SMTP sont autorisées.|  
|**SocketPermission**|**Se connecter :** Connexions sortantes (tous les ports, tous les protocoles) sur une adresse de transport sont autorisées.|  
|**SqlClientPermission**|**Unrestricted :** Un accès complet à la source de données est autorisé.|  
|**StorePermission**|**Unrestricted :** Accès complet aux magasins de certificats X.509 est accordé.|  
|**WebPermission**|**Se connecter :** Les connexions sortantes vers les ressources web sont autorisées.|  
...
|Le contexte d’exécution correspond à une connexion de Windows et le contexte d’exécution est l’appelant d’origine et l’appelant a été empruntée.|L'accès s'effectue en utilisant le contexte de sécurité de l'appelant, et non le compte de service.|  
