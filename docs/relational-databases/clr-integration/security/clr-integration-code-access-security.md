...
|**Autorisation**|**Context connection = true**, **contexte Connection = Yes**: seule la connexion Context peut être utilisée et la chaîne de connexion ne peut spécifier que la valeur « context connection = true » ou « context connection = Yes ».<br /><br /> **AllowBlankPassword = false :**  Les mots de passe vides ne sont pas autorisés.|  
...
|**Autorisation**|Non **restreint :** L’accès complet à la source de source est autorisé.|  
