...
description: En savoir plus sur la création de vues XML de données relationnelles à l’aide du langage XSD (XML Schema Definition) (SQLXML 4,0).
ms.date: 01/11/2019
ms.prod: sql
...
```  
 ...
 Dans le schéma de mappage, l' **\<Contact>** élément est mappé à la table Person. contact de l’exemple de base de données AdventureWorks à l’aide de l’annotation **SQL : relation** . Les attributs ConID, FName et LName sont mappés aux colonnes ContactID, FirstName et LastName dans la table Person. contact à l’aide des annotations **SQL : Field** .  
