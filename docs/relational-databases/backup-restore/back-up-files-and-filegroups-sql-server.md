...
ms.topic: "article"
helpviewer_keywords: 
  - "sauvegarde des groupes de fichiers [SQL Server]"
  - "sauvegardes de fichiers [SQL Server], rubriques de procédures"
  - "sauvegarde des fichiers [SQL Server]"
...
## Sauvegarder des groupes de fichiers et des fichiers à l’aide de SSMS   
...
## Sauvegarder des groupes de fichiers et des fichiers à l’aide de T-SQL
...
1.  Pour créer une sauvegarde de fichier ou de groupe de fichiers, utilisez une instruction [BACKUP DATABASE <fichier_ou_groupe_de_fichiers>](../../t-sql/statements/backup-transact-sql.md). Au minimum, cette instruction doit spécifier les actions suivantes :  
    ...
    |Option|Description|  
    |------------|-----------------|  
    |*base de données*|Correspond à la base de données à partir de laquelle va être opérée la sauvegarde du journal des transactions, c'est à dire la sauvegarde complète ou partielle.|  
    |FILE **=***nom_fichier_logique*|Indique le nom logique d'un fichier à inclure dans la sauvegarde de fichiers.|  
    |FILEGROUP **=***nom_groupe_fichiers_logique*|Indique le nom logique d'un groupe de fichiers à inclure dans la sauvegarde de fichiers. En mode de récupération simple, la sauvegarde d'un groupe de fichiers n'est autorisée que pour un groupe de fichiers en lecture seule.|  
...
#### A. Créer une sauvegarde de fichiers de deux fichiers  
...
#### B. Créer une sauvegarde de fichiers complète des groupes de fichiers secondaires  
...
#### C. Créer une sauvegarde de fichiers différentielle des groupes de fichiers secondaires  
...
1.  Utilisez l’applet de commande **Backup-SqlDatabase** et spécifiez **Files** comme valeur du paramètre **-BackupAction**. Spécifiez également l'un des paramètres suivants :  
    ...
    -   Pour sauvegarder tous les fichiers d’un groupe de fichiers donné, spécifiez le paramètre **-DatabaseFileGroup***String*, où *String* représente un ou plusieurs groupes de fichiers de base de données à sauvegarder.  
