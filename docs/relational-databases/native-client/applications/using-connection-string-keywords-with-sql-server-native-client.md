...
|**MOT DE PASSE**|Mot de passe du compte de connexion [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] spécifié dans le paramètre UID. **PWD** ne doit pas être spécifié si la connexion a un mot de passe NULL ou lors de l’utilisation de l’authentification Windows (`Trusted_Connection = yes`).|  
