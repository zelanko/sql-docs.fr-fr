 ...
 **ISequentialStream** doit être utilisée pour la récupération de documents XML volumineux. Les mêmes techniques utilisées pour les types de valeur élevée s'appliquent également à XML. Pour plus d’informations, consultez [à l’aide des Types de valeur volumineux](../../../relational-databases/native-client/features/using-large-value-types.md).  
 ...
 Les données stockées dans des colonnes de type XML dans un ensemble de lignes peuvent également être extraites, insérées ou mises à jour par une application via des interfaces classiques tels que **IRow::GetColumns**, **IRowChange::SetColumns**et **ICommand::Execute**. De même pour les cas de la récupération, une application peut passer une chaîne de texte ou un **ISequentialStream** à la [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Client fournisseur OLE DB natif.  
  
> [!NOTE]  
>  Pour envoyer des données XML sous forme de chaîne via la **ISequentialStream** interface, vous devez obtenir **ISequentialStream** en spécifiant DBTYPE_IUNKNOWN et définir son *pObject* argument null dans la liaison.  
...
### <a name="data-bindings-and-coercions"></a>Liaisons de données et forçages de type  
 Le tableau suivant décrit la liaison et le forçage de type survenant lorsque vous utilisez les listées types de données avec le [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] **xml** type de données.  
...
|---------------|---------------------------|--------------------------------|-----------------------------|----------------------------------|  
|DBTYPE_XML|Traverser<sup>6,7</sup>|Erreur<sup>1</sup>|OK<sup>11, 6</sup>|Erreur<sup>8</sup>|  
|DBTYPE_BYTES|Traverser<sup>6,7</sup>|N/A<sup>2</sup>|OK <sup>11, 6</sup>|N/A <sup>2</sup>|  
|DBTYPE_WSTR|Traverser<sup>6,10</sup>|N/A <sup>2</sup>|OK<sup>4, 6, 12</sup>|N/A <sup>2</sup>|  
|DBTYPE_BSTR|Traverser<sup>6,10</sup>|N/A <sup>2</sup>|OK <sup>3</sup>|N/A <sup>2</sup>|  
...
|DBTYPE_VARIANT (VT_UI1 &AMP;#124; VT_ARRAY)|Traverser<sup>6,7</sup>|N/A <sup>2</sup>|Néant|N/A <sup>2</sup>|  
|DBTYPE_VARIANT (VT_BSTR)|Traverser<sup>6,10</sup>|N/A <sup>2</sup>|OK<sup>3</sup>|N/A <sup>2</sup>|  
 ...
 <sup>3</sup>format est UTF-16, aucune marque d’ordre d’octet (BOM), aucune spécification d’encodage, aucune marque de fin null.  
  
 <sup>4</sup>format n’est UTF-16, aucune marque BOM, aucune spécification d’encodage, le caractère null de fin.  
  
 <sup>5</sup>format est caractères multioctets encodés dans la page de codes client avec la marque de fin null. Toute conversion à partir de code Unicode sur le serveur risque d'endommager les données. Cette liaison est donc fortement déconseillée.  
 ...
 <sup>8</sup>validation peut se produire au moment de l’accesseur de la création, ou à des temps de récupération. L'erreur est DB_E_ERRORSOCCURRED et l'état de la liaison est défini sur DBBINDSTATUS_UNSUPPORTEDCONVERSION.  
 ...
 <sup>10</sup>un BOM est toujours ajoutée aux données envoyées au serveur. Si les données commencent déjà par une marque d'ordre d'octet, deux marques d'ordre d'octet apparaissent alors au démarrage de la mémoire tampon. Le serveur utilise le premier octet pour reconnaître l’encodage UTF-16 et puis les ignore. La deuxième marque d'ordre d'octet est interprétée comme un espace insécable de largeur nulle.  
  
 <sup>11</sup>format est UTF-16, aucune spécification d’encodage, un BOM est ajouté aux données reçues à partir du serveur. Si une chaîne vide est retournée par le serveur, une marque BOM est quand même retourné à l’application. Si la longueur de la mémoire tampon est un nombre impair d’octets, les données sont tronquées correctement. Si la valeur entière est retournée par segments, ils peuvent être concaténées pour reconstituer la valeur correcte.  
...
#### <a name="the-isscommandwithparameters-interface"></a>Interface ISSCommandWithParameters  
 Pour prendre en charge la **xml** type de données via OLE DB, [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client implémente plusieurs modifications, notamment l’ajout de la [ISSCommandWithParameters](../../../relational-databases/native-client-ole-db-interfaces/isscommandwithparameters-ole-db.md) interface. Lorsque vous travaillez avec une liaison SQL_C_BINARY, ** Native Client ne nécessitent ni ajouter une nomenclature, puisque l’encodage provient implicitement par la liaison. Outre les trois méthodes héritées de **ICommandWithParameters** (**GetParameterInfo**, **MapParameterNames** et **SetParameterInfo**), **ISSCommandWithParameters** fournit les méthodes [GetParameterProperties](../../../relational-databases/native-client-ole-db-interfaces/isscommandwithparameters-getparameterproperties-ole-db.md) et [SetParameterProperties](../../../relational-databases/native-client-ole-db-interfaces/isscommandwithparameters-setparameterproperties-ole-db.md) employées pour la gestion des types de données spécifiques au serveur.  
...
#### <a name="the-irowset-interface"></a>Interface IRowset  
 ...
 Si le consommateur spécifie DBTYPE_IUNKNOWN et définit le *pObject* argument sur NULL, ou définit le *pObject* argument sur IID_ISequentialStream, le fournisseur retourne un **ISequentialStream**  interface au consommateur afin que le consommateur peut diffuser les données XML hors de la colonne. **ISequentialStream** puis retourne les données XML comme un flux de caractères Unicode.  
...
#### <a name="the-irowsetchange-interface"></a>Interface IRowsetChange  
 Il existe deux moyens pour un consommateur de mettre à jour une instance XML dans une colonne. Le premier est via l’objet de stockage **ISequentialStream** créés par le fournisseur. Le consommateur peut appeler le **ISequentialStream::Write** méthode pour mettre à jour directement l’instance XML retourné par le fournisseur.  
 ...
 En cas de DBTYPE_IUNKNOWN/ISequentialStream, si le consommateur ne spécifie pas de n’importe quel objet de stockage, le consommateur doit créer un **ISequentialStream** de l’objet à l’avance, lier le document XML avec l’objet, puis passez l’objet le fournisseur via le **IRowsetChange::SetData** (méthode). Le consommateur peut également créer un stockage d’objet, définir l’argument pObject sur IID_ISequentialStream, créer un **ISequentialStream** de l’objet, puis passer la **ISequentialStream** objet à la **IRowsetChange::SetData** (méthode). Dans les deux cas, le fournisseur peut récupérer l’objet XML via la **ISequentialStream** de l’objet et l’insérer dans une colonne appropriée.  
  
#### <a name="the-irowsetupdate-interface"></a>Interface IRowsetUpdate  
 **IRowsetUpdate** interface fournit des fonctionnalités mises à jour retardées. Les données mises à disposition pour les ensembles de lignes ne sont pas rendues disponibles à d’autres transactions jusqu'à ce que le consommateur appelle la **IRowsetUpdate : Update** (méthode).  
  
#### <a name="the-irowsetfind-interface"></a>Interface IRowsetFind  
 Le **IRowsetFind::FindNextRow** méthode ne fonctionne pas avec le **xml** type de données. Lorsque **IRowsetFind::FindNextRow** est appelée et le *hAccessor* argument spécifie une colonne de type DBTYPE_XML, la valeur DB_E_BADBINDINFO est retournée. Cette situation se produit quel que soit le type de colonne recherché. Pour tout autre type de liaison, le **FindNextRow** échoue avec DB_E_BADCOMPAREOP si la colonne à rechercher est du **xml** type de données.  
