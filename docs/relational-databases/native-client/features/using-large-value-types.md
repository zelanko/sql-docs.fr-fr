...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
  ...
  Avant [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)], l'utilisation de types de données de valeur élevée nécessitait une gestion particulière. Les types de données à valeur élevée sont ceux dont la taille de ligne maximale dépasse 8 Ko. [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)]a introduit un spécificateur **max** pour les types **de données varchar,** **nvarchar** et **varbinary** pour permettre le stockage de valeurs aussi grandes que 2-31 -1 octets. Les [!INCLUDE[tsql](../../../includes/tsql-md.md)] colonnes et les variables de table peuvent spécifier les types de données **varchar(max),** **nvarchar(max)** ou **varbinaire (max).**  
...
> [!NOTE]  
>  Si vous reproduisez des données, vous devrez peut-être configurer [l’option de configuration maximale de la taille de repl de texte](../../../database-engine/configure-windows/configure-the-max-text-repl-size-server-configuration-option.md) à -1.  
...
## <a name="sql-server-native-client-ole-db-provider"></a>Fournisseur OLE DB SQL Server Native Client  
 ...
 La mise en œuvre de l’objet de commande **IAccessor** a été modifiée pour permettre la liaison comme DBTYPE_IUNKNOWN. Si le consommateur spécifie DBTYPE_IUNKNOWN et affecte la valeur Null à *pObject*, le fournisseur retourne l’interface **ISequentialStream** au consommateur afin que celui-ci puisse extraire les données **varchar(max)**, **nvarchar(max)** ou **varbinary(max)** des variables de sortie.  
 ...
 Afin de soutenir le [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] streaming, le fournisseur de DB OLE de native Client exige que les paramètres de longueur variable soient accessibles par ordre séquentiel. Cela signifie que DBPROP_ACCESSORDER doit avoir la valeur DBPROPVAL_AO_SEQUENTIALSTORAGEOBJECTS ou DBPROPVAL_AO_SEQUENTIAL chaque fois que des colonnes ou des paramètres de sortie **varchar(max)**, **nvarchchar(max)** ou **varbinary(max)** sont liés à DBTYPE_IUNKNOWN. Les appels à **IRowset::GetData** échouent avec le message DBSTATUS_E_UNAVAILABLE si cette restriction en matière d’ordre d’accès n’est pas respectée. Cette restriction ne s'applique pas lorsqu'aucune liaison de sortie n'utilise DBTYPE_IUNKNOWN.  
  
 Le [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] fournisseur de DB OLE de client autochtone prend également en charge les paramètres de sortie contraignants comme DBTYPE_IUNKNOWN pour les types de données de grande valeur afin de faciliter les scénarios où une procédure stockée renvoie des types de grande valeur comme valeurs de rendement qui sont exposées comme DBTYPE_IUNKNOWN au client.  
...
-   Lier comme DBTYPE_IUNKNOWN et utiliser la diffusion en continu.  
 ...
 Lorsqu’il déclare la taille [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] maximale d’une colonne, le fournisseur de DB OLE du client autochtone signalera :  
  
-   La taille maximale définie, qui, par exemple, est de 2000 pour une colonne **varchar(** 2000), ou **)**  
...
```  
 ...
 Pour plus d’informations sur la façon dont le [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] fournisseur de DB OLE de client autochtone expose les types de données de grande valeur, voir [BLOBs et objets OLE](../../../relational-databases/native-client-ole-db-blobs/blobs-and-ole-objects.md).  
  
## <a name="sql-server-native-client-odbc-driver"></a>Pilote ODBC SQL Server Native Client  
 Le [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] conducteur native De l’ODBC expose les types **varchar(max)**, **varbinaire(max)** et **nvarchar(max)** comme SQL_VARCHAR, SQL_VARBINARY et SQL_WVARCHAR dans les fonctions ODBC API qui acceptent ou retournent les types de données SQL D’ODBC.  
  
 Lors du signalement de la taille maximale d'une colonne, le pilote signalera :  
  
-   La taille maximale définie, qui, par exemple, est de 2000 pour une colonne **varchar(2000),** ou  
  
-   La valeur "illimitée" qui dans le cas d’une colonne **varchar(max)** équivaut à 0.  
  
 Les règles de conversion standard s’appliquent à une colonne **varchar(max),** ce qui signifie que toute conversion valable pour une colonne **varchar**(2000) sera également valable pour une colonne **varchar(max).** **)** Il en va de même pour les colonnes **nvarchar(max)** et **varbinary(max)**.  
