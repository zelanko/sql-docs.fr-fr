---
title: Exécuter une procédure stockée avec le service RPC et traiter la sortie | Documents Microsoft
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
2.  Appelez le **ICommandWithParameters::SetParameterInfo** méthode pour décrire les paramètres au fournisseur. Remplissez les informations de paramètre dans un tableau de structures PARAMBINDINFO.  
  
3.  Créez un jeu de liaisons (une pour chaque fabricant de paramètre) en utilisant un tableau de structures DBBINDING.  
  
4.  Création d’un accesseur pour les paramètres définis à l’aide de la **IAccessor::CreateAccessor** (méthode). **CreateAccessor** crée un accesseur à partir d’un jeu de liaisons.  
  
5.  Remplissez la structure DBPARAMS.  
  
6.  Appelez le **Execute** (dans ce cas, un appel à une procédure stockée).  
  
7.  Traiter l’ensemble de lignes et libérez-le à l’aide de la **IRowset::Release** (méthode).  
