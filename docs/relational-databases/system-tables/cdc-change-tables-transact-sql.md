...
|**partition_switch**|**bit**|Indique si le **SWITCH PARTITION** commande de **ALTER TABLE** peuvent être exécutées sur une table prenant en charge pour la capture de données modifiées. 0 indique que le basculement de partition est bloqué. Les tables non partitionnées retournent toujours la valeur 1.|  
