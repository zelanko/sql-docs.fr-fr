...
|**sync_method**|**tinyint**|Méthode de synchronisation :<br /><br /> **0** = utilitaire de copie en bloc en mode natif (**BCP**).<br /><br /> **1** = BCP en mode caractère.<br /><br /> **3** = simultanées, ce qui signifie qu’en mode natif BCP est utilisé mais les tables ne sont pas verrouillées lors de l’instantané.<br /><br /> **4** = Concurrent_c, ce qui signifie que BCP en mode caractère est utilisé mais les tables ne sont pas verrouillées lors de l’instantané.|  
...
|**enabled_for_internet**|**bit**|Indique si les fichiers de synchronisation pour la publication sont exposés à Internet via le protocole de transfert de fichiers (FTP) et d’autres services, où **1** signifie qu’ils soient accessibles à partir d’Internet.|  
...
|**autogen_sync_procs**|**bit**|Indique si la procédure stockée de synchronisation pour les abonnements de mise à jour immédiate est générée par le serveur de distribution. **1** signifie qu’elle est générée sur le serveur de publication.|  
|**retention**|**int**|La quantité de modifications, en heures, à enregistrer pour la publication concernée.|  
|**allowed_queued_tran**|**bit**|Indique si la mise en file d'attente des modifications sur l'Abonné jusqu'à leur application sur le serveur de publication est activée. Si **1**, les modifications sur l’abonné sont en file d’attente.|  
|**snapshot_in_defaultfolder**|**bit**|Spécifie si les fichiers d’instantanés sont stockés dans le dossier par défaut.<br /><br /> **0** = instantané fichiers ont été stockés dans l’emplacement secondaire spécifié par *alternate_snapshot_folder*.<br /><br /> **1** = instantané vous trouverez les fichiers dans le dossier par défaut.|  
...
|**compress_snapshot**|**bit**|Spécifie que l’instantané qui est écrite dans le *alt_snapshot_folder* emplacement consiste à être compressées dans le [!INCLUDE[msCoName](../../includes/msconame-md.md)] format CAB. **1** signifie que l’instantané sera compressé.|  
