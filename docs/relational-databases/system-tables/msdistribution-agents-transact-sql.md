...
|**queue_id**|**sysname**|Identificateur permettant de localiser la file d'attente pour les abonnements de mise à jour en attente. Pour les abonnements qui ne sont pas en attente, la valeur est NULL. Pour les publications [!INCLUDE[msCoName](../../includes/msconame-md.md)] basées sur Message Queuing, la valeur est un GUID qui identifie de manière unique la file d'attente à utiliser pour l'abonnement. Pour les publications de la file d’attente basée sur SQL Server, la colonne contient la valeur **SQL**.<br /><br /> Remarque : [!INCLUDE[msCoName](../../includes/msconame-md.md)] Message Queuing est déconseillé et n'est plus pris en charge.|  
