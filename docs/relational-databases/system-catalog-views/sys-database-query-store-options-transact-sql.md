...
|**size_based_cleanup_mode**|**smallint**|Contrôle si le nettoyage est activé automatiquement quand la quantité totale de données approche de la taille maximale :<br /><br /> 0 = OFF - taille en fonction de nettoyage n’est pas activé automatiquement.<br /><br /> **1** = AUTO - taille en fonction de nettoyage doit être activé automatiquement lors de la taille sur le disque atteint **90 pour cent** de *max_storage_size_mb*. Il s’agit de la valeur de configuration par défaut.<br /><br />Le nettoyage basée sur la taille supprime les requêtes les moins coûteuses et les plus anciennes en premier. Il s’arrête lorsqu’environ **80 pour cent** de *max_storage_size_mb* est atteinte.|  
