...
Soutient la gestion des bibliothèques de colis liées aux temps d’exécution externes tels que R, Python et Java.
...
Le catalogue vue sys.external_libraries répertorie une ligne pour chaque bibliothèque externe qui a été téléchargée dans la base de données.
...
|principal_id |int |ID du directeur qui possède cette bibliothèque externe. |
|langage | sysname | Nom de la langue ou de l’heure d’exécution qui prend en charge la bibliothèque externe. Les valeurs valides sont 'R', 'Python' et 'Java'. Des temps d’exécution supplémentaires pourraient être ajoutés à l’avenir.|
|scope |int |0 pour la portée du public; 1 pour une portée privée |  
|scope_desc |varchar(7) |Indique si le paquet est public ou privé|
