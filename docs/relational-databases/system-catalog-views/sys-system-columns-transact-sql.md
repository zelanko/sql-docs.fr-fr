...
|**nomme**|**sysname**|Nom de la colonne. Unique dans l'objet.|  
...
|**default_object_id**|**int**|ID de l’objet par défaut, qu’il s’agisse d’un [sys. sp_bindefault](../../relational-databases/system-stored-procedures/sp-bindefault-transact-sql.md)autonome ou d’une contrainte par défaut en ligne, au niveau des colonnes. La **parent_object_id** colonne d’un objet par défaut au niveau de la colonne Inline fait référence à la table elle-même. Il aura la valeur 0 en l'absence de valeur par défaut.|  
