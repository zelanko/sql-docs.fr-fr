...
|**mirroring_failover_lsn**|**numérique (25, 0)**|Numéro séquentiel dans le journal (LSN) du dernier enregistrement du journal des transactions dont le renforcement sur disque est garanti sur les deux partenaires. Après un basculement, le **mirroring_failover_lsn** est utilisé par les partenaires comme point de réconciliation à partir duquel le nouveau serveur miroir commence à synchroniser la nouvelle base de données miroir avec la nouvelle base de données principale.|  
...
|**mirroring_end_of_log_lsn**|**numérique (25, 0)**|end-of-log local qui a été vidé sur le disque. Cela est comparable au LSN renforcé du serveur miroir (voir la **mirroring_failover_lsn** colonne).|  
|**mirroring_replication_lsn**|**numérique (25, 0)**|Numéro séquentiel dans le journal maximum que la réplication peut envoyer.|  
