...
## <a name="arguments"></a>Arguments  
 ...
 Retourne des informations pour toutes les modifications de suivi à une table qui se sont produites depuis la version spécifiée par *last_sync_version*.  
 ...
 *last_sync_version*  
 ...
 En règle générale, avant d’obtenir les modifications, l’application appelle **CHANGE_TRACKING_CURRENT_VERSION()** pour obtenir la version qui sera utilisée, les modifications de temps suivantes sont requises. Par conséquent, l'application n'a pas besoin d'interpréter ou comprendre la valeur réelle.  
 ...
 *Value*  
 ...
 [EN] *table_alias* [(*column_alias* [,... *n* ])]  
 Fournit des noms pour les résultats retournés par CHANGETABLE.  
  
 *table_alias*  
 Nom d'alias de la table retournée par CHANGETABLE. *table_alias* est obligatoire et doit être valide [identificateur](../../relational-databases/databases/database-identifiers.md).  
...
## <a name="changetablechanges"></a>CHANGETABLE(CHANGES...)  
 Pour obtenir des données de ligne pour des lignes nouvelles ou modifiées, joignez le jeu de résultats à la table utilisateur en utilisant les colonnes clés primaires. Qu’une seule ligne est retournée pour chaque ligne dans la table utilisateur qui a été modifiée, même s’il y a eu plusieurs modifications à la même ligne depuis le *last_sync_version* valeur.  
...
## <a name="permissions"></a>Permissions  
 Requiert les autorisations suivantes sur la table qui est spécifié par le *table* valeur pour obtenir des informations de suivi des modifications :  
