...
|\<les colonnes @column_list>|**diffère**|Colonnes identifiées dans l’argument *column_list* pour sp_cdc_generate_wrapper_function quand elle est appelée pour générer le script qui crée la fonction wrapper.|  
...
|\<les colonnes @update_flag_list>|**bit**|Un indicateur binaire est nommé en ajoutant _uflag au nom de la colonne. L’indicateur a toujours la valeur NULL quand \__CDC_OPERATION est’d', 'I', de’UO'. Lorsque \__CDC_OPERATION est’UN', il est défini à 1 si la mise à jour a produit une modification apportée à la colonne correspondante. Sinon, il prend la valeur 0.|  
