...
|\<les colonnes @column_list|**diffère**|Colonnes identifiées dans l’argument *liste_de_colonnes* de sp_cdc_generate_wrapper_function lorsqu’elle est appelée pour générer le script qui crée la fonction wrapper.|  
...
|\<les colonnes @update_flag_list|**bit**|Un indicateur binaire est nommé en ajoutant _uflag au nom de la colonne. L’indicateur a toujours la valeur NULL quand \__CDC_OPERATION est’d', 'I', de’UO'. Lorsque \__CDC_OPERATION est’UN', il est défini sur 1 si la mise à jour a produit une modification de la colonne correspondante. Sinon, il prend la valeur 0.|  
