...
ms.topic: "article"
helpviewer_keywords: 
  - "hiérarchies [SQL Server], tables à prendre en charge"
  - "hierarchyid [moteur de base de données], concepts"
  - "tables hiérarchiques [moteur de base de données]"
  - "SqlHierarchyId"
  - "hierarchyid [moteur de base de données]"
  - "requêtes hiérarchiques [SQL Server], utilisation du type de données hierarchyid"
...
###  <a name="BKMK_MovingSubtrees"></a> Déplacement de sous-arborescences  
 Une autre opération courante concerne le déplacement de sous-arborescences. La procédure suivante prend la sous-arborescence de **@oldMgr** pour en faire une sous-arborescence de **@oldMgr** (en y incluant **@oldMgr**).  
