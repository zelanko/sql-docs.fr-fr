...
---
# Scénarios d’utilisation du Magasin des requêtes
<a id="query-store-usage-scenarios" class="xliff"></a>
[!INCLUDE[tsql-appliesto-ss2016-asdb-xxxx-xxx_md](../../includes/tsql-appliesto-ss2016-asdb-xxxx-xxx-md.md)]

  Le Magasin des requêtes peut être utilisé dans un large éventail de scénarios où il est essentiel de suivre et de garantir les performances de charges de travail prévisibles. Voici quelques exemples que vous pouvez examiner :  
...
2.  Vérifiez si l’optimiseur suggère des index absents dans le plan XML. Si tel est le cas, créez les index absents et utilisez le Magasin des requêtes pour évaluer les performances des requêtes après la création de l’index.  
...
-   Application d’une stratégie de filtrage pour la sécurité au niveau des lignes. Pour plus d’informations, consultez [Optimisation de la sécurité au niveau des lignes avec le Magasin des requêtes](http://blogs.msdn.com/b/sqlsecurity/archive/2015/07/21/optimizing-rls-performance-with-the-query-store.aspx).  
...
1.  Exécutez votre charge de travail avec le Magasin des requêtes avant la modification planifiée pour générer une ligne de base des performances.  
...
5.  Décidez s’il faut conserver la modification ou effectuer une restauration dans le cas où les nouvelles performances ne seraient pas acceptables.  
 ...
 L’illustration suivante montre l’analyse du Magasin des requêtes (étape 4) en cas de création d’index absents. Ouvrez **Principales requêtes consommatrices de ressources** / Volet Résumé du plan pour obtenir l’affichage suivant pour la requête qui doit être affectée par la création d’index :  
