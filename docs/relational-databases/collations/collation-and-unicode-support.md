...
|\_BIN2 <sup>1</sup> <sup>2</sup>|Ordre de tri de point de code binaire.|
...
<sup>2</sup> L’ajout de l’option UTF-8 (\_UTF8) active l’encodage des données Unicode avec UTF-8. Pour plus d’informations, consultez la section [Prise en charge d’UTF-8](#utf8) de cette page. 
...
Le classement par défaut du serveur est défini lors de l’installation de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], et il devient également le classement par défaut des bases de données système et de toutes les bases de données utilisateur. 

Le tableau suivant montre les désignations de classement par défaut déterminées par les paramètres régionaux du système d’exploitation, avec les identificateurs de code de langue (LCID) respectifs de Windows et de SQL :
...
Le classement d’une base de données utilisateur peut être modifié avec une instruction `ALTER DATABASE` similaire à celle-ci :
...
Le classement effectif d’une base de données peut être récupéré via une instruction similaire à celle-ci :
...
Le classement d’une colonne peut être changé via une instruction `ALTER TABLE` similaire à celle-ci :
...
Quand vous utilisez seulement des colonnes de caractères et des pages de codes, vous devez vérifier que la base de données est installée avec une page de codes qui va gérer les caractères dans les trois langues. Vous devez aussi faire en sorte de garantir la traduction correcte des caractères d’une des langues quand ils sont lus par des clients exécutant une page de codes pour une autre langue.
...
La sélection d’une page de codes pour les types de données des caractères qui va prendre en charge l’ensemble des caractères nécessaires pour une audience internationale est difficile.
Le moyen le plus simple de gérer les données de type caractère dans les bases de données internationales est de toujours utiliser un type de données qui prend en charge Unicode. 
...
> [!NOTE]
> Vérifiez que vos outils clients, comme [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)], utilisent la police Dengxian pour afficher correctement les chaînes contenant des caractères encodés en GB18030.
