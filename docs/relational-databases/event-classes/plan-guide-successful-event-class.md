...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)] La classe d’événements Plan Guide Successful indique que [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] a produit correctement un plan d’exécution pour une requête ou un lot qui contenait un repère de plan. L'événement est déclenché lorsque les conditions suivantes sont remplies :  
