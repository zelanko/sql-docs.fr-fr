...
-   Augmentez la valeur de MAX_MEMORY_PERCENT.   
    ...
    > Consultez la rubrique [Meilleures pratiques : utilisation de l’OLTP en mémoire dans un environnement de machine virtuelle](../Topic/Using%20In-Memory%20OLTP%20in%20a%20VM%20Environment.md) pour plus d’informations.  
...
-   Augmentez **max server memory**.  
    Pour plus d’informations sur la configuration de **max server memory** see the topic [Optimisation des performances du serveur à l’aide des options de configuration de la mémoire](http://technet.microsoft.com/library/ms177455\(v=SQL.105\).aspx).  
...
## Voir aussi  
 ...
 [Meilleures pratiques : utilisation de l'OLTP en mémoire dans un environnement de machine virtuelle](../Topic/Using%20In-Memory%20OLTP%20in%20a%20VM%20Environment.md)  
