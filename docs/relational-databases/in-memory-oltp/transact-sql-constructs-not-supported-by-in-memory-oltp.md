...
|Fonctionnalité|FOREIGN KEY|Dans les tables optimisées en mémoire, les contraintes FOREIGN KEY ne sont prises en charge que pour les clés étrangères faisant référence à des clés primaires. Supprimez la contrainte de la définition de table si la clé étrangère fait référence à une contrainte unique.|  
|Fonctionnalité|index cluster|Spécifiez un index non cluster. Dans le cas d’un index de clé primaire, veillez à spécifier **PRIMARY KEY NONCLUSTERED [HASH]**.|  
...
|Fonctionnalité|SELECT INTO|La clause **INTO** n'est pas prise en charge avec l'instruction **SELECT** . Réécrivez la requête sous la forme **p INTO***Table***SELECT**.|  
...
|Fonctionnalité|CASE|L'instruction **CASE** n'est pas prise en charge dans les requêtes à l'intérieur de procédures stockées compilées en mode natif. Créez des requêtes pour chaque cas. Pour plus d’informations, consultez [Implémentation d’une expression CASE dans une procédure stockée compilée en mode natif](../../relational-databases/in-memory-oltp/implementing-a-case-expression-in-a-natively-compiled-stored-procedure.md).|  
...
|Fonctionnalité|DDL|Aucune DDL n’est prise en charge.|  
