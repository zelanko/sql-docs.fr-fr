---
title: Manipulation des paramètres de gros objets (LOB) dans le CLR Microsoft Docs
description: Cet article décrit comment gérer les valeurs de grand objet (LOB) pour les paramètres dans l’intégration SQL Server CLR. Utilisez SqlBytes et SqlChars pour les types LOB.
ms.custom: ''
ms.date: 03/04/2017
ms.prod: sql
ms.reviewer: ''
...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]
  Utilisez **SqlBytes** et **SqlChars** pour passer les paramètres binaires de gros objets (LOB)**(varbinaires(max)**) et lob **(nvarchar(max)**) respectivement. Ces types autorisent la diffusion en continu des valeurs LOB de la base de données à la routine CLR (Common Language Runtime), au lieu de copier la valeur entière dans l'espace managé. **SqlBinary** et **SqlString** ne doivent être utilisés que pour les petites valeurs binaires et de chaînes de caractères.  
