...
Quand vous exécutez le code ci-dessus dans Oracle, une fonction `F_RETURN_CUR` portant le nom est créée. Cette fonction retourne le curseur `refCursorValue`. Lors de la migration de cette fonction, SSMA génère un message d’erreur :
