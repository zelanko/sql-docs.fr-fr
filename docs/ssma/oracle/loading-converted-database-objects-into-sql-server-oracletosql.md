...
Une fois que vous avez enregistré les définitions d’objets SQL Server comme un ou plusieurs scripts, vous pouvez utiliser [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] pour afficher et modifier les scripts.  
...
L’étape suivante du processus de migration consiste à [migrer des données dans SQL Server](migrating-oracle-data-into-sql-server-oracletosql.md).  
