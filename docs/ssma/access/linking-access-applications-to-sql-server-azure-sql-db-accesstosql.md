...
---
# <a name="linking-access-applications-to-sql-server---azure-sql-db-accesstosql"></a>Liaison accéder aux Applications sur SQL Server - base de données SQL Azure (AccessToSQL)
...
## <a name="linking-access-and-sql-server-tables"></a>Liaison d’accès et des Tables SQL Server  
Lorsque vous liez une table de l’accès à un [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] ou SQL Azure, le moteur de base de données Jet stocke des informations de connexion et les métadonnées de la table, mais les données sont stockées dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] ou SQL Azure. Cela permet à vos applications Access fonctionnent sur les tables de l’accès, même si les tables réelles et les données sont dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] ou SQL Azure.  
  
> [!NOTE]  
> Si vous utilisez [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] l’authentification, votre mot de passe est stocké dans le texte en clair sur les tables Access liés. Nous recommandons d’utiliser l’authentification Windows.  
...
Après avoir lié les tables, les tables de SSMA apparaît avec une icône de lien petit. Dans Access, les tables s’affichent avec une icône de « liée ». Il s’agit d’un globe avec une flèche pointant vers lui.  
  
Lorsque vous ouvrez une table dans Access, les données sont récupérées à l’aide d’un curseur keyset. Pour les tables volumineuses, cela signifie que toutes les données ne sont pas récupérées en même temps. Toutefois, lorsque vous parcourez la table, accès récupère les données supplémentaires si nécessaire.  
  
> [!IMPORTANT]  
> Pour lier des tables access avec une base de données azure, nous devons Client(SNAC) Native de SQL Server version 10.5 ou version ultérieure.   
...
Si vous avez lié des tables de l’accès à une instance de SQL Server, puis vous souhaitez modifier les liens vers une autre instance, vous devez relier les tables.  
...
Si le [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] ou modifier des définitions de table SQL Azure, vous pouvez supprimer la liaison et puis reconnectez les tables de SSMA en procédant comme indiqué plus haut dans cette rubrique. Ou bien, vous pouvez mettre à jour les tables à l’aide de l’accès.  
...
Les sections suivantes les problèmes de liste qui peuvent se produire dans les applications Access existantes après la migration des bases de données à partir de l’accès à [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] ou SQL Azure, puis lier les tables, en fournissant les causes et les résolutions.  
...
-   L’application dépend des fonctions qui n’existent pas dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] ou SQL Azure. Cela provoque une Jet extraire des tables localement pour exécuter une requête SELECT.  
  
-   Requêtes update ou delete de nombreuses lignes sont envoyées par Jet comme une requête paramétrable pour chaque ligne.  
  
**Résolution :** convertir les requêtes à exécution lente des requêtes directes, des procédures stockées ou des vues. Convertir les requêtes pass-through aura les problèmes suivants :  
...
**Cause :** lorsque vous ajoutez un enregistrement à un [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] ou une table SQL Azure à l’aide de VBA, si le champ de table index unique a la valeur par défaut, et vous n’affectez pas une valeur de ce champ, le nouvel enregistrement n’apparaît pas jusqu'à ce que vous la rouvrirez dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] ou SQL Azure. Si vous essayez d’obtenir une valeur de l’enregistrement, le message d’erreur suivant s’affiche :  
...
### <a name="after-migration-some-queries-will-not-let-the-user-add-a-new-record"></a>Après la migration, certaines requêtes ne permettent pas l’utilisateur Ajouter un nouvel enregistrement  
...
**Résolution :** garantir que toutes les colonnes qui sont inclus dans au moins un index unique font partie de la requête.  
