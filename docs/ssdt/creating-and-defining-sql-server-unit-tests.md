...
|----------------|----------------------|  
|**Effectuer des exercices pratique** : suivez une procédure d’introduction pour vous familiariser avec la création et l’exécution d’un test unitaire SQL Server simple.|-   [Procédure pas à pas : Créer et exécuter un test unitaire SQL Server](../ssdt/walkthrough-creating-and-running-a-sql-server-unit-test.md)|  
