...
* **Moteur de base de données Microsoft Jet 4.0 :** À compter de la version 2.6, MDAC ne contient plus les composants Jet. En d’autres termes, MDAC 2.6, 2.7 et 2.8 ne contiennent pas Microsoft Jet, le fournisseur Microsoft Jet OLE DB, les pilotes de base de données ODBC Desktop ou Jet objets DAO (Data Access). 
...
* **Oracle ODBC et Oracle OLE DB :** Le pilote Microsoft Oracle ODBC (ODBC Oracle) et le fournisseur Microsoft OLE DB pour Oracle (Oracle OLE DB) donnent accès aux serveurs de base de données Oracle. Ils sont générés à l’aide de l’Interface OCI (Oracle Call) version 7 et fournissent une prise en charge complète pour Oracle 7. En outre, il utilise une émulation de Oracle 7 pour fournir la prise en charge limitée pour les bases de données Oracle 8. Oracle prend en charge n’est plus les applications qui utilisent des appels de la version 7 OCI. Ces technologies sont déconseillés. Si vous utilisez des sources de données Oracle, vous devez migrer au fournisseur et le pilote Oracle.
...
* **Communication à distance de Out-of-Process Interface OLE DB :** Communication à distance OLEDB Interface (msdaps.dll) une tentative pour permettre aux fournisseurs OLE DB exécuter en dehors du processus. Communication à distance OLEDB Out-of-Process Interface est déconseillée.
* **AppleTalk et Banyan Vines les bibliothèques réseau SQL :** Les bibliothèques réseau Banyan Vines, AppleTalk, ServerNet, IPX/SPX, Giganet et RPC SQL sont déconseillés. Si vous utilisez l’un de ces technologies, vous devez modifier vos applications pour utiliser une des autres bibliothèques réseau, tels que TCP/IP et canaux nommés.
...
* **MDAC 1.5, MDAC 2.0 et 2.1 de MDAC :** Ces versions de MDAC étaient des versions indépendantes qui ont été publiées par Microsoft Windows NT Option Pack, le SDK de plate-forme Microsoft Windows ou le site Web de MDAC. Ces versions de MDAC ne sont plus prises en charge.
