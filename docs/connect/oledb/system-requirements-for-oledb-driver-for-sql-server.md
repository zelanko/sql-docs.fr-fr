...
## <a name="operating-system-requirements"></a>Système d'exploitation requis  
 ...
 Lorsque vous utilisez des méthodes d’authentification Azure Active Directory avec le pilote OLE DB, assurez-vous que la [bibliothèque d’authentification Active Directory pour SQL Server](https://go.microsoft.com/fwlink/?LinkID=513072) a été installée. ADAL n’est pas requis pour les autres méthodes d’authentification ou les opérations OLE DB.
Pour plus d'informations, consultez les pages suivantes : [Utilisation d’Azure Active Directory](features/using-azure-active-directory.md).
