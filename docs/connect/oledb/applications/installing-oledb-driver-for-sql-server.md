...
Pour télécharger la version la plus récente du pilote OLE DB pour SQL Server, allez à Microsoft Download Center.
...
>  Tous les paramètres de Registre appropriés pour le pilote OLE DB pour SQL Server sont effectuées dans le cadre du processus d’installation.  
 ...
 Le pilote OLE DB pour les fichiers d’en-tête et de la bibliothèque SQL Server (msoledbsql.h et msoledbsql.lib) sont installés à l’emplacement suivant :  
