...
|**UseFMTONLY**|SSPROP_INIT_USEFMTONLY|[!INCLUDE[ssSQL11](../../../includes/sssql11-md.md)] contrôle la façon dont les métadonnées sont récupérées pendant la connexion à  et les versions ultérieures. Les valeurs possibles sont « yes » et « no ». La valeur par défaut est « no ».<br /><br />Par défaut, le pilote OLE DB pour SQL Server utilise [sp_describe_first_result_set](../../../relational-databases/system-stored-procedures/sp-describe-first-result-set-transact-sql.md) et [sp_describe_undeclared_parameters](../../../relational-databases/system-stored-procedures/sp-describe-undeclared-parameters-transact-sql.md) des procédures stockées pour récupérer des métadonnées. Ces procédures stockées ont certaines limitations (par exemple, ils échoueront lors de l’exploitation sur des tables temporaires). Paramètre **UseFMTONLY** sur « yes » indique au pilote d’utiliser [SET FMTONLY](../../../t-sql/statements/set-fmtonly-transact-sql.md) pour la récupération de métadonnées à la place.|  
...
|**Use FMTONLY**|SSPROP_INIT_USEFMTONLY|[!INCLUDE[ssSQL11](../../../includes/sssql11-md.md)] contrôle la façon dont les métadonnées sont récupérées pendant la connexion à  et les versions ultérieures. Les valeurs possibles sont « true » et « false ». La valeur par défaut est « false ».<br /><br />Par défaut, le pilote OLE DB pour SQL Server utilise [sp_describe_first_result_set](../../../relational-databases/system-stored-procedures/sp-describe-first-result-set-transact-sql.md) et [sp_describe_undeclared_parameters](../../../relational-databases/system-stored-procedures/sp-describe-undeclared-parameters-transact-sql.md) des procédures stockées pour récupérer des métadonnées. Ces procédures stockées ont certaines limitations (par exemple, ils échoueront lors de l’exploitation sur des tables temporaires). Paramètre **FMTONLY utiliser** à « true » indique au pilote d’utiliser [SET FMTONLY](../../../t-sql/statements/set-fmtonly-transact-sql.md) pour la récupération de métadonnées à la place.|  
...
<b id="table2_1">[1] :</b> pour améliorer la sécurité, comportement de validation de chiffrement et le certificat est modifié lors de l’utilisation des propriétés d’initialisation du jeton d’accès / d’authentification ou leurs mots clés de chaîne de connexion correspondante. Pour plus d’informations, consultez [chiffrement et validation des certificats](../features/using-azure-active-directory.md#encryption-and-certificate-validation).
...
<b id="table3_1">[1] :</b> pour améliorer la sécurité, comportement de validation de chiffrement et le certificat est modifié lors de l’utilisation des propriétés d’initialisation du jeton d’accès / d’authentification ou leurs mots clés de chaîne de connexion correspondante. Pour plus d’informations, consultez [chiffrement et validation des certificats](../features/using-azure-active-directory.md#encryption-and-certificate-validation).
