...
|**Authentification** <a href="#table1_1"> <sup id="table1_authmode"> **1**</sup></a>|SSPROP_AUTH_MODE|Spécifie l’authentification SQL ou Active Directory utilisée. Les valeurs valides sont :<br/><ul><li>`(not set)`: Mode d’authentification déterminé par les autres mots clés.</li><li>`ActiveDirectoryPassword:`Active Directory l’authentification à l’aide de l’ID de connexion et du mot de passe.</li><li>`ActiveDirectoryIntegrated:`Authentification intégrée à Active Directory à l’aide des informations d’identification du compte Windows de l’utilisateur actuellement connecté.</li><br/>**Remarque:** Il est **recommandé** que les applications `Integrated Security` utilisant des `Trusted_Connection`Mots clés d’authentification (ou) ou leurs propriétés `Authentication` correspondantes définissent la valeur du mot clé `ActiveDirectoryIntegrated` (ou de sa propriété correspondante) sur pour activer New le chiffrement et le comportement de validation des certificats.<br/><br/><li>`SqlPassword:`Authentification à l’aide de l’ID de connexion et du mot de passe.</li><br/>**Remarque:** Il est **recommandé** que les applications `SQL Server` utilisant l’authentification définissent `Authentication` la valeur du mot clé (ou de `SqlPassword` sa propriété correspondante) sur pour activer le nouveau chiffrement et le comportement de validation du certificat.</ul>|
|**Traduction automatique**|SSPROP_INIT_AUTOTRANSLATE|Synonyme de « AutoTranslate ».|  
...
|**UseProcForPrepare**|SSPROP_INIT_USEPROCFORPREP|Ce mot clé est déconseillé, et son paramètre est ignoré par le pilote OLE DB pour SQL Server.|  
