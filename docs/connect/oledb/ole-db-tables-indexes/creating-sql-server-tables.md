...
## <a name="dbcolumndesc-usage"></a>Utilisation de la structure DBCOLUMNDESC  
 Le consommateur peut indiquer un type de données de colonne en utilisant soit le membre *pwszTypeName*, soit le membre *wType*. Si le consommateur spécifie le type de données dans *pwszTypeName*, le pilote OLE DB pour SQL Server ignore la valeur de *wType*.  
 ...
 Le pilote OLE DB pour SQL Server reconnaît un sous-ensemble de valeurs DBTYPE énumérées par OLE DB dans le membre *wType* . Pour plus d’informations, consultez [mappage de type de données dans ITableDefinition](../../oledb/ole-db-data-types/data-type-mapping-in-itabledefinition.md).  
