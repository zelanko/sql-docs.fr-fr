...
<xref:System.Guid> et <xref:System.Data.SqlTypes.SqlGuid> avoir une méthode de `CompareTo` pour comparer différentes valeurs de GUID. Toutefois, les `System.Guid.CompareTo` et les `SqlTypes.SqlGuid.CompareTo` sont implémentés différemment. <xref:System.Data.SqlTypes.SqlGuid> implémente `CompareTo` à l’aide d’SQL Server comportement, les six derniers octets d’une valeur sont plus significatifs. <xref:System.Guid> évalue les 16 octets. L’exemple suivant illustre cette différence de comportement. La première section de code affiche des valeurs de <xref:System.Guid> non triées, et la deuxième section de code affiche les valeurs de <xref:System.Guid> triées. La troisième section affiche les valeurs de <xref:System.Data.SqlTypes.SqlGuid> triées. La sortie s’affiche sous la liste de code.  
