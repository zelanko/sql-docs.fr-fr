...
Par défaut, une opération de copie en bloc est sa propre transaction. Lorsque vous souhaitez effectuer une opération de copie en bloc dédiée, créez une nouvelle instance de <xref:Microsoft.Data.SqlClient.SqlBulkCopy> avec une chaîne de connexion, ou utilisez un objet <xref:Microsoft.Data.SqlClient.SqlConnection> existant sans transaction active. Dans chaque scénario, l’opération de copie en bloc crée, puis valide ou restaure la transaction.  
...
Si vous devez restaurer l’intégralité de l’opération de copie en bloc en raison d’une erreur, ou si la copie en bloc doit s’exécuter dans le cadre d’un processus plus volumineux qui peut être restauré, vous pouvez fournir un objet <xref:Microsoft.Data.SqlClient.SqlTransaction> au constructeur <xref:Microsoft.Data.SqlClient.SqlBulkCopy>.  
