...
> [!IMPORTANT]  
>  Cet exemple ne s’exécutera pas à moins que vous ayez créé les tables de travail comme décrit dans [le programme d’installation de Table](../../connect/jdbc/using-bulk-copy-with-the-jdbc-driver.md#BKMK_TableSetup). Vous devez également créer un fichier CSV avec des données exportées de la table Production.Product. SQL Server Management Studio (SSMS) peut vous aider à le faire.  Si vous n’avez pas de SSMS, vous pouvez visiter [télécharger SQL Server Management Studio](https://msdn.microsoft.com/library/mt238290.aspx) pour l’obtenir.  
