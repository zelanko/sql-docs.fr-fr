...
+ Vous souhaitez exécuter le code dans le *@script* paramètre de [sp_execute_external_script](../../relational-databases/system-stored-procedures/sp-execute-external-script-transact-sql.md) et paramétrer également la procédure stockée.
...
+ Exécutez toutes les requêtes à l’avance et passez en revue les SQL Server plans de requête pour identifier les tâches qui peuvent être effectuées en parallèle.
    ...
    Si la requête d’entrée peut être parallélisée, définissez `@parallel=1` dans le cadre de vos arguments sur [sp_execute_external_script](../../relational-databases/system-stored-procedures/sp-execute-external-script-transact-sql.md). 
