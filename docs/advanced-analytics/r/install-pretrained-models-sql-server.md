...
Cet article décrit comment ajouter des modèles dont l’apprentissage à une instance de SQL Server qui a déjà R Services ou Machine Learning Services est installé.

Les modèles dont l’apprentissage inclus dans MicrosoftML ont été créés pour aider les clients qui ont besoin effectuer des tâches telles que la personnalisation de la sentiment analysis ou image, mais qui n’ont pas les ressources pour obtenir des jeux de données volumineux ou de former un modèle complex. L’équipe Machine Learning Server créé et formé de ces modèles pour vous aider à démarrer sur le texte et image traiter efficacement. Pour plus d’informations, consultez la [ressources](#bkmk_resources) section de cet article.

Pour obtenir un exemple montrant comment utiliser les modèles dont l’apprentissage avec des données SQL Server, consultez le blog par l’équipe SQL Server Machine Learning :
...
## <a name="install-the-pre-trained-models"></a>Installer les modèles dont l’apprentissage

Vous pouvez utiliser les modèles dont l’apprentissage avec les produits suivants :
...
> [!NOTE]
> Il n’est pas possible de lire ou modifier les modèles dont l’apprentissage, car elles sont compressées à l’aide d’un format natif pour améliorer les performances.
...
### <a name="install-pretrained-models-with-sql-server-2016-r-services"></a>Installer les modèles préformés avec SQL Server 2016 R Services
...
Cela en exécutant le programme d’installation Windows distinct pour Microsoft R Server ou Machine Learning et en sélectionnant une instance ou des instances de **lier**. Un moyen d’instance associé à la stratégie de prise en charge de l’instance de liaison est modifiée pour autoriser les mises à jour plus fréquentes sur R. 
...
2. Sélectionnez l’instance à mettre à niveau, puis sélectionnez l’option pour obtenir les modèles préformés.
...
### <a name="install-pretrained-models-on-sql-server-2017"></a>Installer les modèles préformés sur SQL Server 2017

Si vous avez déjà installé SQL Server 2017, vous pouvez obtenir les modèles préformés de deux manières :

+ Mettre à niveau les composants de Python et R en utilisant une liaison et installez les modèles préformés en même temps
...
### <a name="installing-pretrained-models-with-r-server-standalone"></a>L’installation de modèles préformés avec R Server (autonome)
...
3. Exécutez `RSetup.exe` et indiquer le composant à installer, la version et le dossier contenant les fichiers de source de modèle, à l’aide de cette syntaxe :
    ...
    Par exemple, pour activer l’utilisation de la version la plus récente des modèles préformés pour R, dans une installation par défaut de R Server (autonome), vous exécuteriez cette instruction :
...
### <a name="installing-pretrained-models-with-machine-learning-server-standalone"></a>L’installation de modèles préformés avec Machine Learning Server (autonome)
...
Pour plus d’informations sur les algorithmes utilisés dans ces approfondie d’apprentissage des modèles et comment ils om implémentée et formé à l’aide de CNTK, consultez les articles suivants :
...
### <a name="how-to-use-pretrained-models-for-image-detection"></a>Comment utiliser des modèles préformés pour la détection de l’image
