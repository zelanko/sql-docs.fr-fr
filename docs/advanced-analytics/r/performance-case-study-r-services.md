...
Cet article est le quatrième et dernière d’une série qui décrit l’optimisation des performances pour R Services. Cet article résume les méthodes, les résultats et les conclusions deux études de cas qui testé différentes méthodes d’optimisation.

Les études de cas deux avions des objectifs différents :

+ La première étude de cas, par l’équipe de développement R Services, cherchait à mesurer l’impact des techniques d’optimisation spécifiques
+ La deuxième étude de cas, par une équipe spécialiste des données, expérimenté plusieurs méthodes pour déterminer les meilleures optimisations pour un scénario de calcul de score de haut volume spécifique.
...
1. Le jeu de données de compagnies aériennes compose d’une seule table de 10 millions de lignes. Il a été téléchargé et en bloc chargées dans SQL Server.
2. Six copies de la table ont été apportées.
3. Diverses modifications ont été appliquées aux copies de la table, pour tester les fonctionnalités de SQL Server, telles que la compression de page, la compression de ligne, l’indexation, le magasin de données en colonnes, etc.
...
Chaque test était constitué des étapes suivantes :
...
4. Chaque test a été exécuté six fois. L’heure de la première exécution (le « exécuter à froid ») a été supprimé. Pour permettre la valeur hors norme occasionnelle, le **maximale** temps entre les cinq exécutions restantes a été supprimé également. Le temps d’exécution écoulé moyen de chaque test a été calculé à partir de la moyenne des quatre exécutions restantes.
5. Les tests ont été exécutés à l’aide de la *reportProgress* paramètre avec la valeur 3 (= les durées de rapport et la progression). Chaque fichier de sortie contient des informations concernant le temps consacré aux e/s, temps de transition et temps de calcul. Ces informations sont utiles pour le diagnostic et la résolution des problèmes.
...
Par exemple, les résultats suivants sont les heures à partir d’un test unique. Les valeurs les plus intéressantes sont **Total read time** (le temps d’E/S) et **Transition time** (le temps passé à créer les processus de calcul).
...
Nous vous recommandons de télécharger et de modifier les scripts de test pour vous aider à résoudre les problèmes avec R Services ou des fonctions de RevoScaleR.
...
#### <a name="data-size-with-compression-and-a-columnar-table-store"></a>Taille des données avec la compression et un magasin de la table en colonnes
...
La réduction de plus grande taille de données a été obtenue en appliquant un index columnstore, suivi de la compression de page.

#### <a name="effects-of-compression"></a>Effets de la compression

Ce test comparé les avantages de la compression de ligne, la compression de page et aucune compression. Un modèle a été formé à l’aide de `rxLinMod` sur les données de trois tables de données différents. La même formule et requête a été utilisée pour toutes les tables.
...
La compression seule ne semble pas à l’aide. Dans cet exemple, l’augmentation du processeur pour gérer la compression compense la diminution du temps e/s.
...
### <a name="effect-of-windows-power-plan-options"></a>Effet des options de plan d’alimentation Windows
...
La durée d’exécution est plus cohérente et plus rapide lors de l’utilisation de la Windows **hautes performances** de l’alimentation.

#### <a name="using-integer-vs-strings-in-formulas"></a>À l’aide des entiers et chaînes dans les formules

Ce test évalué l’impact de la modification du code R pour éviter un problème courant avec les facteurs de chaîne. Plus précisément, un modèle a été formé à l’aide de `rxLinMod` à l’aide de deux tables : dans le premier, les facteurs sont stockés sous forme de chaînes ; dans la deuxième table, les facteurs sont stockées sous forme d’entiers.

+ Pour le *compagnies aériennes* table, la colonne [DayOfWeek] contient des chaînes. Le _colInfo_ paramètre a été utilisé pour spécifier les niveaux de facteur (lundi, mardi,...)
...
Il existe un avantage évident lorsque vous utilisez des entiers plutôt que de chaînes à des facteurs externes.
...
+ Dans la première exécution, une fonction de transformation a été appliquée dans le cadre de la création de modèles. 
+ Dans la seconde exécution, les valeurs de fonctionnalité ont été précalculées et disponibles, afin qu’aucune fonction de transformation a été nécessaire.
...
Le temps d’apprentissage a été plus court when **pas** à l’aide d’une fonction de transformation. En d’autres termes, le modèle a été formé plus rapidement lorsque vous utilisez des colonnes qui sont précalculées et conservées dans la table.

Les économies sont censé être supérieure si il y avait de nombreuses transformations de plus, le jeu de données ont été plus grand (\> 100 millions).

### <a name="using-columnar-store"></a>À l’aide du stockage en colonnes

Ce test évalué les gains de performance de l’utilisation d’un magasin de données en colonnes et les index. Le même modèle a été formé à l’aide de `rxLinMod` et aucune transformation de données.

+ Dans la première exécution, la table de données utilisé une banque de lignes standard.
...
Les performances sont meilleures avec le magasin de colonnes qu’avec le magasin de ligne standard. Une différence significative des performances peut être prévue sur les jeux de données volumineux (\> 100 millions).

### <a name="effect-of-using-the-cube-parameter"></a>Effet de l’utilisation du paramètre de cube

L’objectif de ce test était de déterminer si l’option dans RevoScaleR pour l’utilisation de la précalculées **cube** paramètre peut améliorer les performances. Un modèle a été formé à l’aide de `rxLinMod`, à l’aide de cette formule :
...
| Nom du test     | Paramètre cube | numTasks | Temps moyen | Unique-ligne (arrdelay_pred) |
|---------------|----------------|----------|--------------|---------------------------------|
...
### <a name="effect-of-changing-maxdepth-for-rxdtree-models"></a>Effet de la modification de maxDepth pour rxDTree les modèles

Dans cette expérience, le `rxDTree` algorithme a été utilisé pour créer un modèle sur le *airlineColumnar* table. Pour ce test, *numTasks* a été défini sur 4. Plusieurs valeurs différentes pour *maxDepth* ont été utilisés pour illustrer comment modifier la profondeur d’arborescence affecte moment de l’exécution.
...
À mesure que la profondeur de l’arborescence augmente, le nombre total de nœuds augmente de façon exponentielle. Le temps écoulé pour la création du modèle est également augmenté de manière significative.

### <a name="prediction-on-a-stored-model"></a>PRÉDICTION sur un modèle stocké

L’objectif de ce test a été de déterminer les impacts sur les performances de calcul de score lorsque le modèle formé est enregistré dans une table SQL Server plutôt que générée en tant que partie du code en cours d’exécution. Pour calculer les scores, le modèle stocké est chargé à partir de la base de données et des prédictions sont créées à l’aide d’une trame de données d’une ligne dans la mémoire (contexte de calcul local).

Les résultats de test indiquent l’heure d’enregistrement du modèle et le temps nécessaire pour charger le modèle et de prédire.
...
Les packages RevoScaleR et de MicrosoftML ont été utilisées pour former un modèle prédictif dans une solution R complexe impliquant des jeux de données volumineux. Requêtes SQL et le code R étaient identiques dans tous les tests. Les tests ont été effectués sur une machine virtuelle Azure unique avec SQL Server est installé. Ensuite, l’auteur comparé les temps de calcul de score avec et sans les optimisations suivantes fournies par SQL Server :
...
Affinage de l’UC a été appliqué dans le scénario de mise en correspondance de reprise, afin d’évaluer l’impact sur les travaux R. Quatre **pools de ressources SQL** et quatre **pools de ressources externes** ont été créés, et l’affinité du processeur a été spécifiée pour garantir que le même ensemble de processeurs serait être utilisé dans chaque nœud.

Chacun des pools de ressources a été affecté à un groupe de charges de travail différent, pour optimiser l’utilisation du matériel. La raison est que Soft-NUMA et l’affinité du processeur ne peut pas diviser la mémoire physique dans les nœuds NUMA physiques ; Par conséquent, par définition, tous les nœuds NUMA logicielles qui sont basés sur le même nœud NUMA physique doivent utiliser mémoire dans le même bloc de mémoire du système d’exploitation. En d’autres termes, il n’existe aucune affinité de mémoire et processeur.
...
5. Créer une fonction classifieur définie par l’utilisateur (UDF) pour affecter des différentes tâches sur les groupes de charges de travail différent.
...
La configuration qui avait des performances optimales dans la correspondance de reprise à étudier était le suivant :
...
-   Chaque pool de ressources est affecté à des processeurs différents

-   Utilisation maximale de mémoire interne (pour SQL Server) = 30 %

-   Mémoire maximale pour une utilisation par les sessions R = 70 %

Pour le modèle de mise en correspondance de reprise, utilisation du script externe a été lourde et ne portaient aucune autre base de données des services de moteur en cours d’exécution. Par conséquent, les ressources allouées aux scripts externes ont été augmentées jusqu'à 70 %, ce qui constitue la meilleure configuration pour les performances de script.

Cette configuration a été équivaut à expérimenter avec différentes valeurs. Si vous utilisez un matériel différent ou une autre solution, la configuration optimale peut être différente. Toujours faire des essais pour trouver la meilleure configuration pour votre cas !

Dans la solution optimisée, 1.1 millions de lignes (avec des fonctionnalités de 100) ont été notées dans moins de 8,5 secondes sur un ordinateur de 20 cœurs. Optimisations amélioré de manière significative les performances en termes de temps de calcul de score.

Les résultats également conseillé que le **nombre de fonctionnalités** eu un impact significatif sur le temps de calcul de score. L’amélioration a été encore plus importante lorsque davantage de fonctionnalités ont été utilisés dans le modèle de prévision.

Nous vous recommandons de lire cet article de blog et le didacticiel qui accompagne cet article pour une présentation détaillée.

-   [Conseils d’optimisation et astuces pour machine learning dans SQL Server](https://azure.microsoft.com/blog/optimization-tips-and-tricks-on-azure-sql-server-for-machine-learning-services/)

Nombre d’utilisateurs ont indiqué qu’il existe une petite pause comme le runtime R (ou Python) est chargé pour la première fois. Pour cette raison, comme décrit dans ces tests, l’heure de la première exécution est souvent mesurée mais rejeté par la suite. Mise en cache ultérieure peut entraîner des différences de performances notables entre la première et deuxième s’exécute. Il existe également une certaine surcharge lorsque les données sont déplacées entre SQL Server et le runtime externe, en particulier si les données sont transmises via le réseau, plutôt que directement à partir de SQL Server en cours de chargement.
