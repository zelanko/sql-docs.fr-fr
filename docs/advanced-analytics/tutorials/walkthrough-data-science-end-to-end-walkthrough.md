...
Vous utilisez une combinaison de code R, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] de données et de fonctions SQL personnalisées pour créer un modèle de classification qui indique la probabilité que le pilote puisse obtenir un pourboire sur un voyage de taxi particulier. Vous déployez également votre modèle R [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sur et utilisez les données du serveur pour générer des scores basés sur le modèle.
...
Cette procédure pas à pas requiert plusieurs bibliothèques R qui ne sont pas installées [!INCLUDE[rsql_productname](../../includes/rsql-productname-md.md)]par défaut dans le cadre de. Vous devez installer les packages sur le client sur lequel vous développez la solution et sur l' [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ordinateur sur lequel vous déployez la solution.
...
2. À l’invite R, exécutez les commandes R suivantes :
  ...
  ```
  Cet exemple utilise la fonction grep R pour rechercher le vecteur de chemins disponibles et rechercher le chemin d’accès qui contient «Program Files». Pour plus d’informations, [https://www.rdocumentation.org/packages/base/functions/grep](https://www.rdocumentation.org/packages/base/functions/grep)consultez.

  Si vous pensez que les packages sont déjà installés, vérifiez la liste des packages installés en `installed.packages()`exécutant.
