---
title: 'Tutoriel : Déployer un modèle dans Python pour catégoriser les clients'
description: Dans la quatrième partie de cette série de didacticiels en quatre parties, vous allez déployer un modèle de clustering dans Python avec SQL Server Machine Learning Services.
ms.prod: sql
...
Dans la quatrième partie de cette série de didacticiels en quatre parties, vous allez déployer un modèle de clustering, développé en Python, dans une base de données SQL à l’aide de SQL Server Machine Learning Services.

Pour effectuer régulièrement le clustering, à mesure que de nouveaux clients s’inscrivent, vous devez être en mesure d’appeler le script Python à partir de n’importe quelle application. Pour ce faire, vous pouvez déployer le script Python dans SQL Server en plaçant le script Python à l’intérieur d’une procédure stockée SQL dans la base de données. Étant donné que votre modèle s’exécute dans la base de données SQL, il peut facilement être formé en fonction des données stockées dans la base de données.
...
Dans la [deuxième partie](python-clustering-model-prepare-data.md), vous avez appris à préparer les données à partir d’une base de données SQL pour effectuer le clustering.

Dans la [troisième partie](python-clustering-model-build.md), vous avez appris à créer et à former un modèle de clustering K-means dans Python.

## <a name="prerequisites"></a>Prérequis

* La quatrième partie de cette série de didacticiels suppose que vous avez rempli les conditions préalables de la [**première partie**](python-clustering-model.md)et effectué les étapes de la deuxième [**partie**](python-clustering-model-prepare-data.md) et de la troisième [**partie**](python-clustering-model-build.md).
...
Supposons que vous souhaitiez envoyer un e-mail promotionnel aux clients dans le cluster 0, le groupe qui était inactif (vous pouvez voir comment les quatre clusters ont été décrits dans la [troisième partie](python-clustering-model-build.md#analyze-the-results) de ce didacticiel). Le code suivant sélectionne les adresses de messagerie des clients dans le cluster 0.
