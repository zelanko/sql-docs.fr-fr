---
title: Préparer les données à l’aide de PowerShell (procédure pas à pas) | Microsoft Docs
ms.prod: sql
...
À ce stade, vous devez avoir un des éléments suivants :

+ SQL Server 2016 R Services
+ SQL Server 2017 Machine Learning Services, avec le langage R est activé

Dans cette leçon, vous téléchargez les données, les packages R et les scripts R utilisés dans la procédure pas à pas d’un référentiel Github. Vous pouvez télécharger tout à l’aide d’un script PowerShell pour des raisons pratiques.
...
Ensuite, vous utilisez un deuxième script PowerShell, RunSQL_R_Walkthrough.ps1, pour configurer la base de données qui est utilisé pour la modélisation et la notation. Que le script effectue un chargement en masse des données dans la base de données vous spécifiez et crée ensuite des fonctions SQL et les procédures stockées qui simplifient les tâches de science des données.
...
3.  Exécutez la commande Powershell suivante pour télécharger les fichiers de script dans un répertoire local. Si vous ne spécifiez pas un répertoire différent, par défaut, le dossier `C:\tempR` est créé et tous les fichiers enregistrés.
    ...
    Si vous souhaitez enregistrer les fichiers dans un répertoire différent, modifiez les valeurs du paramètre *DestDir* et spécifiez un dossier différent sur votre ordinateur. Si vous tapez un nom de dossier qui n’existe pas, le script PowerShell crée le dossier pour vous.
...
Cette procédure pas à pas nécessite certaines bibliothèques R qui ne sont pas installées par défaut dans le cadre de [!INCLUDE[rsql_productname](../../includes/rsql-productname-md.md)]. Vous devez installer les packages sur le client lorsque vous développez la solution et sur le [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ordinateur où vous déployez la solution.
...
2. Mettez en surbrillance et exécutez ces lignes.
    ...
    Certains packages installent également les packages requis. En tout, environ 32 packages sont nécessaires.
...
> [!NOTE]
> Sur le serveur, **ne le faites pas** installer dans une bibliothèque utilisateur même si vous y êtes invité. Si vous installez dans une bibliothèque utilisateur, l’instance de SQL Server ne peut pas trouver ou exécutez le package. Pour plus d’informations, consultez [Installer des packages R supplémentaires sur SQL Server](../r/install-additional-r-packages-on-sql-server.md).
...
Ainsi que les fichiers de données, des scripts R et scripts T-SQL, le téléchargement inclut le script PowerShell `RunSQL_R_Walkthrough.ps1`. Le script effectue les actions suivantes :
...
Exécutez ce script sur l’ordinateur où vous générez la solution : par exemple, l’ordinateur portable où vous développez et testez votre code R. Cet ordinateur, que nous allons appeler le client de science des données, doit pouvoir se connecter à l’ordinateur [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] à l’aide du protocole Canaux nommés.
...
3.  Vous êtes invité pour chacun des paramètres suivants :
  
    **Nom du serveur de base de données**: le nom de l’instance de SQL Server où Machine learning Services ou R Services est installé.
...
Si vous rencontrez des problèmes avec le script PowerShell, vous pouvez exécuter tout ou partie des étapes manuellement, en utilisant les lignes du script PowerShell comme exemples. Cette section répertorie quelques problèmes courants et les solutions de contournement.
...
Le fichier de données est volumineux. Vous devez utiliser un ordinateur qui dispose d’une connexion Internet relativement bonne, ou le téléchargement risque d’expirer.
...
Si votre base de données contient une table existante du même nom et le même schéma, **bcp** insère une nouvelle copie de ces données plutôt que l’écrasement des données existantes.

Pour éviter les doublons, tronquez les tables existantes avant de réexécuter le script.

## <a name="whats-included-in-the-sample"></a>Ce qui est inclus dans l’exemple

Lorsque vous téléchargez les fichiers à partir du référentiel GitHub, vous obtenez ce qui suit :

+ Données au format CSV ; consultez [formation et la notation des données](#bkmk_data) pour plus d’informations
...
### <a name="bkmk_data"></a>Apprentissage et l’évaluation des données
...
+ Jeu de données public : () [NYC Taxi and Limousines Commission]http://www.nyc.gov/html/tlc/html/about/trip_record_data.shtml)
...
+ **RunSQL_R_Walkthrough.ps1** vous exécutez ce script tout d’abord, à l’aide de PowerShell. Il appelle les scripts SQL pour charger les données dans la base de données.
...
+ **RSQL_R_Walkthrough.R** il s’agit du script R de base qui est utilisé dans le reste des leçons pour l’analyse des données et de modélisation. Il fournit tout le code R dont vous avez besoin pour explorer les données [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] , générer le modèle de classification et de créer des tracés.

### <a name="t-sql-script-files"></a>Fichiers de script T-SQL

Le script PowerShell exécute plusieurs [!INCLUDE[tsql](../../includes/tsql-md.md)] scripts sur l’instance de SQL Server. Le tableau suivant répertorie les [!INCLUDE[tsql](../../includes/tsql-md.md)] scripts et ce qu’ils font.
...
Les requêtes T-SQL utilisées dans cette procédure ont été testées et peuvent être exécutés en tant que-dans votre code R. Toutefois, si vous souhaitez faire des expérimentations supplémentaires ou développer votre propre solution, nous vous recommandons d’utiliser un environnement de développement SQL dédié pour tester et optimiser vos requêtes avant de les ajouter à votre code R.
