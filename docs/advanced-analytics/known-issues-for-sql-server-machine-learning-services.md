---
title: "Problèmes connus pour les Services de Machine Learning | Documents Microsoft"
ms.custom:
- SQL2016_New_Updated
ms.date: 06/16/2017
...
---
# <a name="known-issues-for-machine-learning-services"></a>Problèmes connus pour les Services de Machine Learning

Cette rubrique décrit les limitations ou des problèmes connus avec les composants qui sont fournies en tant qu’option dans SQL Server 2016 et SQL Server 2017 d’apprentissage.

S’applique à tous les éléments suivants, sauf si spécifiquement indiqué :
...
## <a name="setup-and-configuration-issues"></a>Le programme d’installation et les problèmes de Configuration

Une description des questions traitées et courantes liées à la configuration initiale et la configuration sont répertoriées ici : [FAQ d’Installation et de mise à niveau](r/upgrade-and-installation-faq-sql-server-r-services.md).

Consultez également cet article pour plus d’informations sur les mises à niveau, installation de côte-à-côte et l’installation de nouveaux composants de R ou Python.

### <a name="unable-to-install-python-components-in-in-offline-installs-of-sql-server-2017"></a>Impossible d’installer les composants de Python dans des installations en mode hors connexion de SQL Server 2017

Si vous installez SQL Server 2017 sur un ordinateur sans accès à Internet, le programme d’installation peut échouer afficher la page qui vous invite à spécifier l’emplacement des composants Python téléchargés ; Par conséquent, vous ne pourrez pas installer la fonctionnalité Services de Machine Learning, mais pas les composants de Python.

Ce problème sera résolu dans une prochaine version. Pour résoudre ce problème, vous pouvez temporairement activer un accès à Internet pour la durée du programme d’installation. Cette limitation ne s’applique pas à R.
...
Si vous installez la dernière version du Client de Microsoft R et l’utiliser pour exécuter R SQL Server, vous utilisez un contexte de calcul, vous pouvez obtenir une erreur semblable à celui-ci :

*9.x.x de version du client de Microsoft R sont en cours d’exécution sur votre ordinateur, ce qui est incompatible avec le 8.x.x de version de Microsoft R Server. Téléchargez et installez une version compatible.*

SQL Server 2016 nécessaire que les bibliothèques R sur le client correspondent exactement aux bibliothèques R sur le serveur. Libère que restriction a été supprimée pour R Server 9.0.1 plus tard. Toutefois, si vous rencontrez cette erreur, vérifiez la version des bibliothèques R utilisée par votre client et le serveur, la détection d’anomalie réseau si nécessaire, mettez à jour le client pour faire correspondre la version du serveur.

La version de R est installé avec SQL Server R Services est mise à jour chaque fois qu’une version de service SQL Server est installée. Par conséquent, pour vous assurer que vous disposez toujours les versions plus récentes des composants R, vous devez installer tous les service packs.

Pour la compatibilité avec Microsoft R Client 9.0.0, vous devez installer les mises à jour qui sont décrites dans cet [article de support](https://support.microsoft.com/kb/3210262).

Pour éviter des problèmes avec les packages R, vous pouvez également mettre à niveau la version des bibliothèques R qui sont installés sur le serveur, en modifiant la stratégie de cycle de vie modernes, comme décrit dans [cette section](#bkmk_sqlbindr). Lorsque vous procédez ainsi, la version de R installé avec SQL Server est mis à jour en même temps que les mises à jour sont publiées pour Microsoft R Server, ce qui garantit que serveur et le client peuvent toujours les dernières versions de Microsoft R.

**S’applique à :** SQL Server 2016 R Services, avec R Server version9.0.0 ou une version antérieure

### <a name="bkmk_sqlbindr"></a>Avertissement de version incompatible lors de la connexion à une version antérieure de SQL Server R Services à partir d’un client à l’aide de[!INCLUDE[ssSQLv14_md](../includes/sssqlv14-md.md)]
...
Pour obtenir les programmes d’installation les plus récentes, consultez [installation des composants Machine Learning sans accès à Internet](r/installing-ml-components-without-internet-access.md).
...
Si vous installez R Services séparément à partir du moteur de base de données, et les versions de build sont différentes, vous pouvez voir cette erreur dans le journal des événements système : _Launchpad de SQL Server service a échoué en raison de l’erreur suivante : le service n’a pas répondu à la demande de démarrage ou de contrôle en temps voulu._
...
Pour afficher une liste des numéros de version R requis pour chaque version de SQL Server 2016, consultez [Installation des composants R sans accès à Internet](r/installing-ml-components-without-internet-access.md).
...
### <a name="performance-limits-when-r-libraries-are-called-from-standalone-r-tools"></a>Limite les performances lorsque les bibliothèques R sont appelées à partir des outils de R autonome

Il est possible d’appeler les outils R et les bibliothèques qui sont installés pour SQL Server R Services depuis une application externe de R comme RGui. Cela peut être pratique lorsque vous installez les nouveaux packages, ou en cours d’exécution des tests ad hoc sur les exemples de code très courte.
...
+ Si vous avez besoin exécuter le code de R complète sur l’ordinateur SQL Server sans l’aide de SQL server, installer une instance distincte de R, comme le Client Microsoft R et assurez-vous que vos outils de développement R pointent vers la nouvelle bibliothèque.

Pour plus d’informations, consultez [Créer un R Server autonome](r/create-a-standalone-r-server.md).

### <a name="r-script-throttled-due-to-resource-governance-default-values"></a>Script R limité en raison de valeurs par défaut de gouvernance des ressources

Dans Enterprise Edition, vous pouvez utiliser des pools de ressources pour gérer les processus de script externe. Dans certaines versions release anticipée, la mémoire maximale qui peut être allouée, pour les processus R a 20 %. Par conséquent, si le serveur possédait 32 Go de RAM, les fichiers exécutables R (RTerm.exe et BxlServer.exe) pouvaient utiliser 6.4 Go maximum dans une seule requête.
...
## <a name="r-code-execution-and-package-or-function-issues"></a>L’exécution du Code R et de Package ou de problèmes de fonction
...
Pour d’autres problèmes connus qui peuvent affecter des solutions R, consultez le site Microsoft R Server : [problèmes connus avec Microsoft R Server](https://msdn.microsoft.com/microsoft-r/rserver-known-issues)

### <a name="limitations-on-processor-affinity-for-r-jobs"></a>Limitations sur l’affinité du processeur pour les tâches R

Dans la génération de la version initiale de SQL Server 2016, vous pouvez définir l’affinité du processeur uniquement pour les processeurs dans le premier groupe de k. Par exemple, si le serveur est une machine à 2 sockets avec 2 K-Groups, seuls les processeurs du premier K-Group sont utilisés pour les processus R. La même restriction s’applique lors de la configuration de la gouvernance des ressources pour les tâches de script R.
...
### <a name="avoid-clearing-workspaces-when-executing-r-code-in-a-includessnoversionincludesssnoversion-mdmd-compute-context"></a>Éviter l’effacement d’espaces de travail lors de l’exécution de code R dans un contexte de calcul [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]
...
### <a name="arguments-varstokeep-and-varstodrop-not-supported-for-sql-server-data-sources"></a>Arguments *varsToKeep* et *varsToDrop* ne pas pris en charge pour les sources de données SQL Server
...
Cette limitation sera supprimée dans une version ultérieure.
...
Pour plus d'informations sur la conversion implicite de types de données, consultez [Working with R Data Types](r/r-libraries-and-data-types.md).
...
Pour éviter cette erreur, réécrivez comme suit :
...
Dans SQL Server 2016, le `rxExec` fonction fournie par le package RevoScaleR peut être utilisée uniquement en mode de thread unique.
...
### <a name="use-of-r-productivity-environment-not-supported"></a>Utilisation de R Productivity Environment ne pas pris en charge
