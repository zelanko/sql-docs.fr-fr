---
description: Propriétés, méthodes et événements de l’objet Connection
title: Propriétés, méthodes et événements de l’objet Connection | Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: ado
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- Connection object [ADO], members
ms.assetid: f571b74d-b796-4009-9c66-6a36ab995a2a
author: rothja
ms.author: jroth
ms.openlocfilehash: e6650847d3db9d07b7c09d41af7895f604ab105d
ms.sourcegitcommit: 18a98ea6a30d448aa6195e10ea2413be7e837e94
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 08/27/2020
ms.locfileid: "88974890"
---
# <a name="connection-object-properties-methods-and-events"></a>Propriétés, méthodes et événements de l’objet Connection
## <a name="propertiescollections"></a>Propriétés/collections  
 [Attributes, propriété](./attributes-property-ado.md)  
  
 [CommandTimeout, propriété](./commandtimeout-property-ado.md)  
  
 [Propriété ConnectionString](./connectionstring-property-ado.md)  
  
 [ConnectionTimeout, propriété](./connectiontimeout-property-ado.md)  
  
 [CursorLocation, propriété](./cursorlocation-property-ado.md)  
  
 [DefaultDatabase, propriété](./defaultdatabase-property.md)  
  
 [Collection d’erreurs](./errors-collection-ado.md)  
  
 [IsolationLevel, propriété](./isolationlevel-property.md)  
  
 [Mode (propriété)](./mode-property-ado.md)  
  
 [Collection Properties](./properties-collection-ado.md)  
  
 [Propriété Provider](./provider-property-ado.md)  
  
 [Propriété State](./state-property-ado.md)  
  
 [Propriété Version](./version-property-ado.md)  
  
## <a name="methods"></a>Méthodes  
 [BeginTrans, CommitTrans et RollbackTrans, méthodes](./begintrans-committrans-and-rollbacktrans-methods-ado.md)  
  
 [Cancel (méthode)](./cancel-method-ado.md)  
  
 [Close, méthode](./close-method-ado.md)  
  
 [Execute, méthode (objet Connection ADO)](./execute-method-ado-connection.md)  
  
 [Open, méthode (objet Connection ADO)](./open-method-ado-connection.md)  
  
 [OpenSchema, méthode](./openschema-method.md)  
  
## <a name="events"></a>Événements  
 [Événements BeginTransComplete, CommitTransComplete et RollbackTransComplete](./begintranscomplete-committranscomplete-and-rollbacktranscomplete-events-ado.md)  
  
 [Événements ConnectComplete et Disconnect](./connectcomplete-and-disconnect-events-ado.md)  
  
 [ExecuteComplete, événement](./executecomplete-event-ado.md)  
  
 [InfoMessage, événement](./infomessage-event-ado.md)  
  
 [Événement WillConnect](./willconnect-event-ado.md)  
  
 [Événement WillExecute](./willexecute-event-ado.md)  
  
## <a name="see-also"></a>Voir aussi  
 [Connection, objet (ADO)](./connection-object-ado.md)