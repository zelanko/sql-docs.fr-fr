...
-   **Événements de connexion**: événements émis quand une transaction sur une connexion commence, est validée ou est restaurée ; quand un [commande](../../../ado/reference/ado-api/command-object-ado.md) exécute ; lorsqu’un avertissement se produit pendant un **l’événement de connexion**opération ; ou quand un **connexion** commence ou se termine.

-   **Événements de jeu d’enregistrements**: événements sont émis autour des opérations d’extraction asynchrones, ainsi que lorsque vous naviguez dans les lignes d’un **Recordset** d’objet, de modifier un champ dans une ligne d’un **Recordset**, modifier une ligne dans un **Recordset**, ouvrez un **Recordset** avec un curseur côté serveur, vous devez fermer un **Recordset**, ou assurez-vous que le modifiez dans le  **Jeu d’enregistrements**.
