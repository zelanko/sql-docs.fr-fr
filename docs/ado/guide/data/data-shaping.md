---
title: "Mise en forme des données | Documents Microsoft"
ms.prod: sql-non-specified
ms.technology:
- drivers
ms.custom: 
ms.date: 01/19/2017
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- data shaping [ADO]
ms.assetid: 62bd7dc9-45b5-4ca9-8b52-457325e0ce9e
caps.latest.revision: 9
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.workload: Inactive
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: 1ad3b257166308fe2c5a4bd94da5427dc0ca2649
ms.contentlocale: fr-fr
ms.lasthandoff: 09/09/2017

---
# <a name="data-shaping"></a>Mise en forme des données
Cette section contient les rubriques suivantes.  
  
-   [Vue d’ensemble de la mise en forme des données](../../../ado/guide/data/data-shaping-overview.md)  
  
-   [ShapingExample de données](../../../ado/guide/data/data-shaping-example.md)  
  
-   [La mise en forme](../../../ado/guide/data/reshaping.md)  
  
-   [Agrégats petits-enfants](../../../ado/guide/data/grandchild-aggregates.md)  
  
-   [Les commandes paramétrables par l’intermédiaire des commandes COMPUTE](../../../ado/guide/data/parameterized-commands-with-intervening-compute-commands.md)  
  
-   [Persistance des jeux d’enregistrements](../../../ado/guide/data/persisting-hierarchical-recordsets.md)  
  
-   [Fournisseurs requis pour la mise en forme des données](../../../ado/guide/data/required-providers-for-data-shaping.md)  
  
-   [En général, les commandes de forme](../../../ado/guide/data/shape-commands-in-general.md)  
  
-   [Clause APPEND de forme](../../../ado/guide/data/shape-append-clause.md)  
  
-   [Clause COMPUTE de forme](../../../ado/guide/data/shape-compute-clause.md)  
  
-   [Fabrication de jeux d’enregistrements](../../../ado/guide/data/fabricating-hierarchical-recordsets.md)  
  
-   [L’accès aux lignes dans un jeu d’enregistrements hiérarchique](../../../ado/guide/data/accessing-rows-in-a-hierarchical-recordset.md)  
  
-   [Grammaire de mise en forme formelle](../../../ado/guide/data/formal-shape-grammar.md)  
  
-   [Visual Basic pour Applications fonctions](../../../ado/guide/data/visual-basic-for-applications-functions.md)

