---
title: "Configurer le conteneur de SQL Server dans Kubernetes pour la haute disponibilité | Documents Microsoft"
...
---
# <a name="configure-sql-server-container-in-kubernetes-for-high-availability"></a>Configurer le conteneur de SQL Server dans Kubernetes pour la haute disponibilité

[!INCLUDE[tsql-appliesto-sslinux-only](../includes/tsql-appliesto-sslinux-only.md)]

Suivez cet article pour configurer une instance de SQL Server sur Kubernetes dans le Service de conteneur Azure (AKS) avec le stockage persistant pour la haute disponibilité. La solution fournit la résilience. Si l’instance de SQL Server échoue, Kubernetes recrée automatiquement dans un nouveau module. AKS fournit la résilience contre une défaillance de nœud Kubernetes. 

Ce didacticiel montre comment configurer une instance de SQL Server à haute disponibilité dans des conteneurs à l’aide de AKS. 

> [!div class="checklist"]
> * Mot de passe SA de création
> * Création de stockage
> * Créer le déploiement
> * Se connecter à SQL Server Management Studio (SSMS)
> * Vérifiez que la défaillance et récupération

### <a name="ha-solution-using-kubernetes-running-in-azure-container-service"></a>Solution de haute disponibilité à l’aide de Kubernetes en cours d’exécution dans le Service de conteneur Azure

Kubernetes 1.6 + prend en charge [Classes de stockage](http://kubernetes.io/docs/concepts/storage/storage-classes/), [persistant Volume revendications](http://kubernetes.io/docs/concepts/storage/storage-classes/#persistentvolumeclaims)et le [pilote de volume de disque Azure](http://github.com/Azure/azurefile-dockervolumedriver). Vous pouvez créer et gérer vos instances de SQL Server en mode natif dans Kubernetes. L’exemple dans cet article montre comment créer un [déploiement](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/) pour obtenir une configuration haute disponibilité similaire à l’instance de cluster de basculement de disque partagé. Dans cette configuration, Kubernetes joue le rôle de l’orchestrateur du cluster. En cas d’échec d’une instance de SQL Server dans un conteneur, l’orchestrateur amorce une autre instance du conteneur qui s’attache à un même stockage persistant.

![Cluster Kubernetes SQL Server](media/tutorial-sql-server-containers-kubernetes/kubernetes-sql.png)

Dans le diagramme précédent, `mssql-server` est un conteneur dans un [pod](http://kubernetes.io/docs/concepts/workloads/pods/pod/). Kubernetes orchestre les ressources du cluster. A [jeu de réplicas](http://kubernetes.io/docs/concepts/workloads/controllers/replicaset/) garantit que le bloc est automatiquement récupérée après une défaillance de nœud. Les applications se connecter au service. Dans ce cas, le service représente un équilibreur de charge qui héberge une adresse IP reste identique après échec de la `mssql-server`.

Dans le diagramme suivant, la `mssql-server` conteneur a échoué. Comme l’orchestrateur, Kubernetes garantit le nombre correct d’instances saines dans le réplica défini et crée un conteneur en fonction de la configuration. L’orchestrateur démarre un nouveau module sur le même nœud, et `mssql-server` se reconnecte au même stockage persistant. Le service se connecte à la recréée `mssql-server`.

![Cluster de serveurs SQL de Kubernetes après](media/tutorial-sql-server-containers-kubernetes/kubernetes-sql-after-pod-fail.png)

Dans le diagramme suivant, le nœud qui héberge le `mssql-server` conteneur a échoué. L’orchestrateur démarre le nouveau module sur un autre nœud, et `mssql-server` se reconnecte au même stockage persistant. Le service se connecte à la recréée `mssql-server`.

![Cluster de serveurs SQL de Kubernetes après](media/tutorial-sql-server-containers-kubernetes/kubernetes-sql-after-node-fail.png)
...
* **Kubernetes cluster**
   ...
   - Vous pouvez suivre les instructions à [déployer un cluster du Service de conteneur Azure (AKS)](http://docs.microsoft.com/en-us/azure/aks/tutorial-kubernetes-deploy-cluster) pour créer et vous connecter à un cluster de Kubernetes nœud unique dans AKS avec `kubectl`. 
...
## <a name="create-sa-password"></a>Mot de passe SA de création
...
1. Créer un manifeste pour définir la classe de stockage et le volume persistant revendication.  Le manifeste spécifie le de fournisseur de stockage, paramètres et le [récupérer la stratégie](http://kubernetes.io/docs/concepts/storage/persistent-volumes/#reclaiming). Le cluster Kubernetes utilise ce manifeste pour créer le stockage persistant. 
...
1. Créez la revendication volume persistant dans Kubernetes.
   ...
   * `<Path to pvc.yaml file>`
   ...
   Le volume persistant est automatiquement créé en tant que compte de stockage Azure et lié à la revendication de volume persistant. 
    ...
    ![Commande de revendication de volume persistant](media/tutorial-sql-server-containers-kubernetes/02_pvc_cmd.png)
...
1. Vérifiez que la revendication volume persistant.
   ...
   * `<PersistentVolumeClaim>`
      * Le nom de la revendication volume persistant.
    ...
    ![Décrire le volume](media/tutorial-sql-server-containers-kubernetes/describe-volume.png)

    La valeur pour le volume, la partie des correspondances du nom de l’objet blob dans l’image suivante à partir du portail Azure : 

    ![Décrire le portail de volume](media/tutorial-sql-server-containers-kubernetes/describe-volume-portal.png)
...
1. Créer un manifeste - yaml - pour décrire le déploiement. L’exemple suivant décrit un déploiement, y compris un conteneur en fonction de l’image de conteneur de SQL Server.
   ...
   * `value: "Developer"`
     * Définit le conteneur pour exécuter SQL Server Developer edition. Édition développeur n’est pas autorisée à des données de production. Si le déploiement est pour la production, définir l’édition appropriée. Peut être une des `Enterprise`, `Standard`, ou `Express`. 

      >[!NOTE]
      >Pour plus d’informations, consultez [la licence SQL Server](http://www.microsoft.com/sql-server/sql-server-2017-pricing).

   * `persistentVolumeClaim`
     * Cette valeur requiert une entrée pour `claimName:` qui mappe au nom utilisé pour la revendication volume persistant. Cet article utilise `mssql-data`. 

   * `name: SA_PASSWORD`
      * Configure l’image de conteneur pour définir le mot de passe SA tel que défini dans cette section.
...
1. Créer le déploiement.
   ...
   * `<Path to sqldeployment.yaml file>`
   ...
   ![Commande de déploiement](media/tutorial-sql-server-containers-kubernetes/04_deploy_cmd.png)

   Le déploiement et le service sont créés. L’instance de SQL Server est dans un conteneur - connecté vers le stockage persistant.

   Pour afficher l’état de la pod, tapez `kubectl get pod`.

   ![Obtenir les commandes de pod](media/tutorial-sql-server-containers-kubernetes/05_get_pod_cmd.png)

   >[!NOTE]
   >Une fois le déploiement est créé, il peut prendre quelques minutes avant que le bloc est visible. Le délai d’attente est parce que le cluster doit extraire le [mssql-server-linux](https://hub.docker.com/r/microsoft/mssql-server-linux/) image à partir du hub Docker. Une fois qu’il est tiré de la première fois, les déploiements suivants peuvent être plus rapides - si le déploiement est à un nœud qui possède déjà l’image mise en cache sur ce dernier. 
...
1. Vérifiez que les services sont en cours d’exécution. Exécutez la commande suivante :
   ...
   ![Obtenir la commande de service](media/tutorial-sql-server-containers-kubernetes/06_get_service_cmd.png)
...
Si vous avez configuré le conteneur comme décrit, vous pouvez vous connecter à une application en dehors du réseau virtuel Azure. Utilisez le `sa` compte et l’adresse IP externe d’adresses pour le service. Utilisez le mot de passe que vous avez configuré en tant que le secret Kubernetes. 
...
* Pour vous connecter avec SQLCMD `sqlcmd`, exécutez la commande suivante :
...
Pour vérifier que la défaillance et récupération, vous pouvez supprimer le bloc. Procédez comme suit :
...
Kubernetes recrée automatiquement du bloc pour récupérer une instance de SQL Server et de connecter le stockage permanent. Utilisez `kubectl get pods` pour vérifier qu’un nouveau module est déployé. Utilisez `kubectl get services` pour vérifier que l’adresse IP pour le nouveau conteneur est le même. 
...
>[Introduction - Kubernetes](http://docs.microsoft.com/en-us/azure/aks/intro-kubernetes)
