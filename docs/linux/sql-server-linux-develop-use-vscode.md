...
1. Dans la **palette de commandes**, sélectionnez extensions   **: Installez les extensions** à partir de la liste déroulante. 

1. Dans le volet **Extensions**, tapez *mssql*.

1. Sélectionnez l'extension **SQL Server (mssql)** , puis sélectionnez **Installer**.
...
1. Sélectionnez **Texte brut** dans la barre d’état inférieure, ou appuyez sur **Ctrl**+**K** > **M** et sélectionnez **SQL** à partir du menu déroulant de langages. 
...
1. Tapez *sql* pour afficher les commandes mssql ou tapez sur *sqlcon*, puis sélectionnez **MS SQL : Connect** à partir de la liste déroulante.
...
1. Sélectionnez la commande **MS SQL : gérer les profils de connexion**.

1. Sélectionnez ensuite **Créer** afin de créer un profil de connexion pour votre instance SQL Server.

1. Suivez les invites afin de spécifier les propriétés du nouveau profil de connexion. Après avoir spécifié chaque valeur, appuyez sur **Entrée** pour continuer.
...
1. Appuyez sur **Ctrl**+**MAJ**+**E** pour exécuter les commandes Transact-SQL. Affichez les résultats dans la fenêtre de requête.

   ![Créer des messages de base de données](./media/sql-server-linux-develop-use-vscode/vscode-create-database-messages.png)
...
1. Tapez *sql* pour afficher les commandes mssql ou tapez sur *sqluse*, puis sélectionnez la commande **MS SQL : Utiliser la base de données**.

1. Sélectionnez la nouvelle base de données **TutorialDB**. 

   ![Utiliser une base de données](./media/sql-server-linux-develop-use-vscode/vscode-use-database.png)
...
1. Dans l’extrait de code, tapez `Employees` pour le nom de la table.

1. Appuyez sur **Onglet** pour accéder au champ suivant, puis tapez `dbo` pour le nom du schéma.

1. Remplacez les définitions de colonne par les colonnes suivantes :
...
1. Appuyez sur **Ctrl**+**MAJ**+**E** pour exécuter les commandes. Les deux jeux de résultats s’affichent dans la fenêtre **Résultats**. 
...
1. Sélectionnez les en-têtes **Résultats** et **Messages** du panneau pour réduire et développer les panneaux.
...
1. Sélectionnez l’icône d’agrandissement de la grille sur la deuxième grille de résultats pour effectuer un zoom avant sur ces résultats.

   ![Agrandir la grille](./media/sql-server-linux-develop-use-vscode/vscode-maximize-grid.png)

   > [!NOTE]
   > L’icône d’agrandissement s’affiche lorsque votre script T-SQL produit au moins deux grilles de résultats.

1. Ouvrez le menu contextuel de la grille en cliquant avec le bouton droit sur la grille. 

   ![Menu contextuel](./media/sql-server-linux-develop-use-vscode/vscode-grid-context-menu.png)

1. Sélectionnez **Sélectionner tout**.

1. Rouvrez le menu contextuel de la grille et sélectionnez **Enregistrer au format JSON** pour enregistrer le résultat dans un fichier *.json*.

1. Spécifiez un nom de fichier pour le fichier JSON. 

1. Vérifiez que le fichier JSON est enregistré et s’ouvre dans Visual Studio Code.

   ![Enregistrer au format JSON](./media/sql-server-linux-develop-use-vscode/vscode-save-as-json.png)
