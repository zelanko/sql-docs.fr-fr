...
Si vous rencontrez des difficultés pour vous connecter à votre serveur SQL sous Linux, il existe quelques éléments à vérifier.  
...
- Vérifiez que le nom du serveur ou l’adresse IP est accessible à partir de votre ordinateur client.
   ...
   >   ```
   > Une exception à cette technique est relative aux machines virtuelles Azure. Pour les machines virtuelles Azure, [rechercher l’adresse IP publique de l’ordinateur virtuel dans le portail Azure](https://docs.microsoft.com/azure/virtual-machines/linux/sql/provision-sql-server-linux-virtual-machine#connect).
...
- Vérifiez que le nom d’utilisateur et le mot de passe ne contiennent pas de fautes de frappe ou espaces ni d’une casse incorrecte.
...
Le programme d’installation enregistre ici : / var/opt/mssql/setup-< horodatage qui représente la date d’installation > vous pouvez parcourir les journaux d'erreurs avec n’importe quel outil compatible UTF-16 comme « vim » ou « cat » comme suit : 
...
Les événements étendus peuvent être interrogées via une commande SQL.  Plus d’information sur les événements étendus est accessible [ici](https://technet.microsoft.com/en-us/library/bb630282.aspx):

## <a name="crash-dumps"></a>Vidages sur incident 

Recherchez les vidages dans le répertoire de journal dans Linux. Vérifiez dans le répertoire /var/opt/mssql/log pour les Linux Core dumps (. tar.gz2 extension) ou SQL minidumps (extension .mdmp)
...
> [!WARNING]  
>  Démarrez SQL Server sous Linux avec l’utilisateur « mssql » afin d’éviter les problèmes de démarrage futurs. Exemple « sudo -u mssql /opt/mssql/bin/sqlservr [OPTIONS de démarrage] » 
...
2. Erreur : Le nom d’hôte doit avoir 15 caractères ou moins.
...
4. Utiliser des caractères spéciaux dans le mot de passe.
   ...
   Si vous utilisez des caractères spéciaux dans le mot de passe du compte de connexion SQL Server, vous devrez peut-être utiliser un caractère d’échappement lors de leur utilisation avec le terminal Linux. Vous devez échapper le caractère $ à chaque fois à l’aide de la barre oblique inverse (antislash) lorsque vous l'utilisez dans un script de shell/terminal :
