...
- Un cluster ACS Kubernetes avec la dernière version
...
>
> Utiliser la dernière version de Kubernetes. La version spécifique dépend de votre abonnement et région. Consultez [versions prises en charge de Kubernetes dans AKS](https://docs.microsoft.com/en-us/azure/aks/supported-kubernetes-versions).  
>
> Le script suivant crée un cluster à quatre nœuds Kubernetes dans Azure. Avant d’exécuter l’opération de remplacement script `<latest version>` avec la dernière version disponible. Par exemple, `1.12.5`.
>
> ```azure-cli
