...
1. Créer un secret Kubernetes avec mots de passe pour le `sa` compte et la clé principale d’instance SQL Server.
...
- La sortie de `kubectl get services` pour l’espace de noms `ag1`.
 ...
 Les services, les services d’équilibrage de charge qui sont créés pour chaque conteneur de SQL Server. Utilisez ces adresses IP en tant que points de terminaison pour se connecter directement aux instances de SQL Server dans le cluster.
...
Vous pouvez afficher l’état du groupe de disponibilité à partir du tableau de bord de groupe de disponibilité de SQL Server Management Studio.
