...
> [!div class="checklist"]
> * Extrait et exécuter la dernière image de conteneur de SQL Server 2017 Linux.
> * Copiez le fichier de base de données wideworldimporters dans le conteneur.
...
> * Sauvegarde la base de données modifiée.
...
> [!IMPORTANT]
> La valeur par défaut sur Docker pour Mac et Docker pour Windows est de 2 Go pour la VM Moby, donc vous devez le modifier à 4 Go. Si vous exécutez sur Mac ou Windows, augmentez vos paramètres de mémoire à l’aide de la [des instructions dans le démarrage rapide de Docker](quickstart-install-connect-docker.md).
...
Ce didacticiel utilise le [base de données exemple Wide World Importers](../sample/world-wide-importers/wide-world-importers-documentation.md). Utilisez les étapes suivantes pour télécharger et copier le fichier de sauvegarde de base de données de Wide World Importers dans votre conteneur de SQL Server.
...
1. Ensuite, téléchargez le [WideWorldImporters-full.bak](https://github.com/Microsoft/sql-server-samples/releases/tag/wide-world-importers-v1.0) fichier sur votre ordinateur hôte. Les commandes suivantes accédez au répertoire d’accueil/utilisateur et télécharge le fichier de sauvegarde en tant que **wwi.bak**.
...
1. Utilisez **docker cp** pour copier le fichier de sauvegarde dans le conteneur dans le **/var/opt/mssql/backup** active.
...
Le fichier de sauvegarde se trouve désormais dans le conteneur. Avant de restaurer la sauvegarde, il est important de connaître les noms de fichiers logiques et les types de fichiers à l’intérieur de la sauvegarde. Les commandes Transact-SQL suivantes inspecter la sauvegarde et effectuer la restauration à l’aide de **sqlcmd** dans le conteneur.
...
1. Exécutez **sqlcmd** dans le conteneur à la liste des noms de fichiers logiques et les chemins d’accès à l’intérieur de la sauvegarde. Cette opération s’effectue avec la **RESTORE FILELISTONLY** instruction Transact-SQL.
...
1. Exécuter une requête pour afficher les premiers 10 éléments dans le **Warehouse.StockItems** table.
...
1. Mettre à jour la description du premier élément par le code suivant **mise à jour** instruction :
...
## <a name="create-a-new-backup"></a>Créez une sauvegarde
...
1. Utilisez le **sauvegarde de base de données** commande Transact-SQL pour créer une sauvegarde de base de données dans le conteneur. Ce didacticiel crée un nouveau fichier de sauvegarde, **wwi_2.bak**, dans précédemment créé **/var/opt/mssql/backup** active.
...
En plus des sauvegardes de base de données pour protéger vos données, vous pouvez également utiliser des conteneurs de volumes de données. Début de ce didacticiel créé le **sql1** contenant le `-v sql1data:/var/opt/mssql` paramètre. Le **sql1data** conteneur de volume de données persiste le **/var/opt/mssql** données même après le conteneur est supprimé. Les étapes suivantes suppriment complètement la **sql1** conteneur, puis créer un nouveau conteneur, **sql2**, avec les données persistantes.

1. Arrêter le **sql1** conteneur.
...
1. Créer un nouveau conteneur, **sql2**et de réutiliser le **sql1data** conteneur de volumes de données.
...
1. La base de données de Wide World Importers est présent dans le nouveau conteneur. Exécuter une requête pour vérifier la modification précédente que vous avez apportées.
   ...
   > [!NOTE]
   > Le mot de passe SA n’est pas le mot de passe que vous avez spécifié pour le **sql2** conteneur, `MSSQL_SA_PASSWORD=<YourStrong!Passw0rd>`. Toutes les données de SQL Server a été restauré à partir de **sql1**, y compris le mot de passe modifié à partir de précédemment dans le didacticiel. En effet, certaines options ainsi sont ignorées en raison de la restauration des données dans /var/opt/mssql. Pour cette raison, le mot de passe est `<YourNewStrong!Passw0rd>` comme indiqué ici.
...
> * Utilisez des conteneurs de volumes de données dans Docker pour conserver les données de SQL Server.
