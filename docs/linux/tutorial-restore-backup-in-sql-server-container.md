...
## <a name="verify-the-restored-database"></a>Vérifiez que la base de données est restaurée
...
En plus d'effectuer des sauvegardes de bases de données pour protéger vos données, vous pouvez également utiliser des conteneurs de volumes de données. Le début de ce didacticiel créé le container SQL Server **sql1** avec le paramètre `-v sql1data:/var/opt/mssql`. Le conteneur de volume de données **sql1data** conserve les données de **/var/opt/mssql** même après la suppression du conteneur **sql1**. Les étapes suivantes suppriment complètement le conteneur **sql1**, puis créent un nouveau conteneur, **sql2**, utilisant les mêmes données persistantes.
