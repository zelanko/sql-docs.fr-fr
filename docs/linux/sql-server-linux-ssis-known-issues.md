...
Cet article décrit les limitations et problèmes connus pour SQL Server Integration Services (SSIS) sur Linux.
...
Les composants Integration Services suivants sont pris en charge sur Linux. Certaines d'entre elles ont des limitations sur la plate-forme Linux. Les composants de SSIS qui ne sont pas répertoriés ici ne sont pas pris en charge sous Linux.
...
Les tâches de plan de maintenance suivantes ne sont pas pris en charge sous Linux :
- Notifier l’opérateur
- Exécuter le travail de l’Agent SQL Server

Les tâches de plan de maintenance suivantes sont prises en charge sous Linux :
- Vérifier l’intégrité de base de données
