...
4.  Exécutez le processus de nettoyage assisté par ordinateur en cliquant sur **Démarrer** sur la **Nettoyer** page.  
...
-   **Suggéré**: valeurs pour lesquelles DQS a identifié des suggestions qui ont un niveau de confiance est supérieur à la *seuil de suggestion automatique* mais inférieur à la valeur du *seuil de correction automatique* valeur. Vous devez examiner ces valeurs et les approuver ou les refuser, selon le cas.  
  
-   **Nouvelle**: les valeurs valides pour lesquelles DQS ne dispose pas de suffisamment d’informations (suggestion) et par conséquent ne peut pas être mappé à n’importe quel autre onglet. En outre, cet onglet contient également les valeurs dont le niveau de confiance inférieur à celui *seuil de suggestion automatique* valeur, mais suffisamment élevé pour être considéré comme valide.  
...
> [!NOTE]  
>  Dans la **suggérés**, **corrigés**, et **Correct** onglets, DQS affiche la valeur de début pour un domaine, le cas échéant, dans le **Correct à** colonne par rapport à la valeur de domaine respectif.  
...
|Explique comment nettoyer les données à l'aide des connaissances intégrées à DQS.|[Nettoyer les données à l’aide de DQS & #40 ; interne & #41 ; Base de connaissances](../data-quality-services/cleanse-data-using-dqs-internal-knowledge.md)|  
|Explique comment nettoyer les données à l'aide des connaissances du service de données de référence.|[Nettoyer les données à l’aide des données de référence & #40 ; #41 ; & externes Base de connaissances](../data-quality-services/cleanse-data-using-reference-data-external-knowledge.md)|  
