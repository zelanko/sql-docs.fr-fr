...
##  <a name="Answer"></a> Répondent à ce besoin avec DQS  
...
-   **Nettoyage des données :** la modification, suppression ou enrichissement des données incorrectes ou incomplètes, à l’aide de processus assisté par ordinateur et interactifs. Pour plus d'informations, voir [Data Cleansing](../data-quality-services/data-cleansing.md).  
  
-   **Mise en correspondance :** l’identification des doublons sémantiques dans un processus basé sur des règles qui vous permet de déterminer ce qui constitue une correspondance et de procéder à la déduplication. Pour plus d'informations, voir [Data Matching](../data-quality-services/data-matching.md).  
  
-   **Reference Data Services :** vérification de la qualité des données à l'aide des services d'un fournisseur de données de référence. Vous pouvez utiliser les services de données de référence de [Microsoft Azure Marketplace](http://azure.microsoft.com/marketplace/) pour nettoyer, valider, correspondent et enrichir les données. Pour plus d’informations, consultez [Reference Data Services in DQS](../data-quality-services/reference-data-services-in-dqs.md).  
...
-   **Base de connaissances :** Data Quality Services est une solution intelligente qui analyse les données en fonction des connaissances que vous générez avec DQS. Ainsi, vous pouvez créer des processus de qualité des données qui améliorent continuellement les connaissances, et par conséquent, la qualité des données.  
...
-   **Projet de qualité de données** qui propose des modifications à apporter aux données sources en fonction des connaissances figurant dans la base de connaissances.  
 ...
 Pour plus d’informations, consultez [Bases de connaissances DQS et les domaines](../data-quality-services/dqs-knowledge-bases-and-domains.md) et [projets de qualité des données & #40 ; DQS & #41 ;](../data-quality-services/data-quality-projects-dqs.md).  
