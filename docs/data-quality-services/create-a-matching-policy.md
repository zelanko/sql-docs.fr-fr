...
---
# Cr&#233;er une strat&#233;gie de correspondance
...
3.  Cliquez sur **Ouvrir la base de connaissances** pour créer ou modifier la stratégie de correspondance dans une base de connaissances existante. Sélectionnez la base de connaissances, sélectionnez **Stratégie de correspondance**, puis cliquez sur **Suivant**. Vous pouvez également cliquer sur une base de connaissances sous **Base de connaissances récente**. Si vous ouvrez une base de connaissances a été fermée alors qu’une stratégie de correspondance était en cours de traitement, vous allez passer à l’étape de l’activité de stratégie correspondante a été fermée (comme indiqué par la **état** la colonne de la base de connaissances dans la table de base de connaissances ou dans le nom de la base de connaissances sous **Base de connaissances récente**). Si vous ouvrez une base de connaissances qui inclut une stratégie de correspondance et qui a été terminée, vous accédez à la page **Stratégie de correspondance** . Si vous ouvrez une base de connaissances qui n'inclut pas de stratégie de correspondance et qui a été terminée, vous accédez à la page **Mappage** .  
...
1.  Sur le **carte** page pour créer une stratégie pour une base de données, laissez **Source de données** comme **SQL Server**, sélectionnez la base de données que vous souhaitez créer la stratégie pour en **base de données**, puis sélectionnez la table ou afficher dans **Table/vue**. La base de données source doit être présente dans la même instance de SQL Server que [!INCLUDE[ssDQSServer](../includes/ssdqsserver-md.md)]. Sinon, elle ne s'affichera pas dans la liste déroulante.  
...
5.  Cliquez sur le **plus (+)** contrôle pour ajouter une ligne à la table mappages ou **moins (-)** contrôle à supprimer une ligne.  
...
7.  Cliquez sur **Afficher/sélectionner des domaines composites** pour afficher une liste des domaines composites disponibles dans la base de connaissances et sélectionner en fonction de mappage.  
...
3.  Augmentez la valeur de la **min. de score de correspondance** Si vous souhaitez rendre les exigences de correspondance plus strictes. Pour plus d’informations sur le score de correspondance minimal, consultez la page [comment définir les paramètres de règle de correspondance](#MatchingRules).  
...
13. Vous pouvez utiliser les données de la table **Résultats de correspondance** comme suit :  
    ...
    -   Dans la **% zone**, sélectionnez un pourcentage dans la liste déroulante, par incréments de « 5 ». Toutes les lignes ayant un score de correspondance supérieur ou égal à ce pourcentage seront affichées dans la table des résultats de correspondance.  
  
    -   Si vous double-cliquez sur un enregistrement dans la table des résultats de la correspondance, DQS affiche un **Détails du Score de correspondance** fenêtre contextuelle qui affiche l’enregistrement pivot et enregistrement de la source (et les valeurs de tous leurs champs), le score entre eux et un zoom de l’enregistrement correspondant. L'exploration affiche les valeurs de chaque champ de l'enregistrement pivot et de l'enregistrement source pour vous permettre de les comparer, et indique le score de correspondance auquel chaque champ contribue dans le score de correspondance global pour les deux enregistrements.  
...
1.  Sur le **résultats de correspondance** page, sélectionnez **chevauchement des clusters** dans la liste déroulante pour afficher les enregistrements pivots et les enregistrements suivants pour tous les clusters lorsque la correspondance est exécutée, même si les groupes de clusters ont des enregistrements en commun. Sélectionnez **Clusters qui ne se chevauchent pas** pour afficher les clusters qui ont des enregistrements en commun comme un seul cluster lorsque la correspondance est exécutée.  
  
2.  Cliquez sur **Recharger les données à partir de la source** pour copier des données à partir de la source de données dans la table intermédiaire et les réindexer lorsque vous exécutez la stratégie de correspondance. Cliquez sur **Execute sur les données précédentes** pour exécuter une stratégie de correspondance sans copier les données dans la table intermédiaire et les réindexer. L'option**Exécuter sur les données précédentes** est désactivée pour la première exécution de la stratégie de correspondance, ou si vous modifiez le mappage dans la page **Mapper** et que vous appuyez sur **Oui** dans la fenêtre suivante. Dans ces deux cas, vous devez effectuer une réindexation. Il n'est pas nécessaire d'effectuer une réindexation si la stratégie de correspondance n'a pas changé. L'exécution sur les données précédentes peut améliorer les performances.  
  
3.  Cliquez sur **Démarrer** pour exécuter le processus de correspondance pour toutes les règles que vous avez définies. Le **résultats de correspondance** table affiche l’ID d’enregistrement, numéro de cluster et des colonnes de données (y compris ceux pas dans la règle de correspondance) pour chaque enregistrement dans un cluster. L'enregistrement de début du cluster est sélectionné de manière aléatoire. (Vous déterminez l’enregistrement survivant sélectionné par la règle de SURVIVANCE sur le **Exporter** page lorsque vous exécutez le projet correspondant.) Chaque ligne supplémentaire d'un cluster est considérée comme un doublon ; son score de correspondance (comparé à l'enregistrement pivot) est fourni dans la table des résultats.  
...
8.  Cliquez sur **Fermer** pour enregistrer votre travail et revenir à la page d'accueil de DQS. L'état de la base de connaissances affiche la chaîne « Stratégie de correspondance – », ainsi que l'état actuel. Si vous avez cliqué sur **Fermer** pendant que vous êtes dans le **résultats de correspondance** écran, l’état affiche : « Correspondance de stratégie - résultats ». Si vous cliquez sur Fermer lorsque vous êtes dans le **stratégie de correspondance** écran, l’état affiche : « Correspondance stratégie - stratégie de correspondance ». Après avoir cliqué sur **Fermer**, pour effectuer l'activité **Découverte des connaissances** , revenez à l'activité **Stratégie de correspondance** , cliquez sur **Terminer**, puis sur **Oui** pour publier la base de connaissances ou sur **Non** pour enregistrer le travail dans la base de connaissances et quitter.  
