...
---
# Ex&#233;cuter un projet de correspondance
...
3.  Cliquez sur **Ouvrir le projet de qualité des données** pour effectuer une correspondance dans un projet de qualité des données existant. Sélectionnez le projet, puis cliquez sur **Suivant**. (Vous pouvez également cliquer sur un projet sous **Projet de qualité des données récentes**.) Si vous ouvrez un projet de correspondance qui a été fermé, vous allez passer à l’étape de l’activité de projet correspondant a été fermée (comme indiqué par la **état** colonne dans la table du projet ou du nom de projet sous **projet de qualité des données récentes**). Si vous ouvrez un projet de correspondance s’est terminé, vous accédez à la **Exporter** page (et vous ne pouvez plus revenir aux écrans précédents).  
...
4.  Cliquez sur le **plus (+)** contrôle pour ajouter une ligne à la table mappages ou **moins (-)** contrôle à supprimer une ligne.  
...
6.  Cliquez sur **Afficher/sélectionner des domaines composites** pour afficher une liste des domaines composites disponibles dans la base de connaissances et sélectionner en fonction de mappage.  
...
7.  Si vous sélectionnez un enregistrement non-pivot dans le tableau, cliquez sur le **Afficher les détails** icône (ou double-cliquez sur l’enregistrement), DQS affiche un **Détails du Score de correspondance** fenêtre contextuelle qui affiche l’enregistrement double-cliqué et son pivot (et les valeurs de tous leurs champs), le score entre eux et un zoom de la mise en correspondance évaluer les contributions de chaque champ. Un double-clic sur un enregistrement pivot n'affiche pas le message.  
...
10. Pour modifier la valeur minimale de correspondance minimal qui détermine le degré de correspondance ayant un enregistrement doit être affichée, sélectionnez le **Min. Score de correspondance** icône au-dessus de la partie droite de la table, puis entrez un nombre plus élevé. Le score de correspondance minimal a la valeur 80 % par défaut. Cliquez sur **Actualiser** pour modifier le contenu de la table.  
...
3.  Activez la case à cocher **résultats de correspondance** pour exporter les résultats correspondants (voir ci-dessus pour obtenir une explication) vers la table désignée dans une base de données SQL Server ou au fichier Excel ou .csv désigné. Activez la case à cocher pour **les résultats de SURVIVANCE** pour exporter les résultats de SURVIVANCE (voir ci-dessus pour obtenir une explication) vers la table désignée dans une base de données SQL Server ou au fichier Excel ou .csv désigné.  
...
7.  Sélectionnez la règle de survivance comme suit :  
    ...
    -   Sélectionnez **enregistrement de tableau croisé dynamique** (la valeur par défaut) pour identifier l’enregistrement survivant en tant qu’enregistrement pivot initial choisi arbitrairement par DQS.  
...
### Notifications de stratégie de correspondance  
...
### Onglet Règles de correspondance  
...
### Onglet Résultats de correspondance  
