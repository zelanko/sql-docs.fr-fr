...
7.  Dans la **Sélectionnez fichier** boîte de dialogue, accédez au dossier qui contient le fichier Excel que vous souhaitez importer les valeurs de domaine, sélectionnez le fichier (avec une extension .xlsx, .xls ou .csv), puis cliquez sur **Ouvrir**. Le fichier doit être sur le client à partir duquel vous exécutez DQS ou sur un partage de fichiers auquel l'utilisateur a accès.  
  
8.  À partir de la **feuille de calcul** liste déroulante, sélectionnez la feuille de calcul que vous importez depuis.  
...
-   Les valeurs de la première colonne sont importées dans le domaine. Si en plus de la première colonne, une ou plusieurs colonnes supplémentaires sont remplies, les valeurs de ces colonnes seront ajoutés en tant que synonymes (consultez [importation des synonymes](#Synonyms)).  
