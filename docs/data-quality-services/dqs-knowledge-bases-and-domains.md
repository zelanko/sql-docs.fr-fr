...
### Non-respect de la casse dans DQS  
 ...
 Toutefois, vous pouvez contrôler la casse des valeurs que vous exportez dans les résultats de nettoyage. Faire en définissant le **Format de sortie à** propriété de domaine (voir [définir les propriétés de domaine](../data-quality-services/set-domain-properties.md)) et en utilisant le **normaliser la sortie** case à cocher lorsque vous exportez les résultats du nettoyage (consultez [Nettoyer Data à l’aide de DQS & #40 ; interne & #41 ; Base de connaissances](../data-quality-services/cleanse-data-using-dqs-internal-knowledge.md)).  
...
-   Les données des champs composites peuvent être jointes à une source de données de référence, auquel cas le domaine composite sera envoyé au fournisseur de données de référence. Cela se produit souvent avec les données d'adresse.  
 ...
 La manière dont les données représentées par un domaine composite sont analysées est déterminée par les propriétés du domaine composite. Les données peuvent être analysées par un délimiteur, ordre des domaines, ou en fonction de la base de connaissances dans les domaines joints au domaine composite (en sélectionnant la **analyse de base de connaissances utilisation** propriété du domaine composite). Pour plus d’informations, consultez [Set Composite Domain Properties](../data-quality-services/create-a-composite-domain.md#CompositeDomainProperties).  
