...
|--------------|-----------------|-------------|  
|**Data Source** ou **DataSource**|Spécifie l'instance du serveur. Cette propriété est obligatoire pour toutes les connexions. Les valeurs valides incluent le nom réseau ou l'adresse IP du serveur, local ou localhost pour les connexions locales, une adresse URL si le serveur est configuré pour l'accès HTTP ou HTTPS, ou le nom d'un fichier de cube local (.cub). <br /><br /> Valeur valide pour Azure Analysis Services, `<protocol>://<region>/<servername>` où protocole est asazure de chaîne, la région est l’Uri où le serveur a été créé (par exemple, westus.asazure.windows.net) et servername est le nom de votre serveur unique au sein de la région. |`Data source=asazure://westus.asazure.windows.net/myasserver`<br /><br />`Data source=AW-SRV01` pour l'instance et le port par défaut (TCP 2383).<br /><br /> `Data source=AW-SRV01$Finance:8081` pour une instance nommée ($Finance) et un port fixe.<br /><br /> `Data source=AW-SRV01.corp.Adventure-Works.com` pour un nom de domaine complet, avec le port et l'instance par défaut.<br /><br /> `Data source=172.16.254.1` pour une adresse IP du serveur, en ignorant la recherche de serveur DNS, utile pour résoudre des problèmes de connexion.|  
...
|**DbpropMsmdRequestMemoryLimit**|Cette propriété remplace la [Memory\QueryMemoryLimit](../server-properties/memory-properties.md) valeur de propriété de serveur pour une connexion. Spécifiée en kilo-octets. |
