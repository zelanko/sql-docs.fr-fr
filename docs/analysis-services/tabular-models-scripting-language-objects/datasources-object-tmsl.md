---
title: Sources de données, objet (TMSL) | Microsoft Docs
ms.date: 05/07/2018
ms.prod: sql
...
[!INCLUDE[ssas-appliesto-sqlas-aas](../../includes/ssas-appliesto-sqlas-aas.md)]
 ...
 À moins que vous créez, en remplaçant, ou la modification de l’objet de source de données elle-même, n’importe quelle source de données référencé dans votre script (comme dans le script de la partition) doit être un existant **DataSource** objet dans votre modèle.  
  
## <a name="object-definition"></a>Définition de l'objet  
 Tous les objets ont un ensemble commun de propriétés, notamment le nom, type, description, une collection de propriétés et annotations. **Source de données** objets possèdent également les propriétés suivantes.  
...
-   ImpersonateAnonymous (3) - le serveur utilise le compte d’utilisateur anonyme.  Cette option n’est pas recommandée, mais il est parfois utilisée dans les scénarios d’accès HTTP par des applications personnalisées qui gèrent l’authentification.  
  
-   ImpersonateCurrentUser (4) - le serveur utilise le compte d’utilisateur que le client se connecte en tant que.  
  
-   ImpersonateServiceAccount (5) - le serveur utilise le compte d’utilisateur qui exécute en tant que le serveur.  
...
-   ImpersonateUnattendedAccount (6) : le serveur utilise un compte d’utilisateur sans assistance. Il est utilisé pour les modèles Power Pivot ou tabulaire qui s’exécutent dans un environnement SharePoint.  
 ...
 Le mode DirectQuery peut utiliser impersonateCurrentuser si Analysis Services est configuré pour une délégation approuvée, ou  
 ...
 account  
 Utilisé pour l’emprunt d’identité. Un compte Windows ou de la base de données qui a une connexion valide avec des autorisations de lecture sur la base de données externe.  
 ...
 isolation  
 Le type d’isolation qui est utilisé lors de l’exécution des commandes par rapport à la source de données. Les valeurs valides sont ReadCommitted (1) ou instantané (2).  
  
 délai d'expiration  
 Entier qui spécifie le délai d’expiration en secondes pour les commandes exécutées par rapport à la source de données.  
  
 fournisseur  
 Chaîne facultative qui identifie le nom du fournisseur de données managé utilisé pour la connexion à la base de données relationnelle, indication contraire sur la chaîne de connexion.  
  
## <a name="usage"></a>Utilisation  
 **Source de données** objets sont utilisés dans [Alter, commande &#40;TMSL&#41;](../../analysis-services/tabular-models-scripting-language-commands/alter-command-tmsl.md), [Créer commande &#40;TMSL&#41;](../../analysis-services/tabular-models-scripting-language-commands/create-command-tmsl.md), [commande CreateOrReplace &#40; TMSL&#41;](../../analysis-services/tabular-models-scripting-language-commands/createorreplace-command-tmsl.md), [commande Delete &#40;TMSL&#41;](../../analysis-services/tabular-models-scripting-language-commands/delete-command-tmsl.md), [commande d’actualisation &#40;TMSL&#41;](../../analysis-services/tabular-models-scripting-language-commands/refresh-command-tmsl.md), et [commande MergePartitions &#40;TMSL&#41;](../../analysis-services/tabular-models-scripting-language-commands/mergepartitions-command-tmsl.md).  
  
 Un **DataSource** objet est une propriété d’un modèle, mais peut également être spécifié en tant que propriété d’un objet de base de données étant donné le mappage entre le modèle et de la base de données.  Les partitions basées sur des requêtes SQL également spécifient un **DataSource**, uniquement avec un ensemble réduit de propriétés.  
  
 Lors de la création, de remplacement ou de modification d’un objet de source de données, spécifiez toutes les propriétés en lecture-écriture de la définition d’objet. L’omission d’une propriété en lecture-écriture est considéré comme une suppression.  
  
## <a name="examples"></a>Exemples  
 **Exemple 1** -une connexion à un *FoodMart* base de données sur un référentiel distant une instance nommé de *Sales* sur un serveur réseau nommé *Server01*.  
