...
# Activer le mode DirectQuery dans SSMS
 ...
 Cette rubrique suppose que vous avez créé et validé un modèle tabulaire en mémoire au niveau de compatibilité 1200, et qu’il ne vous reste plus qu’à permettre à DirectQuery d’accéder aux chaînes de connexion et de les mettre à jour. Si vous démarrez à partir d’un niveau de compatibilité inférieur, vous devez commencer par mettre à niveau le modèle manuellement. Pour connaître la procédure à suivre, consultez [Mettre à niveau Analysis Services](../../database-engine/install-windows/upgrade-analysis-services.md).  
...
## Étape 1 : vérifier le niveau de compatibilité  
...
## Étape 2a : passer d’une base de données tabulaire 1200 au mode DirectQuery  
...
## Étape 2 b : passer d’une base de données tabulaire 1100-1103 au mode DirectQuery  
...
-   Une fois le modèle déployé, vous pouvez modifier la méthode de connexion par défaut. Par exemple, vous pouvez utiliser un mode hybride pour le test, et basculer le modèle en mode **DirectQuery uniquement** seulement après avoir testé complètement tous les rapports ou requêtes qui utilisent le modèle. Pour plus d’informations, consultez [Définir ou modifier la méthode de connexion par défaut pour DirectQuery](../Topic/Set%20or%20Change%20the%20Preferred%20Connection%20Method%20for%20DirectQuery.md).  
  
## Étape 3: vérifier les propriétés de connexion sur la base de données  
...
## Étape 4 : valider l’accès de DirectQuery  
