...
2.  **Récupérer des données.**  
     ...
     Comme la commande exécute la requête, ADOMD.NET retourne les résultats dans le **Resultset** mettre en forme, un format tabulaire comme décrit dans la spécification XML for Analysis, à aplatir les données pour le <xref:Microsoft.AnalysisServices.AdomdClient.AdomdDataReader> objet. Le format tabulaire est inhabituel en matière d'interrogation de données analytiques compte tenu de leur dimensionnalité variable.  
...
## <a name="see-also"></a>Voir aussi  
 [Récupération des données à partir d’une Source de données analytiques](../../analysis-services/multidimensional-models-adomd-net-client/retrieving-data-from-an-analytical-data-source.md)   
 [Récupération des données à l’aide de l’ensemble de cellules](../../analysis-services/multidimensional-models-adomd-net-client/retrieving-data-using-the-cellset.md)   
