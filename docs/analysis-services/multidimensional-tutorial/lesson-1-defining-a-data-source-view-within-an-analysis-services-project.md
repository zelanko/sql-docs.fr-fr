...
Dans cette tâche, vous spécifiez le **AdventureWorksDW** base de données comme source de données pour le [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] dimensions et les cubes que vous définissez dans les leçons suivantes.  
  
[Définition d’une vue de source de données](lesson-1-3-defining-a-data-source-view.md)  
Dans cette tâche, vous définissez une vue unifiée unique des métadonnées issues des tables sélectionnées dans le **AdventureWorksDW** base de données.  
  
[Modification des noms de tables par défaut](lesson-1-4-modifying-default-table-names.md)  
Dans cette tâche, vous allez modifier les noms de tables dans la vue de source de données, afin que les noms des prochains [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] objets que vous définissez sont plus conviviales.  
