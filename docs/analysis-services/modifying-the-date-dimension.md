...
#### Pour définir la propriété KeyColumns composite pour l'attribut Calendar Quarter  
...
#### Pour définir la propriété KeyColumns composite pour l'attribut Calendar Semester  
...
12. Passez en revue les membres de la hiérarchie d’attributs **English Month Name**.  
    ...
    Notez que les membres de la hiérarchie sont maintenant triés d'abord par année, puis alphabétiquement par mois. La raison en est que le type de données de la colonne EnglishCalendarMonth dans la vue de source des données est une colonne de chaîne, basée sur le type de données nvarchar dans la base de données relationnelle sous-jacente. Pour plus d’informations sur la façon de trier les mois chronologiquement au sein de chaque année, consultez [Tri des membres d’attribut sur la base d’un attribut secondaire](../analysis-services/sorting-attribute-members-based-on-a-secondary-attribute.md).  
  
## Tâche suivante de la leçon  
[Exploration du cube déployé](../analysis-services/browsing-the-deployed-cube.md)  
