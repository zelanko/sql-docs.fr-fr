...
##  <a name="bkmk_currentsettings"></a> Déterminer les paramètres du pool de threads actuel  
 ...
 N'oubliez pas que l'algorithme permettant de définir **MinThread** et **MaxThread** intègre la configuration du système, notamment le nombre de processeurs. Le billet de blog suivant offre un aperçu du mode de calcul des valeurs : [Analysis Services 2012 Configuration settings (Wordpress Blog)](http://go.microsoft.com/fwlink/?LinkId=330387). Notez que ces paramètres et comportements sont susceptibles d'être modifiés dans les versions à venir.  
