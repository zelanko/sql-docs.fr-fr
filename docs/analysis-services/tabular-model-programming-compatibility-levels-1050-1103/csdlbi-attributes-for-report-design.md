...
|Nom de l'attribut|Type de données|Description|  
|--------------------|---------------|-----------------|  
|**Nom de la référence**|Text|Identificateur utilisé pour référencer cette entité dans une requête DAX. En cas d'omission, le nom est utilisé.|  
|**Légende**|Text|Nom complet de l'entité.|  
|**Documentation**|Text|Texte descriptif pour aider les utilisateurs professionnels à comprendre la signification des données.|  
|**Hidden**|Boolean|Indique si l'entité doit être affichée. La valeur par défaut est **false**.|  
...
|Nom de l'attribut|Type de données|Description|  
|--------------------|---------------|-----------------|  
|**Nom de la référence**|Text|Identificateur utilisé pour référencer cette entité dans une requête DAX. En cas d'omission, le nom du champ est utilisé.|  
|**Légende**|Text|Nom complet de l'entité. Si omis, le champ **ReferenceName** est utilisé.|  
...
|**ContextualNameRule**|Enum|Valeur qui indique si et comment le nom de la propriété doit être modifié en fonction du contexte dans lequel il est utilisé. Les valeurs possibles sont les suivantes :  **Aucun**, **rôle**, **fusion**.|  
...
|**FormatString**|Text|Une chaîne de format .NET indiquant comment la valeur du champ doit être mise en forme par défaut. En cas d'omission, le format suivant est utilisé :<br /><br /> Champs - Datetime : date courte régionale ou « d »<br /><br /> -Fonction d’agrégation des champs virgule flottante et champs intégraux avec une valeur par défaut : nombre local ou « n »<br /><br /> -Fonction d’agrégation entiers sans valeur par défaut : nombre décimal local ou « d »<br /><br /> Pour tous les autres types de champs, aucune chaîne de format ne s'applique.|  
|**Unités**|Text|Symbole qui est appliqué aux valeurs de champ pour exprimer des unités. En cas d'omission, les unités sont inconnues.|  
