---
title: Comparaison des modèles tabulaires et multidimensionnelles d’Analysis Services | Microsoft Docs
ms.date: 05/08/2018
ms.prod: sql
...
ms.locfileid: "68210256"
---
# <a name="comparing-tabular-and-multidimensional-solutions"></a>Comparaison des solutions tabulaires et multidimensionnelles
[!INCLUDE[ssas-appliesto-sqlas](../includes/ssas-appliesto-sqlas.md)]
 ...
 Les solutions tabulaires et multidimensionnelles sont créées à l’aide de SSDT et sont destinées aux projets de BI d’entreprise qui s’exécutent sur un serveur autonome [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] instance sur site et pour les modèles tabulaires, une [Azure Analysis Services](https://azure.microsoft.com/services/analysis-services/) serveur dans le cloud. Les deux solutions fournissent des bases de données analytiques hautes performances qui s’intègrent aisément aux clients BI. Pourtant, chaque solution diffère dans la façon dont elle est créée, utilisée et déployée. L’essentiel de cette rubrique compare ces deux types afin que vous puissiez identifier l’approche qui vous convient.  
  
 Pour les nouveaux projets, nous recommandons généralement les modèles tabulaires. Les modèles tabulaires sont plus rapides à concevoir, tester et déployer ; et seront mieux fonctionner avec les toutes dernières applications d’analyse Décisionnelle libre-service et services à partir de Microsoft cloud.  
...
##  <a name="bkmk_overview"></a> Vue d’ensemble des types de modélisation  
 ...
 > [!NOTE]  
>  **Azure Analysis Services** prend en charge les modèles tabulaires aux niveaux de compatibilité 1200 et supérieur. Toutefois, toutes les fonctionnalités de modélisation tabulaire décrites dans cette rubrique sont pris en charge dans Azure Analysis Services. Créer et déployer des modèles tabulaires dans Azure Analysis Services est très similaire car il s’agit en local, il est important de comprendre les différences. Pour plus d’informations, consultez [qu’est Azure Analysis Services ?](https://docs.microsoft.com/azure/analysis-services/analysis-services-overview)
  
||||  
|-|-|-|  
|**Type**|**Description de la modélisation**|**Publication**|  
...
|Tabulaire dans SQL Server 2016|Constructions (modèle, tables, colonnes), articulées dans les définitions des objets dans les métadonnées tabulaires de modélisation relationnelle [tabulaire modèle Scripting Language (TMSL)](../analysis-services/tabular-model-scripting-language-tmsl-reference.md) et [le modèle d’objet tabulaire (TOM)](../analysis-services/tabular-model-programming-compatibility-level-1200/introduction-to-the-tabular-object-model-tom-in-analysis-services-amo.md) code.|SQL Server 2016 (niveau de compatibilité 1200)| 
|Tabulaire dans SQL Server 2017|Constructions (modèle, tables, colonnes), articulées dans les définitions des objets dans les métadonnées tabulaires de modélisation relationnelle [tabulaire modèle Scripting Language (TMSL)](../analysis-services/tabular-model-scripting-language-tmsl-reference.md) et [le modèle d’objet tabulaire (TOM)](../analysis-services/tabular-model-programming-compatibility-level-1200/introduction-to-the-tabular-object-model-tom-in-analysis-services-amo.md) code.|SQL Server 2017 (niveau de compatibilité 1400)| 
...
|Power Pivot|À l’origine un complément, désormais entièrement intégré dans Excel. Modélisation uniquement, sur une infrastructure tabulaire interne. Vous pouvez importer un modèle PowerPivot dans SSDT pour créer un modèle tabulaire s’exécutant sur une instance Analysis Services.|Via Excel et Power BI Desktop|  
 ...
 <sup>1</sup> les niveaux de compatibilité sont importants dans la version actuelle en raison de la prise en charge pour l’activation de scénario et de moteur de métadonnées tabulaires fonctionnalités disponibles uniquement au niveau supérieur. Versions ultérieures prennent en charge les niveaux de compatibilité antérieurs, mais il est recommandé de créer de nouveaux modèles ou de la mise à niveau des modèles existants vers le niveau de compatibilité le plus élevé pris en charge par la version du serveur.
...
|Expressions M|Non|Oui<sup>1</sup>|
|Relations plusieurs-à-plusieurs|Oui|Non (mais il est [filtres croisés bidirectionnels](../analysis-services/tabular-models/bi-directional-cross-filters-tabular-models-analysis-services.md) niveaux de compatibilité 1200 et supérieur)| 
 ...
 **Taille du modèle et écart de ressource (en mémoire ou sur disque)**  
 ...
 Les bases de données tabulaires s’exécutent soit en mémoire, soit en mode DirectQuery qui permet de décharger l’exécution des requêtes vers une base de données externe. Pour l’analytique tabulaire en mémoire, la base de données est stockée intégralement en mémoire, ce qui signifie que vous devez disposer suffisamment de mémoire non seulement charger toutes les données, mais également les structures de données supplémentaires créées pour prendre en charge des requêtes.  
  
 DirectQuery, revisité dans SQL Server 2016, a moins de restrictions qu’auparavant et améliorer les performances. Le fait de tirer parti de la base de données relationnelle principale pour le stockage et l’exécution des requêtes rend la génération d’un modèle tabulaire à grande échelle plus réalisable qu’auparavant.  
  
 Historiquement, les bases de données plus grands en production sont multidimensionnelles, dont les charges de traitement et de requête en cours d’exécution indépendamment sur du matériel dédié, chacune étant optimisée pour son propre usage.  Les bases de données tabulaires rattrapent rapidement leur retard, et de nouvelles avancées dans DirectQuery contribuent à combler l’écart encore davantage.  
  
 Pour les requêtes et de stockage de données multidimensionnel déchargement l’exécution est disponible via ROLAP.   Un serveur de requêtes permet de mettre en cache des ensembles de lignes et de paginer mes ensembles caduques. L’utilisation efficace et équilibrée des ressources de mémoire et de disque guide souvent les clients vers des solutions multidimensionnelles.  
...
-   Bases de données model tabulaire peuvent utiliser la sécurité au niveau des lignes, à l’aide d’autorisations en fonction du rôle.  
  
-   Bases de données model multidimensionnelles peuvent utiliser dimension et sécurité de niveau cellule, à l’aide d’autorisations en fonction du rôle.  
...
-   Les classeurs[!INCLUDE[ssGemini](../includes/ssgemini-md.md)] sont sécurisés au niveau des fichiers à l’aide d’autorisations SharePoint.  
 ...
 Les classeurs[!INCLUDE[ssGemini](../includes/ssgemini-md.md)] peuvent être restaurés sur un serveur en mode tabulaire. Une fois que le fichier est restauré, il est découplé à partir de SharePoint, ce qui vous permet d’utiliser toutes les fonctionnalités de modélisation tabulaire, y compris la sécurité de niveau ligne.  
...
##  <a name="bkmk_client"></a> Prise en charge des applications clientes  
 Solutions en général, tabulaires et multidimensionnelles prennent en charge les applications clientes à l’aide d’un ou plusieurs des bibliothèques clientes Analysis Services (MSOLAP, utilisent AMOND, ADOMD). Par exemple, Excel, Power BI Desktop et des applications personnalisées.   
