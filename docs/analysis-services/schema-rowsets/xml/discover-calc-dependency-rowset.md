...
|**OBJET**|**DBTYPE_WSTR**||Nom de l'objet pour lequel les informations de dépendance sont générées. Si l'objet est une mesure ou une colonne calculée, utilisez le nom de la mesure. Si l'objet est une relation, le nom de la table (ou la dimension du cube) qui contient la colonne qui participe à la relation.|  
