...
>  Le [!INCLUDE[dnprdnshort](../../../includes/dnprdnshort-md.md)] définit des attributs personnalisés comme une façon de décrire votre code et affecter le comportement à l'exécution. Pour plus d’informations, consultez la rubrique «[vue d’ensemble des attributs](http://go.microsoft.com/fwlink/?LinkId=82929), » dans le [!INCLUDE[dnprdnshort](../../../includes/dnprdnshort-md.md)] Guide du développeur sur MSDN.  
 ...
 Events  
 La création d’objets est déclenchée par les événements de session **OpenedCubeOpened Cube** et **ClosingCubeClosing Cube**.  
...
### <a name="mdx-extensions-and-enhancements"></a>Extensions MDX et améliorations  
 La commande CREATE MEMBER est améliorée avec la **légende** propriété, le **display_folder** propriété et le **associated_measure_group** propriété.  
  
 La commande UPDATE MEMBER est ajoutée pour éviter la récréation du membre lorsqu'une mise à jour est nécessaire avec la perte de priorité conséquente pour la résolution des calculs. Mises à jour ne peut pas modifier l’étendue du membre calculé, déplacer le membre calculé à un autre parent ou définir un autre **solveorder**.  
  
 La commande CREATE SET est améliorée avec la **légende** propriété, le **display_folder** propriété ainsi que le nouveau **statique | DYNAMIQUE** mot clé. *Statique* signifie que le jeu est évalué uniquement lors de la création. *Dynamique* signifie que le jeu est évalué chaque fois que le jeu est utilisé dans une requête. La valeur par défaut est **statique** si un mot clé est omis.  
