...
ms.topic: "article"
helpviewer_keywords: 
  - "requêtes [MDX], instruction SELECT"
  - "requêtes [MDX], à propos des requêtes"
  - "ensembles de cellules [MDX]"
  - "SELECT, instruction [MDX]"
  - "cubes [Analysis Services], instruction SELECT"
...
-   Le jeu de membres ou de tuples à inclure sur chaque axe de secteur. Pour plus d’informations sur les axes de secteur et de requête, consultez [Restriction de la requête avec des axes de requête et de secteur &#40;MDX&#41;](../Topic/Restricting%20the%20Query%20with%20Query%20and%20Slicer%20Axes%20\(MDX\).md).  
...
-   Une clause FROM qui détermine quel cube sera interrogé. Pour plus d’informations sur la clause FROM, consultez [Instruction SELECT &#40;MDX&#41;](../Topic/SELECT%20Statement%20\(MDX\).md).  
...
> [!NOTE]  
>  Pour plus d’informations détaillées sur les diverses clauses de l’instruction SELECT, consultez [Instruction SELECT &#40;MDX&#41;](../Topic/SELECT%20Statement%20\(MDX\).md).  
