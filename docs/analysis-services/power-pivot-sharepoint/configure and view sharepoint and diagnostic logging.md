---
title: "Configurer et afficher les fichiers journaux SharePoint et la journalisation des diagnostics (Power Pivot pour SharePoint) | Microsoft Docs"
ms.custom: ""
ms.date: "12/09/2016"
...
manager: "erikre"
---
# Configurer et afficher les fichiers journaux SharePoint et la journalisation des diagnostics (Power Pivot pour SharePoint)
...
3.  Accédez à **[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] Service**.  
...
4.  Développez la catégorie et sélectionnez des catégories.  
     ...
     **Traitement des demandes** spécifie les événements déclenchés par des requêtes sur une base de données [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] chargée sur un serveur de la batterie.  
  
     **Utilisation** spécifie un événement lié à la collecte des données d’utilisation [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] .  
...
-   Recherchez le statut d'erreur « Élevé » ou « Exception ». Recherche de « Service[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] ».  
...
#### Utiliser une application de visionneuse de journaux  
...
#### Entrées pour les services Power Pivot  
...
|w3wp.exe|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] Service|Web frontal|Commentaires|Starting to locate an application server for data source=\<*chemin*>|Lorsqu'il reçoit une demande de connexion, le service [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] identifie un [!INCLUDE[ssGeminiSrv](../../includes/ssgeminisrv-md.md)] disponible pour gérer la demande. Si la batterie de serveurs ne contient qu'un seul serveur, le serveur local accepte toujours la demande.|  
|w3wp.exe|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] Service|Web frontal|Commentaires|Locating the application server succeeded.|La requête a été allouée à une application de service [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] .|  
|w3wp.exe|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] Service|Web frontal|Commentaires|Redirecting request for the \<*source de données PowerPivot*> to the [!INCLUDE[ssGeminiSrv](../../includes/ssgeminisrv-md.md)].|La demande a été envoyée au [!INCLUDE[ssGeminiSrv](../../includes/ssgeminisrv-md.md)].|  
|w3wp.exe|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] Service|Traitement des demandes|Commentaires|Redirecting request for UserName\<*utilisateur SharePoint*> to the database|Une connexion avec emprunt d'identité à la source de données [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] a été créée au nom de l'utilisateur SharePoint.|  
