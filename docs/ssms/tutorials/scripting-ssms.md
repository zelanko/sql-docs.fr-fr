...
Le résumé de ce tutoriel est que tout objet dans **l’Explorateur d’objets** peut faire l’objet d’un script en cliquant dessus avec le bouton droit et en sélectionnant l’option **Scripter l’objet en tant que**. 
...
### <a name="scriptt-sql-when-backing-up-a-database"></a>Script T-SQL lors de la sauvegarde d’une base de données
...
1. Connectez-vous à votre serveur SQL Server.
2. Développez le nœud **Bases de données**.
3. Cliquez avec le bouton droit sur la base de données > **Générer un script de la base de données en tant que** :
...
6. Une fois que vous êtes prêt à poursuivre, continuez à cliquer sur **Suivant** jusqu’à ce que les scripts soient générés et que vous accédiez à **Terminer**. Votre script de base de données se trouve à l’emplacement où il a été enregistré à l’étape 5. 
    - Le script est ainsi généré pour le schéma et différents objets dans la base de données, mais pas pour les données. 
...
5. Cliquez avec le bouton droit sur la table que vous voulez scripter > **Scripter une table en tant que**
    ...
    ![Scripter une table](media/scripting-ssms/scripttable.png)
