...
|UC|Nombre de sockets.|![Icône de crayon](media/pencil-icon.png "icône de crayon")|Pour le chargement des données non compressées, dwloader n’est pas une application sollicitant beaucoup le processeur.  Au minimum, nous recommandons à l’aide d’un serveur 2 sockets fabriqués récemment.<br /><br />Pour le chargement des données compressées, vous avez besoin de suffisamment de puissance du processeur de décompresser les données avant de les envoyer à PDW. dwloader pouvez exécuter 10 threads actifs en même temps. Si vous envisagez de charger simultanément les 10 fichiers compressés, nous recommandons que le serveur a au moins une UC de 10 cœurs ou deux processeurs 6 cœurs.|  
