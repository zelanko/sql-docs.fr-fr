---
title: Migrer une Installation Reporting Services (Mode natif) | Documents Microsoft
ms.custom: 
ms.date: 05/30/2017
...
Cette rubrique fournit des instructions détaillées pour la migration d’une des versions prises en charge suivantes d’un [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] déploiement en mode natif vers une nouvelle instance de SQL Server Reporting Services :  
...
Pour plus d’informations sur la migration d’un déploiement en mode SharePoint de [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] , consultez [Migrer une installation Reporting Services &#40;mode SharePoint&#41;](../../reporting-services/install-windows/migrate-a-reporting-services-installation-sharepoint-mode.md).  
 ...
 La migration est définie comme étant le déplacement des fichiers de données d’application vers une nouvelle instance de SQL Server. Voici quelques raisons courantes pour lesquelles vous devez migrer votre installation :  
...
-   Installer une nouvelle instance de SQL Server. Si vous utilisez le même matériel, vous pouvez installer SQL Server côte à côte avec votre installation existante s’il s’agissait d’une des versions prises en charge.  
  
    > [!TIP]  
    >  Une installation côte à côte peut nécessiter une installation de SQL Server comme instance nommée.  
  
-   Déplacer la base de données du serveur de rapports et autres fichiers d’application à partir de votre installation existante vers votre nouvelle installation de SQL Server.  
...
Pour plus d’informations sur les modifications apportées dans SQL Server Reporting Services, consultez la documentation relative au conseiller de mise à niveau et [Nouveautés de Reporting Services](../../reporting-services/what-s-new-in-sql-server-reporting-services-ssrs.md).  
...
3.  Sauvegardez les fichiers de configuration du serveur de rapports. Les fichiers à sauvegarder sont les suivants :  
    ...
    6.  Web.config pour le serveur de rapports [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)] application.  
...
1.  Choisissez l'instance [!INCLUDE[ssDE](../../includes/ssde-md.md)] à utiliser. SQL Server Reporting Services requiert que vous utilisez une des versions suivantes pour héberger la base de données du serveur de rapports :  
...
4.  Suivez les instructions fournies dans [Déplacement des bases de données du serveur de rapports vers un autre ordinateur &#40;en mode natif SSRS&#41;](../../reporting-services/report-server/moving-the-report-server-databases-to-another-computer-ssrs-native-mode.md).  
...
2.  Déplacez les assemblys vers le dossier \bin de serveur de rapport. Dans SQL Server, les binaires de serveur de rapports se trouvent dans l’emplacement suivant pour l’instance de serveur de rapports par défaut :  
...
## <a name="bkmk_configure_reportserver"></a> Configuration du serveur de rapports
 ...
 Configurer des URL pour le portail de web et le service Web Report Server et configurer la connexion à la base de données du serveur de rapports.  
  
 Si vous migrez un déploiement avec montée en puissance parallèle, mettez tous les nœuds du serveur de rapports hors connexion et migrez les serveurs un par un. Une fois le premier serveur de rapports est migré et il se connecte avec succès à la base de données du serveur de rapports, la version de base de données de serveur de rapports est automatiquement mis à niveau vers la version de base de données SQL Server.  
...
2.  Configurez les URL pour le serveur de rapports et le portail web. Pour plus d’informations, consultez [Configurer une URL &#40;Gestionnaire de configuration de SSRS&#41;](../../reporting-services/install-windows/configure-a-url-ssrs-configuration-manager.md).  
  
3.  Configurez la base de données du serveur de rapports, en sélectionnant la base de données du serveur de rapports de votre installation précédente. Après configuration réussie, les services de serveur de rapports redémarreront, et une fois qu’une connexion est établie à la base de données du serveur de rapports, la base de données est mis automatiquement à niveau vers SQL Server Reporting Services. Pour plus d’informations sur la façon d’exécuter l’Assistant Modification de base de données que vous utilisez pour créer ou sélectionner une base de données du serveur de rapports, consultez [créer une base de données de serveur de rapports en Mode natif](../../reporting-services/install-windows/ssrs-report-server-create-a-native-mode-report-server-database.md).  
...
Si vous avez modifié le fichier RSReportServer.config ou RSWebApplication.config dans l'installation précédente, vous devez apporter les mêmes modifications au nouveau fichier RSReportServer.config. La liste suivante récapitule certaines des raisons pour lesquelles vous pouvez avoir modifié le fichier de configuration précédent et fournit des liens vers des informations supplémentaires sur la façon de configurer les mêmes paramètres dans SQL Server 2016.  
  
|Personnalisation|Informations|  
|-------------------|-----------------|  
|Remise du courrier électronique du serveur de rapports avec des paramètres personnalisés|[Paramètres de messagerie - mode natif des Services de création de rapports](../../reporting-services/install-windows/e-mail-settings-reporting-services-native-mode-configuration-manager.md).|  
...
## <a name="bkmk_windowsservice_group"></a> Groupe de service Windows et ACL de sécurité
 ...
 Dans [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)], il existe un groupe de service, le [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] groupe de Service Windows, qui permet de créer des ACL de sécurité pour toutes les clés de Registre, fichiers et dossiers qui sont installés avec SQL Server Reporting Services. Ce nom de groupe Windows apparaît au format SQLServerReportServerUser$\<*Nom_Ordinateur*>$\<*nom_instance*>.  
...
2.  Testez les rapports et assurez-vous qu'ils contiennent les données attendues. Passez en revue les informations de la source de données pour vérifier si ses informations de connexion sont toujours spécifiées. Le serveur de rapports utilise le modèle objet de rapport lors du traitement et le rendu des rapports, mais il ne remplace pas [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)], [!INCLUDE[ssKilimanjaro](../../includes/sskilimanjaro-md.md)], [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)], ou [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] construit avec les nouveaux éléments de langage de définition de rapport. Pour en savoir plus sur les rapports existants s’exécuter sur une nouvelle version de votre serveur de rapports, consultez [mise à niveau des rapports](../../reporting-services/install-windows/upgrade-reports.md).  

## <a name="bkmk_remove_unused"></a> Suppression des programmes et fichiers inutiles

Une fois que vous avez bien migré votre serveur de rapports vers une nouvelle instance, vous souhaiterez peut-être effectuer les étapes suivantes pour supprimer des programmes et fichiers qui ne sont plus nécessaires.  
