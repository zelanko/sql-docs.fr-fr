 ...
 **TargetServerVersion**  
 Sélectionnez la version [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] ou sélectionnez **Détecter la version** pour déterminer automatiquement la version installée sur le serveur identifié par la propriété **TargetServer URL**. La valeur par défaut est **[!INCLUDE[ssCurrent_md](../../includes/sscurrent-md.md)]**.  
