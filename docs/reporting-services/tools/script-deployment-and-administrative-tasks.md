...
2.  Utilisez le <xref:ReportService2010.ReportingService2010.GetItemDefinition%2A> et <xref:ReportService2010.ReportingService2010.GetProperties%2A> méthodes pour récupérer la définition de rapport et les propriétés du rapport.  
  
3.  Définissez l'URL de manière à pointer vers le serveur de destination.  
  
4.  Utilisez <xref:ReportService2010.ReportingService2010.CreateCatalogItem%2A> méthode, en passant les propriétés retournées par <xref:ReportService2010.ReportingService2010.GetProperties%2A> et la définition de rapport retournée par <xref:ReportService2010.ReportingService2010.GetItemDefinition%2A>.  
