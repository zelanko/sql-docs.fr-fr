---
title: Serveur de rapports Reporting Services | Documents Microsoft
ms.custom: 
ms.date: 09/25/2017
...
## <a name="overview-of-report-server-modes"></a>Présentation des modes du serveur de rapports
 ...
 Un serveur de rapports Reporting Services prend en charge deux modes de déploiement pour les instances de serveur de rapports :  
  
-   **En mode natif**: notamment le mode natif avec composants WebPart SharePoint, où un serveur de rapports s’exécute en tant que serveur d’applications qui fournit toutes les fonctions de traitement et de gestion exclusivement par le biais de composants de Reporting Services. Vous configurez un serveur de rapports en mode natif avec le Gestionnaire de configuration de Reporting Services et SQL Server Management Studio.  
...
|**Rapports .RDL**|Oui|Oui<br /><br /> . Rapports RDL peuvent exécuter sur Reporting Services serveurs de rapports en mode natif ou en mode SharePoint.|  
|**Rapports .RDLX**|Non|Oui<br /><br /> Power View. Les rapports RDLX ne peuvent exécuter sur Reporting Services les serveurs de rapports en mode SharePoint.|  
...
### <a name="native-mode-with-sharepoint-web-parts"></a>Mode natif avec composants WebPart SharePoint
 ...
 Reporting Services fournit deux composants WebPart que vous pouvez installer et inscrire sur une instance de [!INCLUDE[winSPServ](../../includes/winspserv-md.md)] 2.0 ou version ultérieure, ou [!INCLUDE[spPortalServ](../../includes/spportalserv-md.md)] 2003 ou version ultérieure. À partir d’un site SharePoint, vous pouvez utiliser les composants WebPart pour rechercher et afficher des rapports qui sont stockés et traités sur un serveur de rapports qui s’exécute en mode natif. Ces composants WebPart ont été introduites dans les versions antérieures de Reporting Services.  
  
## <a name="sharepoint-mode"></a>Mode SharePoint

 En mode SharePoint, un serveur de rapports doit s'exécuter dans une batterie de serveurs SharePoint. Les fonctionnalités de traitement, de rendu et de gestion de serveur de rapports sont représentées par un serveur d’applications SharePoint exécutant le service partagé Reporting Services SharePoint et une ou plusieurs applications de service Reporting Services. Un site SharePoint fournit l'accès frontal au contenu et aux opérations du serveur de rapports.  
...
-   Un serveur d’applications SharePoint avec le service partagé Reporting Services installé et au moins une application de service Reporting Services.  
  
 L’illustration suivante montre un environnement de Reporting Services en mode SharePoint :  
...
|| Description|  
|-|-----------------|  
|**(1)**|Serveurs Web ou serveurs Web frontaux (WFE). Le complément Reporting Services doit être installé sur chaque serveur web à partir de laquelle vous souhaitez utiliser les fonctionnalités des applications web telles que l’affichage des rapports ou des pages d’administration de Reporting Services pour des tâches telles que la gestion des abonnements ou des sources de données.|  
|**(2)**|Le complément installe les points de terminaison URL et SOAP pour les clients communiquer avec les serveurs d’applications via le proxy de service Reporting Services.|  
|**(3)**|Serveurs d’applications exécutant le service partagé Reporting Services. Montée en puissance de traitement des rapports est gérée dans le cadre de la batterie de serveurs SharePoint et en ajoutant le service Reporting Services pour les serveurs d’applications supplémentaires.|  
|**(4)**|Vous pouvez créer plusieurs applications de service Reporting Services, avec différentes configurations, y compris les autorisations, par courrier électronique, le proxy et abonnements.|  
...
|**(6)**|Reporting Services créent trois bases de données pour le serveur de rapports, les fonctions d’alerte de données et les données temporaires. Les paramètres de configuration qui s'appliquent à l'ensemble des applications de service SSRS sont stockés dans le fichier **RSReportserver.config** .|  
