...
-   créer des dossiers sur le serveur pour organiser vos rapports comme vous le souhaitez.  
 ...
 ![Barre d'outils Rapport dans le Gestionnaire de rapports](../../reporting-services/report-builder/media/hs-reportserver-blowout.png "Barre d'outils Rapport dans le Gestionnaire de rapports")  
...
|![Bouton d'actualisation du navigateur sur la barre d'outils du rapport](../../reporting-services/media/htmlviewer-refresh.png "Bouton d'actualisation du navigateur sur la barre d'outils du rapport")|Quand vous cliquez dans la barre d’outils Rapports sur le bouton **Actualiser**, le serveur de rapports réexécute la requête et met à jour les données du rapport si ce dernier s’exécute à la demande. Si le rapport est mis en cache ou s'il s'agit d'un instantané, la commande **Actualiser** permet d'afficher le rapport stocké dans la base de données du serveur de rapports.|  
