---
title: "Compteurs de performance du service Web MSRS 2011 en mode SharePoint et des objets de performance du service Windows MSRS 2011 en mode SharePoint (mode SharePoint) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "compteurs de performance [Reporting Services]"
  - "compteurs [Reporting Services]"
  - "Service Report Server Windows, compteurs de performance"
  - "objet de performance du Service Windows Reporting Services"
  - "objet de performance du processeur de planification et de livraison [Reporting Services]"
...
>  Ces objets de performance contrôlent des événements sur le serveur de rapports local. Si vous exécutez un serveur de rapports dans un déploiement avec montée en puissance parallèle, les chiffres s'appliquent au serveur actuel et non au déploiement avec montée en puissance parallèle dans son ensemble.  
 ...
 Les objets de performance sont disponibles dans l’Analyseur de performances Windows (**Perfmon.exe**). Pour plus d'informations, consultez la documentation Windows. [Génération de profils d’exécution](http://msdn.microsoft.com/library/w4bz2147.aspx) (https://msdn.microsoft.com/fr-fr/library/w4bz2147.aspx).  
...
|**Présences dans le cache mémoire/s**|Nombre de fois par seconde où les rapports sont récupérés du cache mémoire. Le *cache en mémoire* fait partie du cache qui stocke les rapports dans la mémoire de l’unité centrale. Quand le cache en mémoire est utilisé, le serveur de rapports n’interroge pas [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] par rapport au contenu mis en cache.|  
...
|**Total des présences dans le cache mémoire**|Nombre total de rapports mis en cache retournés par le cache mémoire après le démarrage du service. Ce compteur est réinitialisé à chaque interruption du service Web Report Server par [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)] . Le *cache en mémoire* fait partie du cache qui stocke les rapports dans la mémoire de l’unité centrale. Quand le cache en mémoire est utilisé, le serveur de rapports n’interroge pas [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] par rapport au contenu mis en cache.|  
...
##  <a name="bkmk_powershell"></a> Utiliser des applets de commande PowerShell pour retourner des listes  
 ![Contenu relatif à PowerShell](../../analysis-services/instances/install-windows/media/rs-powershellicon.png "Contenu relatif à PowerShell")Le script Windows PowerShell suivant retourne les ensembles de compteurs dans lesquels CounterSetName commence par « msr »  
