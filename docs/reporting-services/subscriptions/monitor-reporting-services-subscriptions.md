...
ms.topic: "article"
helpviewer_keywords: 
  - "abonnements [Reporting Services], inactifs"
  - "abonnements [Reporting Services], état"
  - "analyse [Reportions Services], abonnements"
  - "informations d'état [Reporting Services]"
  - "abonnements inactifs [Reporting Services]"
...
|Terminé : \<*nombre*> traités sur \<*nombre*> au total ; \<*nombre*> erreurs.|Indique l'état de l'exécution d'un abonnement piloté par les données. Ce message provient du processeur de planification et de livraison.|  
...
|Échec d’écriture dans le fichier \<nom_fichier> : \<message>|Indique que la remise à l'emplacement du partage de fichiers a échoué ; ce message provient de l'extension de remise dans un partage de fichiers.|  
