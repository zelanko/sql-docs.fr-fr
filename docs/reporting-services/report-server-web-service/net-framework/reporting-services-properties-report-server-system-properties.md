...
|ExecutionLogDaysKept|Nombre de jours pendant lesquels conserver les informations sur l'exécution du rapport dans le journal des exécutions. Les valeurs valides pour cette propriété sont comprises entre **0** et **2****147** **483****647**. Si la valeur est **0** entrées ne sont pas supprimées de la table du journal d’exécution. La valeur par défaut est **60**.|  
