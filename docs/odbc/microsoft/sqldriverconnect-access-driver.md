 ...
 **SQLDriverConnect** vous permet de se connecter à un pilote sans création d’une source de données (DSN).  
 ...
 Le **UID** et **PWD** mots clés sont également acceptés.  
  
 Le mot clé PWD ne doit pas inclure les caractères spéciaux (voir SQL_SPECIAL_CHARACTERS dans **SQLGetInfo** valeurs retournées).  
