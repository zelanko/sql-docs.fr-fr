---
title: Des erreurs ODBC | Documents Microsoft
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.suite: sql
ms.technology: connectivity
ms.tgt_pltfrm: ''
ms.topic: conceptual
helpviewer_keywords:
- errors (ODBC desktop database drivers)
ms.assetid: 33542afe-4dc7-4db8-b625-fec51a4718d3
caps.latest.revision: 5
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: 6d20729306c453f6691eb08bb93ad148ae72475a
ms.sourcegitcommit: 1740f3090b168c0e809611a7aa6fd514075616bf
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 05/03/2018
ms.locfileid: "32899884"
---
# <a name="odbc-errors"></a>Erreurs ODBC
Lorsqu’une erreur se produit, les pilotes de base de données de bureau Microsoft ODBC retourne le numéro d’erreur native, la valeur SQLSTATE (un code d’erreur ODBC) et un message d’erreur.  
  
 Cette section contient les rubriques suivantes.  
  
-   [Erreur native](../../odbc/microsoft/native-error.md)  
  
-   [SQLSTATE](../../odbc/microsoft/sqlstate.md)  
  
-   [Messages d’erreur](../../odbc/microsoft/odbc-jet-error-messages.md)  
  
-   [Erreurs arithmétiques](../../odbc/microsoft/arithmetic-errors.md)
