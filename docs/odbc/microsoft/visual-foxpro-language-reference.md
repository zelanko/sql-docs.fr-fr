---
title: Informations de référence sur le langage Visual FoxPro | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.prod_service: connectivity
ms.reviewer: ''
ms.technology: connectivity
ms.topic: conceptual
helpviewer_keywords:
- commands for FoxPro ODBC driver [ODBC]
- Visual FoxPro ODBC driver [ODBC], commands
ms.assetid: 8e052213-ca6f-44ca-917e-054ecb3eb0e5
author: MightyPen
ms.author: genemi
ms.openlocfilehash: 88993a80fa629c2f4cdc8448483fc8a65a6f2adf
ms.sourcegitcommit: b87d36c46b39af8b929ad94ec707dee8800950f5
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 02/08/2020
ms.locfileid: "68087825"
---
# <a name="visual-foxpro-language-reference"></a>Informations de référence sur le langage Visual FoxPro
Cette section contient les rubriques suivantes :  
  
-   [ALTER TABLE, commande SQL](../../odbc/microsoft/alter-table-sql-command.md)  
  
-   [CREATE TABLE, commande SQL](../../odbc/microsoft/create-table-sql-command.md)  
  
-   [DELETE, commande SQL](../../odbc/microsoft/delete-sql-command.md)  
  
-   [DELETE TAG, commande](../../odbc/microsoft/delete-tag-command.md)  
  
-   [DROP TABLE, commande](../../odbc/microsoft/drop-table-command.md)  
  
-   [INDEX, commande](../../odbc/microsoft/index-command.md)  
  
-   [INSERT, commande SQL](../../odbc/microsoft/insert-sql-command.md)  
  
-   [SELECT, commande SQL](../../odbc/microsoft/select-sql-command.md)  
  
-   [SET ANSI, commande](../../odbc/microsoft/set-ansi-command.md)  
  
-   [SET BLOCKSIZE, commande](../../odbc/microsoft/set-blocksize-command.md)  
  
-   [SET COLLATE, commande](../../odbc/microsoft/set-collate-command.md)  
  
-   [SET DELETED, commande](../../odbc/microsoft/set-deleted-command.md)  
  
-   [SET EXACT, commande](../../odbc/microsoft/set-exact-command.md)  
  
-   [SET EXCLUSIVE, commande](../../odbc/microsoft/set-exclusive-command.md)  
  
-   [SET NULL, commande](../../odbc/microsoft/set-null-command.md)  
  
-   [SET PATH, commande](../../odbc/microsoft/set-path-command.md)  
  
-   [SET REPROCESS, commande](../../odbc/microsoft/set-reprocess-command.md)  
  
-   [SET UNIQUE, commande](../../odbc/microsoft/set-unique-command.md)  
  
-   [UPDATE, commande SQL](../../odbc/microsoft/update-sql-command.md)  
  
-   [Types de données Visual FoxPro](../../odbc/microsoft/visual-foxpro-field-data-types.md)
