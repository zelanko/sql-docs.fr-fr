---
title: "Référence du langage Visual FoxPro | Documents Microsoft"
ms.custom: 
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.reviewer: 
ms.suite: 
ms.technology:
- drivers
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- commands for FoxPro ODBC driver [ODBC]
- Visual FoxPro ODBC driver [ODBC], commands
ms.assetid: 8e052213-ca6f-44ca-917e-054ecb3eb0e5
caps.latest.revision: 5
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: a833509f772205258e10ec4b589da10ab2a1cbbe
ms.contentlocale: fr-fr
ms.lasthandoff: 09/09/2017

---
# <a name="visual-foxpro-language-reference"></a>Référence du langage Visual FoxPro
Cette section contient les rubriques suivantes.  
  
-   [ALTER TABLE - commande SQL](../../odbc/microsoft/alter-table-sql-command.md)  
  
-   [CRÉER la TABLE - commande SQL](../../odbc/microsoft/create-table-sql-command.md)  
  
-   [Supprimer : la commande SQL](../../odbc/microsoft/delete-sql-command.md)  
  
-   [SUPPRIMER des commandes de balise](../../odbc/microsoft/delete-tag-command.md)  
  
-   [Commande DROP TABLE](../../odbc/microsoft/drop-table-command.md)  
  
-   [Commande INDEX](../../odbc/microsoft/index-command.md)  
  
-   [INSERT - commande SQL](../../odbc/microsoft/insert-sql-command.md)  
  
-   [Sélectionnez - commande SQL](../../odbc/microsoft/select-sql-command.md)  
  
-   [Commande SET ANSI](../../odbc/microsoft/set-ansi-command.md)  
  
-   [Commande de taille de bloc SET](../../odbc/microsoft/set-blocksize-command.md)  
  
-   [Commande SET COLLATE](../../odbc/microsoft/set-collate-command.md)  
  
-   [Commande SET DELETED](../../odbc/microsoft/set-deleted-command.md)  
  
-   [Commande exacte de jeu](../../odbc/microsoft/set-exact-command.md)  
  
-   [Commande exclusif SET](../../odbc/microsoft/set-exclusive-command.md)  
  
-   [Commande NULL SET](../../odbc/microsoft/set-null-command.md)  
  
-   [Commande de chemin d’accès de SET](../../odbc/microsoft/set-path-command.md)  
  
-   [Commande de RETRAITER SET](../../odbc/microsoft/set-reprocess-command.md)  
  
-   [Commande UNIQUE SET](../../odbc/microsoft/set-unique-command.md)  
  
-   [Commande de mise à jour - SQL](../../odbc/microsoft/update-sql-command.md)  
  
-   [Types de données Visual FoxPro](../../odbc/microsoft/visual-foxpro-field-data-types.md)
