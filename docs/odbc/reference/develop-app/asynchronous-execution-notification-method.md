...
ODBC permet l’exécution asynchrone de connexion et les opérations de l’instruction. Un thread d’application peut appeler une fonction ODBC en mode asynchrone et la fonction peut retourner avant que l’opération est terminée, ce qui permet le thread d’application effectuer d’autres tâches. Dans le Kit de développement Windows 7 pour les opérations de connexion, ou l’instruction asynchrone, une application déterminé que l’opération asynchrone a été terminée à l’aide de la méthode d’interrogation. Pour plus d’informations, consultez [exécution asynchrone (méthode d’interrogation)](../../../odbc/reference/develop-app/asynchronous-execution-polling-method.md). À compter dans le Kit de développement logiciel Windows 8, vous pouvez déterminer qu’une opération asynchrone est terminée à l’aide de la méthode de notification.  
 ...
 Dans la méthode d’interrogation, les applications doivent appeler la fonction asynchrone chaque fois qu’il veut l’état de l’opération. La méthode de notification est similaire à rappeler et la durée d’attente dans ADO.NET. ODBC, utilise toutefois, les événements Win32 en tant que l’objet de notification.  
  
 La bibliothèque de curseurs ODBC et la notification asynchrone ODBC ne peut pas être utilisés en même temps. Définition de ces deux attributs retournera une erreur avec SQLSTATE S1119 (bibliothèque de curseurs et Notification asynchrone ne peut pas être activée en même temps).  
  
 Consultez [Notification de fin de fonction asynchrone](../../../odbc/reference/develop-driver/notification-of-asynchronous-function-completion.md) pour plus d’informations pour les développeurs de pilotes.  
  
> [!NOTE]  
>  La méthode de notification n’est pas pris en charge avec la bibliothèque de curseurs. Une application message d’erreur si elle tente d’activer la bibliothèque de curseurs par le biais de SQLSetConnectAttr, lorsque la méthode de notification est activée.  
  
## <a name="overview"></a>Vue d'ensemble  
 Lorsqu’une fonction ODBC est appelée en mode asynchrone, le contrôle est retourné à l’application appelante immédiatement avec le code de retour SQL_STILL_EXECUTING. L’application doit interroger à plusieurs reprises la fonction jusqu'à ce qu’elle retourne différent de SQL_STILL_EXECUTING. La boucle d’interrogation augmente l’utilisation du processeur provoque ces performances médiocres dans de nombreux scénarios asynchrones.  
...
### <a name="integrate-data-from-multiple-odbc-sources"></a>Intégrer des données provenant de plusieurs Sources ODBC  
 En mode asynchrone, une application d’intégration de données extrait les données à partir de plusieurs sources de données. Certaines données proviennent de sources de données distantes et des données sont à partir de fichiers locales. L’application ne peut pas continuer jusqu'à ce que les opérations asynchrones sont terminées.  
  
 Au lieu de l’interrogation à plusieurs reprises une opération pour déterminer si elle est terminée, l’application peut créer un objet d’événement et l’associer à un handle de connexion ODBC ou un handle d’instruction ODBC. L’application appelle ensuite les API d’attente sur l’objet d’événement d’un ou plusieurs objets d’événements (événements d’ODBC et autres événements de Windows) de synchronisation de système d’exploitation. ODBC signalera que l’objet d’événement lorsque l’opération asynchrone ODBC correspondante est terminée.  
  
 Sur Windows, les objets d’événements Win32 seront utilisés et qui fournira l’utilisateur un modèle de programmation unifié. Gestionnaires de pilotes sur d’autres plateformes peuvent utiliser l’implémentation d’objet événement spécifique pour ces plateformes.  
  
 L’exemple de code suivant illustre l’utilisation de connexion et de notification asynchrone d’instruction :  
...
### <a name="associating-a-win32-event-handle-with-an-odbc-handle"></a>Association d’un Handle d’événement Win32 avec un descripteur ODBC  
 Les applications sont chargées de créer des objets d’événements Win32 à l’aide de fonctions Win32 correspondantes. Une application peut associer un handle d’événement Win32 un handle de connexion ODBC ou un handle d’instruction ODBC.  
...
-   **SQLSetStmtAttr** ne prend pas en charge l’exécution asynchrone.  
 ...
 Scénario de l’erreur-out  
 Lorsque **SQLSetConnectAttr** est appelée avant d’effectuer une connexion, le Gestionnaire de pilotes ne peut pas déterminer le pilote à utiliser. Par conséquent, le Gestionnaire de pilotes réussit pour **SQLSetConnectAttr** mais l’attribut ne peut pas être prêt à définir dans le pilote. Le Gestionnaire de pilotes définirez ces attributs lorsque l’application appelle une fonction de la connexion. Le Gestionnaire de pilotes peuvent hors de l’erreur, car le pilote ne prend pas en charge les opérations asynchrones.  
 ...
 Pour associer un handle d’événement à un handle de connexion ODBC, une application ODBC appelle des API ODBC **SQLSetConnectAttr** et spécifie SQL_ATTR_ASYNC_DBC_EVENT dans la mesure où l’attribut et l’événement en tant que la valeur d’attribut. Le nouvel attribut ODBC SQL_ATTR_ASYNC_DBC_EVENT est de type SQL_IS_POINTER.  
...
```  
 ...
 En règle générale, les applications créent des objets d’événement d’auto-réinitialisation. ODBC ne réinitialise pas l’objet d’événement. Applications doivent vous assurer que l’objet n’est pas dans l’état signalé avant d’appeler une fonction ODBC asynchrone.  
...
```  
 ...
 La valeur par défaut de SQL_ATTR_ASYNC_DBC_EVENT est NULL. Si le pilote ne prend pas en charge la notification asynchrone, obtention ou définition SQL_ATTR_ASYNC_DBC_EVENT retourne SQL_ERROR avec SQLSTATE HY092 (identificateur d’option/attribut non valide).  
  
 Si la dernière valeur SQL_ATTR_ASYNC_DBC_EVENT définie sur un handle de connexion ODBC n’est pas NULL et que l’application activée en mode asynchrone en définissant l’attribut SQL_ATTR_ASYNC_DBC_FUNCTION_ENABLE avec SQL_ASYNC_DBC_ENABLE_ON, appel de toute connexion ODBC fonction qui prend en charge le mode asynchrone obtiendra une notification d’achèvement. Si la dernière valeur SQL_ATTR_ASYNC_DBC_EVENT définie sur un handle de connexion ODBC est NULL, ODBC n’envoie pas l’application toute notification, quel que soit si le mode asynchrone est activé.  
 ...
 Avant d’appeler une fonction de connexion, les applications peuvent définir l’attribut SQL_ATTR_ASYNC_DBC_EVENT le sur un handle de connexion ODBC (**SQLConnect**, **SQLBrowseConnect**, ou  **SQLDriverConnect**). Étant donné que le Gestionnaire de pilotes ODBC ne sait pas quel pilote ODBC que l’application utilisera, elle retourne SQL_SUCCESS. Lorsque l’application appelle une fonction de connexion, le Gestionnaire de pilotes ODBC vérifie si le pilote prend en charge la notification asynchrone. Si le pilote ne prend pas en charge la notification asynchrone, le Gestionnaire de pilotes ODBC retourne SQL_ERROR avec SQLSTATE S1_118 (pilote ne prend pas en charge la notification asynchrone). Si le pilote prend en charge la notification asynchrone, le Gestionnaire de pilotes ODBC appelle le pilote et définir les attributs correspondants SQL_ATTR_ASYNC_DBC_NOTIFICATION_CALLBACK et SQL_ATTR_ASYNC_DBC_NOTIFICATION_CONTEXT.  
  
 De même, une application appelle **SQLSetStmtAttr** sur une instruction ODBC gèrent et spécifie l’attribut SQL_ATTR_ASYNC_STMT_EVENT pour activer ou désactiver la notification asynchrone au niveau instruction. Car une instruction fonction est toujours appelée une fois la connexion est établie, **SQLSetStmtAttr** retourne SQL_ERROR avec SQLSTATE S1_118 (pilote ne prend pas en charge la notification asynchrone) immédiatement si le correspondant pilote ne prend pas en charge les opérations asynchrones ou le pilote prend en charge l’opération asynchrone, mais ne prend pas en charge la notification asynchrone.  
...
```  
 ...
 La valeur par défaut de SQL_ATTR_ASYNC_STMT_EVENT est NULL. Si le pilote ne prend pas en charge la notification asynchrone, l’obtention ou définition de l’attribut SQL_ATTR_ASYNC_ STMT_EVENT retourne SQL_ERROR avec SQLSTATE HY092 (identificateur d’option/attribut non valide).  
...
### <a name="calling-asynchronous-odbc-functions"></a>Appel de fonctions ODBC asynchrone  
 Après l’activation des notifications asynchrones et commencer une opération asynchrone, l’application peut appeler n’importe quelle fonction ODBC. Si la fonction appartient à l’ensemble de fonctions qui prennent en charge une opération asynchrone, l’application obtiendra une notification de fin lors de l’opération terminée, indépendamment de si la fonction a échoué ou a réussi.  La seule exception est que l’application appelle une fonction ODBC avec un handle de connexion ou une instruction non valide. Dans ce cas, ODBC ne sera pas obtenir le handle d’événement et définissez-le sur l’état signalé.  
...
### <a name="getting-notification-from-odbc"></a>Obtention de Notification à partir d’ODBC  
 Un thread d’application peut appeler **WaitForSingleObject** d’attente sur le handle d’un événement ou appel **WaitForMultipleObjects** d’attendre sur un tableau de handles d’événement et d’être suspendu jusqu'à ce qu’un ou tous les objets d’événement signalé ou que l’intervalle de délai s’écoule.  
