---
title: SQLPrepare, fonction | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
...
## <a name="diagnostics"></a>Diagnostics  
 Lorsque **SQLPrepare** retourne SQL_ERROR ou SQL_SUCCESS_WITH_INFO, une valeur SQLSTATE associée peut être obtenu en appelant **SQLGetDiagRec** avec un *HandleType* de SQL_ HANDLE_STMT et un *gérer* de *au paramètre StatementHandle*. Le tableau suivant répertorie les valeurs SQLSTATE généralement retournées par **SQLPrepare** et explique chacune dans le contexte de cette fonction ; la notation « (DM) » précède les descriptions de SQLSTATE retournée par le Gestionnaire de pilotes. Le code de retour associé à chaque valeur SQLSTATE est SQL_ERROR, sauf indication contraire.  
...
|HYC00|Fonctionnalité optionnelle non implémentée|Le paramètre d’accès concurrentiel n’est pas valide pour le type de curseur défini.<br /><br /> L’attribut d’instruction SQL_ATTR_USE_BOOKMARKS a été défini sur SQL_UB_VARIABLE, et l’attribut d’instruction SQL_ATTR_CURSOR_TYPE a été défini pour un type de curseur pour lequel le pilote ne prend pas en charge les signets.|  
...
>  Si une application utilise **SQLPrepare** pour préparer et **SQLExecute** pour soumettre un **valider** ou **ROLLBACK** instruction, il ne sera pas interopérabilité entre les produits de SGBD. Pour valider ou restaurer une transaction, appelez **SQLEndTran**.  
 ...
 Certains pilotes ne peut pas retourner des erreurs de syntaxe ou de violations d’accès lorsque l’application appelle **SQLPrepare**. Un pilote peut gérer les erreurs de syntaxe et les violations d’accès, uniquement les erreurs de syntaxe, ou les erreurs de syntaxe ni violations d’accès. Par conséquent, une application doit être en mesure de gérer ces conditions lors de l’appel suivantes des fonctions associées telles que **SQLNumResultCols**, **SQLDescribeCol**, **SQLColAttribute**, et **SQLExecute**.  
  
 Selon les fonctionnalités du pilote et de la source de données, les informations de paramètre (par exemple, les types de données) peuvent être archivées lorsque l’instruction est préparée (si tous les paramètres ont été liés) ou lorsqu’elle est exécutée (si tous les paramètres n’ont pas été liés). Pour une interopérabilité maximale, une application doit dissocier tous les paramètres appliquaient à une ancienne instruction SQL avant de préparer une nouvelle instruction SQL sur la même instruction. Cela empêche les erreurs dues à des anciennes informations de paramètre qui est appliquées à la nouvelle instruction.  
