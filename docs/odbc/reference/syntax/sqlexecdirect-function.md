...
|22025|Séquence d’échappement non valide|\**StatementText* contenait une instruction SQL contenant «**comme** *valeur de modèle* **échappement** *caractère d’échappement* « dans le **où** clause et le caractère qui suit le caractère d’échappement dans la valeur de modèle n’est pas une « % » ou « _ ».|  
