...
|-------------|---------------------------------|  
|**DIAGNOSTIC**|Nom d’une source de données telle qu’elle est retournée par **SQLDataSources** ou la boîte de dialogue sources de données de **SQLDriverConnect**.|  
...
|**Identificateur d’utilisateur**|Un ID d’utilisateur.|  
