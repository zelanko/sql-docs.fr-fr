---
title: Fonction SQLTablePrivileges (fr) Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
 ...
 **Résumé**  
 **SQLTablePrivileges** retourne une liste de tables et les privilèges associés à chaque table. Le conducteur renvoie les informations sur l’instruction spécifiée.  
 ...
 *SchemaName (SchemaName)*  
 [Entrée] Modèle de recherche de cordes pour les noms de schéma. Si un conducteur prend en charge des schémas pour certaines tables, mais pas pour d’autres, comme lorsque le conducteur récupère des données de différents DBMS, une chaîne vide («)) dénote les tables qui n’ont pas de schémas.  
...
## <a name="comments"></a>Commentaires  
 Les arguments *SchemaName* et *TableName* acceptent les modèles de recherche. Pour plus d’informations sur les modèles de recherche valides, voir [Arguments de valeur de modèle](../../../odbc/reference/develop-app/pattern-value-arguments.md).  
  
 **SQLTablePrivileges** retourne les résultats comme résultat standard, commandés par TABLE_CAT, TABLE_SCHEM, TABLE_NAME, PRIVILEGE et GRANTEE.  
  
 Pour déterminer la longueur réelle des colonnes TABLE_CAT, TABLE_SCHEM et TABLE_NAME, une application peut appeler **SQLGetInfo** avec les options SQL_MAX_CATALOG_NAME_LEN, SQL_MAX_SCHEMA_NAME_LEN et SQL_MAX_TABLE_NAME_LEN.  
...
|SUBVENTION (ODBC 1.0)|4|Varchar|Nom de l’utilisateur qui a accordé le privilège; NULL s’il n’est pas applicable à la source de données.<br /><br /> Pour toutes les lignes dans lesquelles la valeur de la colonne GRANTEE est propriétaire de l’objet, la colonne GRANTOR sera "_SYSTEM".|  
|GRANTEE (ODBC 1.0)|5|Varchar pas NULL|Nom de l’utilisateur à qui le privilège a été accordé.|  
|PRIVILÈGE (ODBC 1.0)|6|Varchar pas NULL|Le privilège de la table. Peut être l’un des privilèges suivants ou spécifiques à la source de données.<br /><br /> SÉLECTION : Le bénéficiaire est autorisé à récupérer des données pour une ou plusieurs colonnes de la table.<br /><br /> INSERT: Le bénéficiaire est autorisé à insérer de nouvelles lignes contenant des données pour une ou plusieurs colonnes dans la table.<br /><br /> MISE À JOUR : Le bénéficiaire est autorisé à mettre à jour les données dans une ou plusieurs colonnes du tableau.<br /><br /> DELETE : Le bénéficiaire est autorisé à supprimer les lignes de données de la table.<br /><br /> RÉFÉRENCES : Le bénéficiaire est autorisé à se référer à une ou plusieurs colonnes de la table dans une contrainte (par exemple, une contrainte unique, référentielle ou de vérification de table).<br /><br /> La portée de l’action permise par un privilège de table donné dépend de la source de données. Par exemple, le privilège UPDATE pourrait permettre au bénéficiaire de mettre à jour toutes les colonnes d’un tableau sur une source de données et uniquement les colonnes pour lesquelles le subventionnaire a le privilège UPDATE sur une autre source de données.|  
|IS_GRANTABLE (ODBC 1.0)|7|Varchar|indique si le bénéficiaire est autorisé à accorder le privilège à d’autres utilisateurs; "OUI", "NON", ou NULL s’il est inconnu ou non applicable à la source de données.<br /><br /> Un privilège est soit exausible, soit non exausible, mais pas les deux. Le résultat établi par **SQLColumnPrivileges** ne contiendra jamais deux rangées pour lesquelles toutes les colonnes, à l’exception de la colonne IS_GRANTABLE contiennent la même valeur.|  
