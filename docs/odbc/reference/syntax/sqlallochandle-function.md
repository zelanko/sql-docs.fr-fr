...
## <a name="allocating-a-statement-handle"></a>Allocation d'un descripteur d'instruction  
 ...
 Lorsque le descripteur d’instruction est alloué, le pilote alloue automatiquement un ensemble de quatre descripteurs et assigne les handles de ces descripteurs au SQL_ATTR_APP_ROW_DESC, SQL_ATTR_APP_PARAM_DESC, SQL_ATTR_IMP_ROW_DESC et SQL_ATTR_IMP_PARAM_DESC attributs d’instruction. Celles-ci sont appelées descripteurs alloués de *manière implicite* . Pour allouer explicitement un descripteur d’application, consultez la section « allocation d’un handle de descripteur ».  
