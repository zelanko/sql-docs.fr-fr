...
|HYC00|Fonctionnalité facultative non implémentée|La valeur spécifiée pour l' *attribut* argument était un attribut d’environnement ODBC valide pour la version de ODBC prise en charge par le pilote, mais n’était pas pris en charge par le pilote.<br /><br /> (DM) l’argument d' *attribut* était SQL_ATTR_OUTPUT_NTS et *ValuePtr* était SQL_FALSE.|  
