 ...
 *ColumnCountPtr*  
 [Sortie] Pointeur vers une mémoire tampon dans lequel retourner le nombre de colonnes dans le résultat défini. Ce nombre n’inclut pas une colonne liée de signet.  
  
## <a name="returns"></a>Valeur renvoyée  
 SQL_SUCCESS, SQL_SUCCESS_WITH_INFO, SQL_STILL_EXECUTING, SQL_ERROR ou SQL_INVALID_HANDLE.  
  
## <a name="diagnostics"></a>Diagnostics  
 Lorsque **SQLNumResultCols** retourne SQL_ERROR ou SQL_SUCCESS_WITH_INFO, une valeur SQLSTATE associée peut être obtenu en appelant **SQLGetDiagRec** avec un *HandleType* de SQL_HANDLE_STMT et un *gérer* de *au paramètre StatementHandle*. Le tableau suivant répertorie les valeurs SQLSTATE généralement retournées par **SQLNumResultCols** et explique chacune dans le contexte de cette fonction ; la notation « (DM) » précède les descriptions de SQLSTATE retournée par le Gestionnaire de pilotes. Le code de retour associé à chaque valeur SQLSTATE est SQL_ERROR, sauf indication contraire.  
 ...
 **SQLNumResultCols** peut retourner n’importe quel SQLSTATE qui peut être retourné par **SQLPrepare** ou **SQLExecute** lorsqu’elle est appelée après **SQLPrepare** et avant  **SQLExecute**, en fonction lorsque la source de données évalue l’instruction SQL associée à l’instruction.  
  
## <a name="comments"></a>Commentaires  
 **SQLNumResultCols** peut être appelée avec succès uniquement lorsque l’instruction est dans l’état préparé, exécuté ou positionnée.  
 ...
 Pour plus d’informations, consultez [a un résultat défini créé ?](../../../odbc/reference/develop-app/was-a-result-set-created.md) et [comment les métadonnées sont-elles utilisées ?](../../../odbc/reference/develop-app/how-is-metadata-used.md).  
