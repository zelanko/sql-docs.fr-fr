 ...
 *cursor_name*  
 ...
 SCROLL  
 Spécifie que toutes les fonctions d’extraction (`FIRST`, `LAST`, `PRIOR`, `NEXT`, `RELATIVE`, `ABSOLUTE`) sont disponibles. Si `SCROLL` n’est pas spécifié dans une instruction `DECLARE CURSOR` ISO, seule l’option d’extraction `NEXT` est prise en charge. `SCROLL` ne peut pas être spécifié si `FAST_FORWARD` est également spécifié.  
...
>  Si ni `GLOBAL` ni `LOCAL` ne sont spécifiés, la valeur par défaut est contrôlée par la valeur de l’option de base de données **default to local cursor**.  
 ...
 FORWARD_ONLY  
 Spécifie que le curseur peut seulement défiler de la première à la dernière ligne. `FETCH NEXT` est la seule fonction d’extraction prise en charge. Si vous spécifiez `FORWARD_ONLY` sans les mots clés `STATIC`, `KEYSET` ou `DYNAMIC`, le curseur fonctionne comme un curseur DYNAMIC. Quand ni `FORWARD_ONLY` ni `SCROLL` ne sont spécifiés, `FORWARD_ONLY` est la valeur par défaut, sauf si les mots clés `STATIC`, `KEYSET` ou `DYNAMIC` sont spécifiés. Les curseurs `STATIC`, `KEYSET` et `DYNAMIC` sont par défaut de type `SCROLL`. Contrairement aux API de base de données comme ODBC et ADO, `FORWARD_ONLY` est pris en charge avec les curseurs [!INCLUDE[tsql](../../includes/tsql-md.md)] `STATIC`, `KEYSET` et `DYNAMIC`.  
...
> Si la requête fait référence à au moins une table sans index unique, le curseur de jeu de clés est converti en curseur statique.  
 ...
 FAST_FORWARD  
 Spécifie un curseur `FORWARD_ONLY`, `READ_ONLY` pour lequel les optimisations de performances sont activées. `FAST_FORWARD` ne peut pas être spécifié si `SCROLL` ou `FOR_UPDATE` est également spécifié.  
