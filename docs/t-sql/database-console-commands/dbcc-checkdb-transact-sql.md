...
> [!WARNING]
> L’option REPAIR_ALLOW_DATA_LOSS est une fonctionnalité prise en charge, mais il ne peut pas être toujours la meilleure option pour mettre une base de données dans un état physiquement cohérent. En cas de réussite, l'option REPAIR_ALLOW_DATA_LOSS peut entraîner une perte de données. En fait, elle peut entraîner une perte de données supérieure à celle que vous pourriez constater si un utilisateur restaurait la base de données à partir de la dernière bonne sauvegarde. 
>
...
EXTENDED_LOGICAL_CHECKS  
 ...
 Pour plus d’informations, consultez *effectuant des vérifications de cohérence logique sur les index*, dans le [notes](#remarks) section plus loin dans cette rubrique.  
...
En commençant par [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] SP2, l’exécution de DBCC CHECKDB **n’est plus** efface le cache de plan pour l’instance de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Avant de [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] SP2, l’exécution de DBCC CHECKDB efface le cache du plan. Cette opération entraîne la recompilation de tous les plans d'exécution ultérieurs et peut entraîner une baisse temporaire et brutale des performances des requêtes. 
