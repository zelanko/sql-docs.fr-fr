...
L’exemple suivant défragmente toutes les partitions de la `PK_Product_ProductID` de l’index dans le `Production.Product` de table dans le `AdventureWorks` base de données.
