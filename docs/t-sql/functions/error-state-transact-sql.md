---
title: ERROR_STATE (Transact-SQL) | Documents Microsoft
ms.custom: 
ms.date: 03/16/2017
...
## <a name="remarks"></a>Notes  
 ...
 ERROR_STATE renvoie l'état de l'erreur, indépendamment du nombre d'exécutions ou de l'endroit où elle est exécutée dans le champ du bloc CATCH. Cela diffère des fonctions telles que @@ERROR, qui retourne uniquement le numéro d’erreur dans l’instruction immédiatement après celle qui provoque une erreur, ou dans la première instruction d’un bloc CATCH.  
...
### <a name="a-using-errorstate-in-a-catch-block"></a>A. Utilisation d'ERROR_STATE dans un bloc CATCH  
 L’exemple suivant montre un `SELECT` instruction qui génère une erreur de division par zéro. L'état de l'erreur est renvoyé.  
...
### <a name="b-using-errorstate-in-a-catch-block-with-other-error-handling-tools"></a>B. Utilisation d'ERROR_STATE dans un bloc CATCH avec d'autres outils de gestion des erreurs  
 L’exemple suivant montre un `SELECT` instruction qui génère une erreur de division par zéro. Outre l'état de l'erreur, la procédure renvoie également les informations relatives à l'erreur.  
...
### <a name="c-using-errorstate-in-a-catch-block"></a>C. Utilisation d'ERROR_STATE dans un bloc CATCH  
 L’exemple suivant montre un `SELECT` instruction qui génère une erreur de division par zéro. L'état de l'erreur est renvoyé.  
...
### <a name="d-using-errorstate-in-a-catch-block-with-other-error-handling-tools"></a>D. Utilisation d'ERROR_STATE dans un bloc CATCH avec d'autres outils de gestion des erreurs  
 L’exemple suivant montre un `SELECT` instruction qui génère une erreur de division par zéro. Outre l'état de l'erreur, la procédure renvoie également les informations relatives à l'erreur.  
