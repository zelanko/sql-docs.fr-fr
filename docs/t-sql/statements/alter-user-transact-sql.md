---
title: ALTER USER (Transact-SQL) | Documents Microsoft
ms.custom: 
ms.date: 05/05/2017
 ...
 *nom d’utilisateur*  
 ...
 Spécifie le nouveau nom de cet utilisateur. *Nouveau_nom_utilisateur* ne doit pas exister dans la base de données actuelle.  
...
> [!NOTE]  
>  Cette option est uniquement disponible pour les utilisateurs à relation contenant-contenu. Consultez [Contained Databases](../../relational-databases/databases/contained-databases.md) et [sp_migrate_user_to_contained &#40; Transact-SQL &#41; ](../../relational-databases/system-stored-procedures/sp-migrate-user-to-contained-transact-sql.md) pour plus d’informations.  
  
 OLD_PASSWORD  **=**  *'ancien_mot_de_passe'*  
 **S’applique aux**: [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] via [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)], [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)].  
  
 Le mot de passe utilisateur actuel sera remplacé par «*mot de passe*'. Les mots de passe respectent la casse. *OLD_PASSWORD* est requise pour modifier un mot de passe, sauf si vous avez **ALTER ANY USER** autorisation. Nécessitant *OLD_PASSWORD* empêche les utilisateurs avec **l’emprunt d’identité** autorisation de modifier le mot de passe.  
...
> [!IMPORTANT]  
>  La valeur de DEFAULT_SCHEMA est ignorée si l’utilisateur est membre de la **sysadmin** rôle serveur fixe. Tous les membres de la **sysadmin** rôle serveur fixe possèdent le schéma par défaut `dbo`.  
...
Le nom d’un utilisateur mappé à un [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] connexion, un certificat ou une clé asymétrique ne peut pas contenir le caractère barre oblique inverse (\\).  
...
> [!NOTE]  
>  Un utilisateur qui a **ALTER ANY USER** autorisation permettre modifier le schéma par défaut de n’importe quel utilisateur. Un utilisateur dont le schéma a été modifié peut, sans le savoir, sélectionner des données dans la mauvaise table ou exécuter du code à partir du mauvais schéma.  
  
### <a name="permissions"></a>Permissions  
 Pour modifier le nom d’un utilisateur requiert la **ALTER ANY USER** autorisation.  
  
 Pour modifier la connexion cible d’un utilisateur requiert la **contrôle** autorisation sur la base de données.  
  
 Pour modifier le nom d’utilisateur d’un utilisateur ayant **contrôle** autorisation sur la base de données requiert le **contrôle** autorisation sur la base de données.  
  
 Pour modifier le schéma par défaut ou la langue nécessite **ALTER** autorisation sur l’utilisateur. Les utilisateurs peuvent modifier leur propre schéma ou langue par défaut.  
  
## <a name="examples"></a>Exemples  

Tous les exemples sont exécutées dans une base de données utilisateur.  
