...
> [!IMPORTANT]
> Les connexions AD Azure pour SQL Database Managed Instance sont en **préversion publique**. Seules les options suivantes sont prises en charge pour Azure SQL Database Managed Instance lors de l’application à des utilisateurs avec des connexions Azure AD : `DEFAULT_SCHEMA = { schemaName | NULL }` et `DEFAULT_LANGUAGE = { NONE | lcid | language name | language alias }`
