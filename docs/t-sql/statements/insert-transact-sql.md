---
title: INSERT (Transact-SQL) | Documents Microsoft
ms.custom: 
ms.date: 08/10/2017
...
## <a name="arguments"></a>Arguments  
 ...
 Spécifie le jeu de résultats nommé temporaire, également appelé expression de table commune, défini dans l'étendue de l'instruction INSERT. Le jeu de résultats est dérivé d'une instruction SELECT. Pour plus d’informations, consultez [avec common_table_expression &#40; Transact-SQL &#41; ](../../t-sql/queries/with-common-table-expression-transact-sql.md).  
 ...
 **S'applique à**: [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] et [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
 ...
 Nom du serveur lié sur lequel se trouve la table ou la vue. *nom_serveur* peut être spécifié comme un [serveur lié](../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md) nom, ou à l’aide de la [OPENDATASOURCE](../../t-sql/functions/opendatasource-transact-sql.md) (fonction).  
  
 Lorsque *nom_serveur* est spécifié comme un serveur lié, *nom_base_de_données* et *nom_schéma* sont requis. Lorsque *nom_serveur* est spécifié avec OPENDATASOURCE, *nom_base_de_données* et *schema_name* ne peuvent pas s’appliquer à toutes les sources de données et est soumise aux capacités du fournisseur OLE DB qui accède à l’objet distant.  
 ...
 *table_or view_name*  
 ...
 A [table](../../t-sql/data-types/table-transact-sql.md) variable, dans son étendue, peut être utilisée comme source de table dans une instruction INSERT.  
  
 La vue référencée par *nom_table_ou_vue* doit être mis à jour et faire référence exactement à une table de base dans la clause FROM de la vue. Par exemple, une instruction INSERT dans une vue à plusieurs tables doit utiliser un *column_list* qui référence uniquement les colonnes d’une table de base. Pour plus d’informations sur les vues de mettre à jour, consultez [CREATE VIEW &#40; Transact-SQL &#41; ](../../t-sql/statements/create-view-transact-sql.md).  
  
 *rowset_function_limited*  
 **S'applique à**: [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] et [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
  
 Est soit le [OPENQUERY](../../t-sql/functions/openquery-transact-sql.md) ou [OPENROWSET](../../t-sql/functions/openrowset-transact-sql.md) (fonction). L'utilisation de ces fonctions dépend des fonctionnalités du fournisseur OLE DB qui accède à l'objet distant.  
...
>  La possibilité de spécifier les indicateurs HOLDLOCK, SERIALIZABLE, READCOMMITTED, REPEATABLEREAD ou UPDLOCK sur les tables qui sont des cibles d'instructions INSERT sera supprimée dans une future version de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Ces indicateurs n'affectent pas les performances des instructions INSERT. Évitez de les utiliser dans les nouveaux travaux de développement et prévoyez la modification des applications qui les utilisent actuellement.  
 ...
 (*column_list*)  
 Liste d'une ou plusieurs colonnes dans lesquelles insérer des données. *column_list* doit être placée entre parenthèses et délimitée par des virgules.  
  
 Si une colonne n’est pas dans *column_list*, la [!INCLUDE[ssDE](../../includes/ssde-md.md)] doit être en mesure de fournir une valeur basée sur la définition de la colonne ; sinon, la ligne ne peut pas être chargée. Le [!INCLUDE[ssDE](../../includes/ssde-md.md)] fournit automatiquement une valeur pour la colonne si :  
...
-   A un **timestamp** type de données. la valeur d'horodateur actuelle est utilisée ;  
  
-   Autorise la valeur NULL. Une valeur Null est utilisée.  
  
-   Colonne calculée. la valeur calculée est utilisée.  
  
*column_list* doit être utilisé lors de l’insertion de valeurs explicites dans une colonne d’identité et l’option SET IDENTITY_INSERT doit avoir la valeur ON pour la table.  
...
Clause OUTPUT  
 ...
 Le [clause OUTPUT](../../t-sql/queries/output-clause-transact-sql.md) n’est pas pris en charge dans les instructions DML qui font référence à des vues partitionnées locales, des vues partitionnées distribuées ou des tables distantes ou des instructions INSERT qui contiennent un *execute_statement*. La clause OUTPUT INTO n’est pas pris en charge dans les instructions INSERT qui contiennent un \<dml_table_source > clause. 
  
 VALUES  
 Présente la ou les listes de valeurs de données à insérer. Il doit y avoir une valeur de données pour chaque colonne de *column_list*, si spécifié, ou dans la table. La liste de valeurs doit être mise entre parenthèses.  
  
 Si les valeurs dans la liste de valeurs ne sont pas dans le même ordre que les colonnes de la table ou que vous n’avez pas une valeur pour chaque colonne dans la table, *column_list* doit être utilisé pour spécifier explicitement la colonne qui stocke chaque valeur entrante.  
 ...
 DEFAULT  
 Force le [!INCLUDE[ssDE](../../includes/ssde-md.md)] à charger la valeur par défaut définie pour une colonne. S'il n'existe pas de valeur par défaut pour la colonne et si celle-ci autorise les valeurs NULL, NULL est inséré. Pour une colonne définie avec la **timestamp** type de données, la valeur d’horodateur suivante est insérée. DEFAULT n'est pas valide pour une colonne d'identité.  
 ...
 *execute_statement* peut être utilisé pour exécuter des procédures stockées sur le même serveur ou sur un serveur distant. La procédure du serveur distant est exécutée et les jeux de résultats sont retournés au serveur local où ils sont chargés dans la table. Dans une transaction distribuée, *execute_statement* ne peut pas être émise sur un serveur lié en boucle lors de la connexion a plusieurs jeux de résultats actifs (MARS) activé.  
  
 Si *execute_statement* retourne des données avec l’instruction READTEXT, chaque instruction READTEXT peut retourner un maximum de 1 Mo (1 024 Ko) de données. *execute_statement* peut également être utilisé avec les procédures étendues. *execute_statement* insère les données retournées par le thread principal de la procédure étendue ; Toutefois, les sorties de threads autres que le thread principal ne sont pas insérées.  
...
-   Elle ne peut pas participer à la réplication de fusion ou à des abonnements pouvant être mis à jour pour la réplication transactionnelle.  
 ...
 \<select_list >  
 Liste séparée par des virgules qui spécifie les colonnes retournées par la clause OUTPUT qu'il convient d'insérer. Les colonnes de \<select_list > doivent être compatibles avec les colonnes dans lesquelles les valeurs sont insérées. \<select_list > ne peut pas référencer des fonctions d’agrégation ni à TEXTPTR. 
  
> [!NOTE]  
>  Toutes les variables répertoriées dans la liste SELECT font référence à leurs valeurs d’origine, indépendamment de toutes les modifications apportées des \<dml_statement_with_output_clause >.  
 ...
 OÙ \<search_condition >  
 Est toute clause WHERE contenant une valide \<search_condition > qui filtre les lignes retournées par \<dml_statement_with_output_clause >. Pour plus d’informations, consultez [Condition de recherche &#40; Transact-SQL &#41; ](../../t-sql/queries/search-condition-transact-sql.md). Lorsqu’il est utilisé dans ce contexte, \<search_condition > ne peut pas contenir de sous-requêtes, fonctions scalaires définies par l’utilisateur qui effectuent l’accès aux données, les fonctions d’agrégation, TEXTPTR ou des prédicats de recherche en texte intégral. 
...
**S'applique à**: [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] et [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
 ...
 Utilisé par les outils externes pour télécharger un flux de données binaires. Cette option n’est pas destinée pour une utilisation avec des outils tels que [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)], SQLCMD, OSQL ou des données accès aux interfaces de programmation d’application telles que [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client.  
 ...
 **S'applique à**: [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] et [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
 ...
 Spécifie le nombre approximatif de kilo-octets (Ko) de données par lot en tant que *kilobytes_per_batch*. Pour plus d’informations, consultez [BULK INSERT &#40;Transact-SQL&#41;](../../t-sql/statements/bulk-insert-transact-sql.md).  
...
>  [!NOTE]
>  Une erreur de syntaxe est générée si une liste de colonnes n’est pas fournie.  

## <a name="remarks"></a>Notes  
Pour plus d’informations spécifiques à l’insertion de données dans des tables SQL graph, consultez [INSERT (graphique SQL)](../../t-sql/statements/insert-sql-graph.md). 
...
#### <a name="using-insert-intoselect-to-bulk-import-data-with-minimal-logging"></a>Utilisation d'INSERT INTO…SELECT pour les données d'importation en bloc avec une journalisation minimale  
 Vous pouvez utiliser `INSERT INTO <target_table> SELECT <columns> FROM <source_table>` pour transférer efficacement un grand nombre de lignes d’une table, telles qu’une table intermédiaire, à une autre table avec une journalisation minimale. La journalisation minimale peut améliorer les performances de l'instruction et réduire le risque de voir l'opération remplir l'espace disponible du journal des transactions au cours de la transaction.  
...
-   Si une valeur est chargée dans des colonnes ayant un **char**, **varchar**, ou **varbinary** type de données, le remplissage ou la troncation des espaces de fin (espaces pour **char** et **varchar**, zéros pour **varbinary**) est déterminé par la valeur de SET ANSI_PADDING définie pour la colonne lorsque la table a été créée. Pour plus d’informations, consultez [SET ANSI_PADDING &#40;Transact-SQL&#41;](../../t-sql/statements/set-ansi-padding-transact-sql.md).  
    ...
    |**varchar**|Espaces de fin de la suppression pour le dernier caractère autre qu’un espace ou un caractère pour les chaînes composées uniquement d’espaces d’espacement simple.|  
    |**varbinary**|Supprime les zéros à droite.|  
  
-   Si une chaîne vide (' ') est chargé dans une colonne avec un **varchar** ou **texte** type de données, l’opération par défaut est pour charger une chaîne de longueur nulle.  
  
-   Insertion d’une valeur null dans une **texte** ou **image** colonne ne crée pas un pointeur de texte valide et ne prédéfinit pas une page texte de 8 Ko.  
  
-   Les colonnes créées avec le **uniqueidentifier** magasin de type de données spécialement mise en forme de valeurs binaires de 16 octets. Contrairement aux colonnes d’identité, le [!INCLUDE[ssDE](../../includes/ssde-md.md)] ne génère pas automatiquement de valeurs pour les colonnes avec la **uniqueidentifier** type de données. Pendant une opération d’insertion, les variables comportant des données le type de **uniqueidentifier** et constantes sous la forme de chaîne *xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx* (36 caractères, tirets inclus, où *x* est un chiffre hexadécimal dans la plage 0-9 ou a-f) peut être utilisé pour **uniqueidentifier** colonnes. Par exemple, 6F9619FF-8B86-D011-B42D-00C04FC964FF est une valeur valide pour un **uniqueidentifier** variable ou une colonne. Utilisez le [NEWID()](../../t-sql/functions/newid-transact-sql.md) fonction pour obtenir un ID global unique (GUID).  
...
Insérez des requêtes qu’utilisation de SELECT avec ORDER BY pour remplir les garanties de lignes comment les valeurs d’identité sont calculées, mais pas l’ordre dans lequel les lignes sont insérées.    
  
## <a name="logging-behavior"></a>Comportement de journalisation  
 L’instruction INSERT est toujours entièrement journalisée, sauf lors de l’utilisation de la fonction OPENROWSET avec le mot clé BULK ou lors de l’utilisation `INSERT INTO <target_table> SELECT <columns> FROM <source_table>`. Ces opérations peuvent faire l'objet d'une journalisation minimale. Pour plus d'informations, consultez la section « Recommandations pour le chargement en masse des données », précédemment dans cette rubrique.  
  
## <a name="security"></a>Sécurité  
 Au cours d'une connexion à un serveur lié, le serveur émetteur fournit un nom et un mot de passe de connexion afin de se connecter au serveur récepteur. Pour cette connexion fonctionne, vous devez créer un mappage de connexion entre les serveurs liés à l’aide de [sp_addlinkedsrvlogin](../../relational-databases/system-stored-procedures/sp-addlinkedsrvlogin-transact-sql.md).  
  
 Lorsque vous utilisez OPENROWSET(BULK…), il est important que vous compreniez la manière dont [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] gère l'emprunt d'identité. Pour plus d’informations, consultez « Considérations de sécurité » dans [importer en bloc des données par l’utilisation de BULK INSERT ou OPENROWSET &#40; BULK... &#41; &#40; SQL Server &#41; ](../../relational-databases/import-export/import-bulk-data-by-using-bulk-insert-or-openrowset-bulk-sql-server.md).  
...
|[La gestion des valeurs de colonne](#ColumnValues)|IDENTITY • NEWID • valeurs par défaut • types définis par l'utilisateur|  
|[Insertion de données provenant d’autres tables](#OtherTables)|INSERT…SELECT • INSERT…EXECUTE • WITH expression de table commune • TOP • OFFSET FETCH|  
|[Spécification d’objets cibles autres que les tables standards](#TargetObjects)|Vues • variables de table|  
...
|[En bloc des données de chargement à partir de fichiers de données ou de tables](#BulkLoad)|INSERT…SELECT • fonction OPENROWSET|  
|[Substituer le comportement par défaut de l’optimiseur de requête à l’aide d’indicateurs](#TableHints)|Indicateurs de table|  
...
#### <a name="c-inserting-data-that-is-not-in-the-same-order-as-the-table-columns"></a>C. Insertion de données qui ne sont pas dans le même ordre que les colonnes de la table  
 L'exemple suivant utilise une liste de colonnes afin de spécifier de manière explicite les valeurs insérées dans chaque colonne. L’ordre des colonnes dans le `Production.UnitMeasure` de table dans le [!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)] base de données est `UnitMeasureCode`, `Name`, `ModifiedDate`; Toutefois, les colonnes ne sont pas répertoriées dans cet ordre en *column_list*.  
...
###  <a name="ColumnValues"></a>La gestion des valeurs de colonne  
 Exemples de cette section présentent des méthodes d’insertion de valeurs dans des colonnes qui sont définies avec une propriété IDENTITY, la valeur par défaut, ou qui sont définis avec des types de données tels que **uniqueidentifer** ou les colonnes de type défini par l’utilisateur.  
...
#### <a name="f-inserting-data-into-a-uniqueidentifier-column-by-using-newid"></a>F. Insertion de données dans une colonne uniqueidentifier en utilisant NEWID()  
 L’exemple suivant utilise le [NEWID](../../t-sql/functions/newid-transact-sql.md)(fonction) () pour obtenir un GUID pour `column_2`. Contrairement aux colonnes d’identité, le [!INCLUDE[ssDE](../../includes/ssde-md.md)] ne génère pas automatiquement de valeurs pour les colonnes avec la [uniqueidentifier](../../t-sql/data-types/uniqueidentifier-transact-sql.md) type de données, comme indiqué par le deuxième `INSERT` instruction.  
...
###  <a name="OtherTables"></a>Insertion de données provenant d’autres Tables  
...
#### <a name="k-inserting-data-by-specifying-a-view"></a>K. Insertion de données en spécifiant une vue  
 L'exemple suivant spécifie un nom de vue comme objet cible ; cependant, la nouvelle ligne est insérée dans la table de base sous-jacente. L'ordre des valeurs dans l'instruction `INSERT` doit correspondre à l'ordre des colonnes de la vue. Pour plus d’informations, consultez [modifier les données via une vue](../../relational-databases/views/modify-data-through-a-view.md).  
...
#### <a name="n-inserting-data-into-a-remote-table-by-using-the-openquery-function"></a>N. Insertion de données dans une table distante en utilisant la fonction OPENQUERY  
 L’exemple suivant insère une ligne dans une table distante en spécifiant le [OPENQUERY](../../t-sql/functions/openquery-transact-sql.md) fonction d’ensemble de lignes. Le nom de serveur lié créé dans l'exemple précédent est utilisé dans cet exemple.  
...
###  <a name="BulkLoad"></a>Chargement des données à partir des Tables ou des fichiers de données en bloc  
...
#### <a name="r-using-the-openrowset-function-with-bulk-to-bulk-load-data-into-a-table"></a>R. Utilisation de la fonction OPENROWSET avec BULK pour charger en masse des données dans une table  
 L'exemple suivant insère des lignes d'un fichier de données dans une table en spécifiant la fonction OPENROWSET. L'indicateur de table IGNORE_TRIGGERS est spécifié en vue d'optimiser les performances. Pour plus d’exemples, consultez [importer en bloc des données par l’utilisation de BULK INSERT ou OPENROWSET &#40; BULK... &#41; &#40; SQL Server &#41; ](../../relational-databases/import-export/import-bulk-data-by-using-bulk-insert-or-openrowset-bulk-sql-server.md).  
...
###  <a name="TableHints"></a>Substituer le comportement par défaut de l’optimiseur de requête à l’aide d’indicateurs  
 Exemples de cette section montrent comment utiliser [indicateurs de table](../../t-sql/queries/hints-transact-sql-table.md) pour remplacer temporairement le comportement par défaut de l’optimiseur de requête lors du traitement de l’instruction INSERT.  
...
**S’applique aux**: [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)], [!INCLUDE[ssSDS_md](../../includes/sssds-md.md)].  
...
#### <a name="w-inserting-data-using-the-select-option"></a>W. Insertion de données à l’aide de l’option Sélectionner  
 L’exemple suivant montre comment insérer plusieurs lignes de données à l’aide d’une instruction INSERT avec une option de sélection. La première `INSERT` instruction utilise un `SELECT` instruction directement pour récupérer des données à partir de la table source, puis définissez pour stocker le résultat dans le `EmployeeTitles` table.  
