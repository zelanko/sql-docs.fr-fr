...
-   Dans SQL Server 2016 et base de données SQL Azure, utilisez un index non cluster sur un index columnstore pour améliorer les performances des requêtes d’entreposage de données. Consultez [Columnstore index - Data Warehouse](../../relational-databases/indexes/columnstore-indexes-data-warehouse.md)  
...
>  Opérations d’index parallèles ne sont pas disponibles dans toutes les éditions de [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Pour obtenir la liste des fonctionnalités prises en charge par les éditions de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], consultez [Fonctionnalités prises en charge par les éditions de SQL Server 2016](../../sql-server/editions-and-supported-features-for-sql-server-2016.md).  
 ...
 SUR les PARTITIONS **(** { \<partition_number_expression > | \<plage >} [ **,**...  *n*  ] **)** **S’applique aux**: [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] via [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] et [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)].  
...
-   Un index cluster ou non cluster rowstore sur une table de l’entrepôt de données lorsqu’il existe déjà un index columnstore. Ce comportement est différent de SMP SQL Server qui permet à un index rowstore et columnstore de coexister sur la même table.  
...
## <a name="version-notes"></a>Notes de version  
 Base de données SQL ne prend pas en charge les options de groupe de fichiers et de filestream.  
...
### <a name="p-add-a-column-to-an-index"></a>P. Ajouter une colonne à un index  
 L’exemple suivant crée un index IX_FF avec deux colonnes à partir de la table dbo. Table FactFinance.  L’instruction suivante montre la reconstruction des index avec le même nom et une colonne supplémentaire.  
