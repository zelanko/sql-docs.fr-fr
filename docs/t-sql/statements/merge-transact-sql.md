...
### <a name="e-using-merge-to-perform-insert-or-update-on-a-target-edge-table-in-a-graph-database"></a>E. Utilisation de MERGE pour effectuer une opération INSERT ou UPDATE sur une table d’arêtes cible dans une base de données de graphes
Dans cet exemple, nous créons des tables de nœuds `Person` et `City` et une table d’arêtes `livesIn`. Nous utiliserons l’instruction MERGE sur l’arête `livesIn` pour insérer une nouvelle ligne si l’arête n’existe pas encore entre une `Person` et `City`. Si l’arête existe déjà, nous mettrons simplement à jour l’attribut StreetAddress sur l’arête `livesIn`.
