---
title: DROP TABLE (Transact-SQL) | Documents Microsoft
ms.custom: 
ms.date: 05/12/2017
...
[!INCLUDE[tsql-appliesto-ss2008-all-md](../../includes/tsql-appliesto-ss2008-all-md.md)]
  ...
  Supprime une ou plusieurs définitions de table ainsi que toutes les données, index, déclencheurs, contraintes et spécifications d'autorisation se rapportant à celles-ci. Toute vue ou procédure stockée qui fait référence à la table supprimée doit être supprimée explicitement à l’aide de [DROP VIEW](../../t-sql/statements/drop-view-transact-sql.md) ou [DROP PROCEDURE](../../t-sql/statements/drop-procedure-transact-sql.md). Pour signaler les dépendances sur une table, utilisez [sys.dm_sql_referencing_entities](../../relational-databases/system-dynamic-management-views/sys-dm-sql-referencing-entities-transact-sql.md).  
 ...
 **S’applique à**: [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ([!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] jusqu’à [version actuelle](http://go.microsoft.com/fwlink/p/?LinkId=299658)).  
 ...
 Conditionnellement supprime la table uniquement s’il existe déjà.  
...
## <a name="remarks"></a>Notes  
 ...
 Si vous supprimez toutes les lignes dans une table à l’aide de DELETE *tablename* ou utilisez l’instruction TRUNCATE TABLE, la table existe jusqu'à ce qu’il est supprimé.  
...
### <a name="c-dropping-a-temporary-table"></a>C. Suppression d'une table temporaire  
 Cet exemple crée une table temporaire, teste son existence, la supprime et teste une nouvelle fois son existence. Cet exemple n’utilise pas le **IF EXISTS** syntaxe qui est disponible à partir de [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)].  
...
**S’applique à**: [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ([!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] jusqu’à [version actuelle](http://go.microsoft.com/fwlink/p/?LinkId=299658)).  
 ...
 L’exemple suivant crée une table nommée T1. La deuxième instruction dépose ensuite la table. La troisième instruction n’effectue aucune action, car la table est déjà supprimée, mais cela n’entraîne pas une erreur.  
