---
title: "CRÉER le schéma (Transact-SQL) | Documents Microsoft"
ms.custom: 
ms.date: 12/01/2016
...
> [!NOTE]  
>  Instructions qui contiennent CREATE SCHEMA AUTHORIZATION mais ne spécifient pas un nom, sont autorisées pour la compatibilité descendante uniquement. L'instruction ne génère pas d'erreur, mais un schéma n'est pas créé.  
 ...
 **Création de l’utilisateur et de schéma implicite**  
...
>  [!NOTE]
>  La création implicite d’un utilisateur Azure Active Directory n’est pas possible sur [!INCLUDE[ssSDS_md](../../includes/sssds-md.md)]. Étant donné que la création d’un utilisateur Azure AD à partir du fournisseur externe doit vérifier l’état des utilisateurs dans le DAS, création de l’utilisateur échoue avec l’erreur 2760 : **le nom de schéma spécifié «\<user_name@domain> » n’existe pas ou vous n’êtes pas autorisé à le faire.** Et puis erreur 2759 : **Échec de CREATE SCHEMA en raison d’erreurs précédentes.** Pour résoudre ces erreurs, créez l’utilisateur Azure AD à partir du fournisseur externe d’abord, puis réexécutez l’instruction de création de l’objet.
...
### <a name="a-creating-a-schema-and-granting-permissions"></a>A. Création d’un schéma et l’octroi d’autorisations  
...
### <a name="b-creating-a-schema-and-a-table-in-the-schema"></a>B. Création d’un schéma et une table dans le schéma  
 L’exemple suivant crée un schéma `Sales` , puis crée une table `Sales.Region` dans ce schéma.  
...
### <a name="c-setting-the-owner-of-a-schema"></a>C. Définir le propriétaire d’un schéma  
