...
```
L'identifiant utilisé doit être créé en utilisant `SHARED ACCESS SIGNATURE` comme identité, ne doit pas avoir le premier `?` dans le jeton SAS, doit avoir au moins les droits de lecture sur le fichier à charger (par exemple `srt=o&sp=r`), et la période d'expiration doit être valide (toutes les dates sont en heure UTC). Par exemple :
```sql
...
>[!NOTE] 
>Pour effectuer le chargement du Stockage Blob Azure vers SQL DW ou Parallel Data Warehouse, le secret doit être la clé de stockage Azure.

  
 RESOURCE_MANAGER_LOCATION = '*ResourceManager_URI*[:*port*]'  
 Spécifie l’emplacement du Gestionnaire de ressources Hadoop. S’il est spécifié, l’optimiseur de requête peut prendre la décision basée sur le coût de prétraiter les données d’une requête PolyBase en utilisant les fonctionnalités de calcul d’Hadoop avec MapReduce. Appelée pushdown de prédicats, cette opération peut considérablement réduire le volume des données transférées entre Hadoop et SQL, et donc améliorer les performances des requêtes.  
...
Actuellement, un jeton SAS de type `hadoop` n’est pas pris en charge. Il n’est pris en charge qu’avec une clé d'accès au compte de stockage. Une tentative de créer une source de données externe de type `hadoop` et l’utilisation d’informations d'identification SAS peuvent échouer avec l’erreur :
...
```   
Pour voir une utilisation de cet exemple, consultez [BULK INSERT](../../t-sql/statements/bulk-insert-transact-sql.md#f-importing-data-from-a-file-in-azure-blob-storage).
