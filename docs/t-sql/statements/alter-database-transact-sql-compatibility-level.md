...
> [!NOTE]  
> **Azure [!INCLUDE[ssSDS](../../includes/sssds-md.md)]**  V12 a été publiée en décembre 2014. Un aspect de cette version était que les bases de données nouvellement créées sont leur niveau de compatibilité à 120. Base de données SQL 2015 compter prise en charge pour le niveau 130, bien que la valeur par défaut est restée à 120.  
> 
> À compter de **mid-juin 2016**, dans [!INCLUDE[ssSDS](../../includes/sssds-md.md)], le niveau de compatibilité par défaut sont 130 au lieu de 120 pour **nouvellement créé** bases de données. Bases de données existants créés avant mid-juin 2016 ne sont pas affectés et à maintenir leur niveau de compatibilité actuel (100, 110 et 120). 
> 
...
>  
>  Pour plus d’informations sur la façon d’évaluer les différences de performances de vos requêtes importantes, entre deux niveaux de compatibilité sur [!INCLUDE[ssSDS](../../includes/sssds-md.md)], consultez [amélioré les performances de requête avec 130 de niveau de compatibilité de base de données SQL Azure](http://azure.microsoft.com/documentation/articles/sql-database-compatibility-level-query-performance-130/).
...
> [!IMPORTANT]
> Fonctionnalités introduites dans supprimées une donnée [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] version n’est pas protégée par le niveau de compatibilité.
> 
> Par exemple, le `FASTFIRSTROW` indicateur a été abandonné dans [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] et remplacée par la `OPTION (FAST n )` indicateur. Définir le niveau de compatibilité de base de données à 110 ne restaure pas l’indicateur supprimée.
> Pour plus d’informations sur les fonctionnalités supprimées, consultez [fonctionnalités du moteur de base de données supprimées dans SQL Server 2016](../../database-engine/discontinued-database-engine-functionality-in-sql-server-2016.md), [fonctionnalités du moteur de base de données supprimées dans SQL Server 2014](http://msdn.microsoft.com/library/ms144262(v=sql.120)), [Supprimées des fonctionnalités du moteur de base de données dans SQL Server 2012](http://msdn.microsoft.com/library/ms144262(v=sql.110)), et [supprimées des fonctionnalités du moteur de base de données dans SQL Server 2008](http://msdn.microsoft.com/library/ms144262(v=sql.100)).

> [!IMPORTANT]
> Modifications avec rupture introduites dans une donnée [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] version **peut** ne pas être protégé par le niveau de compatibilité. [!INCLUDE[tsql](../../includes/tsql-md.md)]comportement est généralement protégé par niveau de compatibilité. Toutefois, les objets système modifiés ou supprimés sont **pas** protégé par le niveau de compatibilité.
>
> Un exemple d’une modification avec rupture **protégé** par la compatibilité de niveau est une conversion implicite de datetime aux types de données datetime2. Sous le niveau de compatibilité de base de données 130, ils apparaissent précision améliorée par la gestion des comptes pour les fractions de milliseconde, résultant dans différentes valeurs converties. Pour restaurer le comportement précédent de la conversion, définissez le niveau de compatibilité de base de données à 120 ou inférieure.
>
> Exemples de modifications avec rupture **non protégé** par la compatibilité de niveau sont :
> -  Noms de colonne modifiée dans les objets système. Dans [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] la colonne *single_pages_kb* dans sys.dm_os_sys_info a été renommé en *pages_kb*. Quel que soit le niveau de compatibilité, la requête `SELECT single_pages_kb FROM sys.dm_os_sys_info` génère l’erreur 207 (nom de colonne non valide).
> -  Objets système supprimées. Dans [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] le `sp_dboption` a été supprimé. Quel que soit le niveau de compatibilité, l’instruction `EXEC sp_dboption 'AdventureWorks2016CTP3', 'autoshrink', 'FALSE';` génère l’erreur 2812 (pu trouver la procédure stockée 'sp_dboption').
>
> Pour plus d’informations sur les modifications avec rupture, consultez [modifications avec rupture des fonctionnalités du moteur de base de données dans SQL Server 2017](../../database-engine/breaking-changes-to-database-engine-features-in-sql-server-2017.md), [modifications avec rupture des fonctionnalités du moteur de base de données dans SQL Server 2016](../../database-engine/breaking-changes-to-database-engine-features-in-sql-server-2016.md), [ Des modifications à la base de données du moteur de fonctionnalités de SQL Server 2014](http://msdn.microsoft.com/library/ms143179(v=sql.120)), [changements essentiels apportés aux fonctionnalités du moteur de base de données dans SQL Server 2012](http://msdn.microsoft.com/library/ms143179(v=sql.110)), et [changements essentiels apportés aux fonctionnalités du moteur de base de données SQL Server 2008](http://msdn.microsoft.com/library/ms143179(v=sql.100)).
...
|Le `sp_execute_external_script` UDX (opérateur) ne peut être exécuté en mode ligne.|Le `sp_execute_external_script` UDX (opérateur) est éligible pour une exécution en mode batch.|  
|Fonctions table de plusieurs instructions (TVF) n’ont pas l’exécution entrelacée |Exécution entrelacée de multi-instruction améliorer la qualité du plan. |
...
|Les références récursives dans la partie droite d’une `EXCEPT` clause créer une boucle infinie. Exemple C dans la section exemples ci-dessous illustre ce comportement.|Fait référence à récursive dans une `EXCEPT` clause génère une erreur conformément à la norme SQL ANSI.|  
|Expression de table commune récursive (CTE) autorise les noms de colonne en double.|Les expressions CTE récursives n'autorisent pas les noms de colonnes en double.|  
...
|La clause de table OUTPUT INTO ignore le `IDENTITY_INSERT SETTING = OFF` et permet l’insertion de valeurs explicites.|Impossible d’insérer des valeurs explicites pour une colonne d’identité dans une table lorsque `IDENTITY_INSERT` est définie sur OFF.|  
|Lorsque la relation contenant-contenu de base de données est définie à partielle, valider le `$action` champ dans le `OUTPUT` clause d’un `MERGE` instruction peut retourner une erreur de classement.|Le classement des valeurs retournées par la `$action` clause d’un `MERGE` instruction est le classement de base de données au lieu du classement du serveur et une erreur de conflit de classement n’est pas retournée.|  
...
|`PIVOT`est autorisée dans une requête de l’expression (CTE) de table commune récursive. Cependant, la requête retourne des résultats incorrects lorsqu'il existe plusieurs lignes par regroupement.|`PIVOT`n’est pas autorisée dans une requête de l’expression (CTE) de table commune récursive. Une erreur est retournée.|  
...
|`SOUNDEX`la fonction implémente les règles suivantes :<br /><br /> (1) MAJUSCULE H ou W majuscules sont ignorés lors de la séparation de deux consonnes qui ont le même numéro dans le `SOUNDEX` code.<br /><br /> 2) si les 2 premiers caractères de *character_expression* ont le même numéro dans le `SOUNDEX` code, les deux caractères sont inclus. Sinon, si un jeu de consonnes de côte-à-côte ont le même numéro dans le `SOUNDEX` code, toutes sont exclues, sauf le premier.|`SOUNDEX`la fonction implémente les règles suivantes :<br /><br /> (1) si les majuscules H ou majuscule W séparent deux consonnes qui ont le même numéro dans le `SOUNDEX` code, la consonne à droite est ignoré.<br /><br /> (2) si un jeu de consonnes de côte-à-côte ont le même numéro dans le `SOUNDEX` code, toutes sont exclues, sauf le premier.<br /><br /> <br /><br /> Les règles supplémentaires peuvent générer des valeurs calculées par la `SOUNDEX` fonction qui sont différentes des valeurs calculées sous des niveaux de compatibilité antérieurs. Après la mise à niveau vers le niveau de compatibilité 110, vous devrez peut-être reconstruire les index, de segments de mémoire ou de contraintes CHECK qui utilisent la `SOUNDEX` (fonction). Pour plus d’informations, consultez [SOUNDEX &#40; Transact-SQL &#41;](../../t-sql/functions/soundex-transact-sql.md)|  
...
|Le `FOR BROWSE` clause est autorisée (et ignorée) dans `INSERT` et `SELECT INTO` instructions.|Le `FOR BROWSE` clause n’est pas autorisée dans `INSERT` et `SELECT INTO` instructions.|Moyenne|  
|Les prédicats de recherche en texte intégral sont autorisés dans les `OUTPUT` clause.|Prédicats de texte intégral ne sont pas autorisés dans les `OUTPUT` clause.|Faible|  
...
|`MERGE`n’est pas appliquée comme un mot clé réservé.|MERGE est un mot clé entièrement réservé. La `MERGE` instruction est pris en charge sous les niveaux de compatibilité 100 et 90.|Faible|  
...
|À moins que `NOINDEX` est spécifié, `DBCC CHECKDB` ou `DBCC CHECKTABLE` effectue des vérifications de cohérence physique et logique sur une seule table ou la vue indexée et sur tous ses non ordonnés en clusters et les index XML. Les index spatiaux ne sont pas pris en charge.|À moins que `NOINDEX` est spécifié, `DBCC CHECKDB` ou `DBCC CHECKTABLE` effectue les vérifications de cohérence physique et logique sur une seule table et tous ses index non-cluster. Toutefois, seules des vérifications de cohérence physique sont effectuées par défaut sur les index XML, les index spatiaux et les vues indexées.<br /><br /> Si `WITH EXTENDED_LOGICAL_CHECKS` est spécifié, les vérifications logiques sont effectuées sur les vues indexées, les index XML et les index spatiaux, le cas échéant. Par défaut, les vérifications de cohérence physique sont effectuées avant les vérifications de cohérence logique. Si `NOINDEX` est également spécifié, seules les vérifications logiques sont effectuées.|Faible|  
|Lorsqu'une clause OUTPUT est utilisée avec une instruction des langages de manipulation de données (DML) et une erreur d'exécution se produit pendant l'exécution d'instruction, la transaction complète est terminée et restaurée.|Lorsqu’un `OUTPUT` clause est utilisée avec une instruction data manipulation language (DML) et une erreur d’exécution se produit pendant l’exécution des instructions, le comportement varie selon le `SET XACT_ABORT` paramètre. Si `SET XACT_ABORT` a la valeur OFF, une erreur d’annulation d’instruction générée par l’instruction DML à l’aide du `OUTPUT` clause mettra fin à l’instruction, mais l’exécution du lot continue et la transaction n’est pas restaurée. Si `SET XACT_ABORT` a la valeur ON, toutes les erreurs d’exécution générées par l’instruction DML à l’aide de la clause OUTPUT termineront le lot, et la transaction est restaurée.|Faible|  
...
|Les propriétés des colonnes `ROWGUIDCOL` et `IDENTITY` peut être nommé incorrectement en tant que contrainte. Par exemple, l'instruction `CREATE TABLE T (C1 int CONSTRAINT MyConstraint IDENTITY)` s'exécute, mais le nom de contrainte n'est pas conservé et n'est pas accessible à l'utilisateur.|Les propriétés des colonnes `ROWGUIDCOL` et `IDENTITY` ne peut pas être nommé en tant que contrainte. L'erreur 156 est retournée.|Faible|  
|Mise à jour de colonnes en utilisant une affectation bidirectionnelle telle que `UPDATE T1 SET @v = column_name = <expression>` peut produire des résultats inattendus, car la valeur dynamique de la variable peut être utilisée dans d’autres clauses telles que la `WHER`E et `ON` clause pendant l’exécution d’instruction à la place de la valeur de départ d’instruction. Cette opération peut modifier les significations des prédicats de façon imprévisible et ligne par ligne.<br /><br /> Ce comportement est applicable uniquement lorsque le niveau de compatibilité est défini à 90.|La mise à jour de colonnes en utilisant une affectation bidirectionnelle produit des résultats attendus car seule la valeur de départ d'instruction de la colonne fait l'objet d'un accès pendant l'exécution de l'instruction.|Faible|  
...
|La fonction ODBC {fn CONVERT()} utilise le format de date par défaut de la langue. Pour certaines langues, le format par défaut est YDM, ce qui peut entraîner des erreurs de conversion lorsque CONVERT() est associé à d’autres fonctions, telles que `{fn CURDATE()}`, qui attendent un format YMD.|La fonction ODBC `{fn CONVERT()}` utilise style 121 (format YMD indépendant du langage) lorsque vous convertissez des données ODBC tape SQL_TIMESTAMP, SQL_DATE, SQL_TIME, SQLDATE, SQL_TYPE_TIME et SQL_TYPE_TIMESTAMP.|Faible|  
|Les intrinsèques datetime tels que DATEPART ne requièrent pas que les valeurs d'entrée de chaîne soient des littéraux datetime valides. Par exemple, `SELECT DATEPART (year, '2007/05-30')` se compile correctement.|Les intrinsèques DateTime tels que `DATEPART` requièrent des valeurs d’entrée de chaîne soient des littéraux de date et heure valides. L'erreur 241 est retournée lorsqu'un littéral datetime non valide est utilisé.|Faible|  
